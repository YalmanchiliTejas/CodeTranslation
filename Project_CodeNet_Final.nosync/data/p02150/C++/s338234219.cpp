...
template<class T>decltype(auto) operator |(T v, Slicer s) {
	typedef T::iterator::value_type Ty;
	vector<Ty> ret; int size = (int)v.size();
	if (s.from_.none)s.from_ = 0;
	if (s.to_.none)s.to_ = size;
	if (s.from_ < 0)s.from_ = size + s.from_;
	if (s.to_ < 0)s.to_ = size + s.to_;
	if (s.step_ > 0)
		for (int i = max(s.from_.val, 0); i < s.to_ && i < size && i >= 0; i += s.step_)ret.push_back(v[i]);
	else
		for (int i = min(s.from_.val, size - 1); i > s.to_ && i >= 0; i += s.step_)ret.push_back(v[i]);
	return ret;
}
*/

int main() {
	Modl n = max(inl[2] - inl[1], 0LL) / (inl[0] - inl[1]);
	n = n * (inl[1] % MOD) + inl[2];
	out, n, out;
}
