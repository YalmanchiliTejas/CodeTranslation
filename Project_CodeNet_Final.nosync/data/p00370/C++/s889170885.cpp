...
bool Intersect(Polygon p, Segment s1) {
  for (int i = 0; i < p.size(); ++i) {
    if (Intersect(s1, p[i])) return true;
...
signed main(int argc, char *argv[]) {
  ...
  if (Intersect(p, Segment(s, g))) {
    ...
    int idx = 0;
    for (int i = 0; i < p.size(); ++i) {
      sum += distance(p[i], p[(i + 1) % p.size()]);
      if (p[i] == s) idx = i;
    }

    ans = 0;
    for (int i = idx; p[i] != g; i = (i + 1) % p.size()) {
      ans += distance(p[i], p[(i + 1) % p.size()]);
    }
    ...
    ans = inf;
    for (int i = 0; i < n; ++i) {
      Point l = p[i], r = p[(i + 1) % n];
      int cnt = 100;
      while (cnt--) {
        Point midL = (l + l + r) * (1 / 3.0);
        Point midR = (l + r + r) * (1 / 3.0);
        if (distance(s, midL) + distance(g, midL) < distance(s, midR) + distance(g, midR)) {
          r = midR;
        } else {
          l = midL;
        }
      ...
      chmin(ans, distance(s, l) + distance(g, l));
    }
  }
