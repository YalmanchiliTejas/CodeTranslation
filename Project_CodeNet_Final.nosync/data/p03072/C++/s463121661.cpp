...
// list declaration
VarList list_H({Var(9), Var(5), Var(6), Var(8), Var(4)});
...
int main(){
    ...
    var_ans = Var(1);
    var_maxH = list_H.getLineOfList(Var(1));
    var_i = Var(2);
    while (!(var_i == (var_N + Var(1)))){
        if (((var_maxH == list_H.getLineOfList(var_i)) || (var_maxH < list_H.getLineOfList(var_i)))){
            var_maxH = list_H.getLineOfList(var_i);
            var_ans += Var(1);
        }
        var_i += Var(1);
    }
