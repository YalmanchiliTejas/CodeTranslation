...
//https://ferin-tech.hatenablog.com/entry/2018/02/03/062941
...
signed main(){
    ...
    for(int i=0;i<L;i++){
        if(dp[i]==-1) continue;
        if(i+sz<=L)chmax(dp[i+sz],dp[i]+1);
        if(i+tz<=L)chmax(dp[i+tz],dp[i]);
    }
    int x=dp[L];
    string ret="";
    for(int i=0;i<x;i++) ret+=s;
    for(int i=0;i<(L-x*sz)/tz;i++){
        ret+=t;
    }
    if(ret.size()!=L) return 1;
    cout<<ret<<endl;
}
