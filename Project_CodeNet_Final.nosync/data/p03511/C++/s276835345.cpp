...
int main(void){
	int L;cin>>L;
	string s,t;cin>>s>>t;
	llint lc=lcm(s.size(),t.size());
	//s,t,lcmの何か
	string ssss;
	string tttt;
	while(ssss.size()<min((llint)L,lc)){ssss+=s;}
	while(tttt.size()<min((llint)L,lc)){tttt+=t;}
	if(ssss>tttt){swap(s,t);}
	string ans;
	while(!((L-ans.size())<lc&&(L-ans.size())%t.size()==0)){ans+=s;}
