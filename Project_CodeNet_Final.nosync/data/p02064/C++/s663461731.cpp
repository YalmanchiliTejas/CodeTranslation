...
void solve(); int main() {
  locale loc(locale(), new yes_no); cout.imbue(loc); cout << fixed << setprecision(10) << boolalpha;
  solve();
}
// ^^^^^^^^^^^^ TEMPLATE ^^^^^^^^^^^^
...
void solve() {
  ...
  rep(i, n) {
    ...
    ll pos = query(s, i);
    if (pos > len) continue;
    if (pos + query(i, t) == len) {
      auto it = ord.lower_bound(P(pos, -linf));
      if (it->first == pos) continue;
      P nxt = *it;
      P prv = *prev(it, 1);
      if (query(prv.second, i) + query(i, nxt.second) == nxt.first - prv.first) {
        ord.insert(P(pos, i));
      }
    }
  }
