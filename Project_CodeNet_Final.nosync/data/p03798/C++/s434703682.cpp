...
bool check(ll x, ll y) {
  ...
  // 0: hontou
  for (ll i = 1; i < n; i++) {
    if ((d[i] == 1) xor (s[i] == 'o')) d[i+1] = d[i-1];
    else d[i+1] = 1 - d[i-1];
  }
  if (d[0] != d[n]) return false;
  if (d[0]) {
    if ((s[0] == 'o') xor (d[1] != d[n-1])) return false;
  } else {
    if ((s[0] == 'x') xor (d[1] != d[n-1])) return false;
  }
  for (ll i = 0; i < n; i++) printf("%c", d[i] ? 'W' : 'S');
  printf("\n");
  return true;
}
...
int main() {
  scanf("%lld", &n);
  scanf("%s", s);
  if (!(check(0, 0) || check(0, 1) || check(1, 0) || check(1, 1))) printf("-1\n");
}
