...
int main(void){
    ...
    for(char c0 : {'S', 'W'}) for(char c1 : {'S', 'W'}){
        string ans {c0, c1};
        for(int i = 2; i < n; i ++){
            ans += ((ans[i-2] == 'W') ^ (ans[i-1] == 'W') ^ (s[i-1] == 'x')) ? 'W' : 'S';
        }
        if((ans[n-1] == 'W') ^ (ans[  0] == 'W') ^ (ans[1] == 'W') ^ (s[  0] == 'x')) continue;
        if((ans[n-2] == 'W') ^ (ans[n-1] == 'W') ^ (ans[0] == 'W') ^ (s[n-1] == 'x')) continue;
        Cout(ans);
        return 0;
    }
    Cout(-1);
