...
signed main(){
	ic(n) sc(s)
	for(char i='S';i<='W';i+='W'-'S'){
		for(char j='S';j<='W';j+='W'-'S'){
			string ans;
			ans+=i;
			ans+=j;
			rep(1,l,n-1){
				if((s[l]=='o'&&ans[l]=='S')||(s[l]=='x'&&ans[l]=='W')){
					ans+=ans[l-1];
				}
				else{
					ans+=char('W'+'S'-ans[l-1]);
				}	
			}
			if(((((s[0]=='o'&&ans[0]=='S')||(s[0]=='x'&&ans[0]=='W'))&&ans[1]==ans[n-1])||(((s[0]=='x'&&ans[0]=='S')||(s[0]=='o'&&ans[0]=='W'))&&ans[1]!=ans[n-1]))){
	 			if((((((s[n-1]=='o'&&ans[n-1]=='S')||(s[n-1]=='x'&&ans[n-1]=='W'))&&ans[0]==ans[n-2])||(((s[n-1]=='x'&&ans[n-1]=='S')||(s[n-1]=='o'&&ans[n-1]=='W'))&&ans[0]!=ans[n-2])))){
	 				c(ans)
					return 0;
				}	
			}	
