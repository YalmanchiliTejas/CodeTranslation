...
void solve(int N, string s) {
  vb same(N);
  rep(i, N)same[i] = s[i] == 'o';
  each(x, tf)each(y, tf){
    vb is_s(N);
    is_s[0] = x;
    is_s[1] = y;
    rep(i, N - 2) is_s[i + 2] = (is_s[i + 1] == same[i + 1]) == is_s[i];
    if((is_s[N - 1] == same[N - 1]) == (is_s[N - 2] == is_s[0])
      && (is_s[0] == same[0]) == (is_s[N - 1] == is_s[1])){
      each(b, is_s)cout<<(b ? 'S' : 'W');
      cout<<endl;
      return;
    }
  }
