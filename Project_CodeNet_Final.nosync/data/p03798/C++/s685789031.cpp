...
bool JudgeSheep(char talk, bool mysheep, bool othersheep) {
...
bool Judge(bool zero, bool one,string s,ll n) {
	...
	for (ll i = 0; i < n; i++) {
		if (i == 0)sheep[N - 1] = JudgeSheep(s[0], sheep[0], sheep[1]);
		else if (i == N - 2) {
			if (JudgeSheep(s[i], sheep[i], sheep[i - 1]) != sheep[i + 1]) {
				
				return false;
			}
		}
		else if (i == N - 1) {
			if (JudgeSheep(s[i], sheep[i], sheep[i - 1]) != sheep[0]) {
				return false;
			}
		}
		else {
			sheep[i + 1] = JudgeSheep(s[i], sheep[i], sheep[i - 1]);
		}
...
int main() {
	...
	if (Judge(true, true, SS, N))return 0;
	if (Judge(true, false, SS, N))return 0;
	if (Judge(false, true, SS, N))return 0;
	if (Judge(false, false, SS, N))return 0;
