...
bool next_is_sheep(bool cur, bool pre, char remark){
    if (cur && remark == 'o') return pre;
    else if (cur && remark == 'x') return !pre;
    if (!cur && remark == 'o') return !pre;
    else if (!cur && remark == 'x') return pre;
}

void prl_sheep(bool b1){
    if (b1) pr('S');
    else pr('W');
}

void prl_result(bool s1, bool s2){
    prl_sheep(s1); prl_sheep(s2);
    For(i, 2, n) prl_sheep(sheep[i]);
    prl("");
}

void solve(){
    okay = false; bool c1, c2;
    // case 1. 0 and 1 is a sheep

    rep(i, 4){
        rep(j, 2) sheep[j] = (1 << j & i);
        For(i, 1, n - 1) sheep[i + 1] = next_is_sheep(sheep[i], sheep[i - 1], s[i]);
        c1 = next_is_sheep(sheep[n - 1], sheep[n - 2], s[n - 1]) == sheep[0];
        c2 = next_is_sheep(sheep[0], sheep[n - 1], s[0]) == sheep[1];
        if (c1 && c2) okay = true;
        if (okay){prl_result(sheep[0], sheep[1]); return;}
    }
