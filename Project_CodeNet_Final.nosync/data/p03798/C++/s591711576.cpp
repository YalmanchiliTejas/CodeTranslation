...
/*
    ...
    ll bfs(ll i, ll parents){
        ll color_idx = 1; ll n_edge = tree[i].size();
        ll n_colors = 0; ll org = cnt[{i, parents}];
        rep(idx, n_edge){
            ll j = tree[i][idx].to; ll col = cnt[{i, j}];
            while (org == color_idx) color_idx++;
    
            if (col == -1) {
                q.push(mp(j, i));
                cnt[{i, j}] = color_idx; cnt[{j, i}] = color_idx;
                n_colors = max(color_idx, n_colors);
                color_idx++;
            }
        }
        return n_colors;
    ...
    int main(void){
        ...
        rep(i1, h * w)rep(i2, h * w) if (i1 == i2) dist[i1][i2] = 0; else dist[i1][i2] = INF;
...
void solve(){
    ...
    rep(i, 4) {
        ...
        For(j, 1, n){
            ll val;
            if (sheep[i][j] == 1 && s[j] == 'o') val = sheep[i][j - 1];
            else if (sheep[i][j] == 1) val = 1 - sheep[i][j - 1];
            else if (s[j] == 'o') val = 1 - sheep[i][j - 1];
            else val = sheep[i][j - 1];

            if (j == n - 2 && sheep[i][n - 1] != val) okay = false;
            else if (j == n - 1 && sheep[i][0] != val) okay = false;
            else sheep[i][j + 1] = val;
        }
        if (okay) {rep(k, n) if (sheep[i][k])pr("S"); else pr("W"); prl(""); return;}
    }
    prl(-1);
}
