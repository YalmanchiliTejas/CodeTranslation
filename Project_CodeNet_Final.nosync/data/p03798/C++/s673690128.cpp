...
int main(){
	...
	REP(i, 2) REP(j, 2){
		...
		REPS(k, n - 2) res[k + 1] = (((((s[k] == 'o') == res[k]) xor res[k - 1])) ? 1 : 0);
		if(res[0] == ((((s[n - 1] == 'o') == res[n - 1]) xor res[n - 2])) ? 1 : 0)
		if(res[1] == ((((s[0] == 'o') == res[0]) xor res[n - 1])) ? 1 : 0){
			REP(t, n) cout << (res[t] ? 'W' : 'S');
			cout << endl;
			return 0;
		}
	}
