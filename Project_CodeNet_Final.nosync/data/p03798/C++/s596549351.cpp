...
namespace abc055d {
	void abc055d()
	{
		cin >> N;
		string str; cin >> str;
		static bool ans[100005];
		static bool init[4][2] = { {false,false}, {false, true}, {true, false}, {true, true} };
		bool found = false;
		REP(i, 4) {
			REP(j, 2) ans[j] = init[i][j];
			FOR(j, 1, N - 1) {
				int next = j + 1;
				int before = j - 1;
				if (j == N - 1) next = 0;
			...
			if ((str[0] == 'o' and ans[0] == false) or (str[0] == 'x' and ans[0] == true)) {
				if (ans[N - 1] == ans[1]) {
					out1 = true;
				}
			}
			if ((str[0] == 'x' and ans[0] == false) or (str[0] == 'o' and ans[0] == true)) {
				if (ans[N - 1] != ans[1]) {
					out1 = true;
				}
			}

			if ((str[N - 1] == 'o' and ans[N - 1] == false) or (str[N - 1] == 'x' and ans[N - 1] == true)) {
				if (ans[N - 2] == ans[0]) {
					out2 = true;
				}
			}
			if ((str[N - 1] == 'x' and ans[N - 1] == false) or (str[N - 1] == 'o' and ans[N - 1] == true)) {
				if (ans[N - 2] != ans[0]) {
					out2 = true;
				}
			}
