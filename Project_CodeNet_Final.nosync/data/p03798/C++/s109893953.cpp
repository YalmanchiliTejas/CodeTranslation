...
int main() {
	uint32 N = ReadUint32(); // 数字Nが与えられます。
	String S = ReadWord();	// 文字列Sが与えられてます。

	// 要素1 要素2を仮定するので、4種類仮定すればいい。
	// SS, SW, WS, WW
	...
	// 仮定1 SS
	{
		...
		if (ans.back() == 'S') {
			if (S.back() == 'o') {
				hoge += ans[ans.size()-2];	// *(ans.end() - 1)
			}
			else if (ans[ans.size()-2] == 'S') {
				hoge += 'W';
			}
			else {
				hoge += 'S';
