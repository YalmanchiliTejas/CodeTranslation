...
plain pla[200005];
ll ans = -1;
vector<ll> vlD[400005];
vector<ll> vlU[400005];
vector<ll> vlR[400005];
vector<ll> vlL[400005];

void CheckUD() {
	lp(i, N)if (pla[i].u == 'D')vlD[pla[i].x].push_back(pla[i].y);
	lp(i, N)if (pla[i].u == 'U')vlU[pla[i].x].push_back(pla[i].y);
	lp(i, 400005)sort(ALL(vlD[i]));
	lp(i, 400005)sort(ALL(vlU[i]));

	lp(i, 400005) {
		if (vlD[i].size() > 0 && vlU[i].size() > 0) {
			a = 0;
			b = 0;
			while (a < vlD[i].size() && b < vlU[i].size()) {
				d = vlD[i][a];
				c = vlU[i][b];
				if (c <= d) {
					e = (d - c)*5;
...
void CheckRL() {
	lp(i, N)if (pla[i].u == 'R')vlR[pla[i].y].push_back(pla[i].x);
	lp(i, N)if (pla[i].u == 'L')vlL[pla[i].y].push_back(pla[i].x);
	lp(i, 400005)sort(ALL(vlR[i]));
	lp(i, 400005)sort(ALL(vlL[i]));

	lp(i, 400005) {
		if (vlR[i].size() > 0 && vlL[i].size() > 0) {
			a = 0;
			b = 0;
			while (a < vlR[i].size() && b < vlL[i].size()) {
				c = vlR[i][a];
				d = vlL[i][b];
				if (c <= d) {
					e = (d - c) * 5;
