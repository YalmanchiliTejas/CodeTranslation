...
P operator-(const P& l,const P& r){return P(l.f-r.f,l.s-r.s);}
P operator-(const P& r){return P(-r.f,-r.s); }
P operator*(const lint& l,const P& r){return P(l*r.f,l*r.s); }
P operator/(const P& l,const lint& r){return P(l.f/r,l.s/r); }
#pragma endregion
...
#pragma region lib_abc_a

vector<string>in_str;
vector<lint>in_num;
lint a,b,c,d,e,f;
string s,t,u,x,y,z;
lint* pn[6] = { &a,&b,&c,&d,&e,&f };
string* ps[6] = { &s,&t,&u,&x,&y,&z };
lint n = 0;
...
struct hayatoki{
	hayatoki(){
		...
		rep(in_str.size())* ps[i] = in_str[i];
		rep(in_num.size())* pn[i] = in_num[i];
	}
} a_input;
