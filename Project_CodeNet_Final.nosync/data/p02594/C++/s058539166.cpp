...
template<class S,class T> common_type<S,T> nPr(const S &n,const T &k){if(n<k||n<0||k<0)return 0;T r=1;brep(i,n-k+1,n)r*=i;return r;}
template<class S,class T> common_type<S,T> nCr(const S &n,T k){if(n<k||n<0||k<0)return 0;T r=1;k=min(k,n-k);brep(i,n-k+1,n)r*=i;return r/factorial(k);}
...
template<class S,class T> common_type<S,T> dotProduct(const vector<S> &u,const vector<T> &v)  {T r=0;rep(i,u.size())r+=u[i]*v[i];return r;}
...
template<class S,class T> pair<S,T> min(const map<S,T> &m) {pair<S,T> r=*m.begin();for(const auto& p:m)if(chmin(r.se,p.se))r=p;return r;}
template<class S,class T> pair<S,T> max(const map<S,T> &m) {pair<S,T> r=*m.begin();for(const auto& p:m)if(chmax(r.se,p.se))r=p;return r;}
