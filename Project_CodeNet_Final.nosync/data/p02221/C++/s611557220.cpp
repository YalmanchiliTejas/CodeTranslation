...
{
    auto N  = in<int>();
    auto S  = "0" + in<std::string>();
    auto ps = in_v<int>({1 << N});
    ...
    const auto ans = mfp([&](auto&& self, const std::vector<int>& ps) -> std::vector<int> {
        const int n = ps.size();
        if (n == 2) { return std::vector<int>{w(ps[0], ps[1]), w(ps[0], ps[1])}; }
        std::vector<int> qs, rs;
        for (int i = 0; i < n; i++) {
            const int a = ps[i], b = ps[(i + 1) % n];
            (i % 2 == 0 ? qs : rs).push_back(w(a, b));
        }
        const auto sq = self(self, qs);
        const auto sr = self(self, rs);
        std::vector<int> ans(n);
        for (int i = 0; i < n; i++) { ans[i] = (i % 2 == 0 ? sq : sr)[i / 2]; }
        return ans;
    })(ps);
    for (int i = 0; i < (1 << N); i++) { outln(ans[i]); }
    return 0;
}
