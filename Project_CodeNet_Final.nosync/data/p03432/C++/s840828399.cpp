...
const int MXFF=18, MXLL=(1<<MXFF)+5;
...
void doit(){
    ...
    for (int i=0;i<=m;++i) A[i]=f[i]*inv[i]%mod;
    for (int i=1;i<=m;++i) B[i]=inv[i+2];
    poly res=A*B;
    for (int i=0;i<=m;++i)
        f[i]= (f[i]*(C(i+1,2)+1)+res[i]*jc[i+2])%mod;
}

int main(){
    cin>>m>>n;
    jc[0]=1; for (int i=1;i<=m+5;++i) jc[i]=jc[i-1]*i%mod;
    for (int i=0;i<=m+5;++i) inv[i]=K(jc[i]);
    ...
    //         for (int k=0;j+k<=m;++k){
    //             if (k)
    //                 f[i+1][j+k]=(f[i+1][j+k]+f[i][j]*C(j+k+2,k+2))%mod;
    //             else
    //                 f[i+1][j+k]=(f[i+1][j+k]+f[i][j]*(C(j+1,2)+1))%mod;
    //         }
    // }
