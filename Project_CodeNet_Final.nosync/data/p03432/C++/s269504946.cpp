...
const int BIT = 14, MAXN = 1 << BIT, MAXM = 205;
namespace NTT {
...
int main() {
	...
	for (int i = 0; i < m; i++) {
		int a = i & 1, b = !a;
		for (int j = 0; j <= n; j++) f[a][j] = f[a][j] * inv[j] % MOD;
		f[b][0] = 0;
		for (int j = 1; j <= n; j++) f[b][j] = inv[j + 2];
		NTT::get_mul(f[a], f[b], f[b], n + 1, n + 1, n + 1);
		for (int j = 0; j <= n; j++)
			f[b][j] = (f[b][j] * fac[j + 2] + f[a][j] * (C(j + 1, 2) + 1) % MOD * fac[j]) % MOD;
//		for (int j = 0; j <= n; j++) if (f[i][j]) {
//			f[i + 1][j] = (f[i + 1][j] + f[i][j] * (C(j + 1, 2) + 1)) % MOD;
//			for (int k = 1; j + k <= n; k++)
//				f[i + 1][j + k] = (f[i + 1][j + k] + f[i][j] * C(j + k + 2, k + 2)) % MOD;
//		}
	}
	LL ans = 0;
	for (int i = 0; i <= n; i++) ans = (ans + C(n, i) * f[m & 1][i]) % MOD;
	printf("%lld\n", ans);
	return 0;
}