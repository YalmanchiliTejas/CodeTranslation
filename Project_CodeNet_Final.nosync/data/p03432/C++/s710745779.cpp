...
namespace io {
    // void setIn(string s) { freopend(s.c_str(),"r",stdin); }
    // void setOut(string s) { freopend(s.c_str(),"w",stdout); }
    void setIO(string s = "") {
...
void adv() {
    vmi DP(N+1);
    F0R(i,N+1) DP[i] += dp[i]*(C.comb(i+1,2)+1);
    vmi x, z; F0R(i,N+1) x.pb(dp[i]*C.ifac[i]);
    F0R(i,N+1) {
        if (i == 0) z.pb(0);
        else z.pb(C.ifac[i+2]);
    }
    auto y = FFT::conv(x,z); y.rsz(N+1);
    F0R(i,N+1) DP[i] += C.fac[i+2]*y[i];
    swap(dp,DP);
}

int main() {
    setIO(); C.init();
    re(N,M); dp.rsz(N+1); dp[0] = 1;
    F0R(i,M) adv();
    mi ans = 0;
    F0R(i,N+1) ans += C.comb(N,i)*dp[i];
    ps(ans);
}
