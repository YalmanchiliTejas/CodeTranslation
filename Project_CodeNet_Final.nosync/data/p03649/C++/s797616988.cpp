...
int _main()
{
        ...
        for ( ; ; ) {
                sort_reverse(alist);
                if (alist[0] <= n * 2LL) break;

                int end = n;
                rep (i, n) {
                        if (alist[0] - alist[i] > n + 1LL) {
                                end = i;
                                break;
                        }
                }
                ...
                ll loop = (alist[0] - alist[end] - 1LL) / (n + 1LL);
                rep (i, end) alist[i] -= (n - end + 1LL) * loop;
                srep (i, end, n) alist[i] += end * loop;
                ans += end * loop;
        }

        if (alist[0] > n * 2LL) {
                ll loop = alist[0] - n * 2LL;
                rep (i, n) alist[i] -= loop;
                ans += n * loop;
        }
