...
signed main(){
    ...
    ll ans=0;
    rep(k,n-1){
        vector<tuplis>change;
        const ll i1=k*3+2,i2=k*3+3,i3=k*3+4;
        sort(a.begin()+i1,a.begin()+i1+3);
        const ll x=a[i1],y=a[i2],z=a[i3];
        if(x==z){
            ans++;
            continue;
        }
        if(x==y){
            rep(w,n)change.push_back({w,z,dp[w][x]+1});
        }
        if(z==y){
            rep(w,n)change.push_back({w,x,dp[w][z]+1});
        }
        change.push_back({y,z,dp[x][x]+1});
        change.push_back({x,z,dp[y][y]+1});
        change.push_back({x,y,dp[z][z]+1});
        rep(w,n)change.push_back({w,x,yoko[w]});
        rep(w,n)change.push_back({w,y,yoko[w]});
        rep(w,n)change.push_back({w,z,yoko[w]});
        change.push_back({x,y,all});
        change.push_back({z,y,all});
        change.push_back({z,x,all});
        each(i,change){
            chmax(dp[i[0]][i[1]],i[2]);
            chmax(dp[i[1]][i[0]],i[2]);
            chmax(yoko[i[0]],i[2]);
            chmax(yoko[i[1]],i[2]);
            chmax(all,i[2]);
        }
