...
void solve()
{
	...
	for (int i = 3; i < 3 * n; i += 3)
	{
		...
		for (int x = i; x < i + 3; ++x)
			for (int y = i; y < i + 3; ++y) if (x != y)
			{
				int z = 3 * i + 3 - x - y;
				q.push_back({ a[x], a[y], max(f[a[z]][a[z]] + 1, res) });
			}
		for (int x = i; x < i + 3; ++x)
		{
			int y = x + 1; if (y == i + 3) y = i;
			int z = y + 1; if (z == i + 3) z = i;
			for (int c = 1; c <= n; ++c)
			{
				q.push_back({ a[x], c, max(g[c], f[c][a[y]] + (a[y] == a[z])) });
			}
		}
		for (auto [x, y, c] : q)
		{
