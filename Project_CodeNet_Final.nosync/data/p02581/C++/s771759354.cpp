...
main() {
    ...
    VEC(int, a, n * 3);
    a.eb(0), a.eb(0);
    rep(i, 2010) rep(j, 2010) dp[i][j] = -1e8;
    rep(i, 2010) ma2[i] = -1e8;
    rep(i, 5) rep2(j, i + 1, 4) {
        vi bb;
        rep(k, 5) if(k != i and k != j) bb.eb(a[k]);
        if(bb == vi(3, bb[0]))
            dp[a[i]][a[j]] = dp[a[j]][a[i]] = 2, ma = 2, ma2[a[i]] = ma2[a[j]] = 2;
        else
            dp[a[i]][a[j]] = dp[a[j]][a[i]] = 1, chmax(ma2[a[i]], 1), chmax(ma2[a[j]], 1);
    }
    vector<tuple<int, int, int>> mem;
    rep(i, n - 1) {
        int st = i * 3 + 5;
        if(a[st] == a[st + 1] and a[st] == a[st + 2]) {
            lazy++;
            continue;
        }
        rep(j, 3) b[j] = a[st + j];
        sort(b, b + 3);
        do {
            int K = max({dp[b[0]][b[1]], dp[b[2]][b[2]] + 1, ma});
            mem.eb(b[0], b[1], K);
            rep2(i, 1, n) {
                K = dp[i][b[0]];
                if(b[1] == b[2]) { chmax(K, dp[i][b[1]] + 1); }
                chmax(K, ma2[i]);
                mem.eb(i, b[0], K), mem.eb(b[0], i, K);
            }
        } while(next_permutation(b, b + 3));
        for(auto [e, f, g] : mem) chmax(dp[e][f], g), chmax(ma2[e], g), chmax(ma, g);
        mem.clear();
    }
    cout << ma + lazy << endl;
}
