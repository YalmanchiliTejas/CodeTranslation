...
void mappri(mll &m) { foa(s, m) cout << s << endl; }

queue<pair<pll, ll>> q;
using tu = pair<pll, ll>;
...
int main() {
	...
	rep(i, n - 1) {
		...
		while(!q.empty()) {
			tu tp = q.front();
			chmax(dp[tp.first.first][tp.first.second], tp.second);
			chmax(m[tp.first.first], tp.second);
			chmax(m[tp.first.second], tp.second);
			q.pop();
		}
		rep(i, n) { chmax(maxi, m[i]); }
		// debug(dp);
		// debug(m);
	}
	dp[a[3 * n - 1]][a[3 * n - 1]]++;
	chmax(maxi, dp[a[3 * n - 1]][a[3 * n - 1]]);
	cout << maxi + ans << endl;
	// debug(dp);
	// debug(m);
}