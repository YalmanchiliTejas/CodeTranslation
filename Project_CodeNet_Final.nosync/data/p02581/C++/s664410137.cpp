...
void include(int a, int b, int v) {
	todo.emplace_back(a, b, v);
	if (v >= 0) debug(imie(a), imie(b), imie(v));
	assert(dp[a][b] == dp[b][a]);
}
void flush() {
	for (auto [a, b, v] : todo) {
		assert(dp[a][b] == dp[b][a]);
...
vector <tuple <int, int, int> > cyc(int a, int b, int c) {
	return {{a, b, c}, {b, c, a}, {c, a, b}};
}
void solve() {
	memset(dp, -10, sizeof(dp));
	memset(top, -10, sizeof(top));
	int n;
	scanf("%d", &n);
	vi in(n * 3);
	REP(i, 3 * n) scanf("%d", &in[i]);
	include(in[0], in[1], 0);
	flush();
	int inc = 0;
	REP(i, n - 1) {
		int x = in[i * 3 + 2], y = in[i * 3 + 3], z = in[i * 3 + 4];
		debug(imie(x), imie(y), imie(z));
		if (x == y && x == z && y == z) {
			inc++;
			continue;
		}
		for (auto [a, b, c] : cyc(x, y, z)) {
			include(a, b, max(topall, dp[c][c] + 1));
			REP(x, n + 1) {
				include(x, a, top[x]);
				if (b == c) include(x, a, dp[x][b] + 1);
			}
		}
		flush();
	}
