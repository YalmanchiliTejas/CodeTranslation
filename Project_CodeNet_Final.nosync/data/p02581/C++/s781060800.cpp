...
int main(){fastio
    ...
    dp[t[0]][t[1]]=dp[t[1]][t[0]]=0;
    ll mx[n+1];rep(i,0,n)mx[i]=-inf;
    rep(i,1,n){
        rep(j,1,n)chmax(mx[i],dp[i][j]);
    }
    rep(z,1,n){
        ll pl=z*3-1,pr=z*3+1;
        sort(t+pl,t+pr+1);
        vector<vector<ll>>v;
        if(t[pl]==t[pr]){
            TOTAL++;
        }else if(t[pl+1]==t[pl]||t[pl+1]==t[pr]){
            ll a=t[pl+1];
            ll b=t[pl];if(a==b)b=t[pr];
            {
                ll m=-inf;
                rep(i,1,n)chmax(m,mx[i]);
                v.pb({a,a,m});
                v.pb({a,b,m});
            }
            rep(k,1,n){
        ...
        if(v.size()>=1){
            for(auto x:v){
                if(x[2]<=-inf/2)continue;
                chmax(dp[x[0]][x[1]],x[2]);
                chmax(dp[x[1]][x[0]],x[2]);
                chmax(mx[x[0]],x[2]);
                chmax(mx[x[1]],x[2]);
            }
        }
