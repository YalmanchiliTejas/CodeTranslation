...
int main(){
	...
	for(int i=3;i<3*n;i+=3){
		...
		if(a[i]>a[i+1])swap(a[i],a[i+1]);
		if((a[i]==a[i+1]&&num[a[i]]>0)||(a[i+1]==a[i+2]&&num[a[i+1]]>0))for(int j=1;j<=n;j++){
			if(a[i]==a[i+1]){
		...
		for(int j=0;j<t;j++)if(dp[A[j]][B[j]]<C[j]){
			dp[A[j]][B[j]]=dp[B[j]][A[j]]=C[j];
			if(MX[A[j]]<C[j])MX[A[j]]=C[j];
			if(MX[B[j]]<C[j])MX[B[j]]=C[j];
			if(cur<C[j])cur=C[j];
		}
		num[a[i]]++,num[a[i+1]]++,num[a[i+2]]++;
	}
	for(int i=1;i<=n;i++){
