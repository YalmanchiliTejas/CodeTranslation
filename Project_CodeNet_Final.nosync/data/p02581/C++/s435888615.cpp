...
{
  static char buf[1<<22],*s1=buf,*s2=buf;
  return s1==s2&&(s2=(s1=buf)+fread(buf,1,1<<22,stdin),s1==s2)?EOF:*s1++;
}
...
//https://atcoder.jp/contests/abc176/submissions/16571317
//https://atcoder.jp/contests/abc176/submissions/16449316
static inline int Max(int x,int y){return x>y?x:y;}
int work[10006][3],sz=0;
...
int main(void)
{
  ...
  rep(i,N-1)
  {
    if(A[3*i+2]>A[3*i+3])Swap(A[3*i+2],A[3*i+3]);
    if(A[3*i+3]>A[3*i+4])Swap(A[3*i+3],A[3*i+4]);
    if(A[3*i+2]>A[3*i+3])Swap(A[3*i+2],A[3*i+3]);
    int x[3]={A[3*i+2],A[3*i+3],A[3*i+4]};
