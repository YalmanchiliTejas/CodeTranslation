...
int main() {
  ...
  vector dp(n, vector<cint>(n, -inf<>));
  vector<cint> max_dp(n, -inf<>);
  cint offset = 0;
  ...
  for (cint i : rep(n - 1)) {
    ...
    if (x == y) {
      for (cint p : rep(n)) lazy_update(p, z, dp[p][y] + 1);
    }
    if (y == z) {
      for (cint p : rep(n)) lazy_update(p, x, dp[p][y] + 1);
    }
    for (cint e : {x, y, z}) {
      for (cint p : rep(n)) lazy_update(p, e, max_dp[p]);
    }
