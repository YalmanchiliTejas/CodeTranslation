...
void Main()
{
    ...
    rep(i, n-1) {
        ...
        // x,y,zのうち２つだけ等しい場合
        // 左ふたつのうちどちらかがそれら２つと同じであればペアを作る
        if (x == y) {
            rep(p, n) {
                cand.push_back({
                    z,
                    p,
                    // そんなx,pのペアがなければdp[x][p]は-INFになっているから問題ない
                    max(dp[x][p], dp[p][x]) + 1,
                });
            }
        }
        ...
        // 左２つがともにxyzのどれかと等しい場合
        // xxxyz, yyxyzみたいなとき
        // そんな場合がなればdp[z][z]は-INFになっているのでこれでよい
    ...
    // dp[A[3 * n - 1]][A[3 * n - 1]] は、最後の3枚が同じになるように選んだ場合の最高成績
    // このときは追加で1点もらえるので特別扱い
