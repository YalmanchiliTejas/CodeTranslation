...
int main()
{
	...
	while (!que.empty()) {
		// 一番小さいのからとれる
		P point = que.top();
		int pointIdx = point.second;
		int pointCost = point.first;
		que.pop();
		if (isUsed[pointIdx]) {
			// すでに使用済み
			continue;
		}
		if (minCost[pointIdx] < pointCost) {
			// もっと小さいのが入ってる
			continue;
		}
		...
		// 頂点採用
		ans += pointCost;
		isUsed[pointIdx] = true;

		for (auto egdeOne : G[pointIdx]) {
			int nextCost = egdeOne.cost;
			if (minCost[egdeOne.to] > nextCost) {
				minCost[egdeOne.to] = nextCost;
				que.push(make_pair(nextCost, egdeOne.to));
			}
		}
	}
