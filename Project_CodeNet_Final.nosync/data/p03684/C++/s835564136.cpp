...
void _main()
{
    ...
    rep (idx, n-1) {
        auto xyi1 = xyilist[idx];
        ll x1 = get<0>(xyi1);
        ll i1 = get<2>(xyi1);
        auto xyi2 = xyilist[idx+1];
        ll x2 = get<0>(xyi2);
        ll i2 = get<2>(xyi2);
        q.push(t3(x2 - x1, i1, i2));
    }

    rep (idx, n-1) {
        auto yxi1 = yxilist[idx];
        ll y1 = get<0>(yxi1);
        ll i1 = get<2>(yxi1);
        auto yxi2 = yxilist[idx+1];
        ll y2 = get<0>(yxi2);
        ll i2 = get<2>(yxi2);
        q.push(t3(y2 - y1, i1, i2));
    }
    ...
    while (!q.empty()) {
        auto cvv = q.top(); q.pop();
        ll c = get<0>(cvv);
        ll v1 = get<1>(cvv);
        ll v2 = get<2>(cvv);

        if (uf.root(v1) == uf.root(v2)) continue;
