...
#ifdef _DEBUG
template<class T> void outv(vector<T> &a, ll W) {    rep(i, W) { cerr << a[i] << " "; }    cerr << "" << endl;}template<class T> void outv(vector<vector<T> > &a, ll H = linf, ll W = linf) {    rep(h, min(H, sz(a))) {        rep(w, min(W, sz(a[0]))) { if (a[h][w] == linf) cerr << "e" << " "; else cerr << a[h][w] << " "; }        cerr << "" << endl;    }}
#else
...
void solve() {
    ...
    vi yi = sorti(y);
    //-> V<(cost,to(group_id))>
    auto f = [&](int gsz, const vi &bel/*どのグループに属するか*/) {
        vp ret(gsz, {linf, -1});
        //x
        //自分と違うグル―プで最小の物を選ぶ
        //行先はグループのid
        {
            ...
            vp mi{{linf, -1},
                  {linf, -1}};
            //コストを足し算の形で表現できるようにする
            rep(ni, n) {
                int i = xi[ni];
                auto e = (mi[0].se == bel[i]) ? mi[1] : mi[0];
                e.fi += x[i];//iが右側 e.fiは最終的なコスト
                chmi(ret[bel[i]], e);

                P now(-x[i], bel[i]);//iを左側のコストとしてみなした時の置き方
                if (mi[0].se == bel[i]) {
                    chmi(mi[0], now);
                } elf (mi[1].se == bel[i]) {
                    chmi(mi[1], now);
        ...
        }
        {
            ...
            rer(ni, n - 1) {
                ...
                P now(x[i], bel[i]);//iを右側のコストとしてみなした時の置き方
                if (mi[0].se == bel[i]) {
                    chmi(mi[0], now);
                } elf (mi[1].se == bel[i]) {
                    chmi(mi[1], now);
        ...
        //y
        {
            ...
            rep(ni, n) {
                int i = yi[ni];
                auto e = (mi[0].se == bel[i]) ? mi[1] : mi[0];
                e.fi += y[i];//iが右側 e.fiは最終的なコスト
                chmi(ret[bel[i]], e);

                P now(-y[i], bel[i]);//iを左側のコストとしてみなした時の置き方
                if (mi[0].se == bel[i]) {
                    chmi(mi[0], now);
                } elf (mi[1].se == bel[i]) {
                    chmi(mi[1], now);
        ...
        }
        {
            ...
            rer(ni, n - 1) {
                ...
                P now(y[i], bel[i]);//iを右側のコストとしてみなした時の置き方
                if (mi[0].se == bel[i]) {
                    chmi(mi[0], now);
                } elf (mi[1].se == bel[i]) {
                    chmi(mi[1], now);
