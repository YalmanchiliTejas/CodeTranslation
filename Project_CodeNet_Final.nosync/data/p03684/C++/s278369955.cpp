...
/**************************************************************Begin***************************************************************/
int n=READ(),a[100010][2],par[100010],u[100010];
ll ans;
...
int main()
{
	rep1(i,n) a[i][0]=READ(),a[i][1]=READ();
	
	rep1(i,n) u[i]=par[i]=i;
	
	sort(u+1,u+n+1,cmp1);
	rep1(i,n-1) v.push_back(MP(a[u[i+1]][0]-a[u[i]][0],MP(u[i],u[i+1])));
	
	sort(u+1,u+n+1,cmp2);
	rep1(i,n-1) v.push_back(MP(a[u[i+1]][1]-a[u[i]][1],MP(u[i],u[i+1])));
	
	sort(All(v));
	repV(i,v)
	{
		if(FIND(i.sc.fs)==FIND(i.sc.sc)) continue;
		ans+=i.fs;
		par[FIND(i.sc.fs)]=FIND(i.sc.sc);
	}
