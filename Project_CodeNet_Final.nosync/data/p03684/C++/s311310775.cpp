...
signed main() {
	ll n;
	cin >> n;
	vector<pair<ll, PAIR>> v;
	v.reserve(n);
	REP(i, n) {
		ll x, y;
		cin >> x >> y;
		v.emplace_back(i, PAIR(x, y));
	}

	unordered_multimap<ll, PAIR> graph;
	sort(ALL(v), [](cauto a, cauto b) {
		return a.second.first < b.second.first;
	});
	REPI(it, v) {
		if ((++it)-- != v.end()) {
			ll cost = (++it)--->second.first - it->second.first;
			graph.emplace(it->first, PAIR((++it)--->first, cost));
			graph.emplace((++it)--->first, PAIR(it->first, cost));
		}
	}
	sort(ALL(v), [](cauto a, cauto b) {
	...
	REPI(it, v) {
		if ((++it)-- != v.end()) {
			ll cost = (++it)--->second.second - it->second.second;
			graph.emplace(it->first, PAIR((++it)--->first, cost));
			graph.emplace((++it)--->first, PAIR(it->first, cost));
		}
	}
	auto compare = [](cauto a, cauto b) {
	...
	priority_queue<PAIR, vector<PAIR>, decltype(compare)> q(compare);
	q.emplace(0, 0);
	while (!q.empty()) {
		ll from = q.top().first;
