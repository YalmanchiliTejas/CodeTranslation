...
ll dist(int x,int y){
	return min(abs(a[x].first.first-a[y].first.first),abs(a[x].first.second-a[y].first.second));
}
...
int I_Love_Hashimoto_Kanna() {
  ...
  FOR(i,0,n){
	...
	FOR(tot,0,2){
	  sort(a,a+n);
	  FOR(i,0,n-1){
      e.insert({dist(i,i+1),{a[i].second,a[i+1].second}});
			swap(a[i].first.first,a[i].first.second);
	  }
	  swap(a[n-1].first.first,a[n-1].first.second);
	}
	ll ans=0;
	for(set<pair<ll,pii > >::iterator itr=e.begin();itr!=e.end();itr++){
		int a,b,c;
		pair<ll,pii > edge=*itr;
		c=edge.first;
		a=edge.second.first;
