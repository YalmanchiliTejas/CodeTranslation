...
int main()
{
  ...
  rep(i,n)p[i]=make_tuple(x[i],y[i],i);
  sort(all(p));
  rep(i,n-1){
    q[get<2>(p[i])].push_back(get<2>(p[i+1]));
    q[get<2>(p[i+1])].push_back(get<2>(p[i]));
  }
  rep(i,n){
    swap(get<1>(p[i]),get<0>(p[i]));
  }
  sort(all(p));
  rep(i,n-1){
    q[get<2>(p[i])].push_back(get<2>(p[i+1]));
  ...
  vector<tuple<int,int,int>>t(n*4);
  UnionFind u(n);
  rep(i,n){
    rep(j,q[i].size()){
      t[i*4+j]=make_tuple(min(abs(x[i]-x[q[i][j]]),abs(y[i]-y[q[i][j]])),i,q[i][j]);
    }
  }
  sort(all(t));
  rep(i,t.size()){
    if(u.same(get<2>(t[i]),get<1>(t[i]))==0){
      u.unite(get<2>(t[i]),get<1>(t[i]));
      ans+=get<0>(t[i]);
    }
  }
  printf("%d",ans);
