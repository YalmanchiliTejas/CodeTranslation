...
int main(){
	...
	sort(a+1,a+n+1,cmp);
	for(int i=1;i<n;i++)
		v.insert(mer(a[i+1].x-a[i].x,mer(a[i].z,a[i+1].z)));
	
	sort(a+1,a+n+1,cmp2);
	for(int i=1;i<n;i++)
		v.insert(mer(a[i+1].y-a[i].y,mer(a[i].z,a[i+1].z)));
	
	int sum=0;
	while(!v.empty()){
		pair<int,pii> pij=*v.begin();
		v.erase(v.begin());
		if(father(pij.se.fi)!=father(pij.se.se)){
			bcj[father(pij.se.fi)]=father(pij.se.se);
			sum+=pij.fi;
		}
	}
	cout<<sum;
