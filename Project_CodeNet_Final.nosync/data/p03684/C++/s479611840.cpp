...
void solve() {
  ...
  while (!Q.empty()) {
    ...
    if (it != s_x.begin()) {
      ...
      //cerr << "Nxt = " << nxt << "\n";
      if (!visit[nxt]) {
        if (dist[nxt] > abs(ar_x[at]-ar_x[nxt])) {
          dist[nxt] = abs(ar_x[at] - ar_x[nxt]);
    ...
    if (it != s_y.begin()) {
      ...
      //cerr << "Nxt = " << nxt << "\n";
      if (!visit[nxt]) {
        if (dist[nxt] > abs(ar_y[at]-ar_y[nxt])) {
          dist[nxt] = abs(ar_y[at] - ar_y[nxt]);
          Q.insert({dist[nxt], nxt});
        }
      }
      it++;
