...
int main() {
  scanf("%lld", &n);
  for (ll i = 0; i < n; i++) scanf("%lld%lld", &x[i], &y[i]), px[i] = P(x[i], i), py[i] = P(y[i], i);
  sort(px, px+n); sort(py, py+n);
  Graph<ll> graph(n, false);
  for (ll i = 0; i < n-1; i++) graph.add_edge(px[i].second, px[i+1].second, px[i+1].first-px[i].first);
  for (ll i = 0; i < n-1; i++) graph.add_edge(py[i].second, py[i+1].second, py[i+1].first-py[i].first);
  printf("%lld\n", graph.kruskal());
}
