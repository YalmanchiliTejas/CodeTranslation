...
LL cost(pair<LL,LL> A,pair<LL,LL> B){
	LL ret = LINF;
	ret = min(ret,abs(A.first-B.first) );
	ret = min(ret,abs(A.second-B.second) );
	return ret;
}
void solve(){
	int N;
	cin >> N;
	vector< pair<LL,LL> > posXY(N),posYX(N);
	for(int i=0;i<N;++i){
		cin >> posXY[i].first >> posXY[i].second;
		posYX[i].first = posXY[i].second;
		posYX[i].second = posXY[i].first;
	}
	sort( posXY.begin(), posXY.end() );
	sort( posYX.begin(), posYX.end() );
	if( posXY[0] == posXY[N-1] ){
		//全て同じ位置
		cout << 0 << endl;
		return;
	}
	///
	//重複チェック
	vector< pair<LL,LL> > v = posXY;
	UNIQUE( v );
	int NUM = v.size();//位置が異なる町の数
	///
	/*
	N = NUM;
	A = v;
	...
	*/
	...
	priority_queue< vector<LL>
		, vector< vector<LL> > , greater< vector<LL> > > que;
	pair<LL,LL> A,B;
	vector<LL> temp(5);
	...
	int No = 0;
	for(int i=0;i<N;++i){
		if( index.find( posXY[i] ) == index.end() ){
			index[ posXY[i] ] = No;
			++No;
		}
	}
	...
	for(int i=0;i<N;++i){
		...
		///////////////YX逆
		if( i > 0 ){
			A = posYX[i-1];swap(A.first,A.second);
			B = posYX[i];swap(B.first,B.second);
			LL res = cost( A, B );
			if( A > B ){
				swap(A,B);
			}
		...
		if( i+1 < N ){
			A = posYX[i];swap(A.first,A.second);
			B = posYX[i+1];swap(B.first,B.second);
			LL res = cost( A, B );
			if( A > B ){
				swap(A,B);
			}
	...
	while( !que.empty() ){
		...
		if( uf.same( index[A], index[B] ) ){
		}else{
			//
			ans += temp[0];
			uf.add( index[A],index[B] );
		}
	}
	cout << ans << endl;
}
