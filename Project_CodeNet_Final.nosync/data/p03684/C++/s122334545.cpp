...
int _main()
{
        ...
        V<t3> sublist; // (-sub, idx1, idx2)
        rep (i, n-1LL) {
                t3 xyi1 = xyilist[i];
                t3 xyi2 = xyilist[i+1];
                sublist.pb(t3(get<0>(xyi1) - get<0>(xyi2),
                              get<2>(xyi1), get<2>(xyi2)));

                t3 yxi1 = yxilist[i];
                t3 yxi2 = yxilist[i+1];
                sublist.pb(t3(get<0>(yxi1) - get<0>(yxi2),
                              get<2>(yxi1), get<2>(yxi2)));
        }

        V<t2> linklist[100005];  // (idx1 --> (-sub, idx2))
        for (auto sub : sublist) {
                ll minussub = get<0>(sub);
                ll idx1 = get<1>(sub);
                ll idx2 = get<2>(sub);
                linklist[idx1].pb(t2(minussub, idx2));
                linklist[idx2].pb(t2(minussub, idx1));
        }
        ...
        for (auto link : linklist[0LL]) q.push(link);
        checked[0] = true;
        int count = 1;
        ll ans = 0LL;
        ...
        for ( ; count < n; ) {
                t2 item = q.top(); q.pop();
                ll minussub = get<0>(item);
                ll idx = get<1>(item);

                if (checked[idx]) continue;

                for (auto link : linklist[idx]) q.push(link);
                checked[idx] = true;
                count++;
                ans += (-minussub);
        }
