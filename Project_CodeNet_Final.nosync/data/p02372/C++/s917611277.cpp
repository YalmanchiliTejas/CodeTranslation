...
private : 
    struct Edge {
        ...
        E dat;
        T dp, sum;
        Edge (int to, const E &dat, const T &dp, const T &sum) : 
            to(to), dat(dat), dp(dp), sum(sum) { }
    };
...
public : 
    ...
    void dfs1 (int v, int p) {
        for (const auto &e : g[v]) {
            if (e.to == p) continue;
            dfs1(e.to, v);
            subdp[v] = f(subdp[v], h(subdp[e.to], e.dat));
        }
    }
    ...
    void dfs2 (int v, int p, const T &top) {
        ...
        for (int i = 0; i < (int)g[v].size(); i++) {
            Edge &e = g[v][i];
            e.sum = buff;
            e.dp = h(e.to == p ? top : subdp[e.to], e.dat);
            buff = f(buff, e.dp);
        }
        dp[v] = buff;
        buff = id;
