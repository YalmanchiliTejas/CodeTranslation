...
int main()
{
    ...
    auto dfs = [&](auto&& self, ll now, ll p = -1)->void {
        for(auto &e : G[now]) if(e.to != p){ 
            self(self, e.to, e.from);
            chmax(dist[now], dist[e.to] + e.weight);
        }
    };

    vector<ll> res(N);

    auto dfs2 = [&](auto&& self, ll now, ll d_par, ll p = -1)->void {
        vector<P> d_child;
        d_child.emplace_back(0, -1);
        for(auto &e : G[now]){ 
            if (e.to == p) d_child.emplace_back(d_par + e.weight, e.to);
            else d_child.emplace_back(e.weight + dist[e.to], e.to);
        }
        sort(RALL(d_child));

        res[now] = d_child[0].first;
        
        for(auto &e : G[now]){ 
            if (e.to == p) continue;
            self(self, e.to, d_child[d_child[0].second == e.to].first, now);
        }
    };
