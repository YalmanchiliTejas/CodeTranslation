/**
 * [Spaghetti Source - 木の高さ](http://www.prefield.com/algorithm/graph/tree_height.html)
 */
...
vector<i64> tree_height(const vector<vector<pair<i64,i64>>>& g) {
    i64 n = SIZE(g);
    ...
    auto rec = FIX([&g,&h](auto&& self, i64 v, i64 i) -> void {
        i64& memo = h[v][i].second;
        if(memo >= 0) return;
        i64 to,cost; tie(to,cost) = g[v][i];
        memo = cost;
        REP(j, SIZE(g[to])) {
            if(g[to][j].first == v) continue;
            self(to,j);
            chmax(memo, cost + h[to][j].second);
        }
    });
...
void solve() {
    ...
    auto hs = tree_height(G);

    ALL(for_each, hs, LIFT(PRINTLN));
}
