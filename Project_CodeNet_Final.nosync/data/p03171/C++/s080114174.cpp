...
{
    ll n; llin(n);
    static V<ll> alist; llina(alist, n); alist.pb(-1e18);

    static ll dp[3003][3003];  // (len, idx)
    rep (idx, n) dp[1][idx] = alist[idx] * ((n & 1LL) * 2LL - 1LL);

    srep (len, 2, n+1) {
        bool taro = !((n - len) & 1LL);
        rep (idx, n-len+1) {
            if (taro) {
                dp[len][idx] = dp[len-1][idx+1] + alist[idx];
                dp[len][idx] = max(dp[len][idx],
                                   dp[len-1][idx] + alist[idx+len-1]);
            }
            else {
                dp[len][idx] = dp[len-1][idx+1] - alist[idx];
                dp[len][idx] = min(dp[len][idx],
                                   dp[len-1][idx] - alist[idx+len-1]);
            }
        }
    }

    debug_printf("dp[1][0]=%lld\n", dp[1][0]);
    debug_printf("dp[1][1]=%lld\n", dp[1][1]);
    debug_printf("dp[1][2]=%lld\n", dp[1][2]);
    debug_printf("dp[1][3]=%lld\n", dp[1][3]);

    debug_printf("dp[2][0]=%lld\n", dp[2][0]);
    debug_printf("dp[2][1]=%lld\n", dp[2][1]);
    debug_printf("dp[2][2]=%lld\n", dp[2][2]);
    debug_printf("dp[2][3]=%lld\n", dp[2][3]);

    debug_printf("dp[3][0]=%lld\n", dp[3][0]);
    debug_printf("dp[3][1]=%lld\n", dp[3][1]);
    debug_printf("dp[3][2]=%lld\n", dp[3][2]);
    debug_printf("dp[3][3]=%lld\n", dp[3][3]);

    debug_printf("dp[4][0]=%lld\n", dp[4][0]);
