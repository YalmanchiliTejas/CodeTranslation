...
void input() {
	...
	for(int i=0; i<=N; i++) {
		for(int j=0; j<=N; j++) dp[i][j]=LLINF;
		dp[i][i]=0LL;
	}
}

ll dpslv(int L, int R) {
	if(dp[L][R]!=LLINF) return dp[L][R];
	if((N-(R-L))%2==0) { //先手
		return dp[L][R]=max(a[L]+dpslv(L+1,R),dpslv(L,R-1)+a[R-1]);
	}
	else {
		return dp[L][R]=min(-a[L]+dpslv(L+1,R),dpslv(L,R-1)-a[R-1]);
	}
}
