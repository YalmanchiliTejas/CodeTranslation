...
LL solve( const int T, const int K )
{
	if ( T == N )
	{
		return 0;
	...
	static LL dp[ 1 << 12 ][ 1 << 12 ];
	static bool memoized[ 1 << 12 ][ 1 << 12 ];
	...
	res = t ? INF : -INF;
	const auto func = t
		? ( const LL&(*)( const LL&, const LL & ) )min< LL >
		: ( const LL&(*)( const LL&, const LL & ) )max< LL >;

	res = func( res, ( t ? -1 : 1 ) * A[K] + solve( T + 1, K + 1 ) );
	res = func( res, ( t ? -1 : 1 ) * A[ N - 1 - ( T - K ) ] + solve( T + 1, K ) );
