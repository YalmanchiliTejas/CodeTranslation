...
int dfs(int l,int r,int turn){
	if(used[l][r][turn==-1])return dp[l][r][turn==-1];
	used[l][r][turn==-1]=true;
	if(r-l==1){
		return dp[l][r][turn==-1]=a[l]*turn;
	}
	int ans1=dfs(l+1,r,turn*-1)+a[l]*turn;
	int ans2=dfs(l,r-1,turn*-1)+a[r-1]*turn;
	if(turn==1)return dp[l][r][0]=max(ans1,ans2);
	return dp[l][r][1]=min(ans1,ans2);
}
signed main(){
	cin>>N;
	rep(i,N)cin>>a[i];
