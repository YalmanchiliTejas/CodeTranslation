...
long long int solve(int i,int j,int turn)
{
    ...
    if(turn==1)
    {
        dp[i][j][turn] = max(dp[i][j][turn],arr[i]+solve(i+1,j,0));
        dp[i][j][turn] = max(dp[i][j][turn],arr[j]+solve(i,j-1,0));
    }
    else
    {
        dp[i][j][turn] = min(solve(i+1,j,1),solve(i,j-1,1));
    }
