...
ll getXY(bool taro, ll starting, ll ending) {
	if(starting > ending) return 0;
	if(dp[taro][starting][ending] != -1) return dp[taro][starting][ending];
	
	ll output;
	if(taro) {
		ll leftRemove = numbers[starting] + getXY(!taro, starting + 1, ending);
		ll rightRemove = numbers[ending] + getXY(!taro, starting, ending - 1);
		output = max(leftRemove, rightRemove);
	} else {
		ll leftRemove = -numbers[starting] + getXY(!taro, starting + 1, ending);
		ll rightRemove = -numbers[ending] + getXY(!taro, starting, ending - 1);
		output = min(leftRemove, rightRemove);
	}
	
	dp[taro][starting][ending] = output;
	return output;
}
