...
int mainWA() {
    ...
    if (N & 1) {
        ...
        ll best = max(
            -abs((total0 - aFront) - total1) + aFront,
            -abs((total0 - aBack) - total1) + aBack
        );
        printer << best << '\n';
    }
    else {
        // N is even
        printer << abs(total0 - total1) << '\n';
    }
...
ll dp[3030][3030][2];
bool visited[3030][3030][2];

ll dfs(int hd, int tl, bool turn) {
    if (hd == tl) return 0;

    if (visited[hd][tl][turn]) return dp[hd][tl][turn];

    visited[hd][tl][turn] = true;
    if (turn == true) {
        return dp[hd][tl][turn] = 
            max(A[hd] + dfs(hd + 1, tl, !turn),
                A[tl-1] + dfs(hd, tl - 1, !turn));
    }
    else {
        return dp[hd][tl][turn] =
            min(-A[hd] + dfs(hd + 1, tl, !turn),
                -A[tl - 1] + dfs(hd, tl - 1, !turn));
    }
}
