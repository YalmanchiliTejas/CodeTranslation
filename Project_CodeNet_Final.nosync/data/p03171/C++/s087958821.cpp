...
ll dfs(int left, int right){
  if(dp[left][right] != infLL)
    return dp[left][right];
  else if(left == right) 
    return dp[left][right] = (N % 2 == 1 ? a[left] : -1 * a[left]);
  else{
    return dp[left][right] = ( (right - left + N) % 2 == 1 ?
      max(a[left]+dfs(left+1,right),a[right]+dfs(left,right-1)) :
      min(-1*a[left]+dfs(left+1,right),-1*a[right]+dfs(left,right-1))
    );
  }
}
...
int main(){
  ...
  out(dfs(0 , N - 1));
  //rep(i,N) rep(j,N) if(i <= j)trc(i,j,dp[i][j]);
