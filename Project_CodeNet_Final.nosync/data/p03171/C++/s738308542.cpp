...
public:
    void solve() {
        int N = in.readInt();
        arri a = in.readIntArray(N);
        arr3d<ll> dp(N, N, 2, 0);
        arr3d<bool> state(N, N, 2, false);
        RecursiveFunction rec = [&] (auto const& self, int l, int r, int t) -> ll {
            if (l > r)
                return 0;
            if (state(l, r, t))
                return dp(l, r, t);
            state(l, r, t) = true;
            if (t == 0) {
                return dp(l, r, t) = max(a[l] + self(l + 1, r, 1), a[r] + self(l, r - 1, 1));
            } else {
                return dp(l, r, t) = min(-a[l] + self(l + 1, r, 0), -a[r] + self(l, r - 1, 0));
            }
        };
        ll ans = rec(0, N - 1, 0);
        out.printLine(ans);
