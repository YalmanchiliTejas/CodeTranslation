...
int main()
{
    ...
    a.resize(N);
    cin >> a;
    vector<vector<lint>> dp; // i手目で，左からj個取られてるとき，今後加わるX-Y
    ndarray(dp, N + 2, N + 1);

    IFOR(i, 1, N + 1) REP(j, i)
    {
        if (i & 1) dp[i][j] = max(dp[i + 1][j] + a[N - (i - j)], dp[i + 1][j + 1] + a[j]);
        else dp[i][j] = min(dp[i + 1][j] - a[N - (i - j)], dp[i + 1][j + 1] - a[j]);
    }
    cout << dp[1][0] << endl;
}
