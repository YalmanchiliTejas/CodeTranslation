...
bool fnd[1000];
double cl4(double a, int t, double b) {
	switch (t)
	{
	case 0:
		return a + b;
...
void clc5(double a, int t1, double b) {
	double ab = cl4(a, t1, b);
	if (ab != INF && ab > 0 && ab < 1000 && (int)ab == ab) {
		fnd[(int)ab] = 1;
	}
}
void clc4(double a, int t1, double b, int t2, double c) {

	double ab = cl4(a, t1, b);
	double bc = cl4(b, t2, c);
	if (ab != INF) {
		clc5(ab, t2, c);
	}
	if (bc != INF) {
...
void clc3(double a,int t1, double b,int t2, double c,int t3, double d) {

	double ab = cl4(a, t1, b);
	double bc = cl4(b, t2, c);
	double cd = cl4(c, t3, d);
	if (ab != INF) {
		clc4(ab, t2, c, t3,d);
	}
	if (bc != INF) {
...
void clc(ll a, ll b, ll c, ll d) {

	ll t[] = { a,b,c,d };
	do {
		clc2(t[0], t[1], t[2], t[3]);
...
ll memo[2][3010][3010];
ll a[3010];
ll rec(bool p, ll left, ll right) {

	if (~memo[p][left][right])return memo[p][left][right];
	if (left == right) return memo[p][left][right] = (p ? a[left] : -a[left]);
	ll res = 0;
	if (p) {
		res = max(a[left] + rec(!p, left + 1, right), a[right] + rec(!p, left, right - 1));
	}
	else {
		res = min(rec(!p, left + 1, right) - a[left], rec(!p, left, right - 1) - a[right]);
	}
	return memo[p][left][right] = res;
}
void solv() {
	cin >> n;
	for (size_t i = 0; i < n; i++)
