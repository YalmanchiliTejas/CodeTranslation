		...
		F0R(i, n)
		{
			...
			else
			{
				if (i < n - 1)
					...
					dp[i][j][0] = A[i];
				if (j > 0)
					dp[i][j][0] = maximum(dp[i][j][0], A[j] + dp[i][j - 1][1]);
				else
					dp[i][j][0] = maximum(dp[i][j][0], A[j]);
				if (i < n - 1)
					dp[i][j][1] = -A[i] + dp[i + 1][j][0];
				else
					dp[i][j][1] = minimum(dp[i][j][1], -A[i]);
				if (j > 0)
					dp[i][j][1] = minimum(dp[i][j][1], -A[j] + dp[i][j - 1][0]);
				else
					dp[i][j][1] = minimum(dp[i][j][1], -A[j]);
			}
		}	
	}
	//print_vector(dp);
