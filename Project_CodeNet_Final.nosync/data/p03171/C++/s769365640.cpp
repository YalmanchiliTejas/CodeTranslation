...
#define rep(i,s,e) for(decltype(s) i=(s), i##_len=(e); i<i##_len; ++i)
#define repR(i,e,s) for(decltype(e) i=(e), i##_len=(s); i>=i##_len; --i)
...
ll solve(int left, int right, int turnPlayer) {

	ll key = left * 10000 + right;
	...
	ll result = 0;
	ll val1 = solve(left + 1, right, (turnPlayer + 1) % 2) + (turnPlayer == 0 ? a[left] : -a[left]);
	ll val2 = solve(left, right - 1, (turnPlayer + 1) % 2) + (turnPlayer == 0 ? a[right] : -a[right]);
	if (turnPlayer == 0) {
