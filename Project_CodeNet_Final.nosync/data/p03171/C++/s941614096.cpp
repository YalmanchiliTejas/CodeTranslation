...
void solve() {
  ...
  vi32 a(n);
  cin >> a;
  rep(i, 2) rep(j, 3001) rep(k, 3001) dp[i][j][k] = -1e18;
  function<i64(int, int, int)> rec = [&](int turn, int l, int r) {
    if (r - l == 1) {
      if (turn == 0) return (i64) a[l];
      if (turn == 1) return (i64) -a[l];
    }
    if (dp[turn][l][r] != -1e18) return dp[turn][l][r];
    i64 ret = -1e18;
    if (turn == 0) {
      amax(ret, rec(1 - turn, l + 1, r) + a[l]);
      amax(ret, rec(1 - turn, l, r - 1) + a[r - 1]);
    } else {
      ret = 1e18;
      amin(ret, rec(1 - turn, l + 1, r) - a[l]);
      amin(ret, rec(1 - turn, l, r - 1) - a[r - 1]);
    }
    return dp[turn][l][r] = ret;
  };
  cout << rec(0, 0, n) << endl;
}
