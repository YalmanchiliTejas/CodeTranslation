...
class Game{
    const int n;
    std::vector<int> A;
    Wlli Diff; // 0, 1, 2, 3, ..., n-1 : [x, y)での値
public:
    Game(int nn): n(nn){
        A.resize(nn);
        Diff.resize(nn+1, Vlli(nn+1, -INFl));
    }
    ~Game(){};
    void inputA(void){
        REP(i, n) scanf("%d", &A[i]);
    }
    lli doDP(int l, int r, bool senko){
        if(Diff[l][r] != -INFl) return Diff[l][r];
        if(l >= r) return Diff[l][r] = 0;
        lli LeftChoose = doDP(l+1, r, not senko);
        lli RightChoose = doDP(l, r-1, not senko);
        if(senko) return Diff[l][r] = std::max(LeftChoose+A[l], RightChoose+A[r-1]);
        else return Diff[l][r] = std::min(LeftChoose-A[l], RightChoose-A[r-1]);
    }
    void solve(void){
        printf("%lld\n", doDP(0, n, true));
