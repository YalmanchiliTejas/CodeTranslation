...
int main()
{
    ...
    vec2 dp(N + 1, vec(N + 1, -1));

    auto f = [&](auto&& self, ll x, ll y, ll turn)->ll {
        if(x > y) return 0;
        if(dp[x][y] != -1) return dp[x][y];
        if(turn % 2 == 0) return dp[x][y] = max(self(self, x + 1, y, !turn) + A[x], self(self, x, y - 1, !turn) + A[y]);
        else return dp[x][y] = min(self(self, x + 1, y, !turn) - A[x], self(self, x, y - 1, !turn) - A[y]);
    };
