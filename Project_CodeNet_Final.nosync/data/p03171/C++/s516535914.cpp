...
ll memo[2][3010][3010];
ll a[3010];
ll recg(int tn, int fr, int en) {

	if (en < fr) return 0;
	if (~memo[tn][fr][en]) return memo[tn][fr][en];
	ll *res = &memo[tn][fr][en];
	int next = (tn + 1) % 2;
	if (tn == 0) {
		*res = max(recg(next , fr + 1, en) + a[fr], recg(next, fr, en - 1) + a[en]);
	}
	else {
		*res = min(recg(next, fr + 1, en) - a[fr], recg(next, fr, en - 1) - a[en]);
	}
