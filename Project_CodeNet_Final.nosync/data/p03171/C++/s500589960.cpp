...
vector<ll>a;
ll dp[3005][3005][2];
ll find(ll ls,ll rs,ll player)
{
    if(ls>rs)
        return 0;
    ll &ans=dp[ls][rs][player];
    if(ans!=-1)
    {
        return ans;
    }

    if(player==0) //1st player maximize (X-Y)
    {
        return ans=max(a[ls]+find(ls+1,rs,player^1),a[rs]+find(ls,rs-1,player^1));
    }
    else  //2nd player minimize (X-Y) we do not of player 1 because we have to maximize -Y ,minimize Y
    {
        return ans=min(-a[ls]+find(ls+1,rs,player^1),-a[rs]+find(ls,rs-1,player^1));
    }
}
int main()
{
