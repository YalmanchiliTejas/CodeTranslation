...
ll solve(int lidx, int ridx, int flag){
	if(lidx > ridx)return 0;
	if(dp[lidx][ridx][flag] != -1000000000000000)return dp[lidx][ridx][flag];
	ll ans;

	if(flag == 0){
		ans = max(a[lidx] + solve(lidx + 1, ridx, 1), a[ridx] + solve(lidx, ridx - 1, 1));
	}
	else{
		ans = min(-a[lidx] + solve(lidx + 1, ridx, 0), -a[ridx] + solve(lidx, ridx - 1, 0));
	}

	return dp[lidx][ridx][flag] = ans;
}
