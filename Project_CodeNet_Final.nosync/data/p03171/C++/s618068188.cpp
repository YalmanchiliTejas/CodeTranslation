...
int main() {
	...
	vll a(n);
	rep(i, n) cin >> a[i];
	ll dp[n + 1][n + 1];  // dp[i][j]は区間[i,j)が残ったときの得点
	...
	rep(i, n) {
		rep(j, n) {
			...
			ll sign = 1;
			if((n - i - 1) % 2) sign = -1;
			// dp[j][i+j+1]のときの次の手番が先手なら1後手なら-1
			dp[j][i + j + 1] = dp[j][i + j] + sign * a[i + j];
			if(sign == 1)
				dp[j][i + j + 1] = max(dp[j][i + j + 1], dp[j + 1][i + j + 1] + sign * a[j]);
			else
				dp[j][i + j + 1] = min(dp[j][i + j + 1], dp[j + 1][i + j + 1] + sign * a[j]);
		}
	}
