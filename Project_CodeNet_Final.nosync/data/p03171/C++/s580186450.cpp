...
int solve(int player, int l, int r){
    if(l > r) return 0;
    if(l == r){
        if(player) return a[l];
        else return (-a[l]);
    }
    int &res = dp[player][l][r];
    if(vis[player][l][r]) return res;
    vis[player][l][r] = true;
    if(player)
        res = max(solve(!player, l + 1, r) + a[l], solve(!player, l, r - 1) + a[r]);
    else
        res = min(solve(!player, l + 1, r) - a[l], solve(!player, l, r - 1) - a[r]);
    return res;
}
