...
int main(){
    ...
    vec a(n);
    rep(i,n)cin>>a[i];
    auto dp=make_vector<lint>(INF,n,n);
    auto f=[&](auto f,lint l,lint r,bool b)->lint{
        if(r<l)return 0;
        if(dp[l][r]!=INF)return dp[l][r];
        lint ans=0;
        if(b)ans=min(f(f,l+1,r,1-b)-a[l],f(f,l,r-1,1-b)-a[r]);
        else ans=max(f(f,l+1,r,1-b)+a[l],f(f,l,r-1,1-b)+a[r]);
        return dp[l][r]=ans;
    };
    cout<<f(f,0,n-1,0)<<endl;
}