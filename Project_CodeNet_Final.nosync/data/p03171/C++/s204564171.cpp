...
constexpr unsigned int MAX_N = 3000;
int A[MAX_N];
long long int DP[MAX_N][MAX_N + 1];
bool done[MAX_N][MAX_N + 1];

long long int dp(unsigned int l, unsigned int r, bool senko){
    if(l >= r) return 0;
    if(done[l][r]) return DP[l][r];
    done[l][r] = true;
    if(senko){
        const long long int&& p1 = dp(l + 1, r, false) + A[l];
        const long long int&& p2 = dp(l, r - 1, false) + A[r - 1];
        return DP[l][r] = (p1 > p2) ? p1 : p2;
    }
    const long long int&& p1 = dp(l + 1, r, true) - A[l];
    const long long int&& p2 = dp(l, r - 1, true) - A[r - 1];
    return DP[l][r] = (p1 < p2) ? p1 : p2;
}
