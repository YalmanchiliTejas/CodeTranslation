...
int game(int i, int j) {
	if (dp[i][j] != INT64_MAX) return dp[i][j];
	if (!((N - (j - i)) & 1)) //taro
		return dp[i][j] = max(game(i, j - 1) + a[j - 1], game(i + 1, j) + a[i]);
	else //jiro
		return dp[i][j] = min(game(i, j - 1) - a[j - 1], game(i + 1, j) - a[i]);
}
...
signed main() {
	...
	REP(i, N + 1) REP(j, N + 1) dp[i][j] = INT64_MAX;
	REP(i, N) dp[i][i + 1] = (N & 1 ? a[i] : -a[i]);
