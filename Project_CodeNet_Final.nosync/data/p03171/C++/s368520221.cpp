...
#define vvvi(a,b,c,d) vector<vector<vi>> a(b, vector<vi>(c, vi(d)))
#define vvvd(a,b,c,d) vector<vector<vd>> a(b, vector<vd>(c, vd(d)))
...
public :
    ...
    bool isleaf(int v, int r) {
        return g[v].size() == 1 && g[v][0].from != r;
    }
...
vi A,B,C;
vvi (dp,3030,3030);// st end  a-b
vvb (was,3030,3030);
int dfs(int s,int t){
    if(was[s][t])return dp[s][t];
    bool f = (N-(t+1-s))%2==0;
    if(f){
        was[s][t] = 1;
        return dp[s][t] = max(dfs(s+1,t) + A[s], dfs(s,t-1) + A[t]);
    }else{
        was[s][t] = 1;
        return dp[s][t] = min(dfs(s+1,t) - A[s], dfs(s,t-1) - A[t]);
    }
}
signed main() {
    cin>>N;
    addn(A,N);
    rep(i,N)dp[i][i] = A[i] * (N % 2 ? 1 : -1),was[i][i]=1;
