...
LL f(int N, LL i, LL j, VLL &a, VVLL &dp) {
    if (dp[i][j] != INF) return dp[i][j];
    if (i == j) return dp[i][j] = 0;
    bool left = true;
    if ((N - (j - i)) % 2) left = false;
    if (left) {
        LL maxV = LONG_LONG_MIN;
        chmax(maxV, a[i] + f(N, i + 1, j, a, dp));
        chmax(maxV, a[j - 1] + f(N, i, j - 1, a, dp));
        return dp[i][j] = maxV;
    } else {
        LL minV = LONG_LONG_MAX;
        chmin(minV, -a[i] + f(N, i + 1, j, a, dp));
        chmin(minV, -a[j - 1] + f(N, i, j - 1, a, dp));
        return dp[i][j] = minV;
    }
}
