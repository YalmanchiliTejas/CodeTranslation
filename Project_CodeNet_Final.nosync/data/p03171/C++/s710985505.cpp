...
lli score(int turn, int left, int right)
{
  if( dp[turn][left][right] > init )
    {
  ...
  if( right < left )
    {
      return dp[turn][left][right] = 0;
    }

  if( turn == 0 )
    {
      return dp[turn][left][right] = max(score((turn+1)&1,left+1,right) + a[left],
					 score((turn+1)&1,left,right-1) + a[right]);
    }

  return dp[turn][left][right] = min(score((turn+1)&1,left+1,right) - a[left],
				     score((turn+1)&1,left,right-1) - a[right]);
