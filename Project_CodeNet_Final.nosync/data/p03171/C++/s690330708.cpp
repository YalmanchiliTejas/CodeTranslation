...
vector<vector<LL>> dp;
LL func(int i, int l, int r, vector<LL> &A) {
    if (l > r) {
        return 0;
    }

    if (dp[l][r] != LONG_LONG_MIN) {
        return dp[l][r];
    }

    LL a = func(i + 1, l + 1, r, A);
    LL b = func(i + 1, l, r - 1, A);

    if (i % 2 == 0) {
        return dp[l][r] = max(a + A[l], b + A[r]);
    }
    else {
        return dp[l][r] = min(a - A[l], b - A[r]);
    }
}
...
int main() {
    ...
    dp.resize(N + 10, vector<LL>(N + 10, LONG_LONG_MIN));
    print(func(0, 0, N - 1, A));
