...
pii solve(int l, int r, bool turn)
{
	if(done[l][r][turn])
	{
		return dp[l][r][turn];
	...
	if(l == r)
	{
		if(turn)
		{
			return dp[l][r][1] = mp(0, a[l]);
		}
		else return dp[l][r][0] = mp(a[l], 0);
	}
	if(turn)
	{
		//minimise (x - y)
		pii s1 = solve(l + 1, r, 1^turn);
		pii s2 = solve(l, r-1, 1^turn);
		s1.ss += a[l];
		s2.ss += a[r];
		if(s1.ff-s1.ss > s2.ff-s2.ss)
		{
			return dp[l][r][turn] = s2;
		}
		else
		{
			return dp[l][r][turn] = s1;
		}
	}
	else
	{
