...
void get_query(string a,vector<pair<string,int> > & b){
  ...
  if ( b.size() == 0)return;
  if ( a[a.size()-1]=='/')b.push_back(make_pair("index.html",FILE) );
...
void solve(string *q,Tree *root){
  ...
  //cout << "case0 " <<data[0]->myname << " case1 " << data[1]->myname << endl;
  //cout << "case0 " <<data[0] << " case1 " << data[1] << endl;
}
