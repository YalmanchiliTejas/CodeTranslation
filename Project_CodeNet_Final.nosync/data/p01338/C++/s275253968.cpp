...
int rec(int x,int y,int s){
    ...
    for(int i = 0; i <= 3; i++){
        if(x==0){
            ...
            setValue(ns,2,(max(0,field[y+1][x+1])+i)%4);
            // ¶ãÌêÌXRAðmè³¹é
            int res=0;
            if(field[y][x]!=-1)
                res+=seiseki[(getValue(s,2)+i)%4];
            maxCost=max(maxCost,res+rec(x+1,y,ns));
        }
        else if(x==2){
            int ns=0;
            ...
            setValue(ns,3,(max(0,field[y+1][x+1])+i)%4);
            // ¶ãÌêÌXRAðmè³¹é
            int res=0;
            if(field[y][x]!=-1)
                res+=seiseki[(getValue(s,3)+i)%4];
            maxCost=max(maxCost,res+rec(x+1,y,ns));
        }
        else if(x==3){
            int ns=0;
            ...
            setValue(ns,4,(max(0,field[y+1][x+1])+i)%4);
            // ¶ãÌêÌXRAðmè³¹é
            int res=0;
            if(field[y][x]!=-1)
                res+=seiseki[(getValue(s,4)+i)%4];
            // EãÌXRAàmè³¹é
            if(field[y][x+1]!=-1)
                res+=seiseki[(getValue(s,5)+i)%4];
            maxCost=max(maxCost,res+rec(x+1,y,ns));
        }
    }
