...
void solve(ostringstream& aout, long long N, long long M, long long S, std::vector<long long> U, std::vector<long long> V, std::vector<long long> A, std::vector<long long> B, std::vector<long long> C, std::vector<long long> D);
void solve_TLE(ostringstream& aout, long long N, long long M, long long S, std::vector<long long> U, std::vector<long long> V, std::vector<long long> A, std::vector<long long> B, std::vector<long long> C, std::vector<long long> D);

class StressTest {
private:
	mt19937 m_RandEngine;
	bool judge_case(long long N, long long M, long long S, std::vector<long long> U, std::vector<long long> V, std::vector<long long> A, std::vector<long long> B, std::vector<long long> C, std::vector<long long> D) {
		ostringstream fast, tle;
		solve(fast, N, M, S, std::move(U), std::move(V), std::move(A), std::move(B), std::move(C), std::move(D));
		solve_TLE(tle, N, M, S, std::move(U), std::move(V), std::move(A), std::move(B), std::move(C), std::move(D));
		if (fast.str() == tle.str()) {
			return true;
		}
		else {
...
public:
	...
	void test() {
		while (1) {
			...
			//if (!judge_case(N, M, S, std::move(U), std::move(V), std::move(A), std::move(B), std::move(C), std::move(D))) {
				// TODO: output case
				//break;
			//}
		}
