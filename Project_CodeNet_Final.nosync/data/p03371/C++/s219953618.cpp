...
# endif
    ...
    namespace detail
    {
        ...
        {
            std::unique_ptr<FormatData> formatData;
            DebugBuffer() : formatData(std::make_unique<FormatData>()) {}
            DebugBuffer(DebugBuffer&& o) noexcept : formatData(std::move(o.formatData)) {}
            ~DebugBuffer() { if (formatData) std::cerr << formatData->string << '\n'; }
            template <class Type> DebugBuffer& operator <<(const Type& x) { Formatter(*formatData, x); return *this; }
        };

        struct Debug_impl
        {
            void operator()(const String& text) const { std::cerr << text << '\n'; }
            template <class... Args> void operator()(const Args&... args) const { operator()(Format(args...)); }
            template <class Type, class = decltype(Formatter(std::declval<FormatData&>(), std::declval<Type>()))>
            DebugBuffer operator <<(const Type& x) const { DebugBuffer b; Formatter(*b.formatData, x); return b; }
        };
    }

    constexpr auto Debug = detail::Debug_impl();
...
# endif
	...
	inline Array<String> ReadWordArray(size_t n) { return ReadArray_impl<String>(n); }
	...
	// 整数の桁数を返す
	template <class Integer> Integer GetDigit(Integer n) noexcept
    {
        Integer d = 0;
        while (n)
        {
    ...
    // aがbよりも大きければ `true` を返して、a=bとする
    template<class T> constexpr bool chmax(T &a, const T &b) noexcept { if (a<b) { a=b; return 1; } return 0; }

    // aがbよりも小さければ `true` を返して、a=bとする
    template<class T> constexpr bool chmin(T &a, const T &b) noexcept { if (b<a) { a=b; return 1; } return 0; }
    ...
    // Array配列の要素数を符号付整数型で返す
    template <class Type>
    constexpr int32 len(Array<Type> ary)
    {
        return ary.size();
    }
    ...
    template <class Type>
    constexpr Type min(const yat::Array<Type>& a) noexcept
    {
        Type m;
        for (yat::int32 i=0; i<len(a); ++i)
            if (a[i] < m)
                m = a[i];
        return m;
    }
    template <class Type>
    constexpr Type max(const yat::Array<Type>& a) noexcept
    {
        Type m;
        for (yat::int32 i=0; i<len(a); ++i)
            if (a[i] > m)
