...
int cost(int ax, int bx, int abx) { return ax * a + bx * b + abx * c; }
...
int main() {
    ...
    ans = cost(x, y, 0);
    if (x < y) {
        ans = min(ans, cost(0, y - x, 2 * x));
        ans = min(ans, cost(0, 0, 2 * y));
    } else {
        ans = min(ans, cost(x - y, 0, 2 * y));
        ans = min(ans, cost(0, 0, 2 * x));
    }
    print(ans);
}
