...
void divisor(int64_t n) {
    ...
    for(int64_t i = 1; i * i <= n; i++) {
        if(n % i == 0) {
            ...
            if(A[i]>=1 and n!=1){
                cnt++;
                //cout<<n<<" has factor "<<i<<endl;
                return;
            }
            if(A[i]>1 and n==1){
                cnt++;
                //cout<<n<<" has factor(for1) "<<i<<endl;
                return;
            }
            if(i * i != n) {
                ret.push_back(n / i);
                ...
                if(n==n/i and A[n]>1){
                    cnt++;
                    //cout<<n<<" has juukai factor "<<i<<endl;
                    return;
                }
                if(n!=n/i and A[n/i]>=1){
                    cnt++;
                    //cout<<n<<" has factor2 "<<n/i<<endl;
                    return;
                }
            }
        }
...
int main() {
    ...
    IOS;
    prec;
    Int a,b,c,n,m,k,x,y=0,ans=MAX_N_Int;
    string str,s="",t,u;
    cin>>a>>b>>c>>x>>y;
    n=x;
    m=y;
