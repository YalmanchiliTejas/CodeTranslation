...
signed main(void){
	...
	int a, b, c, x, y, min = INF;
	cin >> a >> b >> c >> x >> y;

	if(c*2*max(x, y) < min) min = c*2*max(x, y);
	if(a*x + b*y < min) min = a*x + b*y;
	if(x>=y && c*2*y + a*(x-y) < min) min = c*2*y + a*(x-y);
	if(x<y && c*2*x + b*(y-x) < min) min = c*2*x + b*(y-x);
