...
struct segtako{
  vector<P<double,double>> seg;
  int b;
  segtako(int a){
    b=1;
    while(b<a)
      b*=2;
    seg=vector<P<double,double>>(2*b-1,{1,0});
    fo(i,a){
      seg[i+b-1]={1,0};
    }
    for(int i=b-2;i>=0;i--){
      seg[i]={seg[2*i+1].fi*seg[2*i+2].fi,seg[2*i+1].se*seg[2*i+2].fi+seg[2*i+2].se};
    }
  }
  void update(int i,P<double,double> a){
    i+=b-1;
    seg[i]=a;
    while(i){
      i=(i-1)/2;
