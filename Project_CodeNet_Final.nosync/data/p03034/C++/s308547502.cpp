...
signed main() {
  ...
  function<bool(int, int)> invalid = [&](int A, int B) {
    if (A <= 0 || B <= 0) return true;
    if (A <= B) return true;
    int g = gcd(A, B);
    if (A / g - B / g != 1) return false;
    int mx = (A / g + B / g + 1) * g;
  ...
  REP(d, 1, N) {
  // REP(d, 2, N) {
    vector<ll> acc_a(1, 0), acc_b(1, 0);
    {
      // 前から : acc_a
      for (int i = d; i < N; i += d)
        acc_a.emplace_back(acc_a[i / d - 1] + S[i]);
    }
    {
      // 後ろから : acc_b
      for (int i = N - 1 - d, j = 0; i >= 0; i -= d)
        acc_b.emplace_back(acc_b[j++] + S[i]);
    }
    // debug(acc_a, acc_b);
    int sz = acc_a.size();
    REP(i, sz) {
      ...
      int B = A - d;
      // print(d, i, A, B);
      // if (A == 3 && B == 2) print(111111);
      if (invalid(A, B)) continue;
      // print(d, i, A, B);
      chmax(res, acc_a[i] + acc_b[i]);
    }
  }
