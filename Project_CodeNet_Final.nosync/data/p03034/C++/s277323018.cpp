...
#define forit(st, l, r) for (auto &&it = st.lower_bound(l); it != st.end() && (*it) < r; it++)
...
template<typename V, typename H> void resize(vector<V> &vec,
        const H head
) {
    vec.
            resize(head);
}
template<typename V, typename H, typename ... T> void resize(vector<V> &vec,
        const H &head,
        const T ... tail
) {
    vec.
...
template<class T> bool contains(const std::string &s, const T &v) { return s.find(v) != std::string::npos; }
...
template<class T, class U> void replace(vector<T> &a, T
key,
        U v
) {
    replace(a
...
void replace(str &a, str key, str v) {
    if (v == "")a.erase(remove(all(a), key[0]), a.end());
    else replace(all(a), key[0], v[0]);
}
void replace(str &a, char key, char v) { replace(all(a), key, v); }
//keyと同じかどうか01で置き換える
template<class T, class U> void replace(vector<T> &a, U
k) {
    rep(i, sz(a)) a[i] = a[i] ==
                         k;
}
template<class T, class U> void replace(vector<vector<T >> &a, U
k) {
    rep(i, sz(a))rep(j, sz(a[0])) a[i][j] = a[i][j] ==
                                            k;
}
...
template<class T> inline void sort(vector<T> &a, int
len) {
    sort(a
                 .
                         begin(), a
...
template<class T, class F> inline void sort(vector<T> &a, F
f) {
    sort(a
                 .
                         begin(), a
...
inline void sort(vector<P> &a, ___pcomparator
type) {
    switch (type) {
        case fisi:
            sort(all(a),
                 [&](
                         P l, P
                 r) {
                     return l.fi != r.fi ? l.fi < r.fi : l.se < r.
                             se;
                 });
            break;
        case fisd:
        ...
        case sifi:
            sort(all(a),
                 [&](
                         P l, P
                 r) {
                     return l.se != r.se ? l.se < r.se : l.fi < r.
                             fi;
                 });
            break;
        case sifd:
...
inline void sort(vector<T> &a, ___pcomparator
type) {
    switch (type) {
        case fisi:
            sort(all(a),
...
template<class T> inline void rsort(vector<T> &a, int
len) {
    sort(a
                 .
                         begin(), a
...
template<class U, class F> inline void rsort(vector<U> &a, F
f) {
    sort(a
                 .
                         begin(), a
...
//F = T<T>
//例えばreturn p.fi + p.se;
template<class U, class F> inline void sortp(vector<U> &a, vector<U> &b, F
f) {
    vp c;
    int n = sz(a);
    assert(n == sz(b));
...
template<class U, class F> inline void sortp(vector<U> &a, vector<U> &b, char
type) {
    vp c;
    int n = sz(a);
    assert(n == sz(b));
...
template<class U, class F> inline void rsortp(vector<U> &a, vector<U> &b, F
f) {
    vp c;
    int n = sz(a);
    assert(n == sz(b));
...
template<class U, class F> inline void sortt(vector<U> &a, vector<U> &b, vector<U> &c, F
f) {
    vt r;
    int n = sz(a);
    assert(n == sz(b));
...
template<class U, class F> inline void rsortt(vector<U> &a, vector<U> &b, vector<U> &c, F
f) {
    vt r;
    int n = sz(a);
    assert(n == sz(b));
...
template<class U> inline void rsortt(vector<U> &a, vector<U> &b, vector<U> &c) {
    vt r;
    int n = sz(a);
    assert(n == sz(b));
    assert(n == sz(c));
...
template<typename V, typename T> void fill(vector<V> &vecc, const T vall) { for (auto &&vx: vecc)fill(vx, vall); }
template<typename V, typename T> void fill(vector<V> &vecc, const T vall, const vi &ind) {
    for (auto &&i: ind)vecc[i] = vall;
}
...
//    p2.resize(mint_len);p[0] = 1; for (int i = 1; i < mint_len; ++i) p2[i] = p2[i - 1] * 2 % MOD;
    ...
    for (int i = 1; i < mint_len; ++i) fac[i] = fac[i - 1] * i % MOD, finv[i] = finv[i - 1] * inv[i] % MOD;
}
#define smod setmod
...
template<typename V, typename T> int find(vector<V> &a,
        const T key
) {
    rep(i, sz(a))if (a[i] == key)
            return
                    i;
    return -1;
}
template<typename V, typename T> P find(vector<vector<V >> &a,
        const T key
) {
    rep(i, sz(a)) rep(j, sz(a[0]))if (a[i][j] == key)
                return
                        mp(i, j
                        );
    return mp(-1, -1);
}
template<typename V, typename U> T find(vector<vector<vector<V >>> &a,
        const U key
) {
    rep(i, sz(a))rep(j, sz(a[0]))rep(k, sz(a[0][0]))if (a[i][j][k] == key)
                    return
                            mt(i, j, k
                            );
    return mt(-1, -1, -1);
...
int count(str a, str k) {
    ...
    auto pos = a.find(k);
    while (pos != string::npos)pos = a.find(k, pos + len), ret++;
    return ret;
}
...
struct sint {
    ...
    int set(int i, int k) {
        if (i < 0)return top(abs(i) - 1, k);
        return v += p10[i] * (k - (v / p10[i]) % 10);
    }
    int inc(int i, int k = 1) {
        return v += p10[i] * k;
    }
    int inct(int i, int k = 1) {
        return v += p10[keta(v) - i - 1] * k;
    }
    int dec(int i, int k = 1) {
        return v -= p10[i] * k;
    }
    int dect(int i, int k = 1) {
        return v -= p10[keta(v) - i - 1] * k;
    }
...
template<class T> T sum(T &v) { return v; }
template<class T> T sum(vector<T> &v) {
    T ret = 0;
    rep(i, sz(v))ret += sum(v[i]);
    return ret;
}
template<class T> inline auto sum(priority_queue<T, vector<T>, greater<T> > &r) {
    auto q = r;
    T ret = 0;
    while (sz(q)) {
        ret += q.top();
...
template<class T> inline auto sum(priority_queue<T> &r) {
    auto q = r;
    T ret = 0;
    while (sz(q)) {
        ret += q.top();
...
inline void clear(PQ &q) { q = PQ(); }
template<class T> inline void clear(queue<T> &q) { q = queue<T>(); }
...
//kと同じものの数
template<class T, class U> inline vi imo(vector<T> &a, U
k) {
    vector<T> ret = a;
...
template<class T, class U> void inc(vector<T> &a, U
v = 1
) {
    for (
        auto &u
...
template<class T, class U> void plus(vector<T> &a, U
v = 1
) {
    for (
        auto &u
...
template<class T, class U> void dec(vector<T> &a, U
v = 1
) {
    for (
        auto &u
...
template<class T, class U> void minu(vector<T> &a, U
v = 1
) {
    for (
        auto &u
...
template<class T> inline bool inside(vector<T> &a, int i, int j = 0, int k = 0, int l = 0) { return min({i, j, k, l}) >= 0 && max({i, j, k, l}) < sz(a); }
#define ins inside
...
vi compress(vi &a, umap<int, int> &map) {
    vi b;
    int len = a.size();
    for (int i = 0; i < len; ++i) {
        b.push_back(a[i]);
...
vi compress(vi &a, vi &r) {
    ...
    sort(b);
    unique(b);
    for (int i = 0; i < len; ++i) a[i] = lower_bound(all(b), a[i]) - b.begin();
    for (int i = 0; i < sz(r); ++i) r[i] = lower_bound(all(b), r[i]) - b.begin();
    int blen = sz(b);
    vi ret(blen);
    rep(i, blen) {
        ret[i] = b[i];
...
vi compress(vi &a, vi &r, vi &s) {
    ...
    for (int i = 0; i < len; ++i) a[i] = lower_bound(all(b), a[i]) - b.begin();
    for (int i = 0; i < sz(r); ++i) r[i] = lower_bound(all(b), r[i]) - b.begin();
    for (int i = 0; i < sz(s); ++i) s[i] = lower_bound(all(b), s[i]) - b.begin();
    int blen = sz(b);
    vi ret(blen);
    rep(i, blen) {
        ret[i] = b[i];
...
//                if (i != v / i)res.pb(v / i);
        ...
        rer(i, sz(res) - 1) {
            if (res[i] != v / res[i])res.pb(v / res[i]);
        }
        return res;
    }
...
priority_queue<T, vector<T>, greater<T> > &operator+=(priority_queue<T, vector<T>, greater<T> > &a, vector <U> &v) {
    fora(d, v)a.push(d);
    return a;
}
template<class T, class U>
...
template<class T, class U> vector<T> &operator+=(vector<T> &a, U
v) {
    a.
            pb(v);
    return
...
template<class T, class U> vector<T> &operator+=(vector<T> &a, initializer_list<U>
v) {
    for (
        auto &&va
            :v)
...
template<typename T> void remove(vector<T> &v, unsigned int
i) {
    v.
            erase(v
                          .
...
template<typename T> void removen(vector<T> &v, unsigned int
s,
        unsigned int n
) {
    v.
...
template<typename T> void erase(vector<T> &v, unsigned int
i) {
    v.
            erase(v
                          .
...
template<typename T> void erasen(vector<T> &v, unsigned int
s,
        unsigned int n
) {
    v.
...
template<typename T, typename U> void insert(vector<T> &v, unsigned int
i,
        U t
) {
    v.
...
template<typename T, typename U> void push_front(vector<T> &v, U
t) {
    v.
            insert(v
                           .
...
template<typename T, typename U> void insert(vector<T> &v, initializer_list<U>
list) {
    for (
        auto &&va
            :list)
...
void solve() {
    ...
    rep(c, 1, n) {
        ...
        rep(inf) {
            ...
            if(a<=0||b<=0)break;
            if(a <= b)break;
            if (was.count(l))break;
            was += l;
            if (was.count(r))break;
            was += r;
            if(!ins(s,l,r))break;
            su += s[l] + s[r];
            chma(su);
