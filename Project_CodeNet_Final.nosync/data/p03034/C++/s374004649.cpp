...
{
    ll n; llin(n);
    S V<ll> slist; llina(slist, n);

    ll ans = 0;

    srep (width, 1, n) {
        debug_printf("------- width=%lld\n", width);
        set<ll> checked;
        checked.insert(0);
        checked.insert(n-1);
        ll a = n - width;
        ...
        for (;;) {
            if (a <= 0) break;
            if (b <= 0) break;
            if (checked.find(idx1) != checked.end()) break;
            if (checked.find(idx2) != checked.end()) break;
            if (idx1 == idx2) break;

            checked.insert(idx1);
            checked.insert(idx2);
            debug_printf("idx1=%lld idx2=%lld\n", idx1, idx2);
            total += slist[idx1];
            total += slist[idx2];
            ans = max(ans, total);
