...
#include <bits/stdc++.h>
using namespace std;
using ll = long long; using pii = pair<int, int>; using vi = vector<int>; using vvi = vector<vi>; using pll = pair<ll, ll>; using vl = vector<ll>; using vvl = vector<vl>; using vb = vector<bool>; using vvb = vector<vb>;
...
int main() {
	...
	ll k = 0;
	while (v[k] != A) k++; // 初めの部分
	ll d = v.size() - k; // 循環節の長さ
	// dbs(k, d, v.size());

	ll sum = 0;
	rep2(i, k, v.size()) sum += v[i]; // 1循環節内の余りの和
	ll q = (N - k) / d, r = (N - k) % d;
	ll ans = 0;
	if (N < k) {
		rep(i, N) ans += v[i];
