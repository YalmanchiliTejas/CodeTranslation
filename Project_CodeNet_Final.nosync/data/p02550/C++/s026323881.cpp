...
void solve(long long N, long long X, long long M)
{
    ...
    for (ll i = 1; i <= 100010; i++)
    {
        ...
        if (already[now] != -1)
        {
            loop_start = already[now];
            loop_end = i;
            loop_sum = ans;
            // cout << ans << endl;
            // cout << already[now] << "-" << i << " is Loop!!!" << endl;
            break;
        }
        ans += now;
        // cout << now << endl;
    ...
    now = X;
    ll before_loop = 0;
    for (int i = 1; i <= loop_start - 1; i++)
    {
        before_loop += now;
        now = nv[now];
    }
    loop_sum -= before_loop;
    // cout << before_loop << " " << loop_sum << endl;
    ...
    {
        ans += before_loop;
        ll remain = N - loop_start + 1;

        ans += (remain / (loop_end - loop_start)) * loop_sum;
        now = X;
        ...
        for (int i = 0; i < remain % (loop_end-loop_start); i++)
        {
            ans += now;
            now = nv[now];
        }
