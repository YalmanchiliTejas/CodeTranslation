...
int main()
{
  ...
  ll N, X, M; cin >> N >> X >> M;
  vector<ll> flag(M,-1);//出現したらtrueにする。
  vector<ll> sum(M+2,0);
  ...
  ll ans = 0;
  ll sumCycle = sum[endCycle] - sum[startCycle];
  ll inCycle = N-startCycle+1;
  ans += sum[startCycle-1];
  ans += sumCycle*(inCycle/cycle);
  ans += sum[(startCycle-1)+(inCycle%cycle)]-sum[(startCycle-1)];
