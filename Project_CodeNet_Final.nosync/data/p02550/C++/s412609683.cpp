...
int main() {
	...
	int pos = *(a | Index(dup));
	ll before = 0;
	rep(i, pos) before += a[i];

	ll cycle = 0;
	int cycle_size = sz(a) - pos;
	FOR(i, pos, sz(a)) cycle += a[i];
	...
	if (n <= sz(a)) {
		...
		ll ans = before;
		ll remain = n - pos;
		ans += (remain / cycle_size) * cycle;
		FOR(i, pos, pos + remain % cycle_size) ans += a[i];
		out(ans);
	}
}