...
int main(){
    ...
    auto border = find(all(history), now);
    vector<lint> loop;
    loop.insert(loop.begin(), border, history.end());
    history.erase(border, history.end());
    lint ans = 0;
    for(lint i: history){
        if(!N) break;
        ans += i;
    ...
    ans += (N / loop.size()) * accumulate(all(loop), 0ll);
    N %= loop.size();
    for(lint i: loop){
        if(!N) break;
        ans += i;
