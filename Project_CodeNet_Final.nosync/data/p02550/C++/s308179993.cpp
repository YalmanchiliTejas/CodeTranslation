...
int32_t main(){
	...
	ll n,x,M,ans=0;
	cin>>n>>x>>M;
	if(M==1) {db(0);return 0;}
	vector<int> m(1e5+1,0);auto ne=m;
	vl a;
	ll b=x;int q=n;
	while(m[x]==0&&q){
		if(a.size()==0) a.push_back(x);
		else a.push_back(a.back()+x);
		m[x]=1;
		x*=x;
		x%=M;
		q--;
	...
	while(ne[x]==0) {
		...
		if(p.size()) p.push_back(p.back()+x); else p.push_back(x);
		ne[x]=1,x*=x,x%=M,t++;
	}
		n-=a.size();
		ans=a.back();
