...
int main() {
  ...
  if(count == n) {
    ...
    long f_ans;
    f_ans = ans[count] + (ans[count] - ans[begin]) * ((n - count - 1) / (count - begin));
    long now = count + ((n - count - 1) / (count - begin)) * (count - begin);
    for(long i=now; i<n-1; i++) {
      x = (x * x) % m;
      f_ans += x;
    }
    co(f_ans);
