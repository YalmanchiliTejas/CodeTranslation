...
int main(int argc, char const *argv[])
{
    ...
    A.push_back(X);
    ll next_num = X;
    ll first_len, first_sum, cir_len, cir_sum, cir_times;
    ll rest_len, rest_sum;
    ll cir_point;
    FOR_EQ(i, 2, N){
        next_num = (RepeatSquaring(next_num, 2) % M);
        A.push_back(next_num);
        if(record.find(next_num) == record.end()){  // まだない
            sum += next_num;
            record.insert(next_num);
            board[next_num] = make_pair(sum, i);
        }else{  // 循環に入った(同じあまりなら確実に入る。M <= 10^5 なので確実にTLEにならない)
            in_cir = true;
            cir_len = i - board[next_num].second;
            sum += next_num;
            cir_sum = sum - board[next_num].first;
            
            first_len = board[next_num].second;
            first_sum = board[next_num].first;
    ...
    cir_times = N / cir_len;
    rest_len = N % cir_len;
    ans += cir_sum * cir_times;

    Vector::iterator it = find(A.begin(), A.end(), cir_point);
    it += rest_len;
    rest_sum = board[*it].first - board[cir_point].first;
