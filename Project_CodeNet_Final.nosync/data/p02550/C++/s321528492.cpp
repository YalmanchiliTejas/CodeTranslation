...
int main() {
	...
	const i64 N = read_item();
	const i64 X = read_item();
	const i64 M = read_item();

	vector<pair<i64, i64>> first_steps(M, (pair<i64,i64>){-1LL, 0LL});
	...
	for(i64 i = 2; i <= N; i++) {
		...
		i64 j = first_steps[v].first;
		i64 s = first_steps[v].second;
		...
		if(j == -1) {
			...
			i64 step_diff = i - j;
			i64 sum_diff = sum - s;
			i64 cycle_count = (N - i) / step_diff;
			i += cycle_count * step_diff;
			sum += cycle_count * sum_diff;
		}
	}
