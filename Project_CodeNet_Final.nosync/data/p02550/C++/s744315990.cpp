...
template <typename T> struct DualSegmentTree {
    ...
    const H h;
    const T e;
    DualSegmentTree(int n, const H h, T e) : h(h), e(e) {
        sz = 1, height = 0;
    ...
    DualSegmentTree(vector<T> &v, const H h, T e) : h(h), e(e) {
        sz = 1, height = 0;
...
// Range Add Tree
template <class T> struct RAT : DualSegmentTree<T> {
...
// Range Set Tree
// inf を逆元として利用
template <class T> struct RST : DualSegmentTree<T> {
    using Seg = DualSegmentTree<T>;
    RST(size_t n)
        : Seg(
...
main() {
    ...
    vv(ll, nxt, 40, m);
    vv(ll, sum, 40, m);
    rep(i, m) nxt[0][i] = (i * i % m), sum[0][i] = i;
    rep(i, 39) rep(j, m) nxt[i + 1][j] = nxt[i][nxt[i][j]], sum[i + 1][j] = sum[i][j] + sum[i][nxt[i][j]];
    ll ans = 0;
    rep(i, 40) {
        if(n & 1LL << i) { ans += sum[i][x], x = nxt[i][x]; }
    }
    cout << ans << endl;
}