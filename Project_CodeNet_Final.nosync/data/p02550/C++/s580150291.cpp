...
signed main(){
    ...
    rep(m){
      ...
      lp[i+1]=lp[i]+ai;
      ai=(ai*ai)%m;
      if(!ai)return out(lp[i+1]);
      if(amari[ai]){loop=i+2-amari[ai];break;}
    }
    ans+=(lp[amari[ai]-1+loop]-lp[amari[ai]-1])*((n-amari[ai]+1)/loop);
    if((n-amari[ai]+1)%loop)ans+=lp[amari[ai]-1+(n-amari[ai]+1)%loop]-lp[amari[ai]-1];
    ans+=lp[amari[ai]-1];
    out(ans);
}
