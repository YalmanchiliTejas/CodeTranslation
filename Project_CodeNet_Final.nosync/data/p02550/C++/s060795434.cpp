...
int main()
{
  ...
  int x, m;
  cin >> n >> x >> m;
  vector<vector<llP>> table(50, vector<llP>(m)); // sum, next;
  for(ll i = 0; i < m; ++i) table[0][i] = llP(i, (i*i)%m);

  for(int k = 1; k < 50; ++k) {
    rep(j, m) {
      ll sum = table[k-1][j].first + table[k-1][table[k-1][j].second].first;
      int next = table[k-1][table[k-1][j].second].second;
      table[k][j] = llP(sum, next);
    }
  }
