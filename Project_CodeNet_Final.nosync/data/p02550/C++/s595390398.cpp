...
int main()
{
    ...
    if (is_looping)
    {
        auto loop_start_itr = find(history.begin(), history.end(), cur);
        ll loop_start_idx = distance(history.begin(), loop_start_itr);
        debug(loop_start_idx);

        ll cost_before_loop = 0;
        for (auto i = 0; i < loop_start_idx; i++)
        {
            cost_before_loop += history.at(i);
        }
        debug(cost_before_loop);

        cycle_cost -= cost_before_loop;
        debug(cycle_cost);

        ll cycle_length = idx - loop_start_idx;
        debug(idx, cycle_length);
        ll after = n - loop_start_idx;
        debug(after, n);
        ll cycle = after / cycle_length;
        ll res = n - cycle * cycle_length - loop_start_idx;
        debug(cycle_length, cycle, res);
        debug(history.size());

        ll ans = cost_before_loop + cycle * cycle_cost;
        for (auto i = 0; i < res; i++)
        {
            ans += history[loop_start_idx + i];
        }
