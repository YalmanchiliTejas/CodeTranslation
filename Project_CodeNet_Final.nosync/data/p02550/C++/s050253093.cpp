...
int main()
{
  ...
  ll curr=-1;
  ll latest=0;
  fr(o,1,minim(m+1,n-1))
  {
    latest=o;
    arr[o]=reduce(arr[o-1]*arr[o-1],m);
    if(arr[o]==0)
    {
      cout<<cult[o-1]<<endl;
      return 0;
    }
    cult[o]=cult[o-1]+arr[o];
    if(mx.find(arr[o])!=mx.end())
    {
      last=mx[arr[o]];
      curr=o;
      break;
  ...
  if(last==0)
  {
    ll q=n/(curr-last);
    ll r=n%(curr-last);
    cout<<(q*cult[curr-1])+((r!=0)?cult[r-1]:0)<<endl;
  }
  else
  {
    n-=last+1;
    ...
    ll q=n/(curr-last);
    ll r=n%(curr-last);
    total+=q*(cult[curr]-cult[last]);
    total+=cult[last+r]-cult[last];
    cout<<total<<endl;
  }
