...
int32_t main()
{
    ...
    rep(test, 1, tests+1)
    {
        ...
        dbg(cntr);
        if(m and n){
        	long long int cycle = cntr - cnt;
        	long long int cyclecost = pref[cntr] - pref[cnt];
        	m = n / cycle;
        	n %= cycle;
        	pref[cntr] += pref[cnt + n] - pref[cnt];
        	pref[cntr] += (m) * cyclecost;
        }
        cout << pref[cntr];
    }
    #ifdef LOCAL
