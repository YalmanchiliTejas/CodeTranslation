...
void solve(){
	...
	for(long long k = found[cur]; k < path.size(); ++k){
		ext.push_back(path[k]);
		sum += path[k];
	}

	assert(ext.size() > 0);

	long long new_size = ext.size();
	ans += sum * (rem / new_size);
