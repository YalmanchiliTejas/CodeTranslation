...
void solve(int ti) {
    ...
    while (seenat[a] == -1) {
        ...
        a = modop(a * a, m);//next a
    }
    idx++;
    int oldidx = seenat[a];
    ll period = idx - oldidx;
    ll per_sum = cumsum.back() + a - cumsum[seenat[a]];//7a ] 8b 9c 10d 11a]
    cumsum.eb(cumsum.back() + a);
    ll warp = (n - oldidx) / period;
    n -= warp * period;
    ans = cumsum[n] + warp * per_sum;
    ps(ans);
    //seen[a] !=idx
}
