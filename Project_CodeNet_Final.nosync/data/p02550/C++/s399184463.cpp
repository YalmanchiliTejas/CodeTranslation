...
LL(N,X,M);
N--;
vector<ll> tw;//time -> what
vector<ll> wt(M);//wgat ->time

ll now=X,nowt=0;
ll roopsize,roopstart;
while(true){
tw.push_back(now);
if(wt[now]!=0){
roopsize=nowt-wt[now];
roopstart=wt[now];
break;
}
wt[now]=nowt;
...
if(N<roopstart){
    ...
    ll ans=0;
    rep(i,roopstart)ans+=tw[i];
    N-=roopstart-1;
    ll roopsum=0;
    rep2(i,roopstart,nowt)roopsum+=tw[i];
    ans+=roopsum*(N/roopsize);
    rep2(i,roopstart,roopstart+(N%roopsize))ans+=tw[i];
    out(ans);
