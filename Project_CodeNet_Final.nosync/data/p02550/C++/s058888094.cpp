...
int main() {
  ...
  REP(i,M){
    amint ne = po.pow(2);
    dub.at(0).at(po.x) = ne.x;
    sum.at(0).at(po.x) = po.x;
    po = ne;
  }

  REP(i,59){
    REP(j,M){
      if(dub.at(i).at(j)<0)continue;
      dub.at(i+1).at(j) = dub.at(i).at(dub.at(i).at(j));
      sum.at(i+1).at(j) = sum.at(i).at(j)+sum.at(i).at(dub.at(i).at(j));
    }
  }
