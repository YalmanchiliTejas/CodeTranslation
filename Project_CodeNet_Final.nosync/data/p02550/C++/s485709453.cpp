...
int main()
{
    const auto [N, X, M] = in.tup<ll, int, int>();
    auto nextss          = nd_array<int>({M, 40}, -1);
    auto sumss           = nd_array<ll>({M, 40}, 0);
    for (int i = 0; i < M; i++) {
        nextss[i][0] = (ll)i * i % M;
        sumss[i][0]  = i;
    }
    for (int j = 1; j < 40; j++) {
        for (int i = 0; i < M; i++) {
            const int ni = nextss[i][j - 1];
            nextss[i][j] = nextss[ni][j - 1];
            sumss[i][j]  = sumss[i][j - 1] + sumss[ni][j - 1];
        }
    }
    int pos = X;
    ll ans  = 0;
