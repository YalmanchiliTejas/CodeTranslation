...
class SegmentTree {
    ...
    // 初期化
        ...
        ll vl = getmin(a, b, 2 * now + 1, l, (l + r) / 2);  // 子(左)
        ll vr = getmin(a, b, 2 * now + 2, (l + r) / 2, r);  // 子(右)
...
pair<vector<ll> , T >sequence(T x){  //xの連続してる部分をvectorにまとめる
...
//pascalを使用するので前計算が必要
//n項目までの計算を高速に行う
//オーバーフローしない限り、O((指数)^2)で行うことができる
//しかし活用できるほどの大きさの計算ではまず間違いなくオーバーフローする


//n項目までのa乗和を保存する。nはsigmaCalc(a , n)で指定
//sigmaInitする必要あり
//SigmaN[k] = n項目までのk乗和
vector<long long> SigmaN(100,0);
...
int main(){
    ...
    REP(i,n){
        ...
        if(p[a[i]]==0){
            ...
            //debug(b.size());
            //REP(u,b.size())debug(b[u])

            ll num = n- i -1;
            ll loopnum = num/(i-p[a[i]]);
            REP(o,loopnum){
                fans += toB;
            }
            ll thenum = num%(i-p[a[i]]);

            fans += b[p[a[i]] + thenum] - b[p[a[i]]];
