...
void solve(ostringstream& aout, long long N, long long X, long long M);
void solve_TLE(ostringstream& aout, long long N, long long X, long long M);

class StressTest {
private:
	mt19937 m_RandEngine;
	bool judge_case(long long N, long long X, long long M) {
...
// 区間 op 機能追加、と MSVC で動くように clz を不要にした
// コンストラクタ -> SetNext -> Build -> Query の順に呼ぶこと
class Dubling
{
private:
	int m_LogMax;
	...
	// op の単位元
	int m_E;
	// TODO: std::optional 使う(ジャッジが C++17 対応するまで使えないので優先度低)
	const bool m_HasOp;

	// m_RangeOp[k][i] := [i, i+2^k) の区間 op
	vector<vector<int>> m_RangeOp;

public:
	/**
	* @fn
	* @brief op を渡さず、要素数 N, 最大クエリ queryMax の Dubling クラスを構築します。 op を渡さないため、区間 op の取得には対応しません。
	* @param N 要素数です。
	* @param queryMax クエリとして与えられうる最大値です。
	*/
	Dubling(int N, int queryMax) 
		: m_HasOp(false)
	{
		m_LogMax = 1;
	...
	/**
	* @fn
	* @brief op を渡して、要素数 N, 最大クエリ queryMax の Dubling クラスを構築します。渡した op を使って区間 op を計算できます。
	* @param N 要素数です。
	* @param queryMax クエリとして与えられうる最大値です。
	* @param op 区間に適用したい演算です。(例: 区間和 なら +)
	* @param e op の単位元です。(例: 区間和 なら0)
	*/
	Dubling(int N, int queryMax, function<int(int, int)> op, int e)
		: m_Op(op)
		, m_E(e)
		, m_HasOp(true)
	{
	...
	*/
	...
	/**
	* @fn
	* @brief Dubling の テーブルを構築します。
	* @pre 問題で与えられるすべての距離1の遷移について、SetNext 済 
	*/
	void Build() {
		// k + 1 を更新するので k + 1 < m_LogMax でループすることに注意する
		for (int k = 0; k + 1 < m_LogMax; k++) {
			for (int i = 0; i < m_Next[k].size(); i++) {
				// i に 2^k 個次がなければ、2^(k+1)個次もない
				if (m_Next[k][i] == -1) {
					m_Next[k + 1][i] = -1;
				}
				else {
					// i の 2^(k+1) 個次は i の 2^k 個次の 2^k 個次
					m_Next[k + 1][i] = m_Next[k][m_Next[k][i]];
				}
			}
		}

		// TORIAEZU: [i, i+1) の区間 op は i
		for (int i = 0; i < m_RangeOp[0].size(); i++) {
			m_RangeOp[0][i] = i;
		}

		// 区間 op を計算するための op が与えられていたらそのテーブルも計算する
		if (m_HasOp) {
			for (int k = 0; k + 1 < m_LogMax; k++) {
				for (int i = 0; i < m_Next[k].size(); i++) {
					// 区間 op なので、はみ出してたら何かがおかしいかも(丸めることはしない)
					// TODO: verify できる問題があったらいい感じにする
					assert(m_Next[k][i] != -1);
	...
	*/
	...
	/**
	* @fn
	* @brief k から t 回遷移した時の列を A_0, A_1..., A_tとしたとき、{op(A_i), i = [0, t)} の区間 op を求めます。
	* @param k 遷移元
	* @param x 遷移先
	*/
...
void solve(ostringstream& aout, long long N, long long X, long long M){
	...
	Dubling d(M, N + 10, std::plus<int>(), 0);

	rep(i, M) {
		d.SetNext(i, mod_pow(i, 2, M));
	}

	d.Build();

	int ans = d.QueryRangeOp(X, N);
...
signed main() {
    ...
    long long M;
    scanf("%lld",&M);
	ostringstream ansStream;
    solve(ansStream, N, X, M);
