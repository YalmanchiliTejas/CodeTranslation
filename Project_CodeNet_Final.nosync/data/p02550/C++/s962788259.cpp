...
void solve(){
    ...
    ll sum_loop=sum[loop-1];
    
    if(first) sum_loop-=sum[first-1];
    debug(first);
    debug(sum_loop);
    int size_loop=loop-first;
    debug(size_loop);
    debug((N-first)/size_loop);
    debug(sum[min(first,N)]);


    ll ret=sum_loop*(max((ll)0,(N-first)/size_loop));
    if(first-1>=0) ret+=sum[min(first-1,N-1)];
    ll kosuu=(N-first)%size_loop;
