...
inline ll square(ll a, ll m) {return a*a%m;}
ll lpow(ll a,ll b, ll m){
    ...
    if (b == 1) return a;
    if (b % 2) {
        return (square(lpow(a, b / 2, m) % m, m) * a) % m;
    } else {
        return square(lpow(a, b / 2, m) % m, m);
    }
}
