...
void _main()
{
    ...
    ll ans = 0;
    rep (i, sidx) ans += vals[i];
    n -= sidx;
    ll freq = vals.size() - sidx;
    debug_printf("---- freq=%lld\n", freq);

    ll unit = 0;
    srep (i, sidx, vals.size()) unit += vals[i];

    ans += (n / freq) * unit;
    srep (i, sidx, sidx + n % freq) ans += vals[i];
