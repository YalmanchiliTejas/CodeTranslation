...
//    for(int i = 0; i < len + cir + 5; i++) cout <<a[i] <<" "; cout <<endl;
    ...
    {
        LL suml = 0, sumc = 0;
        for(int i = len; i < len + cir; i++) sumc += a[i];
        for(int i = 0; i < len; i++) suml += a[i];

        LL d = (n - len) / cir;
        int r = (n - len) % cir;

        LL part = 0;
        for(int i = len; i < len + r; i++) part += a[i];

        cout <<suml + d * sumc + part <<endl;
    }
...
//
//    LL sum2 = 0;
//    for(int i = 0; i < r; i++) sum2 += a[i];
//
//    cout <<sum * d + sum2 <<endl;
}
