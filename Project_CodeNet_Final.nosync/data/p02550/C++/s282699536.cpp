...
void solve() {
    ...
    vector<int> val;
    vector<int> used(m + 2, -1);
    mint res = x;
    int cnt = 0;
    while (cnt < n && used[res.x] == -1) {
        used[res.x] = cnt;
        ++cnt;
        val.push_back(res.x);
        res *= res;
    ...
    if (cnt == n || val.back() == 0)
        ...
        auto sum = accumulate(val.begin(), val.begin() + used[res.x], 0LL);
        int tmp = sz(val) - used[res.x];
        sum += accumulate(val.begin() + used[res.x], val.end(), 0LL) * ((n - used[res.x]) / tmp);
        rep(i, (n - used[res.x]) % tmp) {
            int idx = used[res.x] + i;
            if (idx >= sz(val)) idx = used[res.x] + idx % sz(val);
            sum += val[idx];
        }
        cout << sum << endl;
    }
