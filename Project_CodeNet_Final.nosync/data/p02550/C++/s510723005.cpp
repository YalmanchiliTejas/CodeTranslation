...
int main(){
	...
	for(int i=2;i<=INF;i++){
		...
		if(alre[now]>0){
			ll inisum=rui[alre[now]-1];
			ll loopnum=(i-1)-(alre[now]-1);
			ll loopsum=rui[i-1]-rui[alre[now]-1];
			ll amarisum=(N-(alre[now]-1))/loopnum*loopsum;
			ll amarinum=(N-(alre[now]-1))%loopnum;
			rep(j,amarinum){
				amarisum+=are[alre[now]+j];
			}
			ll ans=inisum+amarisum;
			prin(ans);
			return 0;
		}
		alre[now]=i;
