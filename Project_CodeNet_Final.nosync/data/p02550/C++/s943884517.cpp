...
bool cmps(pair<LL,LL> a,pair<LL,LL> b){if(a.SE!=b.SE)return a.SE<b.SE;return a.FI<b.FI;}
...
int main(){
  ...
  cin>>N>>X>>M;
	...
	FOR(i,1,N){
		...
		r=i-v[X];
		s[i+1]=s[i]+X;
		ans+=((N-v[X])/r-1)*(s[i]-s[v[X]]);
		a=(N-v[X])%r;
		ans+=s[v[X]+a]-s[v[X]];
		//cout<<(N-v[X])/r*(s[i]-s[v[X]])<<" "<<s[v[X]+a]-s[v[X]]<<endl;
		break;
	}
	cout<<ans<<endl;
}
