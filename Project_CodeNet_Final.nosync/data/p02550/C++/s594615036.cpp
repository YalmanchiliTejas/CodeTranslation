...
//(親,現在地,現在の距離,最大距離,最大距離となる頂点,隣接リスト)
void tyokkei_init(int a,int b,int c,int &d,int &e,V<V<int>> &f){
  if(c>d){
    d=c;
    e=b;
  }
  fo(i,f[b].sz){
    if(f[b][i]==a)
      continue;
    tyokkei_init(b,f[b][i],c+1,d,e,f);
  }
}
//aは隣接リスト
int kinotyokkei(V<V<int>> &a){
  int b=0,c=0,d=0;
  tyokkei_init(-1,b,0,d,c,a);
  b=c,c=0,d=0;
  tyokkei_init(-1,b,0,d,c,a);
  return d;
  //直径となる2点はbとc
...
//aとbの距離、cは隣接リスト
int tree_kyori(int a,int b,V<V<int>> &c){
  if(a==b)
    return 0;
  Q<int> e;
  e.push(a);
...
//aは入次数のvector,bは隣接リスト
P<V<int>,bool> topological_sort(V<int> &a,V<V<int>> &b){
  stack<int> c;
  fo(i,a.sz){
    if(a[i]==0)
      c.push(i);
...
signed main(){
  ...
  int h=0;
  h+=e[f-1+1];
  h+=((a-(f-1))/(g-f+1))*(e[g+1]-e[f-1+1]);
  h+=e[f-1+((a-f+1)%(g-f+1))-1+1]-e[f-1+1];
  cout<<h<<endl;
}