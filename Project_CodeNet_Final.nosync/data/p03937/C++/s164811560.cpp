...
//        PII down = MP(p.first + 1, p.second);
//        PII right = MP(p.first, p.second + 1);

        vector<PII> vec = {MP(p.first + 1, p.second), MP(p.first, p.second + 1)};
        int count = 0;
        FOREACH(pii, vec) {
            if (pii.first < H && pii.second < W && A[pii.first][pii.second] == '#') {
                Q.push(pii);
                B[pii.first][pii.second] = false;
                count++;
            }
        }
        if(count==2){
