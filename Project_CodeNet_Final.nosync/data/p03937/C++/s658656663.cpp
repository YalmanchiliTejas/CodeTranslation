...
int main()
{
    constexpr int di[] = {0, 1};
    constexpr int dj[] = {1, 0};
    int N = in<int>(), M = in<int>();
    auto ss = in_v<std::string>({N});
    mfp([&](auto&& self, const int i, const int j) -> void {
        ss[i][j] = '.';
        for (int d = 0; d < 2; d++) {
            const int ni = i + di[d];
            const int nj = j + dj[d];
            if (ni < N and nj < M and ss[ni][nj] == '#') {
                self(self, ni, nj);
                break;
            }
        }
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < M; j++) {
            if (ss[i][j] == '#') { return outln("Impossible"); }
        }
    }
    outln("Possible");
    return 0;
