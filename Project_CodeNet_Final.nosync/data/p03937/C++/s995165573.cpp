...
bool ok=false;
ll sharp;
void search(ll h,ll w){
 //cout<<h<<" "<<w<<endl;
 if(vis[h+1][w]=='#'&&!ban[h+1][w]){
  cnt[h+1][w]=cnt[h][w]+1;
  maxcnt=max(maxcnt,cnt[h+1][w]);
  //cout<<"h "<<h+1<<" "<<w<<" "<<cnt[h+1][j]<<" "<<cnt[h][w]<<endl;
  search(h+1,w);
 }
 if(vis[h][w+1]=='#'&&!ban[h][w+1]){
  cnt[h][w+1]=cnt[h][w]+1;
  maxcnt=max(maxcnt,cnt[h][w+1]);
 // cout<<"w "<<h<<" "<<w+1<<" "<<cnt[h][w+1]<<" "<<cnt[h][w]<<endl;
  search(h,w+1);
 }
}
int main(){
