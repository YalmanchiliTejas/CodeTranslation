...
int H, W;
VVB visit(8, VB(8, false));
VS S(8);

bool dfs(int fromy, int fromx){
	bool flag = true;

	if(fromy == H - 1 && fromx == W - 1){
		REP(i, H) {
			REP(j, W) {
				if (S[i][j] == '#' && visit[i][j]) flag = false;
			}
		}
		return flag;
	}

	if(fromy < H - 1 && S[fromy + 1][fromx] == '#'){
		visit[fromy + 1][fromx] = false;
		flag = dfs(fromy + 1, fromx);
		if (flag) return flag;
		visit[fromy + 1][fromx] = true;
	}

	if (fromx < W - 1 && S[fromy][fromx + 1] == '#') {
		visit[fromy][fromx + 1] = false;
		flag = dfs(fromy, fromx + 1);
		if (flag) return flag;
		visit[fromy][fromx + 1] = true;
	}
...
int main() {
	cin >> H >> W;
	REP(i, H) cin >> S[i];
	REP(i, H)REP(j, W) if (S[i][j] == '#') visit[i][j] = true;
