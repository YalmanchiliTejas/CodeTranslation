...
void dfs(const int& nowh, const int& noww, const vector<string> &s, int &cnt) {
	++cnt;
	if (s[nowh][noww + 1] == '#')dfs(nowh, noww + 1, s, cnt);
	else if (nowh != s.size() - 1) {
		if (s[nowh + 1][noww] == '#')dfs(nowh + 1, noww, s, cnt);
	}
	return;
}
