...
int main()
{
	...
	while (1) {
	    if (pos.F + 1 < h && grid[pos.F + 1][pos.S] == '#') {
			pos.F++;
			table[pos] = 1;

			if (peq(pos, goal)) break;
		} else if (pos.S + 1 < w && grid[pos.F][pos.S + 1] == '#') {
			pos.S++;
			table[pos] = 1;
	...
	rep(i, h) {
		rep(j, w) {
			if (grid[i][j] == '#' && table.find(mp(i, j)) == table.end()) {
				Out("Impossible");
