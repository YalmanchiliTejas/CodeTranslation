...
bool check(vector<string> ban, bool used[10][10],int H,int W) {
	rep(h, H)rep(w, W) {
		if ((ban[h][w] == '#'&&!used[h][w]) || (ban[h][w] == '.'&&used[h][w])) {
			return false;
		}
	}
	return true;
