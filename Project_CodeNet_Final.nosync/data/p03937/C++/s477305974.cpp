...
#define rand mtrand
  ...
  #define trca(v,...) do { cout << #v << " = "; array_out(v , ##__VA_ARGS__);cout << endl;} while(0)
#else
  #define trc(...)
  #define trca(...)
#endif
...
int main(){
  ...
  while(h!=H-1||w!=W-1){
    ...
    rep(k,2){
      dh = h + dx[k] , dw = w + dy[k];
      if(ok(dh,dw) && v[dh][dw] == '#') cnt++;
    }
    if(cnt != 1) die("Impossible");
    rep(k,2){
      dh = h + dx[k] , dw = w + dy[k];
      if(ok(dh,dw) && v[dh][dw] == '#'){
        v[h][w] = '.'; h = dh,w = dw; break;
      }
    }
  }
  v[h][w] = '.';
  rep(i,H) rep(j,W){
    if(v[i][j] == '#') die("Impossible");
  }
  out("Possible");
