...
char S[10][10];ll f[10][10];
void solve(){ll N,M;cin>>N>>M;for(int i=0;i<N;i++)cin>>S[i];f[0][0]=1;ll c=0;for(int i=0;i<N;i++)for(int j=0;j<M;j++)if(S[i][j]=='#'){c++;if(i)f[i][j]+=f[i-1][j];if(j)f[i][j]+=f[i][j-1];}if(c!=N+M-1){cout<<"Impossible"<<endl;return;}for(int i=0;i<N;i++)for(int j=0;j<M;j++){if((S[i][j]=='#')^(f[i][j]==1)){cout<<"Impossible"<<endl;return;}}cout<<"Possible"<<endl;}
