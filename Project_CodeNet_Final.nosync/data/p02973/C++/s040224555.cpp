...
int tree[mx << 2], ara[mx];
void add(int p, int l, int r, int x){
    if(l == r){ ara[l] = x; tree[p] = x; return;}
    if(tree[left(p)] < x) add(left(p), l, mid(l,r), x);
    else add(right(p), mid(l,r)+1, r, x);
    tree[p] = min(tree[left(p)], tree[right(p)]);
}

void build(int p, int l, int r){
    if(l == r) {ara[l] = -1; tree[p] = -1;}
    else{
...
int main(){
    int n; sfi(n); build(1, 0, n-1);
    int x; fr0(i, n) sfi(x), add(1, 0, n-1, x);
    int ans = 0;
    for(int i=0; i<n; i++)
        ans += (ara[i] != -1);
    pfi(ans);
