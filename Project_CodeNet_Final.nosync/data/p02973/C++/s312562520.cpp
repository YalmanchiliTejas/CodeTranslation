...
{
    ll n; llin(n);
    V<ll> alist; llinl1(alist, n);

    set<ll> aset;
    for (auto a : alist) aset.insert(a);
    ...
    BIT tree(n+100LL, 0LL);
    ll ans = 0LL;

    for (auto a : alist) {
        ll val = map[a];
        ll base = ((val > 1LL) ? tree.sum(val - 1LL) : 0LL);
        ...
        for ( ; left <= right; now = (left + right) / 2LL) {
            ll sub = ((now > 1LL) ? tree.sum(now - 1LL) : 0LL);
            assert(base >= sub);
            if (base - sub) {
                pos = max(pos, now);
                left = now + 1LL;
        ...
        debug_printf("val=%lld pos=%lld\n", val, pos);

        tree.add(val, 1LL);

        if (pos < 0LL) ans++;
        else tree.add(pos, -1LL);
    }
