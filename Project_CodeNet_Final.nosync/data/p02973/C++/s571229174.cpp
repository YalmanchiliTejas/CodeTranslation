...
namespace NyaGadget
{
	...
	{
		vector<T> lds;         // 最長広義減少部分列
		vector<vector<T>> mis; // 増加部分列に分解した集合
	};

	/**
	@brief 最長広義減少部分列を返す関数。
	@note
	 計算量O(NlogN)で最長広義減少部分列が得られる。
	 内部的には配列vを分解個数が最小になるよう増加部分列集合に分解している。
	 よって、副産物として分解個数最小の増加部分列集合が得られる。
	 分解個数が最小であり、最長増加部分列でないことに注意。
	**/
	template <class T> void DS_NyaaLDS
	(const vector<T>& v, ResultLDS<T>& res)
	{	// Monotonically Increasing Subsequence
		vt<ll> test(v.size() + 1, LLONG_MIN);
		for (auto& e : v)
		{
			auto it = --lower_bound(all(test), e);
			if (res.mis.size() <= v.size() - distance(test.begin(), it))
				res.mis.resize(res.mis.size() + 1);
			res.mis[v.size() - distance(test.begin(), it)].push_back(e);
			*it = e;
		}
		for (auto& e : test)
		{
			if (e != LLONG_MIN) res.lds.push_back(e);
		}
	}
}
