...
void solve(ostringstream& aout, long long N, long long Q, std::vector<long long> S, std::vector<long long> T, std::vector<long long> X, std::vector<long long> D);
void solve_TLE(ostringstream& aout, long long N, long long Q, std::vector<long long> S, std::vector<long long> T, std::vector<long long> X, std::vector<long long> D);

class StressTest {
private:
	mt19937 m_RandEngine;
	bool judge_case(long long N, long long Q, std::vector<long long> S, std::vector<long long> T, std::vector<long long> X, std::vector<long long> D) {
		ostringstream fast, tle;
		solve(fast, N, Q, std::move(S), std::move(T), std::move(X), std::move(D));
		solve_TLE(tle, N, Q, std::move(S), std::move(T), std::move(X), std::move(D));
		if (fast.str() == tle.str()) {
			return true;
		}
		else {
...
public:
	...
	void test() {
		while (1) {
			...
			//if (!judge_case(N, Q, std::move(S), std::move(T), std::move(X), std::move(D))) {
				// TODO: output case
				//break;
			//}
		}
...
void solve(ostringstream& aout, long long N, long long Q, std::vector<long long> S, std::vector<long long> T, std::vector<long long> X, std::vector<long long> D){
	...
	vector<int> poses;
	rep(i, N) {
		int l = max(0LL, S[i] - X[i]);
		int r = max(0LL, T[i] - X[i]);
		poses.push_back(l);
		poses.push_back(r);
	}
	rep(i, Q) {
	...
	auto myMin = [](int a, int b) {return min(a, b); };
	SegmentTree<int, int> seg(cp.unzip.size() + 10, myMin, myMin, myMin, LL_HALFMAX, LL_HALFMAX);

	rep(i, N) {
		int l = max(0LL, S[i] - X[i]);
		int r = max(0LL, T[i] - X[i]);
		seg.update(cp.zip[l], cp.zip[r], X[i]);
	}

	rep(i, Q) {
		if (seg.query(cp.zip[D[i]]) == LL_HALFMAX) {
			cout << -1 << "\n";
			continue;
		}

		cout << seg.query(cp.zip[D[i]]) << "\n";
	}
...
// dp[i][j] := i桁目まで見て、j == 1 : 小さいことが確定している 0 : していない 場合の数
int dp[11][2];
