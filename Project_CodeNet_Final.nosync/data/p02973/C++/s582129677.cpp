...
template<class T, class U, class V> void deleate_map_near(map<T, U> &m, V it) {
    if (it != m.end()) {
        auto tit = it;
        ++tit;
        while (tit != m.end()) {
            if ((*tit).second != 0)break;
            tit = m.erase(tit);
        }
    }
...
void solve() {
    ...
    rep(i, n) {
        ...
        auto it = rupper_map(m, a[i]);
        (*it).se--;
        m[a[i]]++;
        deb(m);
    }
    auto ve = values(m);
    cout << sum(ve) - m[-1] << endl;
}
auto my(ll n, vi &a) {
    return 0;
}
