...
{	
	...
	/**
	@brief 単調増加部分列で分解する関数
	@note
	 一次元配列vを単調増加部分列に分解する。
	 部分列の長さが最大になるように分解して結果をresに格納する。
	 計算量 O(NlogN)
	**/
	template <class T> void DS_NyaaMIS
	(const vector<T>& v, vector<vector<T>>& res)
	{	// Monotonically Increasing Subsequence
		vt<ll> nyaa(v.size() + 1, LLONG_MIN);
		for (auto& e : v)
		{
			auto it = --lower_bound(all(nyaa), e);
			if (res.size() <= v.size() - distance(nyaa.begin(), it))
				res.resize(res.size() + 1);
			res[v.size() - distance(nyaa.begin(), it)].push_back(e);
			*it = e;
		}
	}
}
