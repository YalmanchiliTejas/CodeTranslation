...
public:
  ...
  //! 各rootに属する子を返す 
  unordered_map<LL, vector<LL>> get_root_child2() {
  ...
  //! 標準エラーに出力する
  void print() { 
    map<LL, vector<LL>> M = get_root_child();
    for(auto m :M){
      dump(m.first, m.second);
    }
    // for (LL i = 0; i < (LL)UFT.size(); ++i) {
    //   cerr << root(i) << " ";
...
struct IntegralImage{
  ...
  LL get_sum( LL y, LL h, LL x, LL w){
    ...
    LL get_sum2( LL y0, LL y1, LL x0, LL x1){
      if(y0>y1){
        swap(y0,y1);
      }
      if(x0>x1){
    ...
    return get_sum(y0,y1-y0 ,x0,x1-x0);
  }
  void print( ){
    REP(i,(LL)I.size()){
      PRINTSP(I[i]);
...
tuple<VLL,VLL,VLL> my_dycstra(VVVLL &R,LL start, LL MOD = 0)
{
  LL N = R.size();
  LL INFINF = 1e15;
  ...
  while(!Q.empty()) {
    ...
    for(auto r : R[from]) {
      ...
      if(chmin(Cost[to], (cost + cost_to))) {
        Q.push({cost + cost_to, to});
        Path[to] = from;
        Num[to] = Num[from];
      }
      else if(Cost[to] == (cost + cost_to)){
        Num[to] += Num[from];
        if(MOD!=0){
          Num[to] %= MOD;
        }
  ...
  // dump(C);
  return move(make_tuple(Cost,Path,Num));
}
...
struct SegmentTree{
  ...
  SegmentTree(LL N,string mode = "min"){
    init(N,mode);      
  }
...
tuple<LD,LD,LD> point2line(LD y1,LD x1,LD y2,LD x2){
  LD A = y1-y2;
  LD B = x2-x1;
  LD C = -A*x1-B*y1;
  return make_tuple(A,B,C);
}
LD dist_point_line(LD y1,LD x1,LD A,LD B,LD C){
  return abs(A*x1+B*y1+C)/sqrtl(A*A+B*B);
}

string rec(LL n,LL k, VLL &C){
  if(k==0){
    if(n==0){
      return "A";
    }
...
bool isOK(LL b,VLL &Futosa, VLL &Price,LL K,LL N,LL Yosan){
  auto it = lower_bound(ALL(Futosa),b);
  LL index = distance(Futosa.begin(),it);
  dump(b,index);
  dump(Futosa);
  if(N-index<K){
    dump("tariai",N,index,K);
    return false;
  }
  ...
  if(cnt <= Yosan){
   dump("OK",N,b,index,K,cnt,Yosan);
    return true;
  }
  dump("over",N,index,K,cnt,Yosan);
  return false;
}



void dfs(LL me, LL oya, VVVLL &T,VLL &C){
...
tuple<VLD,VLL,VLL> my_dycstra_v1(vector<vector<pair<LL,LD>>> &R,LL start, LL MOD = 0)
{
  LL N = R.size();
  LL INFINF = 1e15;
  using PQ = pair<LD,LL>;
...
vector<tuple<LD,LL,LL>> makeT(VVLL LS){
  LL NM = LS.size();
  vector<tuple<LD,LL,LL>> T;
  REP(i,NM-1){
    LD xi = LS[i][0];
    LD yi = LS[i][1];
    LD ci = LS[i][2];
    REPM(j,NM,i+1){
      ...
      LD yj = LS[j][1];
      LD cj = LS[j][2];
      LD dist = sqrtl((xi-xj)*(xi-xj) + (yi-yj)*(yi-yj));
      if(ci!=cj){
        dist*=10;
      }
      T.emplace_back(dist,i,j);
    }
  }
  sort(ALL(T));
  return move(T);
...
void dfs(LL me , LL oya, VVLL &A ,VLL &C){
