...
int main() {
	...
	auto nl = numeric_limits<pll>::max();
	...
	rep(i, 0, n)
		ai[i] = { a[i],-i };
	ll color = 0;
	segment_tree<M::min_indexed_t<>> seg(n, ai);
	ll prev=-1;
	ll mn;
	while (true) {
		if (prev == -1 || prev ==n-1) {
			...
			if (m.first == INF)
				...
				mn = m.first;
				color++;
				seg.update(-m.second, { INF,-m.second });
			}
		}
		else {
			auto m = seg.query(prev+1, n);
			if (mn < m.first && m.first!= INF) {
				seg.update(-m.second, { INF,-m.second });
				prev = -m.second;
				mn = m.first;
			}
			else if (mn == m.first) {
				seg.update(prev, { mn,-prev });
				prev = -m.second;
				seg.update(prev, { INF, -prev });
			}
			else {
				prev = -1;
			}
