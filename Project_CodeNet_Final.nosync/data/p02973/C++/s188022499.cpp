...
ll rec(int l, int r){
  ...
  ll ret = 0;
  chmax(ret, rec((l-1+n)%n, r) + a[l]);
  chmax(ret, rec(l, (r+1)%n)   + a[r]);
  return memo[{l, r}] = ret;
}
...
struct edge {
  ...
  ll cost;
  ll time;
  edge(int from, int to, ll cost, ll time): from(from), to(to), cost(cost), time(time){};
};
...
int main(void) {
  ...
  cout << lower_bound(all(dp), longinf) - dp.begin() << endl;
