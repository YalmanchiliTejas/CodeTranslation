...
//========================================================================
ll calc(vll vec, ll size)
{
	vll ret(size + 1); // 長さjを作った時の右端の最小値
	for (ll i = 0; i <= size; i++)
	{
		ret[i] = LLINF;
	}
	...
	{
		// 初めてvec[i]の方が小さくなるところを探す
		auto Iter = upper_bound(ret.begin(), ret.end(), vec[i]);
		ll idx = Iter - ret.begin();

		if ( ret[idx - 1] <= vec[i]) // 条件文の前半怪しい
		{
			ret[idx] = vec[i];
		}
	}
