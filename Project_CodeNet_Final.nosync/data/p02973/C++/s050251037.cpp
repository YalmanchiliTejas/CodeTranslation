...
public:
    ...
    SegmentTree(ll n){
        size=1;
        while(size<n)size*=2;
        data=vector<pll>(size*2,{LINF,LINF});
    }
    void set(ll at,pll val){
        at+=size;
        data[at]=val;
        while(at/=2)data[at]=min(data[at*2],data[at*2+1]);
    }
    pll get(ll l,ll r){
        if(l>=r)return {LINF,LINF};
        pll ans={LINF,LINF};
        l+=size;
        r+=size;
        while(l!=r){
            if(l&1)chmin(ans,data[l++]);
            if(r&1)chmin(ans,data[r-1]);
            l/=2;
            r/=2;
        }
        return ans;
...
signed main(){
    ...
    VEC(ll,a,n);
    SegmentTree seg(n);
    rep(n)seg.set(i,{a[i],-i});
    ll ans=0;
    while(seg.data[1].first!=LINF){
        ans++;
        ll at=-seg.get(0,n).second;
        while(0<=at&&at<n){
            seg.set(at,{LINF,LINF});
            at=-seg.get(at+1,n).second;
        }
    }
    out(ans);
}
