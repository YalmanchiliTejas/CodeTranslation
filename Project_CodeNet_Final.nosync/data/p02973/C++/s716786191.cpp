...
inline unsigned int ctz(unsigned int x) { unsigned long r; _BitScanForward(&r, x); return r; }
#endif
...
struct Calc {
	...
	//vector<pair<u6, u6>> buf;
	vector<u6>buf;
	auto calc() {
		buf.reserve(N);
		//rep(i, N)buf.emplace_back(read<u6>(), i);
		rep(i, N)buf.emplace_back(read<u6>());
		//sort(ALL(buf));// .sort();
		//vector<pair<u6, u6> > result;
		//u6 result = 0;
		multiset<u6> result;
		result.emplace(buf[0]);
		for (auto i = buf.begin() + 1; i != buf.end();++i) {
			auto ite = result.lower_bound(*i);
			if (ite != result.begin()) {
				result.erase(--ite);
			}
			result.emplace(*i);
