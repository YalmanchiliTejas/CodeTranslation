...
int main(){
  ...
  ll ans = 0;
  REP(b, 1, N+1){
    // 各ループで b を 1 ~ N に固定して、mod b >= Kとなるようなaの数を求める
    if (K >= b) continue;
    else{
      // まず、bの倍数までの間には、K周期で (b-1)-(K-1)=b-K 個ずつある
      // 正確には、bの倍数それ自身は余り0なので当てはまらないが、影響しないので無視

      // 他に、例えばN%b=5で、K=3なら、a%b=3,4,5となるものがあるので、仕上げにそれを足す
