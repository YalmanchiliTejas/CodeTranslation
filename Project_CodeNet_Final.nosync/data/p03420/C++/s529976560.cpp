...
int str_replace(string &str, const string &from, const string &to, int limit = -1) {
...
signed main() {
    ...
    ll count = 0;
    for (int base = k + 1; base <= n; ++base) {
        for (int i = k; i <= n; i += base) {
            ll d = min(n, (i + base - 1) / base * base);
            count += min(base - (ll)k, d - i + 1);
        }
    }
    cout << count << "\n";
    return 0;
