...
#define REP(i,a,n) assert(n < 0x80000000); for(int i = (a), i##_max = n; i < i##_max; ++i)
#define REM(i,a,n) assert(n < 0x80000000); for(int i = int(n) - 1, i##min = a; i >= i##min; --i)
...
LL func(LL b) {
	if (b <= K) return 0;
	LL turn = (N + 1) / b;
	return turn * (b - K) + max(0LL, (N + 1) - turn * b - K);
}
