...
template<class T> inline void rotate(T&a) { rotate(a.begin(), a.begin() + 1, a.end()); }
...
public:
  ...
  template <class T> operator T() {assert(n==-1); assert(m==-1); T ret; cin >> ret; return ret; }
  template <class T> operator vector<T>() {assert(n>=0); assert(m==-1); vector<T> ret(n); for(ll i=0;i<(ll)n;++i) cin>>ret[i]; return ret; }
