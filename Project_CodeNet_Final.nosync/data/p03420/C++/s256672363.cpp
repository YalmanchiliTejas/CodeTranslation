...
void _main()
{
    ...
    srep (mod, resi+1LL, maxval+1LL) {
        ll floor = resi;
        ll ceil = mod-1LL;
        for ( ; ; ) {
            if (floor > maxval) break;
            debug_printf("mod=%lld floor=%lld ceil=%lld\n", mod, floor, ceil);
            ll lfloor = min(floor, maxval);
            ll lceil = min(ceil, maxval);
            ans += (lceil - lfloor + 1LL);
            floor += mod;
            ceil += mod;
        }
    }
