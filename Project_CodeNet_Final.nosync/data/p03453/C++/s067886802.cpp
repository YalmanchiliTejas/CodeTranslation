    ...
    while ( !Q.empty() ) {
        ...
        each(e, G[v]) {
            if (res[v] + e.cost < res[e.to]) {
                ...
                Q.push({res[e.to], e.to});
            }
        }
    }
    return res;
}
vector<ll> count(const vector<vector<ll>>& G, ll s) {
    const ll n = G.size();
    vector<ll> dp(n, 0), cnt(n, 0);
    rep(v, n) each(to, G[v]) cnt[to]++;
    dp[s] = 1;
    queue<ll> Q;
...
int main() {
    ...
    // cout << dist << endl;
    ll T = dist[t];
    vector<ll> scnt = count(g, s);
    vector<ll> tcnt = count(rg, t);
    // cout << scnt << endl;
    // cout << tcnt << endl;
    assert(scnt[t] == tcnt[s]);
    ll ans = mul(scnt[t], tcnt[s]);
    // 辺で会う
    rep(v, n) each(to, g[v]) {
        assert(dist[v] < dist[to]);
        if (dist[v]*2 < T && dist[to]*2 > T) {
            ll x = mul(scnt[v], tcnt[to]);
            ans = sub(ans, mul(x, x));
        }
    }
    // 頂点で会う
