...
public:
	...
	Graph(ll sizeget = 0, ll modeget = 0, ll mapmodeget = 0, lentype zlenget = 0, lentype ilenget = INF) {
		size = sizeget;
		mode = modeget;
		mapmode = mapmodeget;
		zlen = zlenget;
...
signed main() {
	...
	Ssr(T);
	Tsr(S);
	ll ans = Mod(Sre[T].fi*Tre[S].fi);
	REP(i, N) {
		if (Sd[i].fi * 2 == ti) {
			if (Sre[i].se&&Tre[i].se) ans = Mod(ans - Mod(Sre[i].fi * Sre[i].fi) * Mod(Tre[i].fi * Tre[i].fi));
		}
		else {
			if (Sd[i].fi * 2 < ti) {
				if (Sre[i].se) {
					ROR(Td[i].se, j) {
						if (Sd[j].fi * 2 > ti) {
							if (Tre[j].se) {
								ans = Mod(ans - Mod(Sre[i].fi * Sre[i].fi) * Mod(Tre[j].fi * Tre[j].fi));
							}
						}
					}
				}
