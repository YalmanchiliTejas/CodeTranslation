...
struct Node {
	// いずれも0:s→t、1:t→s
	long long path_num[2]{}, distance[2]{1LL << 60, 1LL << 60};
...
void dijkstra(int from, int direct)
{
	...
	while (!pq.empty())
	{
		pil dist{pq.top()};
		pq.pop();
		if (dist.first > graph[dist.second].distance[direct]) continue;
		graph[dist.second].distance[direct] = dist.first;
		for (auto& e: graph[dist.second].edges)
			if (graph[dist.second].distance[direct] + e.second < graph[e.first].distance[direct])
			{
				graph[e.first].distance[direct] = graph[dist.second].distance[direct] + e.second;
