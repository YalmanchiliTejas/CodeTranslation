...
vl f(vector<vpli>&E,int s,ll inf,vl&x){
	using P=pair<ll,pii>;
	vl d(E.size(),inf);
	...
	while(pq.size()){
		...
		if(d[v]<a.first)continue;
		if(v==a.second.second){
			assert(v==s);
			x[v]=1;
		}else{
			(x[v]+=x[a.second.second])%=MD;
		}
		if(!b[v]){
			b[v]=true;
			for(pli e:E[v]){
				if(d[v]+e.first<=d[e.second]){
					d[e.second]=d[v]+e.first;
					pq<<P{d[v]+e.first,{e.second,v}};
				}
			}
		}
	}
...
int main(){cin.tie(0);ios::sync_with_stdio(false);
	...
	vl x(N),y(N);
	vl A=f(E,S,e18,x),B=f(E,T,e18,y);
	assert(A[T]==B[S]);
	assert(x[T]==y[S]);
	ll t=A[T];
	ll z=pow(x[T],2,MD);
	fr(i,N){
		if(A[i]==B[i]&&A[i]*2==t){
			z-=pow(x[i],2,MD)*pow(y[i],2,MD)%MD;
			z=(z+MD)%MD;
		}
	}
	fr(i,N){
		for(pli p:E[i]){
			if(A[i]*2<t&&B[p.second]*2<t&&A[i]+p.first+B[p.second]==t){
				z-=pow(x[i],2,MD)*pow(y[p.second],2,MD)%MD;
				z=(z+MD)%MD;
			}
		}
	}
