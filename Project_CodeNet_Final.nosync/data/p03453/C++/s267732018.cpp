...
int main() {
	...
	Dijkstra(g, s, d[0]);
	Dijkstra(g, t, d[1]);
	Matrix dp(2, Array(n, 0));
	dp[0][s] = 1; dp[1][t] = 1;
	auto find = [&](auto f,ll v,ll j)->decltype(v,j) {
		if (dp[j][v] != 0)return dp[j][v];
		ll ret = 0;
		REP(i, g[v].size()) {
			ll to = g[v][i].to;
			ll l = g[v][i].cap;
			if (d[j][v] == d[j][to] + l)(ret += f(f,to, j)) %= MOD;
		}
		return dp[j][v] = ret;
	};
	Y(find)(t, 0);
	Y(find)(s, 1);
	ll total = dp[0][t] * dp[1][s] % MOD;
	ll len = d[0][t];
	REP(i, n) {
		if (len % 2 == 0 && d[0][i] == len / 2) {
			(total += MOD -mod_pow( dp[0][i] * dp[1][i] % MOD,2,MOD)) %= MOD;
		}
		else {
			REP(j, g[i].size()) {
				ll to = g[i][j].to;
				ll cap = g[i][j].cap;
				if (d[0][i] <= len / 2 && d[0][i] + cap == d[0][to] && d[0][to] > len / 2) {
					(total += MOD -mod_pow( dp[0][i] * dp[1][to] % MOD,2,MOD)) %= MOD;
				}
			}
		}
	}
