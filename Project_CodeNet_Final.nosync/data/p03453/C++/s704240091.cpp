...
int main(void)
{
	...
	for(lli i = 0;i < n;i++)
	{
		...
		if(D[i][0]+D[i][1] == dis && D[i][0] == dis/2ll)
		{
			ans += (((W[i][0]*W[i][1])%MOD)*((W[i][0]*W[i][1])%MOD))%MOD;
			ans %= MOD;
		}
		for(auto it: graph[i])
		{
			if(D[i][0]+it.second+D[it.first][1] == dis && D[i][0] < dis/2 && D[it.first][1] < dis/2)
			{
				ans += (((W[i][0]*W[it.first][1])%MOD)*((W[i][0]*W[it.first][1])%MOD))%MOD;
				ans %= MOD;
			}
		}
	}
 
	printf("%lld\n", lliMOD((W[t][0]*W[t][0])%MOD-ans%MOD, MOD));
}