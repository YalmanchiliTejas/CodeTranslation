...
void Run(int ss, LL* dist, LL *cnt) {
    for(int i = 1; i <= n; ++i) dist[i] = 1LL << 60, cnt[i] = 0, deg[i] = 0;
    priority_queue<pair<LL,int>> que;
    dist[ss] = 0; que.push(MP(0LL,ss));
    while(!que.empty()) {
        pair<LL,int> x = que.top(); que.pop();
        if(-x.fi != dist[x.se]) continue;
        for(auto it : G[x.se])
            if(-x.fi + it.se < dist[it.fi]) {
                dist[it.fi] = -x.fi + it.se;
    ...
    queue<int> qu2;
    cnt[ss] = 1; qu2.push(ss);
    while(!qu2.empty()) {
        int x = qu2.front(); qu2.pop();
        for(auto it : G[x])
            if(dist[x] + it.se == dist[it.fi]) {
                cnt[it.fi] = (cnt[it.fi] + cnt[x]) % md, --deg[it.fi];
                if(!deg[it.fi])
                    qu2.push(it.fi);
            }
    }
