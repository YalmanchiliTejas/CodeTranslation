...
int main(void){
	...
	llint syo=Tdis[s];//最短時間
	vector<pair<llint,int>> Sso(n);//sからの時間順にソート
	for(i=0;i<n;i++){Sso[i]=mp(Sdis[i],i);}SO(Sso);
	vector<llint> Sdp(n);Sdp[s]=1;//Sからの生き方
	vector<pair<llint,int>> Tso(n);//tからの時間順にソート
	for(i=0;i<n;i++){Tso[i]=mp(Tdis[i],i);}SO(Tso);
	vector<llint> Tdp(n);Tdp[t]=1;//Tからの生き方
	for(i=0;i<n;i++){
		int ter=Sso[i].sec;
		for(auto it:go[ter]){
			if(Sdis[ter]+Tdis[it.sec]+it.fir>syo){continue;}//not 最短
	...
	for(i=0;i<n;i++){
		...
		for(auto it:go[ter]){
			if(Tdis[ter]+Sdis[it.sec]+it.fir>syo){continue;}//not 最短
