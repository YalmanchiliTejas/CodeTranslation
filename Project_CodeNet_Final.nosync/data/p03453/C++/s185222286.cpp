...
//dijkstra_cou<mint> : 数える型で書く
template<class COU,class T=int> auto dijkstra_cou(const graph<T> &g, int s, int init_value = -1) {
    if (!(0 <= s && s < g.n)) {
        debugline("dijkstra");
        deb(s, g.n);
        ole();
    ...
    while (q.size()) {
        ...
        for (auto &&e  : g.g[i]) {
            ...
            else if (dis[to] == c) { cou[to] += cou[e.f]; }
        }
    }    /*基本、たどり着かないなら-1*/    for (auto &&d :dis) if (d == initValue)d = init_value;
    return vtop(dis, cou);
}
//コストを無限に減らせる := -linf
//たどり着けない := linf
...
void solve() {
    ...
    auto ss = dijkstra_cou<mint>(g, s);
    auto tt = dijkstra_cou<mint>(g, t);
    vi ds = keys(ss);
    vm cs = values(ss);
    vi dt = keys(tt);
    vm ct = values(tt);
...
#define cour ct
    rep(i, N) {
        int nc = ds[i];
        if (nc * 2 == D)zen -= coul[i] * cour[i] * coul[i] * cour[i];
        forg(gi, g[i]) {
            int tc = nc + c;
            //必要最短経路に含まれていたら
            int allc = tc + dt[t];
            if (nc * 2 < D && tc * 2 > D && allc == D) {
                zen -= coul[i] * coul[i] * cour[t] * cour[t];
            }
        }
    }
    //hen
    //s -> t
//    fore(gi, g.edges) {
//        if (ds[f] * 2 < D && ds[t] * 2 > D &&dt[t] * 2 < D && dt[s] * 2 > D) {
//            if (ds[f] + c + dt[t] == D) {
//                deb(f, t);
//                zen -= cs[f] * ct[t] * ct[t] * cs[f];
//            }
//        }
//    }
    cout << zen << endl;
