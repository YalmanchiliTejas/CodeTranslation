...
ll n, m, s, t, u, v, d, cost[2][100000], counter[2][100000];
Q q[200000];
vector<P> e[100000];
bool used[100000];
vector<ll> cord;

void dijkstra(ll s, ll cost[100000], ll counter[100000]) {
  for (ll i = 0; i < n; i++) cost[i] = INF;
  cost[s] = 0;
  counter[s] = 1;
  priority_queue<P, vector<P>, greater<P>> que;
  que.push(P(0, s));
  while (!que.empty()) {
    P p = que.top(); que.pop();
    ll cst = p.first, from = p.second;
    for (const P& i : e[from]) {
      ll to = i.second, newcst = cst + i.first;
      if (newcst < cost[to]) cost[to] = newcst, counter[to] = counter[from], que.push(P(newcst, to));
      else if (newcst == cost[to]) counter[to] = (counter[to] + counter[from]) % mod;
    }
  }
}
...
int main() {
  ...
  dijkstra(s, cost[0], counter[0]);
  dijkstra(t, cost[1], counter[1]);
  ll len = cost[0][t];
  for (ll i = 0; i < n; i++) if (cost[0][i] + cost[1][i] == len && cost[0][i] == cost[1][i]) {
    used[i] = true;
    cord.push_back(counter[0][i] * counter[1][i] % mod);
  }
  for (ll i = 0; i < m; i++) {
    ll cst = q[i].first, c1 = q[i].second.first, c2 = q[i].second.second;
    if (used[c1]) continue;
    if (used[c2]) continue;
    if (cst + cost[0][c1] + cost[1][c2] == len && 2 * cost[0][c1] < len && 2 * cost[1][c2] < len) {
      cord.push_back(counter[0][c1] * counter[1][c2] % mod);
    }
    if (cst + cost[0][c2] + cost[1][c1] == len && 2 * cost[0][c2] < len && 2 * cost[1][c1] < len) {
      cord.push_back(counter[0][c2] * counter[1][c1] % mod);
    }
  }
  ll sum = 0;
  for (const ll& i : cord) sum = (sum + i) % mod;
  ll ans = 0;
  for (const ll& i : cord) ans = (ans + i * (sum - i + mod)) % mod;
  printf("%lld\n", ans);
}
