...
struct Graph {
    ...
    vector<T> dijkstra(int s, vector<mint>& dp);
...
template <typename T>
vector<T> Graph::dijkstra(int s, vector<mint>& dp) {
    // modify T's INF
...
void solve() {
    ...
    auto ds = g.dijkstra<ll>(s, dps);
    auto dt = g.dijkstra<ll>(t, dpt);
