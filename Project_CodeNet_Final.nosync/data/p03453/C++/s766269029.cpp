	...
	while(!que.empty()){
		...
		for(auto u:E[p.second]){
			if(d1[u.second]>p.first+u.first){
				...
				dp1[u.second]=dp1[p.second];
				que.push(P(d1[u.second],u.second));
			}
			else if(d1[u.second]==p.first+u.first){
				(dp1[u.second]+=dp1[p.second])%=MOD;
			}
		}
	}
	
	memset(d2,0x3f,sizeof(d2));
	d2[t]=0;dp2[t]=1;que.push(P(0,t));
	while(!que.empty()){
		P p=que.top();que.pop();
		...
		for(auto u:E[p.second]){
			if(d2[u.second]>p.first+u.first){
				...
				dp2[u.second]=dp2[p.second];
				que.push(P(d2[u.second],u.second));
			}
			else if(d2[u.second]==p.first+u.first){
				(dp2[u.second]+=dp2[p.second])%=MOD;
			}
		}
	}
