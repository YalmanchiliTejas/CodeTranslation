...
#pragma endregion
#pragma region graph shortest path dijkstra
/**
* @brief ダイクストラ法
...
pair<vector<T>, vector<M>> dijkstra_count(const Graph<T> &g, int frm) {
    vector<T> dist(g.V, GINF<T>);   dist[frm] = T(0);
    vector<M> pcnt(g.V, M(0));      pcnt[frm] = M(1);
    using P = pair<T, int>;
    priority_queue<P, vector<P>, greater<P>> pq;
    pq.emplace(dist[frm], frm);
    while (not pq.empty()) {
        T cst; int idx;
        tie(cst, idx) = pq.top(); pq.pop();
        if (dist[idx] < cst) continue;
        for (auto& e : g.mat[idx]) {
            T nxt_cst = cst + e.cst;
            if (dist[e.to] > nxt_cst) {
                dist[e.to] = nxt_cst;
                pcnt[e.to] = pcnt[idx];
                pq.emplace(dist[e.to], e.to);
            } else if (dist[e.to] == nxt_cst) {
                pcnt[e.to] += pcnt[idx];
            }
        }
    }
    return make_pair(dist, pcnt);
}
#pragma endregion
...
void solve() {
    ...
    --S, --T;
    Graph<ll> g(N);
    g.input_edges(M, 1, true);
    auto [dij_s, cnt_s] = dijkstra_count<modint>(g, S);
    auto [dij_t, cnt_t] = dijkstra_count<modint>(g, T);

    modint res = cnt_s[T] * cnt_t[S];
    ll shortest = dij_s[T];

    {   // 頂点で出会うとき
        REP(i, N) {
            ll s = dij_s[i], t = dij_t[i];
            modint sc = cnt_s[i], tc = cnt_t[i];
            if (s != t) continue;
            if (s + t != shortest) continue;
            modint sub = sc * tc;
            res -= sub * sub;
        }
    }
    ...
    {   // 辺上で出会うとき
        REP(i, N) {
            for (auto& e: g.mat[i]) {
                // s -> i と t -> e
                ll cst = e.cst;
                ll s = dij_s[i], t = dij_t[e];
                modint sc = cnt_s[i], tc = cnt_t[e];
                if (s + cst + t != shortest) continue;
                ll d = shortest - 2 * s;
                if (0 < d and d < 2 * cst) {
                    modint sub = sc * tc;
                    res -= sub * sub;
