...
//    cout.rdbuf(out.rdbuf());
    ...
    while (!que.empty()) {
      ...
      FOR (e, g[u]) {
        if (chkmin(d[e.first], d[u] + e.second)) {
          que.ep(d[e.first], e.first);
        }
      }
    }
  };

  vl d_s(n, LLONG_MAX), d_t(n, LLONG_MAX);
  dijkstra(s, d_s);
  dijkstra(t, d_t);
  ...
  sort(all(v), [&](int x, int y){return d_s[x] < d_s[y];});

  vector<Mint> dp_s(n), dp_t(n);
  dp_s[s] = 1;
  FOR (x, v) {
    if (d_s[x] != LLONG_MAX) {
      FOR (e, g[x]) {
        if (d_s[e.first] == d_s[x] + e.second) {
          dp_s[e.first] += dp_s[x];
        }
      }
    }
  ...
  if (D % 2 == 0) {
    rng (i, 0, n) {
      if (d_s[i] == D / 2) {
        ans += dp_s[i] * dp_s[i] * dp_t[i] * dp_t[i];
      }
    }
  }
  ...
  rng (x, 0, n) {
    FOR (e, g[x]) {
      int y = e.first;
      if (d_s[x] < (D + 1) / 2 && d_t[y] < (D + 1) / 2 && d_s[x] + d_t[y] + e.second == D) {
        ans += dp_s[x] * dp_s[x] * dp_t[y] * dp_t[y];
      }
    }
  }
  println(dp_s[t] * dp_s[t] - ans);
