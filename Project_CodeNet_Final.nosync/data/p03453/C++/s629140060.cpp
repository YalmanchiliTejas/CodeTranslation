...
int dist[2][100010];
modInt dp[2][100010];
vector<P> vec[100010], nodes[2];
void dijkstra(int f) {
	REP(i, n)dist[f][i] = INF;
	prique(P) que;
	if (!f) {
		dist[f][s] = 0;
	...
	while (!que.empty()) {
		P p = que.top();
		que.pop();
		if (dist[f][p.second] < p.first)continue;
		for (P e : vec[p.second]) {
			if (dist[f][e.first] > dist[f][p.second] + e.second) {
				dist[f][e.first] = dist[f][p.second] + e.second;
				que.push(make_pair(dist[f][e.first], e.first));
			}
		}
	}
}
signed main() {
	...
	rep(i, m) {
		cin >> u >> v >> d;
		vec[u].emplace_back(v, d);
		vec[v].emplace_back(u, d);
	}
	dijkstra(0);
	dijkstra(1);
	rep(i, 2) {
		REP(j, n)nodes[i].emplace_back(dist[i][j], j);
		sort(all(nodes[i]));
		if (!i)dp[i][s] = 1;
		else dp[i][t] = 1;
		rep(j, n) {
			for (P e : vec[nodes[i][j].second]) {
				if (dist[i][nodes[i][j].second] + e.second == dist[i][e.first]) {
					dp[i][e.first] += dp[i][nodes[i][j].second];
				}
			}
		}
	}
	modInt ans = dp[0][t] * dp[0][t];
	REP(i, n) {
		if (dist[0][i] == dist[1][i] && dist[0][i] + dist[1][i] == dist[0][t]) {
			ans -= dp[0][i] * dp[0][i] * dp[1][i] * dp[1][i];
		}
		for (P e : vec[i]) {
			if (dist[0][i] + dist[1][i] == dist[0][t] && dist[0][e.first] + dist[1][e.first] == dist[0][t] && dist[0][i] < dist[1][i] && dist[0][e.first] > dist[1][e.first] && dist[0][i] + e.second == dist[0][e.first]) {
				ans -= dp[0][i] * dp[0][i] * dp[1][e.first] * dp[1][e.first];
			}
		}
	}
	cout << ans << endl;
