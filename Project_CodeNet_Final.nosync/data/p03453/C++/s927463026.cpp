...
//    DBN(t, nums[t]);
//    DBN(ansDist);
    ll ans = nums[t] * nums[t] % mod;
//    DBN(ans);
    FI(n) {
//        DBN(i, dps[i], dpt[i], ansDist);
        if (dps[i] + dpt[i] == ansDist && dps[i] == dpt[i]) {
            ans -= sqr(nums[i] * numt[i] % mod) % mod;
            if (ans < 0) ans += mod;
            med[i] = true;
//            DBN(i, sqr(nums[i] * numt[i] % mod) % mod);
        }
    }
    FI(n) {
        for (auto e : v[i]) {
            ...
            if (dps[from] + e.second + dpt[to] == ansDist && ((dpt[to] + e.second >= dps[from] && dpt[to] <= dps[from])
                                                              || (dps[from] + e.second >= dpt[to] && dpt[to] >= dps[from])
                                                              )) {
                ans -= sqr(nums[from] * numt[to] % mod) % mod;
//                DBN(i, e, sqr(nums[from] * numt[to] % mod) % mod);
                if (ans < 0) ans += mod;
            }
        }
    }
