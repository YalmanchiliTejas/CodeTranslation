...
void make_linklist(const V<t3> &srclist, V<t2> dstlist[])
{
    for (auto src : srclist) {
        ll a = get<0>(src);
        ll b = get<1>(src);
...
void make_costs(ll costs[100005], ll cnts[100005],
                const V<t2> linklist[100005], ll n, ll s)
{
    priority_queue_incr<t2> q;
    S bool checked[100005];
    memset(checked, 0, sizeof(checked));
    rep (i, 100005) costs[i] = -1LL;
    ...
    while (!q.empty()) {
        ...
        for (auto nvd : linklist[v]) {
            ll nv = get<0>(nvd);
            ll d = get<1>(nvd);
            ll ncost = cost + d;
            if (costs[nv] < 0LL || ncost < costs[nv]) {
                q.push(t2(ncost, nv));
                costs[nv] = ncost;
                cnts[nv] = cnts[v];
            }
            else if (ncost == costs[nv]) {
                cnts[nv] += cnts[v];
                cnts[nv] %= MOD;
            }
        }
    ...
    debug_printf("------- make_costs s=%lld\n", s);
    debug_printf("---- costs\n");
    debug_print(costs, n+1);
    debug_printf("---- cnts\n");
...
void _main()
{
    ...
    V<t2> linklist[100005];
    make_linklist(uvdlist, linklist);

    S ll s_costs[100005];
    S ll s_cnts[100005];
    make_costs(s_costs, s_cnts, linklist, n, s);

    S ll t_costs[100005];
    S ll t_cnts[100005];
    make_costs(t_costs, t_cnts, linklist, n, t);

    assert(s_costs[t] == t_costs[s]);
    assert(s_cnts[t] == t_cnts[s]);

    ll total_cost = s_costs[t];
    ll half_cost = total_cost / 2LL;
    ll ans = s_cnts[t] * s_cnts[t];
    ans %= MOD;

    if (!(total_cost & 1LL)) {
        srep (v, 1, n+1) {
            if (s_costs[v] != t_costs[v]) continue;
            debug_printf("v=%lld %lld %lld\n", v, s_costs[v], half_cost);
            assert(s_costs[v] == half_cost);
            ll decr = 1LL;
            decr *= s_cnts[v];
            decr %= MOD;
            decr *= s_cnts[v];
    ...
    for (auto uvd : uvdlist) {
        ...
        debug_printf("---- edge u=%lld v=%lld d=%lld\n", u, v, d);

        if (abs(s_costs[u] - s_costs[v]) != d) continue;
        if (abs(t_costs[u] - t_costs[v]) != d) continue;
        if (s_costs[u] + t_costs[v] + d != total_cost &&
            s_costs[v] + t_costs[u] + d != total_cost) continue;

        if (s_costs[u] < half_cost && s_costs[v] > half_cost &&
            t_costs[u] > half_cost && t_costs[v] < half_cost) {
            assert(abs(t_costs[u] - t_costs[v]) == d);
            ll decr = 1LL;
            decr *= s_cnts[u];
            decr %= MOD;
            decr *= s_cnts[u];
