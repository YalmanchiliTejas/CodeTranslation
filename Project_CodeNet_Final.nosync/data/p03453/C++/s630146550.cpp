...
void dijkstra_kaizo (int s, sll distance[], ull ways[]) {
	priority_queue<P, std::vector<P>, greater<P> > que; // (最短距離, 頂点番号)
	que.push(P(distance[s], s));
	...
	while (!que.empty()) {
		...
		for (int32_t i = 0; i < maxsize; i++) {
			...
			if (distance[e.to] > distance[v] + e.cost) {
				...
				ways[e.to] = ways[v];
			} else if (distance[e.to] == distance[v] + e.cost) {
...
ull solve () {
	...
	for (i = 0; i < m; i++) {
		path[i].from = path[i + m].to = a[i];
		path[i].to = path[i + m].from = b[i];
		path[i].cost = path[i + m].cost = c[i];
	}
	dijk_init(m * 2, path);

	dijk_distinit(vua, dist1, n);
	dijk_distinit(vub, dist2, n);

	for (i = 0; i < n; i++) {
		ways1[i] = ways2[i] = 0;
	}
	ways1[vua] = ways2[vub] = 1;

	dijkstra_kaizo(vua, dist1, ways1);
	dijkstra_kaizo(vub, dist2, ways2);

	const ull pathlen = dist1[vub];
	...
	for (i = 0; i < n; i++) {
		if (dist1[i] * 2 == pathlen && dist2[i] * 2 == pathlen) {
			// beforesum = (beforesum + ways1[i]) % MOD;
			// aftersum = (aftersum + ways2[i]) % MOD;
			collisionsum = (collisionsum + bitpow((ways1[i] * ways2[i]) % MOD, 2, MOD)) % MOD;
		}
	}
	for (i = 0; i < m * 2; i++) {
		sll f = path[i].from;
		sll t = path[i].to;
		if (dist1[f] + dist2[t] + path[i].cost == pathlen && dist1[f] * 2 < pathlen && dist2[t] * 2 < pathlen) {
			// beforesum = (beforesum + ways1[path[i].from]) % MOD;
			// aftersum = (aftersum + ways2[path[i].to]) % MOD;
			collisionsum = (collisionsum + bitpow((ways1[f] * ways2[t]) % MOD, 2, MOD)) % MOD;
		}
	}

	result = (MOD + (ways1[vub] * ways2[vua] % MOD) - collisionsum) % MOD;
