...
struct graph{
    ...
    graph(ll n): N(n), G(N), d(n,INF2), d2(n,INF2), now(n,0) {}
    ...
    void dijkstra3(ll s){
        ...
        while(!que.empty()){
            ...
            if(d2[v]*2==tyu) ct2[v]=now[v];
            rep(i,G[v].size()){
                edge& E = G[v][i];
                if(mini(d2[E.to], d2[v]+E.cost)){
                    now[E.to]=now[v];
                    que.emplace(d2[E.to], E.to);
                }
                else if(d2[E.to]==d2[v]+E.cost){
                    now[E.to]+=now[v];
                    now[E.to]%=MOD;
                }
                if(d2[v]*2<tyu and (d2[v]+E.cost)*2>tyu) ct2[E.num]=now[v];
            }
        }
    }
};
...
void Main() {
    ...
    rep(i,N+M){
        ...
        if(i>=N and (g.d[A[i-N]]+g.d2[B[i-N]]+D[i-N]!=tyu and g.d2[A[i-N]]+g.d[B[i-N]]+D[i-N]!=tyu)){
            1;
        }
        else{
