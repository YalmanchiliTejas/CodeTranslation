...
//////////////////
int N;
vector< vector<edge_base> > graOrg;
vector< vector<edge_base> > graDij;
vector< vector<LL> > dist;
vector<int> prevS,prevT;

void cal(int S,int T,int pat,vector< vector<edge_base> >& gra,vector<LL>& dp){
	int sideS,sideT;
	sideS = pat;
	sideT = (pat+1)%2;
	//dp = vector<LL>(N,0)
	dp[S] = 1;
	priority_queue< vector<LL>,vector< vector<LL> >, greater< vector<LL> > > minque; 
	int size = gra[S].size();
	vector<LL> temp(2);
	temp[0] = 0;
	temp[1] = S;
	...
	while( minque.size() ){
		vector<LL> now = minque.top();
		minque.pop();
		LL COST = now[0];//現在の総コスト
		int FROM = now[1];
		...
		int size = gra[FROM].size();
		for(int i=0;i<size;++i){
			int TO = gra[FROM][i].to;
			LL Len = gra[FROM][i].cost;
			if( COST + Len + dist[sideT][TO] != dist[sideS][T]){
				continue;
			}
			dp[TO] = (dp[TO] + dp[FROM]) % MOD;
			
			temp[0] = COST+Len;//dist[sideS][TO];
			temp[1] = TO;
			if( use[TO] == false ){
				minque.push( temp );
			}
...
void solve(){
	...
	cin>>N>>M>>S>>T;
	--S;--T;
	graOrg = vector< vector<edge_base> > (N);
	graDij = vector< vector<edge_base> > (N);
	for(int i=0;i<M;++i){
		int u,v,d;
		cin>>u>>v>>d;
		--u;--v;
		graOrg[u].push_back( make_edge_base(v,d) );
		graOrg[v].push_back( make_edge_base(u,d) );
	}
	
	dist = vector< vector<LL> >(2,vector<LL>(N,0));
	prevS =vector<int>(N);
	prevT =vector<int>(N);

	dijkstra( S,N,dist[0],prevS,graOrg);
	dijkstra( T,N,dist[1],prevT,graOrg);
	
	//[S,T]の最短経路で使うグラフに作り直す
	for(int u=0;u<N;++u){
		int size = graOrg[u].size();
		for(int i=0;i<size;++i){
			int v = graOrg[u][i].to;
			LL Len = graOrg[u][i].cost;

			//Su->vTだけみる
			if( dist[0][u]+Len+dist[1][v] == dist[0][T] ){
				graDij[u].push_back( make_edge_base(v,Len) );
				graDij[v].push_back( make_edge_base(u,Len) );
			}
		}
	}
	////////////////

	vector<LL> dpS(N,0),dpT(N,0);
	cal(S,T,0,graDij,dpS);
	cal(T,S,1,graDij,dpT);

	LL ans =  ( dpS[T] * dpS[T] ) % MOD;
	LL sub = 0;//引く数
	for(int u=0;u<N;++u){
		int size = graDij[u].size();
		if( size == 0 ){//最短経路上にない
			continue;
		}
		//頂点でぶつかる。
		...
		if( dist[0][u] == dist[1][u] && 
			(dist[0][u]+dist[1][u]) == dist[0][T] ){
			LL res = (dpS[u]*dpT[u]) % MOD;
			res = (res*res)%MOD;
			ans = (ans+MOD-res) % MOD;
		}
		...
		for(int j=0;j<size;++j){
			int v = graDij[u][j].to;
			LL Len = graDij[u][j].cost;
			//Su-vT
			if( dist[0][u] + Len + dist[1][v] != dist[0][T] ){
				continue;
			}
			LL Half = (dist[0][T]+1)/2;//最短距離が5なら2.5以上を3以上と言い換え
			if( dist[0][u] >= Half || dist[1][v] >= Half ){
				continue;
			}
			LL res = (dpS[u]*dpT[v]) % MOD;
			res = (res*res) % MOD;
			ans = (ans+MOD-res) % MOD;
		}
	}
