...
void counting(int start, vector<Int>& distOpp, vector<Int>& countV)
{
    using P = pair<Int, int>;
    priority_queue<P, vector<P>, greater<P>> Q;
    vector<bool> visited(N);
...
int main()
{
    ...
    int S, T; cin >> S >> T; S--, T--;
    graph.resize(N);
    vector<Int> distS(N), distT(N), countS(N), countT(N);
    ...
    dijkstra(S, distS);
    dijkstra(T, distT);
    counting(S, distT, countS);
    counting(T, distS, countT);
    Int all = 0, collision = 0;
    for (int i = 0; i < N; i++) {
        if (distS[i] * 2 == distS[T]) {
            Int pat = countS[i] * countT[i] % MOD;
            (all += pat) %= MOD;
    ...
    for (int i = 0; i < M; i++) {
        ...
        bool shortest = distS[u] + d + distT[v] == distS[T];
        bool half = distS[u] * 2 < distS[T] && distT[v] * 2 < distS[T];
        if (shortest && half) {
            Int pat = countS[u] * countT[v] % MOD;
            (all += pat) %= MOD;
            (collision += pat * pat) %= MOD;   
        }
    }
