...
int main()
{
	...
	vecl(B)
		B.insert(B.begin(), A.at(0));
	bool flag = true;
	while(true){
		if (flag) {
			B.insert(B.begin(), A.at(right));
			right--;
			if (right < left) { break; }
			B.insert(B.begin()+B.size(), A.at(right));
			right--;
		}
		else {
			B.insert(B.begin(), A.at(left));
			left++;
			if (right < left) { break; }
			B.insert(B.begin() + B.size(), A.at(left));
			left++;
		}
		flag = !flag;
	}
	fx(N-1) {
		ans1 +=abs( B.at(x) - B.at(x + 1) );
	}
	flag = true;
	left = 0;
	right = N - 2;
	B.clear();
	B.insert(B.begin(), A.at(N-1));
	while (true) {
		if (!flag) {
			B.insert(B.begin(), A.at(right));
			right--;
	...
	fx(N - 1) {
		ans2 += abs(B.at(x) - B.at(x + 1));
	}
	//ans2 += abs(A.at(left) - A.at(right));
	ce(max(ans1,ans2))
    return 0;
}
