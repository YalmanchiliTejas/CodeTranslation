...
VLL habayuusen_kyori(VVLL &V, LL n ){
  LL N = V.size();
  VLL D(N,-1);
...
bool ismujun(char a, LL l,LL m, LL r){
  bool ans;
  if(m==1){
    if(a=='o'){
      ans =  l==r;
...
string  funcsub(string &S, LL a0, LL a1)
{
  LL N = S.length();
  VLL A(N,-1);
  A[0] = a0;
  ...
  REPM(i,N-1,1){
    ...
    else if(A[i]==1 and S[i]=='x'){
      A[i+1] = 1-A[i-1];
    }
    else if(A[i]==0 and S[i]=='o'){
      A[i+1] = 1-A[i-1];
    }
    else if(A[i]==0 and S[i]=='x'){
      A[i+1] = A[i-1];
    }
  }
  bool ans = ismujun(S[0], A[N-1],A[0],A[1]) and ismujun(S[N-1], A[N-2],A[N-1],A[0]);
  string result = "";
  if(ans){
    for(auto a :A){
      if(a==0){
...
void func()
{
  ...
  while(not D.empty()){
    LL d1 = abs(D.front()-B.front());
    LL d2 = abs(D.front()-B.back());
    LL d3 = abs(D.back()-B.front());
    LL d4 = abs(D.back()-B.back());

    if(d1>=d2 and d1>=d3 and d1>=d4){
      B.push_front(D.front());
      D.pop_front();
    }
