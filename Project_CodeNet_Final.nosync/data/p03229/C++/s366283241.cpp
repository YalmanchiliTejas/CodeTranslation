...
signed main() {
	...
	sRev = A;
	sort(s.begin(), s.end());
	sort(sRev.begin(), sRev.end(), greater<int>());
	...
	if (N % 2 == 0) {
		...
		rep(i, mergeNum) {
			merge1.push_back(s[i]);
			merge1.push_back(sRev[i]);
			merge2.push_back(sRev[i]);
			merge2.push_back(s[i]);
		}
		cand.push_back(merge1);
		cand.push_back(merge1);
		cand.push_back(merge2);
		cand.push_back(merge2);

		rep(i, 4) {
			if (i % 2 == 0) {
				cand[i].insert(cand[i].begin(), s[N / 2 - 1]);
				cand[i].push_back(s[N / 2]);
			}
			else {
				cand[i].insert(cand[i].begin(), s[N / 2]);
				cand[i].push_back(s[N / 2-1]);
			}
		}
		int ans = 0;
		rep(i, 4) {
