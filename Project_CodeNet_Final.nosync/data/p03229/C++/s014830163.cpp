...
signed main() {
    ...
    if (N == 2) return output(diff(a[0], a[1])), 0;
    intmax_t res1 = 0;
    {
        sort(all(a));
        vector<intmax_t> l, g;
        ...
        if (N & 1) {
            res1 -= l[0] + l[1];
            rep(i, 2, l.size()) res1 -= 2 * l[i];
            rep(i, 0, g.size()) res1 += 2 * g[i];
        } else {
            res1 -= l[0];
            res1 += g[0];
            rep(i, 1, l.size()) res1 -= 2 * l[i];
            rep(i, 1, g.size()) res1 += 2 * g[i];
        }
    }
    intmax_t res2 = 0;
    {
        ...
        if (N & 1) {
            res2 += g[0] + g[1];
            rep(i, 2, g.size()) res2 += 2 * g[i];
            rep(i, 0, l.size()) res2 -= 2 * l[i];
        } else {
            res2 -= l[0];
            res2 += g[0];
            rep(i, 1, l.size()) res2 -= 2 * l[i];
            rep(i, 1, g.size()) res2 += 2 * g[i];
        }
    }
    output(max(res1, res2));
    return 0;
}
