...
ll res(deque<ll>& dq){
    ...
    deque<ll> r;
    r.pb(dq.front()); dq.pop_front();
    while (!dq.empty()) {
        if(!dq.empty()){
            r.push_front(dq.back()); dq.pop_back();
        }
        if(!dq.empty()){
            r.push_back(dq.back()); dq.pop_back();
        }
        if(!dq.empty()){
            r.push_front(dq.front()); dq.pop_front();
        }
        if(!dq.empty()){
            r.push_back(dq.front()); dq.pop_front();
        }
    }
    
    ll res = 0;
    rep(i, N-1) res += abs(r[i+1]-r[i]);
    return res;
}
