...
struct BigInt10{
    ...
    bool operator<(BigInt10& n2) const{
        if (num.length() < n2.n_digits()) return true;
        else if (num.length() > n2.n_digits()) return false;
        return num < n2.num;
    }
...
void read_vector(ll n, vl& v1, vl& v2, ll offset1 = 0, ll offset2 = 0){
...
void solve(){
    ...
    ll n; in1(n); vl as; read_vector(n, as);

    sort(all(as));
    ll ans = 0;
    if (n % 2 == 0){
        rep(i, n / 2) ans -= i != n / 2 - 1 ? 2 * as[i] : as[i];
        For(i, n / 2, n) ans += i != n / 2 ? 2 * as[i] : as[i];
    }else{
        ll v;
        rep(i, n / 2) ans -= 2 * as[i];
        For(i, n / 2, n) ans += i != n / 2 && i != n / 2 + 1 ? 2 * as[i] : as[i];
        v = ans; ans = 0;
        rep(i, n / 2 + 1) ans -= i != n / 2 - 1 && i != n / 2 ? 2 * as[i] : as[i];
        For(i, n / 2 + 1, n) ans += 2 * as[i];
        chmax(ans, v);
    }
    prl(ans);
