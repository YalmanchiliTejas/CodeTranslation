...
int main() {
  ...
  if (N % 2 == 0) {
    ...
    sort(all(A));
    ll ans =
      A[N / 2] + accumulate(A.begin() + N / 2 + 1, A.end(), 0LL) * 2
      - A[N / 2 - 1] - accumulate(A.begin(), A.begin() + N / 2 - 1, 0LL) * 2;
    cout << ans << endl;
  }
  else {
    // odd
    ...
    // 1 2 -2 2 1
    sort(all(A));
    ll lans, rans, h = (N + 1) / 2;
    lans = accumulate(A.begin() + h, A.end(), 0LL) * 2 - A[h - 2] - A[h - 1] - accumulate(A.begin(), A.begin() + h - 2, 0LL) * 2;
    h--;
    rans = A[h] + A[h + 1] + accumulate(A.begin() + h + 2, A.end(), 0LL) * 2 - accumulate(A.begin(), A.begin() + h, 0LL) * 2;
