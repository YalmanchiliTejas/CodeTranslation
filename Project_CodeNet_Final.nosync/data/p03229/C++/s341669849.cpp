...
int main(){
    ...
    if(N % 2 == 0){
        ...
        int idx = 0;
        for(int i = 1; i < N; i += 2){B[i] = A[idx];idx++;}
        B[0] = A[idx]; idx++;
        for(int i = N - 1; i > 0; i -= 2) {B[i] = A[idx];idx++;}
        rep(i, 0, N - 1) ans += abs(B[i+1] -  B[i]);
        // cerr << B << endl;
        ll tans = 0;
        idx = 0;
        for(int i = 2; i < N; i += 2) {B[i] = A[idx];idx++;}
        B[0] = A[idx]; idx++;
        for(int i = N - 2; i >= 0; i -= 2) {B[i] = A[idx];idx++;}
        rep(i, 0, N - 1) tans +=abs(B[i+1] - B[i]);
        chmax(ans, tans);
    }
    cout << ans << endl;
    return 0;
