...
#define repi(i,a,b) for(long long i = static_cast<long long>(a); i < static_cast<long long>(b); i++)
#define pb push_back
...
template<class T1, class T2> bool chmax(T2 &a, T2 b) { return a < b && (a = b, true); }
...
int main(void) {
    ...
    //std::cout << A << std::endl;
    ll ret = 0;
    ll numplus = N % 2 == 1? (N-3)/2 : (N-2)/2;
    ll cnt = 0;
    for (ll i = 0; i < numplus; i++) {
        ret += 2*A[i];
        cnt++;
    }
    ret += A[cnt++];
    ...
    ll ret2 = 0;
    ll numplus2 = N % 2 == 1? (N-1)/2 : (N-2)/2;
    cnt = 0;
    for (ll i = 0; i < numplus2; i++) {
        ret2 += 2*A[i];
        cnt++;
    }
    if (N%2==1) {
    ...
    std::cout << max(ret, ret2) << std::endl;
