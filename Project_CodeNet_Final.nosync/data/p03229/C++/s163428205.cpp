...
int main() {
    ...
    ll c=deq.back(),b=deq.back();
    deq.pop_back();
    while(!deq.empty()){
        if(deq.size()==1){
            t+=max(abs(b-deq.back()),abs(c-deq.back()));
            deq.pop_back();
        }
        else if(abs(deq.back()-c)>=abs(deq.front()-c)){
            t+=abs(c-deq.back());
            c=deq.back();
            deq.pop_back();
        }
    ...
    c=deq.front(),b=deq.front();
    deq.pop_front();
    while(!deq.empty()){
        if(deq.size()==1){
            t2+=max(abs(b-deq.back()),abs(c-deq.back()));
            deq.pop_back();
        }
        else if(abs(deq.back()-c)>=abs(deq.front()-c)){
            t2+=abs(c-deq.back());
