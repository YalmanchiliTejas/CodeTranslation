...
int main(){
  ...
  cin>>n; Int a[n]; rep(i,n)cin>>a[i]; sort(a,a+n);
  Int ans=0,l=a[0],r=a[0],mi=1,ma=n-1;
  rer(i,1,n){
    if(r>l){
      if(a[ma]-l>r-a[mi]){
        ans+=a[ma]-l; l=a[ma]; ma--;
      }else{
        ans+=r-a[mi]; r=a[mi]; mi++;
      }
    }else{
      if(a[ma]-r>l-a[mi]){
        ans+=a[ma]-r; r=a[ma]; ma--;
      }else{
        ans+=l-a[mi]; l=a[mi]; mi++;
      }
    }
  }
  END
