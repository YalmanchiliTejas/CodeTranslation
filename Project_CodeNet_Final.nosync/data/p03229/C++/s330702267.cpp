...
signed main() {
	...
	if(N % 2 == 0) B.at(0) = A.at(N / 2 - 1);
	else B.at(0) = A.at(N / 2);

	if (N % 2 == 0) C.at(0) = A.at(N / 2 - 1);
	else C.at(0) = A.at(N / 2);

	int res1 = 0;
	int res2 = 0;

	bool judge = (A.at(N - 1) - B.at(0) > B.at(0) - A.at(0));

	for (int i = 1; i < N; i++) {

		if (i % 2 == 1) B.at(i) = A.at(N - 1 - i / 2);
		else B.at(i) = A.at(i / 2 - 1);

		if (i % 2 == 1) C.at(i) = A.at(i / 2);
		else C.at(i) = A.at(N - i / 2);
		

		res1 += abs(B.at(i) - B.at(i - 1));
		res2 += abs(C.at(i) - C.at(i - 1));
	}
