...
// BiCoef<mint> bc(2*1024*1024); // 2M * 3 * 8B(ll) = 48MB (0.1sec)
...
int main()
{
	...
	auto dp = make_vector(N + 1, 3, S+3005, static_cast<mint>(0)); // dp[N+1][][S+1] = 初期化0
	...
	for (int i = 0; i < N; ++i)
	{
		for (int f = 2; f >= 0; f--)
		{
			for (int s = 0; s <= S; ++s)
			{
//				cerr << " i=" << i << " f=" << f << " s=" << s << " dp[i][f][s]=" << dp[i][f][s] << endl;

				if (f == 0)
				{
					dp[i + 1][f][s] += dp[i][f][s];  //;
					dp[i + 1][f+1][s + a[i]] += dp[i][f][s];  //;
					dp[i + 1][f + 1][s] += dp[i][f][s];  //;
					dp[i + 1][f + 2][s + a[i]] += dp[i][f][s];  //;
				}
				else if (f== 1)
				{
					dp[i + 1][f][s + a[i]] += dp[i][f][s];  //;
					dp[i + 1][f][s] += dp[i][f][s];  //;
