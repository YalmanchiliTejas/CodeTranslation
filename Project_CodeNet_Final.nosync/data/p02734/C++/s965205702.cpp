...
void read_graph(ll n_vertex, ll n_edges, Graph& graph,
                ll cost = -1, bool directed = false, ll offset = -1){

    graph.resize(n_vertex);
    rep(i, n_edges){
        ll from, to, c = cost;
        if (cost == -1) in3(from, to, c);
        else in2(from, to);
        from += offset, to += offset;

        graph[from].pb({to, cost});
        if (!directed) graph[to].pb({from, cost});
    }
}
...
void read_pair_vector(ll n, vpl& v, ll offset1 = 0, ll offset2 = 0, bool rev = false){
    v.resize(n);
    rep(i, n){
        ll a, b; in2(a, b);
        a += offset1, b += offset2;
...
void solve(){
    ...
    in2(n, m);
    vmi3 dp(3, vmi2(n + 1, vmi(m + 1, 0)));
    read_vector(n, as);

    dp[0][0][0] = 1;
    For(i, 1, n + 1) rep(v, m + 1){
        dp[0][i][v] = dp[0][i - 1][v];
        ...
        ll nxt = as[i - 1];
        if (v - nxt >= 0){
            dp[1][i][v] += dp[0][i - 1][v - nxt] + dp[1][i - 1][v - nxt];
            dp[2][i][v] += dp[0][i - 1][v - nxt] + dp[1][i - 1][v - nxt];
        }
    }
