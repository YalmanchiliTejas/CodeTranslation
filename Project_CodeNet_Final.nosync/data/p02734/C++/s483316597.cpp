...
signed main() {
	...
	vector< int > A(N);
	rep(i, N) cin >> A[i];
	vector< vector< vector< int > > > dp(N + 1, vector< vector< int > >(6005, vector< int >(3, 0)));
	dp[0][0][0] = 1;
	rep(i, N) rep(j, S + 1) {
		(dp[i + 1][j][0] += dp[i][j][0]) %= MOD2;
		(dp[i + 1][j + A[i]][1] += dp[i][j][0] * (i + 1) % MOD2) %= MOD2;
		(dp[i + 1][j + A[i]][2] += dp[i][j][0] * (i + 1) % MOD2 * (N - i) % MOD2) %= MOD2;

		(dp[i + 1][j][1] += dp[i][j][1]) %= MOD2;
		(dp[i + 1][j + A[i]][1] += dp[i][j][1]) %= MOD2;
		(dp[i + 1][j + A[i]][2] += dp[i][j][1] * (N - i) % MOD2) %= MOD2;
