...
const ll modll::MODVAL = 998244353LL;
std::ostream& operator<<(std::ostream& os, const modll& v) { os << v.val; return os; }

/*
LがきまるとN^2で求まる。Lが1こずれたときの関係、なんとかならないか?
	f(L)がわかってたとしてf(L+1)は?
	Lで何も選ばないでL+1の状態になったということ
なにか事前計算しておいてL,Rが決まったときにO(1)で計算して全体N^2でいけないか?
wの初期値{w[0], ..., w[S]} に対してそれを使ったansの式が立てられる?
ans = k*w
*/
int main() {
	cin.tie(0);
