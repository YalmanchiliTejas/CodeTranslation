...
public:
	modInt() : value(0) {}
	template<typename T>
	modInt(T value = 0, bool moded = false) : value(value) {
		if (moded)this->value = value % modulo;
		else {
	...
	template<typename T> modInt operator+(const T& rhs)const { return modInt(*this, true) += rhs; }
