...
signed main() {
  ...
  rep(i, n)rep(j, s+1) {
    ...
    addm(dp[i+1][j][0], sum[i][j]);
    if (j - a[i] >= 0) {
      if (j == a[i]) dp[i+1][j][1] = i+1;
      else dp[i+1][j][1] = sum[i][j-a[i]];
    }
    sum[i+1][j] = (sum[i][j] + dp[i+1][j][1]) % mod;
  }
  cerr << dp[n][s][0] << " " << dp[n][s][1] << endl;
  int ans = (dp[n][s][0] + dp[n][s][1]) % mod;
  cout << ans << endl;
}
