...
int main(){
  ...
  vector<llvec> dp(S+1, llvec(N, 0));
  
  rep(i, S+1){
    if(i==0 or i==A[0])dp[i][0]=1;
  }
  FOR(i, 1, N){
    rep(j, S+1){
      if(j==0){
        dp[j][i] = mod(dp[j][i-1]+1, MOD);
      }else{
        dp[j][i] = mod(dp[j][i-1], MOD);
        if(j-A[i]==0){
          dp[j][i] = mod(dp[j][i] + dp[0][i-1]+1, MOD);
        }else if(j-A[i]>0){
          dp[j][i] = mod(dp[j][i] + dp[j-A[i]][i-1], MOD);          
        }
      }
    }
  }
