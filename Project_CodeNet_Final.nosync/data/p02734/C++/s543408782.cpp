...
#define PI acos(-1)
#define open_read freopen("deleg.in", "r", stdin)
#define open_write freopen("deleg.out", "w", stdout)
...
int main() {
    ...
    FOR (i, 1, N) {
        dpSolve[i][numbers[i]] += i;

        FOR (s, 1, S) {
            dpSolve[i][s] += dpSolve[i - 1][s];
            dpSolve[i][s] %= MOD;
            
            if (s - numbers[i] >= 0) {
                dpSolve[i][s] += dpSolve[i - 1][s - numbers[i]];
                dpSolve[i][s] %= MOD;
            }
        }
    }
