...
int main(){
    ...
    for(int i=1; i<=len; i++){
        ...
        for(int tot=0; tot<=S; tot++){
            if(tot <= arr[i]) dp[i][tot] = (dp[i][tot]+dp[i-1][tot])%MOD;
            else dp[i][tot] = (dp[i-1][tot]+dp[i-1][tot-arr[i]])%MOD;
        }
        if(arr[i] <= S) res = (res+dp[i-1][S-arr[i]]*(len-i+1))%MOD;
        if(arr[i] == S) res = (res+1LL*i*(len-i+1))%MOD;
    }
    printf("%lld\n",res);
}