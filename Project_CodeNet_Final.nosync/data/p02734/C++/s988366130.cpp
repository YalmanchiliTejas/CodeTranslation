...
ll n, s, a[MAXN], dp[MAXN][MAXN], cum[MAXN][MAXN], ans, mod = 998244353;
...
int main()
{
	...
	for(int i=1;i<=s;i++) dp[n - 1][i] = 0;
	for(int i=0;i<n;i++) dp[i][a[i]] = 1;
	for(int i=0;i<=s;i++) cum[n - 1][i] = dp[n - 1][i];
    for(int i=n-2;i>=0;i--)
    {
    	for(int j=0;j<=s;j++)
    	{
    		if(j >= a[i])
    			dp[i][j] = (dp[i][j] + cum[i + 1][j - a[i]]) % mod;
    	}
    	for(int j=0;j<=s;j++) cum[i][j] = (cum[i + 1][j] + dp[i][j]) % mod;
    }
	for(ll i=0;i<n;i++) ans = (ans + ((i + 1) * dp[i][s]) % mod) % mod;
	cout << ans << endl;
    return 0;
}