...
str ts(vector<bool> v) {
	str res = ""; F0R(i,len(v)) res += char('0'+v[i]);
	res += ""; return res; }
...
void setIO(str s = "") {
	...
	if (len(s)) { setIn(s+".in"), setOut(s+".out"); } // for USACO
}
...
template< typename T >
vector<size_t> argsort(vector<T>& v){
...
template< typename T >
int bisect_left(vector<T>& v, T x){
    return lower_bound(v.begin(),v.end(),x)-v.begin();
}
template< typename T >
int bisect_right(vector<T>& v, T x){
    return upper_bound(v.begin(),v.end(),x)-v.begin();
}

template< typename T >
bool erase1(multiset<T>& ms,T x){//erase only 1 element for multiset
	auto ind =ms.find(x);
	if(ind==ms.end()) return false;
	ms.erase(ind);
	return true;
}
