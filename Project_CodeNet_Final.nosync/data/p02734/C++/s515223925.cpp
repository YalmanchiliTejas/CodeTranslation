...
void solve(std::istream& cin, std::ostream& cout, std::ostream& cerr) {
    ...
    cin >> a;
    ERR(a)
    V2(dp, mint,N+2, 3500,0);
    dp[0][0] = 1;
    mint ans = 0;
    REP(i,N){
        if(S > a[i])ans += dp[i][S-a[i]] * (N-i);
        if(S == a[i])ans += dp[i][0] * (N - i)* (i+1);
        ERR(ans)
        REP(j,3300){dp[i+1][j] += dp[i][j];if(j+a[i] < 3400)dp[i+1][j+a[i]] += dp[i][j];}
        dp[i+1][a[i]] += i;
    }
    ERR(dp)
    print(ans);
...
int main() {
	...
	FKnapsackForAllSegments solver;
