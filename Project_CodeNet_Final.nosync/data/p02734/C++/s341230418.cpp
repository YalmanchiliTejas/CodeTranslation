...
int main() {
    ...
    static modint dp[3002][6002][3] = {};

    dp[0][0][0] = 1;
    Rep(ni,N){
        Rep(si,S+1){
            int ssi = si+a[ni];
            dp[ni+1][si][0]  += dp[ni][si][0];
            dp[ni+1][si][1]  += dp[ni][si][0];
            dp[ni+1][ssi][1] += dp[ni][si][0];
            dp[ni+1][si][1]  += dp[ni][si][1];
            dp[ni+1][ssi][1] += dp[ni][si][1];
            dp[ni+1][si][2]  += dp[ni][si][1];
            dp[ni+1][si][2]  += dp[ni][si][2];
        }
    }

    printf ("%lld\n", dp[N][S][0] + dp[N][S][1] + dp[N][S][2] );
