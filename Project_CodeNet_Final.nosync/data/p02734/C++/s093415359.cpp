...
struct polynomial {
  ...
  void shrink() {
    while (coef.size() > 1u and coef.back() == T(0))
      coef.pop_back();
  }
  ...
  polynomial pow(int k) const {
    if (k == 0) return polynomial(vector<T>{T(1)});
    polynomial res = *this;
    rep(i, k-1) res *= *this;
    return res;
  }
...
// as for gcd refer to https://github.com/beet-aizu/library/blob/master/polynomial/polynomial.cpp


int main() {
  int n = IN, s = IN;
  auto a = IN.vi(n);
  polynomial poly(vector<modint>{modint(0)});
  modint res = 0;
  rep(i, n) {
    poly[0]++;
    poly.coef.resize(poly.coef.size()+a[i]+1);
    repr(t, a[i], poly.size()) poly[t] += poly[t-a[i]];
    poly.coef.resize(s+1);
    res += poly[s];
  }
  OUT(res);
}