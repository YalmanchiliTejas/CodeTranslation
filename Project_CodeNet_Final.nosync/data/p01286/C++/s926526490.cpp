...
void solve() {
    while (true) {
        ...
        for (int i = 100; i <= 700; i += 200) {
            for (int q = 0; q < h[i / 200]; ++q) {
                Graphs.add_edge(i + q, i + 100 + q, 1);
                Graphs.add_edge(i + 100 + q, i + 299, 1);
            }
            if (i >= 300) {
                for (int q = 0; q < 99; ++q) {
                    Graphs.add_edge(i - 1, i + q, 1);
                }
                int hoge = (i - 101) / 200;
                REP(q, h[hoge + 1]) {
                    int n;
                    ...
                    REP(t, n) {
                        int b;
                        cin >> b;
                        Graphs.add_edge(i - 100 + (b - 1), i + q, 1);
                    }
                }
            }
        }
        Graphs.add_edge(399, 499, listings[0]);
        Graphs.add_edge(599, 699, listings[1]);
        Graphs.add_edge(799, 899, listings[2]);
        for (int q = 800; q < 900; ++q) {
            Graphs.add_edge(q, 900, 1);
        }
        Graphs.add_edge(899, 900, listings[2]);
        REP(i, h[0]) {
            Graphs.add_edge(0, i + 200, 1);
        }
        cout << Graphs.run(0, 900) << endl;
    }
}
#undef int
int main() {
