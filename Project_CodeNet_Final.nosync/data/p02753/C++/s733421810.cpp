...
#pragma GCC target("sse", "sse2", "sse3", "ssse3", "sse4", "avx", "avx2", "popcnt")
#endif
...
{
	static std::mt19937
		PRNG(static_cast<std::uint64_t>(std::chrono::steady_clock::now().time_since_epoch().count()));
	return PRNG;
}

// Defines MAXN value
constexpr long long MAXN __attribute__((unused)) = static_cast<long long>(1e6 + 1);
// Defines INFINITE Value
constexpr long long INF __attribute__((unused)) = static_cast<long long>(2e18 + 1);
// Defines Modulus
...
void solve()
{
	string s;
	read(s);
	prints(set<char>(ALL(s)).size() == 2 ? "Yes" : "No");
}
