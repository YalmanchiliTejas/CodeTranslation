...
#define readc(var) fscanf(_fin,"%c",var)
int readi() { int var; fscanf(_fin, "%d", &var); return var; }
ll readll() { ll var; fscanf(_fin, "%lld", &var); return var; }
void repreadi(int* data, int n) { repi(n) data[i] = readi(); }
void repreadll(ll* data, int n) { repi(n) data[i] = readll(); }
...
int calc(Veci* nodes, int* ndesc, int n, int current, int parent)
{
	if (ndesc[current] != -1) return ndesc[current];

	ndesc[current] = 0;
	repi(nodes[current].n)
	{
		if (nodes[current].data[i] == parent) continue;
		ndesc[current] += 1 + calc(nodes, ndesc, n, nodes[current].data[i], current);
	}
	return ndesc[current];
}

int main()
{

	RUN_LOCAL("dataabc159C.txt");
	char s[4];
	int len = reads(s,4);
	int nA = 0;
	repi(len) if (s[i] == 'A') nA++;
	if (nA == 0 || nA == 3) writes("No\n");
	else writes("Yes\n");
	return 0;
}