...
inline  ll gcd(ll a,ll b){if(!a||!b)return 0;while(b){ll c=b;b=a%b;a=c;}return a;}
...
inline  ll intpow(ll a,ll b){ll ans=1;for(ll i=1;b;i*=2){if(b&i){b^=i;ans*=a;}a*=a;}return ans;}
template<typename T>
inline  bool chmin(T &mn,const T &cnt){if(mn>cnt){mn=cnt;return 1;}else return 0;}
template<typename T>
inline  bool chmax(T &mx,const T &cnt){if(mx<cnt){mx=cnt;return 1;}else return 0;}
...
template<class T> void print(const deque<T> &vec){ if(vec.empty()) return; print(vec[0]); for(auto i = vec.begin(); ++i != vec.end(); ){ putchar(' '); print(*i); } }
...
int main(){
    ...
    VEC(ll,a,n);
    PartialSum<ll> s(a);
    unordered_map<ll,pll>minmax;
    rep(n+1){
        if(!minmax.count(s.data[i])){
            minmax[s.data[i]]={LINF,-LINF};
        }
        chmin(minmax[s.data[i]].first,i);
        chmax(minmax[s.data[i]].second,i);
    }
    ll ans=0;
    each(i,minmax)chmax(ans,i.second.second-i.second.first);
    out(ans);
}
