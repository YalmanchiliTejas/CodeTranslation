...
int main()
{
	...
	solve();
	for(int i = 1; i <= n; ++i) s.insert(mp(i, 0));
	for(int i = 2; i < tot && b[i].second == 0; ++i)
	{
		if(b[i].first == b[tot].first) continue;
		s.erase(mp(i, 0));
		s.insert(mp(i, 1));
		ans = min(ans, 1ll * (a[s.rbegin()->first][s.rbegin()->second] - a[s.begin()->first][s.begin()->second]) * (ymax - a[b[i].first][0]));
		s.erase(mp(i, 1));
		s.insert(mp(i, 0));
	}
	printf("%lld\n", ans);
