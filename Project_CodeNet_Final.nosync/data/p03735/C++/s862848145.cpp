...
template<typename T>
ostream& operator << (ostream &o, const multiset<T> &st) {
    return _IterOutput_(o, ALL(st), "{", "}");
}
...
int32_t main() {
    ...
    int a2 = (ip[Mi].S - ip[mi].F);

    for (int i=0; i<N; i++) {
        if (i != mi) {
            st.insert(ip[i].F);
        } else st.insert(ip[i].S);
    }

    int cura = *(--st.end()) - *st.begin();
    //cout << st << endl;

    for (int i=0; i<N; i++) {
        if (i == Mi or i == mi) continue;

        st.erase(st.find(ip[i].F));
        st.insert(ip[i].S);
        cura = min(cura, *(--st.end()) - *st.begin());
    }
