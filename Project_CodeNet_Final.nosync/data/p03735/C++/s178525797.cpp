...
ll n, x[200000], y[200000], mx = -1, mn = INF, ans, counter;
vector<P> d;
set<ll> st, st2;
priority_queue<ll> pq, pp;
priority_queue<ll, vector<ll>, greater<ll>> pq2, pp2;
...
int main() {
  ...
  for (ll i = 0; i < n; i++) {
    ...
    d.push_back(P(x[i], i));
    d.push_back(P(y[i], i));
    pq2.push(max(x[i], y[i]));
  }
  ans = (mx - mn) * (mx - mn);
  sort(d.begin(), d.end(), greater<P>());
  for (ll i = 0; i < 2 * n; i++) {
    P p = d[i];
    ll num = p.second, dt = p.first;
    ll another = p.first == x[num] ? y[num] : x[num];
    pq.push(another);
    if (st.count(num) == 1) {
      pp.push(max(x[num], y[num]));
    } else {
      st.insert(num);
      counter++;
    }
    while (!pp.empty() && pp.top() == pq.top()) pp.pop(), pq.pop();
    if (counter == n) ans = min(ans, (mx - dt) * (pq.top() - mn));
    //if (counter == n) printf("%lld, %lld, %lld, %lld\n", mx, dt, pq.top(), mn);
  }

  ans = min(ans, (mx - mn) * (mx - pq2.top()));
  for (ll i = 0; i < 2*n; i++) {
    P p = d[i];
    ll num = p.second, dt = p.first;
    ll another = p.first == x[num] ? y[num] : x[num];
    ...
    ans = min(ans, (mx - mn) * (d[i+1].first - pq2.top()));
  }
