...
int main() {
	...
	REP(i, n)cin >> x[i] >> y[i];
	multiset<P> minblue;
	ll redmin = INF, redmax = 0;
	REP(i, n) {
		ll red = max(x[i], y[i]);
		ll blue = min(x[i], y[i]);
		chmin(redmin, red);
		chmax(redmax, red);
		minblue.insert(make_pair(blue, red));
	}
	ll ans = INF;
	ll bluechma = 0;
	ll bluechmi = INF;
	while (true) {
		ll bluemin = minblue.begin()->first;
		auto itr = minblue.end();
		itr--;
		ll bluemax = itr->first;
		chmin(bluemin, bluechmi);
		chmax(bluemax, bluechma);
		chmin(ans, (redmax - redmin)*(bluemax - bluemin));
		itr = minblue.begin();
		if (itr->second == redmax)break;
		chmax(bluechma, itr->second);
		chmin(bluechmi, itr->second);
		chmin(redmin, itr->first);
		minblue.erase(itr);
	}
	cout << ans;
	return 0;
