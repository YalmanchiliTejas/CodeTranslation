...
int main() {
    ...
    ll ans1 = ll(xh - xl)*ll(yh - yl);

    sort(ALL(pp));

    int low, high;
    low = high = pp[0].second;
    for (int i = 1; i < n; ++i) {
        if (pp[i].second < low)
            low = pp[i].second;
        else if (pp[i].first < low)
            if (pp[i].second <= xh)
                minset(low, pp[i].second),
                maxset(high, pp[i].second);
            else
                minset(low, pp[i].first),
                maxset(high, pp[i].first);
        else
            minset(low, pp[i].first),
            maxset(high, pp[i].first);
    }
    ll ans2 = ll(yh - xl)*ll(high - low);
