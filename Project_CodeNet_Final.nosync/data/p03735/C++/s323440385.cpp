...
ll getAns(){
  pair<ll, ll> leftFirst, rightFirst, leftEnd, rightEnd;
  ...
  leftEnd = *itr3;
  rightEnd = *itr4;
  return (leftEnd.f - leftFirst.f) * (rightEnd.f - rightFirst.f);
}

void solve(){
  sort(xy.begin(), xy.end());
  rep(i, N){
    Left.insert({xy[i].f, i});
    Right.insert({xy[i].s, i});
  }
  ll ans = getAns();
  rep(i, N){
    Left.erase({xy[i].f, i});
    Right.erase({xy[i].s, i});
    Left.insert({xy[i].s, i});
    Right.insert({xy[i].f, i});
    ans = min(ans, getAns());
  }
  cout << ans << endl;
}
