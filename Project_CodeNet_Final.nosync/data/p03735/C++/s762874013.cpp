...
signed main() {
    ...
    ll Midx = vmax(r) - r.begin();
    ll midx = vmin(b) - b.begin();
    sort(All(r));
    sort(All(b));
    ll ans = (r[N - 1] - r[0]) * (b[N - 1] - b[0]);
    MKORDER(N);
    sort(All(od), [&](ll a, ll b) { return x[a] < x[b]; });
    multiset<ll> st;
    st.emplace(x[Midx]);
    st.emplace(y[midx]);
    REP(i, N) {
    ...
    ans = min(ans, (r[N-1]-b[0])* (*(--st.end()) - *st.begin()));
    cout << ans << "\n";
}
