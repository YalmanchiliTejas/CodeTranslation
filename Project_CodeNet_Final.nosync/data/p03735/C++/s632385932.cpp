...
int main() {
	...
	FOR(i, n) {
		...
		mp[vp[i].first].emplace_back(i, 0);
		mp[vp[i].second].emplace_back(i, 1);
	}
	vector<int> cm(st.begin(), st.end());
	int mn = ten(9), mx = 0;
	FOR(i, n) {
	...
	int mx2 = 0;
	FOR(i, n) mx2 = max(mx2, vp[i].first);

	ll ans;
	{
		int a = ten(9), b = 0, c = ten(9), d = 0;
		FOR(i, n) {
			a = min(a, vp[i].first);
			b = max(b, vp[i].first);
			c = min(c, vp[i].second);
	...
	FOR(i, sz(cm)) {
		ll cur = ll(mx - mn) * (mx2 - cm[i]);
		ans = min(ans, cur);
		bool con = true;
		for (auto& kv: mp[cm[i]]) {
			if (kv.second == 0) {
				mx2 = max(mx2, vp[kv.first].second);
			} else {
				con = false;
				break;
			}
