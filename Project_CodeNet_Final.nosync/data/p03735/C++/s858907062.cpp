...
signed main(){
    LL(n);
    VEC(pll,a,n);
    each(i,a)if(i.first>i.second)swap(i.first,i.second);
    ll rmax=-LINF,rmin=LINF,bmax=-LINF,bmin=LINF;
    each(i,a){
        chmax(rmax,i.first);
        chmin(rmin,i.first);
        chmax(bmax,i.second);
    ...
    each(i,x){
        bmin=i.first;
        while(at<x.size()&&x[at].first<bmin){
            chmax(bmax,x[at].second);
            at++;
        }
        if(bmax==LINF)break;
