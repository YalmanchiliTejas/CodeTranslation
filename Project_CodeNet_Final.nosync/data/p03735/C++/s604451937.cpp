...
{
	ll n;  cin >> n;
	vector<pair<ll, ll>> xy = cinv2<ll, ll>(n);
	each(e, xy){
	...
	pll lm=xy.back();  xy.pop_back();

	ll xmn=xy.front().first;
	ll xmx=lm.first;

	sort(ALL(xy), [](pll &a, pll &b){return a.second<b.second; });
	ll ymn=xy.front().second;
	ll ymx=max(xy.back().second, lm.second);
