...
void Solve() {
	...
	si(K);
	li(D);
	auto dp = ven(2, 2, D, mll(0));
	dp[0][1][0] = 1;
	ll now = 0;
	REP(i, K.sz) {
		REP(d, D) {
			REP(j, 10) {
				dp[!now][0][(d + j) % D] += dp[now][0][d];
				if (j == c2l(K[i])) dp[!now][1][(d + j) % D] += dp[now][1][d];
				if (j < c2l(K[i])) dp[!now][0][(d + j) % D] += dp[now][1][d];
			}
			dp[now][0][d] = 0;
			dp[now][1][d] = 0;
		}
