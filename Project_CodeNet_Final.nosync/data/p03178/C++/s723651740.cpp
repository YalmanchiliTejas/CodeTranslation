...
ll rec(ll digit, ll mod, bool strict) {

  if (dp[digit][mod][strict] != -1) return dp[digit][mod][strict];
  ...
  if (strict) {
    ll mx = K[digit] - '0';
    REP(i, mx + 1) {
      ret += rec(digit + 1, (mod + i) % D, i == mx);
      ret %= MOD;
    }
  } else {
    REP(i, 10) {
      ret += rec(digit + 1, (mod + i) % D, false);
      ret %= MOD;
    }
  }

  return dp[digit][mod][strict] = ret;
