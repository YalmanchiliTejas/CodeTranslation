...
constexpr unsigned int MAX_LEN = 10000;
char N[MAX_LEN + 1];
constexpr unsigned int MAX_D = 100;
modint AA[MAX_D];
modint AB[MAX_D];
modint* dp = AA, *ep = AB;

int main(void){
    unsigned int D; cin >> N >> D;
    const unsigned int n = strlen(N);

    unsigned int dig_sum = 0;
    for(unsigned int i = 0; i < n; ++i){
        std::copy(dp, dp + D, ep);
        for(unsigned int j = 0; j < D; ++j){
            for(unsigned int cntr = 1, k = j + 1; cntr < 10; ++cntr, ++k){
                if(k == D) k = 0; 
                ep[k] += dp[j];
            }
        }

        const unsigned int d = N[i] - '0';
        for(unsigned int cntr = 0; cntr < d; ++cntr){
            ++ep[dig_sum++]; if(dig_sum == D) dig_sum = 0;
        }
        std::swap(dp, ep);
    }
    const modint res = (dig_sum) ? --dp[0] : dp[0];
    std::cout << res << eol;
    return 0;
}