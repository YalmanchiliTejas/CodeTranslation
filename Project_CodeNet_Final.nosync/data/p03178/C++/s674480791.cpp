...
int main()
{
  ...
  //no of numbers of length d such that remainder rem
  dp[0][0]=1;
  for(ll digits=0;digits<n;digits++){
    for(ll rem=0;rem<d;rem++){
      for(ll l=0;l<10;l++){
        add(dp[digits+1][(rem+l)%d],dp[digits][rem]);
      }      
    }
  }   
  ...
  for(ll i=0;i<n;i++){
    for(ll digit=(i==0);digit<k[i]-48;digit++){      
      add(ans,dp[n-i-1][(d-(rem+digit)%d)%d]);      
    }
    rem=(rem+(k[i]-48))%d;    
  } 
