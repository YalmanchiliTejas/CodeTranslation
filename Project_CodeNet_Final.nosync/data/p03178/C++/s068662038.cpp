...
ll powmod(ll b, ll e){
    ll r = 1; while(e > 0) {if(e&1) r = (r%MOD)*(b%MOD)%MOD; b = (b%MOD)*(b%MOD)%MOD; e >>= 1;} return r;
}
...
int main(){
    ...
    cin >> s >> d;
    int n = s.size();
    dp.resize(n+1, vector<vector<int> >(d, vector<int>(2)));
    dp[0][0][0] = 1;
    rep(i, n){
        int x = s[i]-'0';
        rep(j, d){
            rep(k, 10) (dp[i+1][(j+k)%d][1] += dp[i][j][1]) %= MOD;
            rep(k, x)  (dp[i+1][(j+k)%d][1] += dp[i][j][0]) %= MOD;
            (dp[i+1][(j+x)%d][0] += dp[i][j][0]) %= MOD;
        }
    }
    ppp((dp[n][0][0]+dp[n][0][1]-1+MOD)%MOD);
