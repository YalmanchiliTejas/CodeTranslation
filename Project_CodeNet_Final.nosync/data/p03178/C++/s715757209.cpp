...
+library{ name:mint, url:"https://tqkoh.github.io/library/library/lib/util/mint.hpp", version:"2020-08-08" }

int solve(){
	cs s; in(s);
	lint d; in(d);
	lint n=s.size();
	vector<vector<mint>>dp0(n+10,vector<mint>(d)),dp1(n+10,vector<mint>(d));
	dp0[0][0] = 1;
	rep(n){
		rep(j,d){
			rep(l,10)dp1[i+1][(j+l)%d] += dp1[i][j];
			rep(l,s[i]-'0')dp1[i+1][(j+l)%d] += dp0[i][j];
			lint l = s[i]-'0';
			dp0[i+1][(j+l)%d] += dp0[i][j];
		}
	}
	out(dp0[n][0]+dp1[n][0]-1);
	return 0;
}
...
int solve(){
	...
	cs s(a.size(),'9');
	lint n=s.size();
	vector<vector<lint>>dp0(n+10,vector<lint>(1ll<<10,linf)),dp1(n+10,vector<lint>(1ll<<10,linf)),dp2(n+10,vector<lint>(1ll<<10,linf));
	dp0[0][0]=0;
	per(n){
		rep(b,1ll<<10){
			rep(l,10)if(dp1[i][b]!=linf)chmin(dp1[i+1][b|(1ll<<l)],dp1[i][b]*10+abs(l-a[i]+'0'));
			rep(l,s[i]-'0')if(dp0[i][b]!=linf)chmin(dp1[i+1][b|(1ll<<l)],dp0[i][b]*10+abs(l-a[i]+'0'));
			lint l = a[i]-'0';
			if(dp0[i][b]!=linf)chmin(dp0[i+1][b|(1ll<<l)],dp0[i][b]*10);
			rep(l,s[i]-'0'+1,10)if(dp0[i][b]!=linf)chmin(dp2[i+1][b|(1ll<<l)],dp0[i][b]*10+abs(a[i]-'0'-l));
			rep(l,10)if(dp2[i][b]!=linf)chmin(dp2[i+1][b|(1ll<<l)],dp2[i][b]*10+abs(a[i]-'0'-l));
		}
	}
	...
	rep(b,1ll<<10){
		lint pc=0;
		rep(10)pc+=(b>>i)&1ll;
		if(pc<=k)chmin(ans,min({dp0[n][b],dp1[n][b],dp2[n][b]})),!dp1[n][b]&&deb(n,b);
	}
	out(ans);
	return 0;
}*/
