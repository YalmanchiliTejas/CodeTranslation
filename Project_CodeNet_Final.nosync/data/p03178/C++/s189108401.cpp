    ...
    for (int r = 0; r < d; r++){
      for (int taken = 0; taken < 2; taken++){
        if (taken){
          for (int c = 0; c <= 9; c++){
            int new_rem = (r + c) % d;
            dp[i + 1][new_rem][taken] += dp[i][r][taken];
            dp[i + 1][new_rem][taken] %= mod;
          }
        } else{
          for (int c = 0; c <= s[i + 1] - '0'; c++){
            int new_rem = (r + c) % d;
            if (c < s[i + 1] - '0') dp[i + 1][new_rem][1] += dp[i][r][taken];
            else dp[i + 1][new_rem][0] += dp[i][r][taken];
            dp[i + 1][new_rem][1] %= mod;
            dp[i + 1][new_rem][0] %= mod;
          }
        }
      }
    }
