        ...
        for (int v = 0; v < d; ++v)
            for (int st = 0; st < 10; ++st)
                for (int nm = 0; nm < 10; ++nm)
                    dp[l + 1][(v + nm) % d][st] = MOD(dp[l + 1][(v + nm) % d][st] + dp[l][v][st]);
}
...
void smain()
{
    ...
    {
        for (int nm = 0; nm < k[i] - '0'; ++nm)
            ans = MOD(ans + dp[SZ(k) - i][cur == 0 ? 0 : (d - cur)][nm]);
