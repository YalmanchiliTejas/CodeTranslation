...
ll M;
ll dp[N][D][2][2] = {};
ll solve(ll ind, ll m, ll tight, ll abtak)
{
	if (ind == s.size())
	{
		if (abtak and m == 0)
	...
	if (dp[ind][m][tight][abtak] != -1)
		return dp[ind][m][tight][abtak];
	ll ans = 0;
	ll end = tight ? (s[ind] - '0') : 9;
	forab(i, 0, end, 1)
	{
		ans += solve(ind + 1, (m + i) % M, tight & (i == (s[ind] - '0')), abtak | (i > 0));
		ans %= MOD;
	}
	dp[ind][m][tight][abtak] = ans;
	return ans;
}
