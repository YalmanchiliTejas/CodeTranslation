...
int main()
{
	...
	for(int i=1; i<=n; i++)
		for(int j=0; j<d; j++){
			dp[i][j][false] = 0, dp[i][j][true] = 0;
			for(int dig=0; dig<=9; dig++){
				int last_sum = ((j-dig)%d+d)%d;
				dp[i][j][false] = addmod(dp[i][j][false], dp[i-1][last_sum][false]);
				if(dig<k[i])
					dp[i][j][false] = addmod(dp[i][j][false], dp[i-1][last_sum][true]);
			}
			int last_sum = ((j-k[i])%d+d)%d;
			dp[i][j][true] = addmod(dp[i][j][true], dp[i-1][last_sum][true]);
		}

	int res = submod(addmod(dp[n][0][true], dp[n][0][false]), 1);
	cout << res << endl;
