...
void Solve() {
	...
	si(K);
	li(D);
	vvvmll dp(K.sz + 1, vvmll(D, vmll(2, 0)));
	dp[0][0][1] = 1;
	REP(k, K.sz) {
		REP(d, D) {
			REP(i, 10) {
				dp[k + 1][(d + i) % D][0] += dp[k][d][0];
				if (i < c2l(K[k])) dp[k + 1][(d + i) % D][0] += dp[k][d][1];
				if (i == c2l(K[k])) dp[k + 1][(d + i) % D][1] += dp[k][d][1];
			}
		}
	}
	sal(dp[K.sz][0][0] + dp[K.sz][0][1] - 1);
