...
template<int MOD>struct modint {
	...
	constexpr bool operator!=(const modint& m)const noexcept { return n != m.n; }
	friend ostream& operator<<(ostream& os, const modint& m)noexcept { return os << m.n; }
	friend istream& operator>>(istream& is, const modint& m)noexcept { return is >> m.n; }
	constexpr modint pow(modint m)const noexcept {
		modint t = n, ret = 1;
		while (m.n > 0) { if (m.n & 1)ret *= t.n; t.n *= t.n; m.n >>= 2; }
		return ret;
	}
...
int main() {
	string K = in;
	int n = sz(K);
	VI k(n); rep(i, n)k[i] = K[i] - '0';
	int d = in;

	vector<VVM> dp(2, VVM(n, VM(d)));

	// dp[0][0]
	dp[0][0][k[0] % d]++;

	// dp[1][0]
	rep(i, k[0])dp[1][0][i % d]++;
	...
	FOR(i, 1, n) {
		...
		// dp[0][i-1] -> dp[0][i]
		rep(j, d) {
			dp[0][i][(j + k[i]) % d] += dp[0][i - 1][j];
		}

		// dp[0][i-1] -> dp[1][i]
		rep(j, d)rep(p, k[i]) {
			dp[1][i][(j + p) % d] += dp[0][i - 1][j];
		}

		// dp[1][i-1] -> dp[1][i]
		rep(j, d)rep(p, 10) {
			dp[1][i][(j + p) % d] += dp[1][i - 1][j];
		}
	}

	out(dp[0].back()[0] + dp[1].back()[0] - 1);
}