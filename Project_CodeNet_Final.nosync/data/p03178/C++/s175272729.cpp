...
const int MAXK = 1e4 + 5, MAXD = 1e2 + 5, MOD = 1e9 + 7;

string K;
int D, dp[MAXK][MAXD][2];
...
int main() {
    ...
    setLength(MAXK);
    read(K, D);
    FOR(i, sz(K) + 1) FOR(j, D) FOR(k, 2) dp[i][j][k] = 0;
    dp[0][0][0] = 1;
    FOR(i, sz(K)) FOR(j, D) FOR(k, 2) FOR(m, 10) if (k || m <= K[i] - '0') (dp[i + 1][(j + m) % D][k | (m < K[i] - '0')] += dp[i][j][k]) %= MOD;
    writeln(((dp[sz(K)][0][0] + dp[sz(K)][0][1]) % MOD + MOD - 1) % MOD);
    return 0;
}
