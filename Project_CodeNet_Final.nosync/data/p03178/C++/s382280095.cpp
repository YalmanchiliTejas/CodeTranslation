	...
	FOR(i, sz(k)) {
		FOR(j, 10) {
			FOR(pd, d) {
				int nd = (pd + j) % d;
				int diff = k[i] - '0' - j;
				(gt[i + 1][nd] += gt[i][pd]) %= MOD;
				(ls[i + 1][nd] += ls[i][pd]) %= MOD;
				if (diff > 0) {
					(ls[i + 1][nd] += eq[i][pd]) %= MOD;
				} else if (diff == 0) {
					(eq[i + 1][nd] += eq[i][pd]) %= MOD;
				} else {
					(gt[i + 1][nd] += eq[i][pd]) %= MOD;
				}
			}
		}
	}
	ll ans = ls[sz(k)][0] + eq[sz(k)][0] - 1;
	ans = (ans % MOD + MOD) % MOD;
	writerLn(ans);
