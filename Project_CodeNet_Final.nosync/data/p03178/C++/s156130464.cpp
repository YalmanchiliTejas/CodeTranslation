    ...
    int rec(int pos,int sum,int flag){
    	if(dp[flag][sum][pos]!=-1)return dp[flag][sum][pos];
    	dp[flag][sum][pos]=0;
    	if(pos==n){
    		if(sum%d==0)return dp[flag][sum][pos]=1;
    		return dp[flag][sum][pos]=0;
    	}
    	int ans=0;
    	int LMT=k[pos]-'0';
    	if(flag)LMT=9;
    	for(int i=0;i<=LMT;i++){
    		if(flag){
    			add(dp[flag][sum][pos],rec(pos+1,(sum+i)%d,flag));
    		}
    		else{
    			if(i<LMT){
    				add(dp[flag][sum][pos],rec(pos+1,(sum+i)%d,1));
    			}
    			else{
    				add(dp[flag][sum][pos],rec(pos+1,(sum+i)%d,flag));
    			}
