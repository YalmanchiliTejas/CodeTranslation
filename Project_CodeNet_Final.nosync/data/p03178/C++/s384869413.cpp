...
ll solve(int digit, int cmod , bool limit = 1){
	if(digit == len) return (cmod == 0);
	ll &ret = dp[digit][cmod][limit];
	if(~ret) return ret;
	ret = 0;
	int bound = 9;
	if(limit) bound = (s[digit] - '0');
	for(int test = 0 ; test <= bound ; test++){
		ret = (ret%mod + solve(digit+1
		,((cmod)%d + test%d)%d,limit&((test+'0')==s[digit])))%mod; 
	 }
	 return ret;	
}
int main()
