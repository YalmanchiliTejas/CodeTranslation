...
//static const int ST_SIZE = (1 << 20) - 1;
//static const int MAX_N = 100010;
//static const int MAX_M = 5010;
//
//ll dat[ST_SIZE];
//ll A[200010];
//ll range = 1;
...
string s;
int D;
ll memo[100010][2][101];
ll recu(int i, int ti,int v) {
	...
	if (~memo[i][ti][v]) return memo[i][ti][v];
	ll *res = &memo[i][ti][v];
	*res = 0;
	if (ti) {
		int c = s[i] - '0';
		*res += recu(i + 1, ti, (v + c) % D);
		for (ll j = c - 1; j >= 0 ; j--)
		{
			ll next = v + j;
			next %= D;
