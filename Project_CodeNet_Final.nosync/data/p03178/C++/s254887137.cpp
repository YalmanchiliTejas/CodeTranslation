...
string K;
LL answer;
LL dpSolve[10010][110];


int newDiff(int diff, int val) {
    return (D + diff - val % D) % D;
}

LL solve(int dig, int diff) {
    if (dig == 0) {
        return (diff == 0);
    }

    LL &ret = dpSolve[dig][diff];
    ...
    REP (i, 0, 10) {
        ret += solve(dig - 1, newDiff(diff, i));
        ret %= MOD;
    }
...
int main() {
    ...
    REP (i, 0, (int)K.size()) {
        REP (j, 0, (int)(K[i] - '0')) {
            answer += solve((int)K.size() - i - 1, newDiff(currDiff, j));
            answer %= MOD;
        }

        currDiff = newDiff(currDiff, (int)(K[i] - '0'));
    }
