...
void solve(){
  ...
  ll n=k.size();
  ll D; cin>>D;
  auto dp=make_v(D,2,mint<MOD>(0)); // dp[r][lt] 桁の和がr mod D, b=1は<=kが確定
  dp[0][0]=1;
  rep(i,n){ // 上からi桁目まで見た
    ll a=k[i]-'0';
    auto newdp=make_v(D,2,mint<MOD>(0));
    rep(r,D)rep(d,10){
      newdp[(r+d)%D][1]+=dp[r][1]; // ltが確定していれば好きに遷移できる
      if(d<a) newdp[(r+d)%D][1]+=dp[r][0];
      else if(d==a) newdp[(r+d)%D][0]+=dp[r][0];
    }
    dp=newdp;
  }
