...
signed main()
{
    ...
    cin >> k >> d;
    int n = k.size();
    rep(i,1,k[0] - '0') dp[i%d][0] ++;
    rep(i,k[0] - '0' + 1 , 10) dp[i%d][1] ++;
    ll eq = (k[0] - '0') % d;
    ll ans = dp[0][0] + dp[0][1] + (eq == 0 ? 1 : 0);
    if(n == 1) ans = dp[0][0] + (eq == 0 ? 1 : 0);
    repp(i,n-1){
        ll new_dp[105][2];
        mem(new_dp,0);
        repp(j,k[i + 1] - '0') new_dp[(eq + j)%d][0] ++;
        rep(j,k[i + 1] - '0' + 1 , 10) new_dp[(eq + j)%d][1] ++;
        eq = (eq + k[i + 1] - '0') % d;
        add(ans , eq == 0 ? 1 : 0);
        repp(j,102){
            repp(k,10){
                int new_mod = (j + k) % d;
                add(new_dp[new_mod][0] ,dp[j][0]);
            }
        }
        add(ans , new_dp[0][0]);
        if(i != n - 2){
            repp(j,102){
                repp(k,10){
                    int new_mod = (j + k) % d;
                    add(new_dp[new_mod][1] , dp[j][1]);
                }
            }
            add(ans , new_dp[0][1]);
        }
        repp(i,104) dp[i][0] = new_dp[i][0] , dp[i][1] = new_dp[i][1];
    }
    cout << ans ;
}   