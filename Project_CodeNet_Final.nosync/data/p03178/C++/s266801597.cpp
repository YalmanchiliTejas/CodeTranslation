...
int main()
{
    ...
    while(q--){
        ...
        ll n = k.size();
        vvl dp(d, vll(2, 0));
        //dp[sum][smaller_already] - the number of ways to chose digits so far such that the sum of digits modulo D is sum and 'smaller already' shows whether we have chosen any digits smaller than the corresponding digit in K
        dp[0][0]=1;
        for(int where=0; where<n; where++){
            vvl new_dp(d, vll(2, 0));
