...
int d;
string num;
lli dp[10000+5][105][2];
bool visited[10000+5][105][2];

lli f(int id,int rem,int small){
    if(id == num.length())
        return (rem == 0);

    if(visited[id][rem][small])
        return dp[id][rem][small];

    visited[id][rem][small] = true;
    lli& ans = dp[id][rem][small];
    ans = 0ll;
    if(small == 0){
        for(int i = 0;i<=(num[id] - '0');i++){
            if(i == (num[id] - '0')){
                ans = (ans%MOD + f(id+1,(rem%d + i%d)%d,0)%MOD)%MOD;
                continue;
            }
            ans = (ans%MOD + f(id+1,(rem%d + i%d)%d,1)%MOD)%MOD;
        }
    }else{
        for(int i = 0;i<=9;i++){
            ans = (ans%MOD + f(id+1,(rem%d + i%d)%d,1)%MOD)%MOD;
