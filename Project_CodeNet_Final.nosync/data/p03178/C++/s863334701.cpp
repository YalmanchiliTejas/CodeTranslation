...
int main(){
  ...
  // 適切な初期化
  rep(j,10){
    if(j < S[0] - '0') dp[0][j%D][0] += 1;
    if(j == S[0]- '0') dp[0][j%D][1] += 1;
  }
  ...
  rep(i,N - 1) rep(j,D){
    ...
    rep(k,10) {
      ...
      // 1のほう
      if(k < S[i+1] - '0') dp[i+1][(j+k)%D][0] += dp[i][j][1];
      if(k ==S[i+1] - '0') dp[i+1][(j+k)%D][1] += dp[i][j][1];
    }
  }
  rep(i,N) rep(j,D) trc(i,j,dp[i][j][0],dp[i][j][1]);
  out( (dp[N-1][0][0] + dp[N-1][0][1]) - 1 );
}