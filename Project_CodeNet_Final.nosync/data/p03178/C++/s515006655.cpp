...
int _main()
{
    ...
    for (auto cc : k) {
        ll c = cc - '0';
        memcpy(dple_prev, dple, sizeof(dple_prev));
        memcpy(dplt_prev, dplt, sizeof(dplt_prev));
        memset(dple, 0, sizeof(dple));
        memset(dplt, 0, sizeof(dplt));
        rep (rem, d) {
            dple[(rem+c)%d] += dple_prev[rem];
            dple[(rem+c)%d] %= MOD;
            rep (cnow, c) {
                dplt[(rem+cnow)%d] += dple_prev[rem];
                dplt[(rem+cnow)%d] %= MOD;
            }
            rep (cnow, 10) {
                dplt[(rem+cnow)%d] += dplt_prev[rem];
                dplt[(rem+cnow)%d] %= MOD;
            }
        }
    }

    ll ans = dple[0] + dplt[0];
    ans %= MOD;
    ans--;
