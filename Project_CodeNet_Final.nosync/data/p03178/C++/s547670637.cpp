...
int main() {
	...
	ll d;
	cin >> k >> d;
	REP(i, k.size())a[i] = k[i] - '0';
	dp[0][0][0] = 1;
	REP(i, k.size()) {
		REP(j, d) {
			REP(k, 10) (dp[i + 1][((j - k) % d + d) % d][1] += dp[i][j][1]) %= MOD;
			ll t = a[i];
			(dp[i + 1][((j - t) % d + d) % d][0] += dp[i][j][0]) %= MOD;
			REP(k, t)(dp[i + 1][((j - k) % d + d) % d][1] += dp[i][j][0]) %= MOD;
		}
	}
	cout << (dp[k.size()][0][1] + dp[k.size()][0][0] - 1 + MOD) % MOD << "\n";
