...
int main() {
    ...
    auto dp = vec<ll>(2, 2, d);
    dp[0][0][0] = 1;
    FOR(i, 0, n){
        FOR(small, 0, 2)FOR(m, 0, d)dp[(i + 1) & 1][small][m] = 0;
        FOR(small, 0, 2)eFOR(x, 0, (small ? 9 : k[i]))FOR(m, 0, d) {
            (dp[(i + 1) & 1][small | (x < k[i])][(m + x) % d] += dp[i & 1][small][m]) %= mod;
        }
    }
