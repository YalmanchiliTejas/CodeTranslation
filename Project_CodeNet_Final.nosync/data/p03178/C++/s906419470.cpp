...
int main(){
	...
	int d;
	cin >> k >> d;
	int n = int(k.size()), sum = accumulate(k.begin(), k.end(), 0, [&](int x, char y){ return x + y - '0'; });
	vector<Zp> dp(d);
	dp[0] = 1;
	Zp res = (sum % d == 0) - 1;
	for(auto i = n - 1; i >= 0; -- i){ // position of the first mismatch
		sum -= k[i] - '0';
		for(auto t = 0; t < k[i] - '0'; ++ t){
			for(auto j = 0; j < d; ++ j){
				if((sum + j + t) % d == 0){
					res += dp[j];
				}
