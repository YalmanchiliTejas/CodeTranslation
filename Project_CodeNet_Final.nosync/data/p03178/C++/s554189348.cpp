...
template <class T,std::size_t Dimension> class mdv_inner<T, Dimension, static_cast<std::size_t>(1)> {
public:
	using value_type = T;
	using size_type = std::size_t;
...
public:
	...
	auto operator[](const size_type index) {
		return mdv_inner<value_type,Dimension,Dimension>(table.begin(), value_size.cbegin())[index];
	}
};
