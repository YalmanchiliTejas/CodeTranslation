...
vector<vvll> dp(1e4+5, vvll(101, vll(2,-1)));
string k;ll d;

ll noOfInt(ll idx, ll sum, bool tight){
  // cout<<idx<<endl;
  if(idx==-1 and sum%d==0)return 1;
  if(idx==-1)return 0;
  if(dp[idx][sum][tight]!=-1)return dp[idx][sum][tight];
  ll pos=k.length()-1-idx;
  ll ul=(tight?(k[pos]-'0'):9);
  ll ans=0;
  rep(i, 0, ul, 1) ans=(ans+noOfInt(idx-1, (sum+i)%d, (i==(k[pos]-'0')?tight:0)))%MOD;
...
int main(){
  fastio;
  cin>>k>>d;
  cout<<(noOfInt(k.length()-1, 0, 1)-1+MOD)%MOD<<endl;
  return 0;
}