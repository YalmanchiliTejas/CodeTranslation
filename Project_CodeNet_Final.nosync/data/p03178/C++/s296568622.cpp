...
int main()
{
    ...
    FOR(i, 0, n) {
        ...
        FOR(rem, 0, d) {
            FOR(nextDig, 0, 10) {
                int nextRem = (rem + nextDig) % d;
                dp[i + 1][nextRem][true] += dp[i][rem][true];
                dp[i + 1][nextRem][true] %= MOD;
            }
            FOR(nextDig, 0, kDig) {
                int nextRem = (rem + nextDig) % d;
                dp[i + 1][nextRem][true] += dp[i][rem][false];
                dp[i + 1][nextRem][true] %= MOD;
            }
            dp[i + 1][(rem + kDig) % d][false] += dp[i][rem][false];
            dp[i + 1][(rem + kDig) % d][false] %= MOD;
        }
    }
    cout << (dp[n][0][true] - 1 + dp[n][0][false] + MOD) % MOD;
}
/************************************** END OF PROGRAM ******************************************/
