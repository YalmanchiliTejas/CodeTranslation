...
void solve() {
    ...
    auto f = MEMOIZE<10001,100,2>([&S,N,D](auto&& self, i64 i, i64 r, bool tight) -> ModP {
        if(i == N) return r == 0 ? 1 : 0;

        ModP res = 0;
        i64 dmax = tight ? digit_ord(S[i]) : 9;
        REP(d, dmax+1) {
            res += self(i+1, modulo(r-d,D), tight && d==dmax);
        }
        return res;
    });

    ModP ans = f(0,0,true) - 1;  // 0 を除く
