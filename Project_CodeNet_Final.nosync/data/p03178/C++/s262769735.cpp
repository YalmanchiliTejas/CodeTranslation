...
void solve1() {
	...
	cin >> n;
	map<ll, Array>mp;
	Matrix p(n - 2, Array(3));
	REP(i, n) REP(j, 3)cin >> p[i][j],p[i][j]--,mp[p[i][j]].push_back(i);
	Array ans;
	deque<int> q;
	ll next;
	//size1のやつを順にたどってく
	REP(i, n)if (mp[i].size() == 1) {
		next = i;
		q.push_back(i);
		break;
	}
...
ll D;
ll a[10101];
ll solve(ll mn, ll d,ll m) {
//	cout << d << ' ' << m << ' ' << mn << endl;
	if (d == 0 && m == 0)return 1;
	if (d == 0)return 0;
	if (dp[d][m][mn] != -1)return dp[d][m][mn];
	ll ret = 0;
	if (mn) {
		REP(i, 10) {
			(ret += solve(mn, d - 1, ((m - i)%D + D) % D)) %= MOD;
		}
	}
	else {
		ll t = a[d];
		ret += solve(mn, d - 1, ((m - t) % D + D) % D);
		REP(i, t) {
			(ret += solve(1, d - 1, ((m - i) % D + D) % D)) %= MOD;
		}
	}
//	cout << d << ' ' << m << ' ' << mn << ' ' << ret << endl;
	return dp[d][m][mn] = ret;
}
...
int main() {
	...
	REP(i, 10101)REP(j, 110)REP(k, 2)dp[i][j][k] = -1;

	cout << (solve(0, k.size(), 0) - 1 + MOD) % MOD << "\n";
