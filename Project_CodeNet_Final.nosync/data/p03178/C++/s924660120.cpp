...
// [i][0] ith digit small than ith digit in s
// [i][1] ith digit equal to ith digit in s

// [i+1][0] = [i][0] + j, (0 <= j <= 9)
//          = [i][1] + j, (j < s[i+1])
// [i+1][1] = [i+1][1] + j, (j == s[i+1])
...
int main() {
    ...
    n = S.length();
    dp[0][0][0] = 1;
    REP(i, n) REP(j, d) REP(b, 2) REP(k, 10) {
        int c = S[i] - '0';
        if(b == 0 && k > c) continue;
        if(b == 0 && k == c) {
            dp[i+1][(j+k)%d][0] += dp[i][j][b];
        } else {
            dp[i+1][(j+k)%d][1] += dp[i][j][b];
        }
    }
    W( (dp[n][0][0] + dp[n][0][1] - 1));
    return 0;
}
