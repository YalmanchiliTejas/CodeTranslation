...
//dp[s.size()][sum][tight]
lli func(int n, int sum, int tight){
	if(n==s.size()){
		return sum==0;
	}
	int &res=dp[n][sum][tight];
	if(res!=-1)return res;
	res=0;
	int mi=0,ma=9;
	if(tight){
	...
	for(int i=mi;i<=ma;i++){
		int ntight = tight & (s[n]-'0'==i);
		res=add(res,func(n+1,(sum+i)%d,ntight));
	}
	return res;
}
int main()
{
	...
	cin>>s;
	cin>>d;
	dp.assign(s.size(),vector<vector<int>>(d,vector<int>(2,-1)));
	cout<<(func(0,0,1)-1+mod)%mod<<endl;
}
