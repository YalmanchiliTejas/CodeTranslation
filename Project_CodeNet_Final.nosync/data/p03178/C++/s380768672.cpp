    ...
    for (int i = 0; i < s.size(); i++)
    {
        ...
        for (int j = 0; j < 2; j++)
        {
            for (int d = 0; d <= (j ? 9 : D); d++)
            {
                for (int k = 0; k <= B; k++)
                {
                    dp[i + 1][j || (d < D)][(k + d) % B] += dp[i][j][k]; //dp[i][j][k] : i桁目まで見たときに
                    dp[i + 1][j || (d < D)][(k + d) % B] %= MOD;
                }
            }
        }
    }

    modint ans =(dp[s.size()][0][0] + dp[s.size()][1][0] - 1);
    cout << ans << endl;
}
...
n以下の数字の中で現れた１の数を求める
dp[i][j][k]:i桁目まで見たときに１の個数がk個の数字の個数と定義
...
赤・青・緑の花束が存在し、ある色を３本or全ての色を１本ずつ使ったときに、できる花束の最大数
