...
void solve() {
	...
	//dp[桁][smaller][3を含む]
	vector<vector<vector<Modint>>> dp = make_vector<Modint>({keta+1, 2LL, D}, 0LL);
	...
	for(ll i = 0; i < keta; i++){
		...
		for(ll smaller = 0; smaller < 2; smaller++){
			for(ll j = 0; j < D; j++){
				for(ll x = 0; x <= (smaller ? 9 : sval); x++){
					ll z=(j+x)%D;
					dp[i + 1][smaller || x < sval][z] += dp[i][smaller][j];
				}
			}
		}
	}
