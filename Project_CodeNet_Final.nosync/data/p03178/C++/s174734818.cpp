...
int main(){
    ...
    vector<modnum<MOD>> dprow(D, 0);
    vector<vector<modnum<MOD>>> dp9(N+1, dprow);
    vector<vector<modnum<MOD>>> dpK(N+1, dprow);

    dp9[0][0] = 1;
    for(int i=0;i<N;i++) {
        for(int d=0;d<D;d++){
            for(int j=0;j<10;j++){
                dp9[i+1][(d+j)%D] += dp9[i][d];
            }
        }
    }
    ...
    for(int i=0;i<N;i++) {
        ...
        for(int d=0;d<D;d++){
            for(int j=0;j<digit;j++){
                dpK[i+1][(d+j)%D] += dp9[i][d];
            }
            dpK[i+1][(d+digit)%D] += dpK[i][d];
        }
    }
