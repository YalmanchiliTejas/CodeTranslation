...
LL rec(string &S, LL k = 0, bool tight = true)
{
  ...
  //dump(k, tight, sum, x, r);
  //! 上位k桁まで見たときの和がsumで、
  //! それ以降のN-k桁を見たときの和がDであるものの数
  //! ただし、tightかどうかで異なるので分ける
  //if(dp[k][tight][sum] != -1) {
    //cerr<<"dp"<<endl;
  //  return dp[k][tight];
  //}
...
void func()
{
  ...
  REP(k,N){
    ...
    REP(d,D){
      REP(e,10){
        dp[k+1][(d+e)%D][0]  +=  dp[k][d][0];
        dp[k+1][(d+e)%D][0] %=MOD;
        if(e<x){
          dp[k+1][(d+e)%D][0] += dp[k][d][1];
          dp[k+1][(d+e)%D][0] %= MOD;
          
        }
        if(e==x){
          dp[k+1][(d+e)%D][1] += dp[k][d][1];
          dp[k+1][(d+e)%D][1] %= MOD;
        }
      }
    }
