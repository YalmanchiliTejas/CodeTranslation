...
#define nl cout<<"\n";
//fo(i,1,10) gives 1,2,3,....,9 and fo(i,10,1) gives 9,8,....,1 also fo(it, end(v), begin(v)) and fo(it, begin(v), end(v))
// #define fo(i, begin, end) for (__typeof(end) i = (begin) - ((begin) > (end)); i != (end) - ((begin) > (end)); i += 1 - 2 * ((begin) > (end)))
#define fo(i,l,r) for( int i = l-(l>r); i != r-(l>r); i += 1-2*(l>r) )
#define se(X,C) for( auto & X : C )
#define ima INT_MAX
#define imi INT_MIN
...
int func( int index, int tight, int req ) {
    if( index == s.length() ) return req == 0;
    int & ret = dp[index][tight][req];
    if( ret != -1 ) return ret;
    ret = 0;
    int mi = 0;
    int ma = 9;
    if( tight ) ma = s[index]-'0';
    fo(i,mi,ma+1) {
        int nt = tight&(s[index]-'0' == i);
        int nr = (req+i)%d; 
        am(ret,func(index+1,nt,nr));
    }
    return ret;
}
