...
void F() {
    ...
    fr1(n) for (int j = 1; j <= m; j++) {
        if (s[i - 1] == t[j - 1]) d0[i][j] = max({d0[i - 1][j], d0[i][j - 1], d0[i - 1][j - 1] + 1});
        else d0[i][j] = max(d0[i - 1][j], d0[i][j - 1]);
    }
    while (n || m) {
        if (n && m && s[n - 1] == t[m - 1] && d0[n - 1][m - 1] + 1 == d0[n][m]) n--, m--, u += s[n];
        else if (n && d0[n - 1][m] == d0[n][m]) n--;
        else m--;
    }
...
void build(int p) {
    while (p) p >>= 1, t[p] = max(t[p << 1], t[p << 1 | 1]) + d1[p];
}
...
void W() {
    int m, o, l[200005] = {}, r[200005] = {}, a[200005] = {};
    ll s = 0, z = 0;
    cin >> n >> m;
    fr(m) cin >> l[i] >> r[i] >> a[i], l[i]--;
    vector<pii> b[200005] = {}, c[200005] = {};
    fr(m) b[l[i]].push_back({r[i], a[i]}), c[r[i]].push_back({l[i], a[i]});
    fr(n) {
        for (pii j : b[i]) s += j.second;
        for (pii j : c[i]) s -= j.second;
        for (pii j : b[i]) inc(i, j.first, -j.second);//, cout << i << ' ' << j.first << ' ' << -j.second << '\n';
        for (pii j : c[i]) inc(j.first, i, j.second);//, cout << ' ' << j.first << ' ' << i << ' ' << j.second << '\n';
        //fr(n) cout << que(i, i + 1) << ' ';
        //cout << que(0, i) << ' ' << s << '\n';
        z = max(z, max(0ll, que(0, i)) + s);
        inc(i, i + 1, max(0ll, que(0, i)) + s);
    }
    //fr(n) cout << que(i, i + 1) << ' ';
    cout << z;
}
