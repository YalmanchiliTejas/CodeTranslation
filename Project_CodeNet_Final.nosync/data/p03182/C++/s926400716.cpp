...
class LazySegmentTree{
  ...
  void ascend(ll i){while(i >>= 1) tree[i] = act(lazy[i], dot(tree[i << 1], tree[i << 1 | 1]));}
  void descend(ll i){
    rep(k, logn, 0, -1){
      ll p = i >> k;
      E f = lazy[p];
...
public:
  LazySegmentTree(const vector<T> &A, function<T(T, T)> dot, T unit ,function<E(E, E)> compose, E identity ,function<T(E, T)> act)
  ...
  void range_act(ll l, ll r, E f){
    ...
    while(l < r){
      if(l & 1){
        tree[l] = act(f, tree[l]); lazy[l] = compose(f, lazy[l]);
        l++;
      }
      if(r & 1){
        r--;
        tree[r] = act(f, tree[r]); lazy[r] = compose(f, lazy[r]);
      }
      l >>= 1; r >>= 1;
    }
