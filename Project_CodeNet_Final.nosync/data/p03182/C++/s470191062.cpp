...
inline namespace {
	...
	template <int D> constexpr ll TEN = TEN<D - 1> * 10;
	template <> constexpr ll TEN<0> = 1;
}  // namespace
...
int main() {
	int n = in, m = in;
	vector<vector<pair<int, ll>>> range(n + 1);
	rep(i, m) {
		int l = in, r = in;
		ll a = in;
	...
	RAQRMQ<ll> seg(VL(n + 1, 0), -TEN<18>);
	FOR(i, 1, n + 1) {
		seg.add(i, seg(0, i));
		if (range[i].empty()) continue;
		for (auto [l, a] : range[i]) {
			seg.add(l, i + 1, a);
		}
	}
	out(seg(0, n + 1));
