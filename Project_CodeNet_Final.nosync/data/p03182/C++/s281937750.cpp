...
int main()
{
    ...
    REP(_, M)
    {
        int l, r, a;
        cin >> l >> r >> a;
        rla[r].push_back(pint(l, a));
    }

    vector<lint> tmp(N + 1, 0);
    LazySegTree<lint, lint> dp(tmp, 0, 0,
                               [](lint a, lint b) { return max(a, b); }, // 本情報のmerge
                               [](lint &a, lint b) { a += b; },          // 遅延情報の子孫への伝播
                               [](lint &a, lint b) { a += b; });         // 遅延情報の本情報への反映

    FOR(r, 1, N + 1)
    {
        dp.update(r, r + 1, dp.getvalue(1, r));
        for (auto pa : rla[r])
        {
            int l = pa.first;
            lint a = pa.second;
