...
class seg{
	vl S,D;
public:
	seg(int n):S(1<<MSB(n-1)+2,0ll),D(1<<MSB(n-1)+2,0ll){}
	...
	void add(int a,int b,ll x,int k=1,int l=0,int r=-1){
		...
		if(a<=l&&r<=b){
			...
			S[k]=std::max(S[k<<1],S[k<<1|1]);
		}
	}
	ll max(int a,int b,int k=1,int l=0,int r=-1){
		if(r<0)r=S.size()>>1;
		eval(k,l,r);
		if(b<=l||r<=a)return -e18;
		if(a<=l&&r<=b)return S[k];
		ll L=max(a,b,k<<1  ,l,l+r>>1);
		ll R=max(a,b,k<<1|1,l+r>>1,r);
		return std::max(L,R);
	}
};
...
int main(){cin.tie(0);ios::sync_with_stdio(false);
	...
	foor(i,1,N){
		for(pii p:q[i-1]){
			s.add(0,p.first,p.second);
		}
		s.add(i,i+1,s.max(0,i));
	}
	for(pii p:q[N]){
		s.add(0,p.first,p.second);
	}
