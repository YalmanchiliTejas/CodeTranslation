...
const int N = 1 << 18, B = 50, oo = 1e18 + 100, M = 1e9 + 7, LOG = 20;
...
int get(int l, int r, int lb, int rb, int ind)
{
    ...
    if (l <= m)
        ans = max(ans, get(l, min(m, r), lb, m, ind << 1));

    if (r > m)
        ans = max(ans, get(l, min(m, r), lb, m, ind << 1 | 1));
