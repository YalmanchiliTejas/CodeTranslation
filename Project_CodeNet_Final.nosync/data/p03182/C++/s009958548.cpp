...
struct range_max_range_add {
  ...
  struct binary_operation {
    first_type identity = std::numeric_limits<first_type>::min() / 2;
...
int main(){
  ...
  segment_tree<range_max_range_add<long long>> st(dp.begin(), dp.end());
  for(int i = 1; i < N+2; ++i){
    for(auto e : I[i]){
      if(e > 0){
        st.modify(0,L[e],A[e]);
    ...
    //cerr << i << " " << st.accumulate(0,i) << endl;
    st.modify(i,i+1,st.accumulate(0,i));
  }
  cout << st.accumulate(0,N+1) << endl;
}
