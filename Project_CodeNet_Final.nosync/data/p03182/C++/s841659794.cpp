...
public:
  ...
  value_type operator [](size_t k) { return aggregate(k, k+1); }
};

template <class Tp>
struct max {
  Tp identity = std::numeric_limits<Tp>::min() / 4;
...
struct add {
  Tp identity = 0;
  Tp operator ()(const Tp& x, const Tp& y, size_t k = 1) const {
    return x + y;
  }
};
...
int main() {
  ...
  std::vector<std::tuple<size_t, size_t, intmax_t>> v(M);
  for (auto& vi: v) {
    size_t l, r;
    intmax_t a;
    scanf("%zu %zu %zu", &l, &r, &a);
    vi = std::make_tuple(r, l, a);
  }
  std::sort(v.begin(), v.end());

  segment_tree<intmax_t, max<intmax_t>, add<intmax_t>> dp(N+1, 0);
  for (size_t i = 1, k = 0; i <= N; ++i) {
    intmax_t cur = dp.aggregate(0, i);
    dp.update(i, i+1, cur-dp[i]);
    while (k < M) {
      size_t l, r;
      intmax_t a;
      std::tie(r, l, a) = v[k];
