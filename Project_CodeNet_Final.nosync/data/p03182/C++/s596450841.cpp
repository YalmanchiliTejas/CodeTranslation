...
// segment tree capable of handling range maximum query and addition of a
// constant to all of the elements in a given range [b, e)

// buf[i] : The leaves are the actual values that were stored directly via st_update.
//          Includes the values added to its child nodes.
//          max(buf[chl] + add[chl], buf[chr] + add[chr])
long buf[SIZE_BUF];
// add[i] : The value added to all the leaves below node i.
long add[SIZE_BUF];
...
long st_query_r(long queryb, long querye, long cur, long curb, long cure) {
  if (queryb >= cure || querye <= curb) {  // no intersection
    ...
    return max(st_query_r(queryb, querye, chl, curb, m),
               st_query_r(queryb, querye, chr, m, cure)) + add[cur];
  }
}
