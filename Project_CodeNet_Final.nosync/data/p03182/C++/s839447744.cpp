...
struct Seg {
  int l, m, r;
  Seg* ch[2] = {};
  long long mi = 0, tag = 0;
  void pull() {
    mi = min(ch[0]->mi, ch[1]->mi);
  }
  void push() {
    if (tag) {
      for (int i : {0, 1}) ch[i]->tag += tag, ch[i]->mi += tag;
      tag = 0;
    }
  }
  ...
  long long query(int ql, int qr) {
    if (ql <= l and r <= qr) return mi;
    else {
      long long ans = 0;
      push();
      if (ql < m) ans = min(ans, ch[0]->query(ql, qr));
      if (m < qr) ans = min(ans, ch[1]->query(ql, qr));
      pull();
      return ans;
    }
  }
