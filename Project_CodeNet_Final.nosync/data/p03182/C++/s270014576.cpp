...
void deal(){
	...
	fori(m){
		ll a , b, v;
		cin>>a>>b>>v;
		nd[b].pb(vector<ll> ({a,b,v}));
	}
	Segtree lz;
	lz.init(1,0,n);
	for(ll i = 1; i<=n; i++){
		Segtree::node que(0, i-1, -inf, 0);
		lz.que(1,que);
		Segtree::node upd(i,i,  0, que.val);
		lz.upd(1,upd);
		for(auto el : nd[i]){
			Segtree::node upd(el[0], el[1], 0, el[2]);
			lz.upd(1, upd);
		}
	}
	Segtree::node que(0,n, -inf, 0);
	lz.que(1, que);
	cout<<que.val;
}
int main(){
