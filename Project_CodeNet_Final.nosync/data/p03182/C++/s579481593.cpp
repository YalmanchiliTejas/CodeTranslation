...
struct node {
    ...
    void pull(node& a, node& b) {
        val = max(a.val + a.lazy, b.val + b.lazy);
    }
...
int base = 1;
const int GET = 0, ADD = 1;
void query(int a, int b, LL& val, int type, int l = 0, int r = -1, int k = 1) {
    if(r == -1) r = base;
