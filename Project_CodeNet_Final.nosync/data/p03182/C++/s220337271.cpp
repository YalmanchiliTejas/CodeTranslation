...
//	mt19937_64 rng64(chrono::steady_clock::now().time_since_epoch().count());
	...
	public:
		...
		template<class T> bool operator != (const T& other) { return *this != initModular(other); }
		template<class T> friend bool operator == (const T& other, const Modular& m) { return initModular(other) == m; }
		template<class T> friend bool operator != (const T& other, const Modular& m) { return initModular(other) != m; }
		...
		template<class T> Modular  operator +  (const T& other) { return *this +  initModular(other); }
		template<class T> friend Modular operator += (const T& other, const Modular& m) { return initModular(other) += m; }
		template<class T> friend Modular operator +  (const T& other, const Modular& m) { return initModular(other) +  m; }
...
struct SegTree {
	...
	inline void push(int id) {
		if (tree[id].lazy != 0) {
			tree[id * 2].val += tree[id].lazy;
			tree[id * 2].lazy += tree[id].lazy;
			tree[id * 2 + 1].val += tree[id].lazy;
			tree[id * 2 + 1].lazy += tree[id].lazy;
			tree[id].lazy = 0;
		}
	}
	
	inline Node pull(const Node& l, const Node& r) const {
		Node tmp;
	...
	void upd(int id, int l, int r, int ql, int qr, ll v) {
		...
		tree[id] = pull(tree[id * 2], tree[id * 2 + 1]);
	}
	...
	Node qry(int id, int l, int r, int ql, int qr) {
		...
		Node ans;
		if (qr <= mid)
			return qry(id * 2, l, mid, ql, qr);
		if (ql > mid)
			return qry(id * 2 + 1, mid + 1, r, ql, qr);
			
		return pull(qry(id * 2, l, mid, ql, qr), qry(id * 2 + 1, mid + 1, r, ql, qr));
	}
	
	SegTree(int n) : LEAF(n) {
		tree.resize(4 * n);
	}
};
...
int main() {
	...
	vector<Event> e;
	for (int i = 0; i < m; i++) {
		int l,r; ll v; R(l,r,v);
		e.emplace_back(Event{l, 0, l - 1, v});
		e.emplace_back(Event{r + 1, 0, l - 1, -v});
	}
	...
	for (int i = 1; i <= n; i++) {
		while (ptr < e.size() && e[ptr].t <= i) {
			st.upd(1, 0, n, e[ptr].l, e[ptr].r, e[ptr].v);
			ptr++;
		}
			
		dp[i] = st.qry(1, 0, n, 0, i - 1).val;
		st.upd(1, 0, n, i, i, dp[i]);
	}
