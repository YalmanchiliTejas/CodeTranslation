...
struct Segtree{
	INT dt[1600005],wp[1600005];
	void jar(int now,int nl,int nr,int l,int r,INT num){
		if(wp[now]){
			dt[now<<1]+=wp[now];
			dt[now<<1|1]+=wp[now];
			wp[now<<1]+=wp[now];
		...
		int m=(nl+nr)>>1;
		jar(now<<1,nl,m,l,r,num);
		jar(now<<1|1,m+1,nr,l,r,num);
		dt[now]=max(dt[now<<1],dt[now<<1|1]);
	}
	INT qiu(int now,int nl,int nr,int l,int r){
		if(wp[now]){
			dt[now<<1]+=wp[now];
			dt[now<<1|1]+=wp[now];
			wp[now<<1]+=wp[now];
		...
		int m=(nl+nr)>>1;
		return max(qiu(now<<1,nl,m,l,r),qiu(now<<1|1,m+1,nr,l,r));
	}
}seg;

vector<pii> ins[200005],del[200005];
INT dp[200005];
INT qzh[200005];
...
int main(){
	...
	INT ans=0;
	for(int i=1;i<=200000;i++){
		dp[i]=seg.qiu(1,0,200000,0,i-1)+qzh[i];
		for(int j=0;j<SZ(ins[i]);j++){
			seg.jar(1,0,200000,i,ins[i][j].first,-ins[i][j].second);
		}
		for(int j=0;j<SZ(del[i]);j++){
			seg.jar(1,0,200000,del[i][j].first,i,del[i][j].second);
		}
		seg.jar(1,0,200000,i,i,dp[i]);
		ans=max(ans,dp[i]);
		//cerr<<i<<' '<<dp[i]<<endl;
	}
	fio<<ans<<endl;
