 ...
 */
const ll ID = -100000000000000000;
...
template<class T, int SZ> struct LazySeg { 
	...
	void pull(int ind) { sum[ind] = max(sum[2*ind], sum[2*ind+1]); }
...
ll ans;
vector<pair<int, ll>> be[200005];
vector<pair<int, ll>> en[200005];
LazySeg<ll, 262144> lseg;
...
int main() {
	...
	for(int i = 0; i <= N; i++){
		ll val = lseg.qsum(0, i);
		//ps("i, val", i, val);
		//ps(lseg.qsum(0, 0));
		ans = max(val, ans);
		if(i > 0) lseg.upd(i, i, -ID);
		lseg.upd(i, i, val);
		//ps("UPD", i, i, val);
		for(auto u: en[i]){
			lseg.upd(0, u.f-1, -(u.s));
			//ps("UPD", u.f, i, -(u.s));
		}
		for(auto u: be[i+1]){
			lseg.upd(0, i, u.s);
