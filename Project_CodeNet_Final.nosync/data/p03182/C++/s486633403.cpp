...
private:
    ...
    T add(int a, int b, T x, int k, int l, int r) {
        ...
        T z = add(a, b, x, 2*k+1, l, (l+r)/2), y = add(a, b, x, 2*k+2, (l+r)/2, r);
        node[k] = nodeUpdate(z, y);
        return node[k];
    }
    ...
    T query(int a, int b, int k, int l, int r) {
        ...
        if (left && right) {
            T x = query(a, b, 2*k+1, l, (l+r)/2), y = query(a, b, 2*k+2, (l+r)/2, r);
            return nodeUpdate(x, y);
        }
...
public:
    ...
    void set(int a, T x) { add(a, a+1, x-query(a, a+1)); }
