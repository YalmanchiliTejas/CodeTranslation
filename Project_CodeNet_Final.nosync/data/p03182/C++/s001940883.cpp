...
struct SegTree {
    ...
    const int GET = 0, UPDATE = 1;
    void query(int a, int b, LL& val, int type, int l, int r, int k) {
        if(b <= l or r <= a) {
            return;
        }
        if(a <= l and r <= b) {
            if(type == GET) {
                val = max(val, st[k].val);
            } else {
                st[k].add(val);
            }
            return;
        }
        st[k].push(st[k << 1], st[k << 1 | 1]);

        int mid = l + (r - l) / 2;
        query(a, b, val, type, l, mid, k << 1);
        query(a, b, val, type, mid, r, k << 1 | 1);

        st[k].pull(st[k << 1], st[k << 1 | 1]);
    }

    void update(int a, int b, LL val) {
        query(a, b, val, UPDATE, 0, base, 1);
    }

    auto get(int a, int b) {
        LL res = -LLINF;
        query(a, b, res, GET, 0, base, 1);
        return res;
    }
};
...
struct Solution {
    Solution(int) {
        int N, M;
        read(N, M);
        vector<vector<pair<int, LL>>> itvs(N + 2);
        for(repeat(M)) {
            int l, r, a;
            read(l, r, a);
            itvs[l].emplace_back(l, a);
            itvs[r + 1].emplace_back(l, -a);
        }
        auto st = SegTree(N + 1);
