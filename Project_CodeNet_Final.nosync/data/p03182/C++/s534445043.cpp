...
signed main(){
	...
	auto g=[](int a,int b,int sz){return a+b;};
	auto h=[](int a,int b){return a+b;};
	Segtree<int,int,decltype(f),decltype(g),decltype(h)>segtree(N,f,g,h,0,0);
	rep(i,M){
		int l,r,m;cin>>l>>r>>m;l--;r--;
		v[r].pb({l,m});
	}
	rep(i,N){
		segtree.update(i,i+1,max(0ll,segtree.query(0,i)));
		for(P j:v[i]){
			segtree.update(j.first,i+1,j.second);
		}
	}
	cout<<max(0ll,segtree.query(0,segtree.size))<<endl;
}
