...
vector<vector<pll>> edges(220000);
...
struct Seg{
  ...
  ll mxv(ll l, ll r, ll pos=1, ll bottom = 0, ll top = 1<<18){
    if(top <= l || r <= bottom)return -INF;
    if(l<=bottom && top <= r)return mx[pos]+sum[pos];

    ll mid = (bottom + top)/2;
    return max<ll>(
      mxv(l,r, pos*2, bottom, mid),
      mxv(l,r, pos*2+1, mid, top)
    )+sum[pos];
  }
};
...
int main(){
  ...
  cout<<max<ll>(0LL, seg.mxv(0, n+1))<<endl;

//  rep(i, n)cout<<seg.mxv(i, i+1)<<' ';cout<<endl;
