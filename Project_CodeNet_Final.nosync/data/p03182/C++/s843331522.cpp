...
class LazySegmentTree {
    // TODO: Change Monid and Operator, their identity, their product, and the action.
...
public:
    ...
    LazySegmentTree(const std::vector<Monoid> &m) : LazySegmentTree(m.size()) {
        int n = m.size();
        for (int i = 0; i < n; i++) {
    ...
    void apply(Operator q, int a, int b, int k = 0, int left = 0, int right = -1) {
        ...
        apply(dat_op[k], left, left + (right - left) / 2, 2 * k + 1, left, left + (right - left) / 2); // Left side
        apply(dat_op[k], left + (right - left) / 2, right, 2 * k + 2, left + (right - left) / 2, right); // Right side
    ...
    Monoid getval(int a, int b, int k = 0, int left = 0, int right = -1) {
        ...
        Monoid vleft = getval(a, b, 2 * k + 1, left, left + (right - left) / 2); // Left side
        Monoid vright = getval(a, b, 2 * k + 2, left + (right - left) / 2, right); // Right side
        return act(dat_op[k], std::max(0, std::min(b, right) - std::max(a, left)), monoid_product(vleft, vright));
    }
};
...
int main() {
    ...
    std::vector<std::vector<int>> ids(n + 1);
    for (int i = 0; i < m; i++) {
        ids[r[i]].push_back(i);
    }
    LazySegmentTree seg(n + 1);
    for (int i = 1; i <= n; i++) {
        seg.apply(seg.getval(0, i), i, i + 1);
        for (int j : ids[i]) {
            seg.apply(a[j], l[j], r[j] + 1);
        }
    }
    std::cout << seg.getval(0, n + 1) << std::endl;
    return 0;
}
