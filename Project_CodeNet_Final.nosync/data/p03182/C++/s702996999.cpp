...
class segtree {
    public :
        vl tree, lazy;
        segtree() {
            tree.assign(4*N+4,lmi);
            lazy.assign(4*N+4,0);
        }
        void update_node( int node, li val ) {
            lazy[node] += val;
            tree[node] += val;
        }
        void shift( int node ) {
        ...
        li query( int node, int l, int r, int ql, int qr ) {
            if( ql > qr ) return lmi;
            if( l == ql && r == qr ) {
                return tree[node];
            } 
            shift(node);
            int m = (l+r)/2;
            li a = query(2*node,l,m,ql,min(m,qr));
            li b = query(2*node+1,m+1,r,max(m+1,ql),qr);
            return max(a,b);
        }
        void update( int node, int l, int r, int ql, int qr, li val ) {
            if( ql > qr ) return;
            if( ql == l && qr == r ) {
            ...
            shift(node);
            int m = (l+r)/2;
            update(2*node,l,m,ql,min(m,qr),val);
            update(2*node+1,m+1,r,max(m+1,ql),qr,val);
            tree[node] = max(tree[2*node+1],tree[2*node]);
        }
        void point_update( int node, int l, int r, int index, li val ) {
            if( l == r ) {
                tree[node] = val;
                return;
            } 
            shift(node);
            int m = (l+r)/2;
            if( index <= m ) point_update(2*node,l,m,index,val);
            else point_update(2*node+1,m+1,r,index,val);
...
void solve() {
    ...
    FOR(i,0,n+1) {
        // tl(i);
        SEL(e,L[i]) tr.update(1,0,N-1,0,i-1,e);
        upd(dp[i],tr.query(1,0,N-1,0,i-1));
        // tl(dp[i],tr.query(1,0,N-1,0,i-1));
        tr.point_update(1,0,N-1,i,dp[i]);
        // tl(dp[i]);
        FOR(j,0,sz(R[i])) tr.update(1,0,N-1,0,RL[i][j]-1,-1*R[i][j]);
        // cout<<"-\n";
        // FOR(j,0,n+1)tl(tr.query(1,0,N-1,j,j));
    }
    FOR(i,0,n+1) upd(ans,dp[i]);
    cout<<ans<<"\n";
}
