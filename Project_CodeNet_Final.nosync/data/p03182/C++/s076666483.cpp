...
struct SegmentTree{
  ...
  //find(l,r,fanc): fancを満たす区間[0,id)なるidの最小値を返す
  //求めたいものが右端の場合は要素を反転したものをセグ木に詰めておくと逆向きに見れる
  template<typename C>
...
signed main(){
  ...
  FOR(i,1,n+1){
    int mx = seg.query(0,i); //最後に1にしたindexがi-1のときのmax
    seg.update(i,i+1,mx);
    for(P x:R[i]) seg.update(x.fs,i+1,x.sc);
  }

  cout << seg.query(0,n+5) << endl;
