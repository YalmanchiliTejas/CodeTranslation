...
template<class T, class U> void operator+=(__gnu_pbds::priority_queue<T, greater<T>, __gnu_pbds::rc_binomial_heap_tag> &a, const U &v) { a.push(v); }
template<class T, class U> void operator+=(__gnu_pbds::priority_queue<T, less<T>, __gnu_pbds::rc_binomial_heap_tag> &a, const U &v) { a.push(v); }
...
struct xorshift {
    ...
    size_t operator()(const std::tuple<ll, ll, ll>& x) const {
        vector<ll> x2 = {get<0>(x), get<1>(x), get<2>(x)};
        return operator()(x2);
    }
    size_t operator()(const std::tuple<ll, ll, ll, ll>& x) const {
        vector<ll> x2 = {get<0>(x), get<1>(x), get<2>(x), get<3>(x)};
        return operator()(x2);
    }
    size_t operator()(const std::tuple<ll, ll, ll, ll, ll>& x) const {
        vector<ll> x2 = {get<0>(x), get<1>(x), get<2>(x), get<3>(x), get<4>(x)};
        return operator()(x2);
    }
...
template<typename K, typename T> using map_f= map<K, T>;
...
template<typename K, typename T> using map_f=__gnu_pbds::tree<K, T, less<K>, __gnu_pbds::rb_tree_tag, __gnu_pbds::tree_order_statistics_node_update>;
...
//マクロ 定数
constexpr ll k3 = 1010;
constexpr ll k4 = 10101;
constexpr ll k5 = 101010;
constexpr ll k6 = 1010101;
constexpr ll k7 = 10101010;
...
namespace std {
    ...
    template<> class hash<std::tuple<ll, ll, ll>> {
    public:
        size_t operator()(const std::tuple<ll, ll, ll> &x_) const {
            static xorshift xor_s;
            return xor_s(x_);
        }
    };
    template<> class hash<std::tuple<ll, ll, ll, ll>> {
    public:
        size_t operator()(const std::tuple<ll, ll, ll, ll> &x_) const {
            static xorshift xor_s;
            return xor_s(x_);
        }
    };
    ...
    public:
        size_t operator()(const std::vector<T> &x_) const {
            static xorshift xor_s;
            return xor_s(x_);
        }
    };
...
ll find_if2(const T &v, F f) {    rep(i, sz(v)) { if (f(v[i]))return i; }    return sz(v);}
...
void replace(str &a, char key, char v) {fora(a_, a){if(a_==key)a_ = v;}}
//keyと同じかどうか01で置き換える
...
void replace(string &a, char key) {    string res;    for(auto c : a){        if(c != key)res.push_back(c);    }    swap(a, res);}
...
void replace(str &a, char key, str v) {replace(a, ""+key, v);}
...
template<class T> vector<T> merge(const vector<T> &a, const vector<T> &b) {    vector<T> res;    std::merge(ALL(a), ALL(b), back_inserter(res));    return res;}

//firstの値が同じ場合secondが最小の物を残す
template<class T, class U> void unique_min(vector<pair<T, U>> &a) {    vector<pair<T, U>> res;    if (sz(a))res.push_back(a[0]);    rep(i, 1, sz(a)) {        if (res.back().fi == a[i].fi) {            chmi(res.back().se, a[i].se);        }else {            res.push_back(a[i]);        }    }   swap(a, res);}
//firstの値が同じ場合secondが最大の物を残す
template<class T, class U> void unique_max(vector<pair<T, U>> &a) {    vector<pair<T, U>> res;    if (sz(a))res.push_back(a[0]);    rep(i, 1, sz(a)) {        if (res.back().fi == a[i].fi) {            chma(res.back().se, a[i].se);        }else {            res.push_back(a[i]);        }    }    swap(a, res);}

template<class T> bool includes(vector<T> &a, vector<T> &b) {    vi c = a;    vi d = b;    sort(c);    sort(d);    return includes(ALL(c), ALL(d));}
template<class T> bool is_unique(const vector<T> &A) {    if ((int) (A).size() == 1)return true;    if ((int) (A).size() == 2)return A[0] != A[1];    if ((int) (A).size() == 3)return (A[0] != A[1] && A[1] != A[2] && A[0] != A[2]);    auto B = A;    sort(B);    int N = (B.size());    unique(B);    return N == (int) (B.size());}
template<class H, class... T> bool is_unique(const H &a, const T &...b) { return is_unique(vector<H>{a, b...}); }
...
template<class T, require_t(!has_value_type<T>::value)>
T in() {    T ret;    cin >> ret;    return ret;}
template<class VT, class T = typename VT::value_type>
VT in(int len) {    VT ret;    T dat;for (int i = 0; i < len; i++){cin >> dat; ret += dat;}    return ret;}
template<class VT, class T = typename VT::value_type>
VT ind(int len) {    VT ret;    T dat;for (int i = 0; i < len; i++){cin >> dat; ret += (dat-1);}    return ret;}
...
template<class T> string deb_tos(const vector<vector<T> > &a, ll H = inf, ll W = inf, int key = -1) {    H = (H != inf) ? H : min({H, sz(a), DEB_LEN_H});    W = 0;    rep(h, H){        chma(W, sz(a[h]));    }    chmi(W, DEB_LEN_H);    vi hs, ws;    rep(h, H) { hs.push_back(h); }    rep(w, W) { ws.push_back(w); }    return deb_tos(a, hs, ws, key);}
/*@formatter:off*/
...
template<class T> string deb_tos(vector<multiset<T> > &a, ll H = inf, ll W = inf, int key = -1) {    vector<vector<T> > b(sz(a));    rep(i, sz(a)) { fora(v, a[i]) { b[i].push_back(v); }}    return deb_tos(b, H, W, key);}
template<class T> string deb_tos(const set<T>& S){    vector<T> b;    fora(s, S)b += s;    return deb_tos(b);}
...
#define dnao2(a, b, n) vi a,b; nao2(a,b,n);
...
/*@formatter:off*/
#ifdef _DEBUG
#define deb(...)  do{if(cou_deb++ >= 1000)break;was_deb=true;cerr<< over10(__VA_ARGS__,deb10,deb9,deb8,deb7,deb6,deb5,deb_4,deb_3,deb_2,deb1)(__VA_ARGS__) <<endl;}while(0)
...
//エラーN
//多次元なら(-1,-1,-1)
template<typename W, typename T> ll find(vector<W> &a, int l, const T key) {rep(i, l, sz(a))if (a[i] == key)return i;return sz(a);}
template<typename W, typename T> ll find(vector<W> &a, const T key) {rep(i, sz(a))if (a[i] == key)return i;return sz(a);}
...
//無ければN
int find(const string &s, const string key) {    int klen = sz(key);    rep(i, sz(s) - klen + 1) {        if (s[i] != key[0])continue;        if (s.substr(i, klen) == key) { return i; }    }return sz(s);}
int find(const string &s, int l, const string key) {    int klen = sz(key);    rep(i, l, sz(s) - klen + 1) {        if (s[i] != key[0])continue;        if (s.substr(i, klen) == key) { return i; }    }    return sz(s);}
int find(const string &s, const char key) {    rep(i, sz(s)) { if (s[i] == key)return i; }    return sz(s);}
int find(const string &s, int l, const char key) {    rep(i, l, sz(s)) { if (s[i] == key)return i; }    return sz(s);}

//N箇所について(自分含む)右のkeyの場所を返す
template<typename W, typename T> vi finds(const W &a, const T &key) {    int n = sz(a);    vi rpos(n, n);    rer(i, n - 1) {        if (i < n - 1) { rpos[i] = rpos[i + 1]; }        if (a[i] == key)rpos[i] = i;    }    return rpos;}
...
ll count(const W &a, const T &k, require_t(!is_same2(W, string))) { return count_if(a, ==k); }
...
ll count(const str &a, const char &k) {return count(a, tos(k));}
/*@formatter:off*/
...
template<class T, class F>vector<T> ruiv(const vector<T> &A, F f, T e) {    vector<T> ret(sz(A) + 1);    ret[0] = e;    rep(i, sz(A)) {        ret[i + 1] = f(ret[i], A[i]);    }    return ret;}
...
template<class T> vector<T> ruix(const vector<T> &a) {    vector<T> ret(a.size() + 1);    rep(i, a.size())ret[i + 1] = ret[i] ^ a[i];    return ret;}
...
#define for_base(...) over3(__VA_ARGS__,base3,base2,base1)(__VA_ARGS__)
...
//便利 数学 math
//sub ⊂ top
bool subsup(int sub, int top) {return (sub & top) == sub;}
//-180 ~ 180 degree
double atand(double h, double w) {return atan2(h, w) / PI * 180;}
double degree(double theta) {return theta / PI * 180;}
//% -mの場合、最小の正の数を返す
...
ll sig0(ll s, ll t) {if (s > t)swap(s, t);return sig0(t - s) + s * (t - s + 1);}
...
//todo stringもセットで
//find_ifはsubrを使う
string sub(const string &A, int l = -1, int r = -1) {    set_lr12(l, r, sz(A));assert(0 <= l && l <= r && r <= sz(A));    return A.substr(l, r - l);}
...
vb::reference operator&=(vb::reference a, bool b){return a = a & b;}
template<class T, class U> void operator+=(pair<T,U> &a, const pair<T,U> & b) {a.fi+=b.fi;a.se+=b.se;}
template<class T, class U> pair<T, U> operator+(const pair<T, U> &a, const pair<T, U> &b) { return pair<T, U>(a.fi + b.fi, a.se + b.se); }
template<class T, class U> pair<T, U> operator*(const pair<T, U> &a, int b) { return pair<T, U>(a.fi * b, a.se * b); }
template<class T, class U> pair<T,U> operator-(const pair<T,U> &a, const pair<T,U> & b) {return pair<T,U>(a.fi-b.fi,a.se-b.se);}
...
ll stoi(string& s){    ll res=0;    for(auto&& v : s){        res*=10;        res += v - '0';    }    return res;}
string itos(int v) {    string s;    while (v) {        s += v % 10;        v /= 10;    }    return rev(s);}
...
//tou分割する
template<typename Iterable, require_t(has_value_type<Iterable>::value)> vector<Iterable> cut(const Iterable &a, int tou = 2) {int N = sz(a);    vector<Iterable> res(tou);    int hab = N / tou;    vi lens(tou, hab);    rep(i, N % tou) { lens[tou - 1 - i]++; }    int l = 0;    rep(i, tou) {        int len = lens[i];        int r = l + len;        res[i].resize(len);        std::copy(a.begin() + l, a.begin() + r, res[i].begin());        l = r;    }    return res;}
//長さn毎に分割する
template<typename Iterable> vector<Iterable> cut_n(const Iterable &a, int len) {    int N = sz(a);    vector<Iterable> res(ceil(N, len));    vi lens(N / len, len);    if (N % len)lens.push_back(N % len);    int l = 0;    rep(i, sz(lens)) {        int len = lens[i];        int r = l + len;        res[i].resize(len);        std::copy(a.begin() + l, a.begin() + r, res[i].begin());        l = r;    }    return res;}
vi cut(int N, int K) {    vi res(K);    int lim = K;    rep(i, lim){        res[i] = N / K;        N -= res[i];        K--;    }    return res;}
...
template<class T> struct my_set : public set<T> {
    template<class... Arg> my_set(Arg... arg) : set<T>(arg...) {}
    T back() { return *(set<T>::rbegin()); }
    T front() { return *(set<T>::begin()); }
};
#define set my_set
...
public:
    ...
    constexpr modint(const Type &x) noexcept {        if (-mod() <= x && x < mod()) value = static_cast<Type>(x); else value = static_cast<Type>(x % mod());        if (value < 0) value += mod();    }
    ...
    constexpr modint operator*(const modint rhs) const noexcept {return modint(*this) *= rhs;}
    constexpr modint operator/(const modint rhs) const noexcept {return modint(*this) /= rhs;}
    constexpr modint &operator+=(const modint rhs) noexcept {        value += rhs.value;        if (value >= mod()) {            value -= mod();        }        return *this;    }
    constexpr modint &operator-=(const modint rhs) noexcept {        if (value < rhs.value) {            value += mod();        }        value -= rhs.value;        return *this;}
    constexpr modint &operator*=(const modint rhs) noexcept {value = value * rhs.value % mod();return *this;}
    constexpr modint &operator/=(const modint& rhs) noexcept {return operator*=(minv(rhs.value));}
    modint &operator++() { return *this += 1; }
    modint &operator--() { return *this -= 1; }
    modint operator++(signed) {modint result(*this);*this += 1;return result;}
    modint operator--(signed) {modint result(*this);*this -= 1;return result;}
...
//4e6 >= n なら O(r)で計算する
mint com_big(int n, int r) {    Type fa = finv[r];    rep(i, r)fa *= n - i, fa %= MOD;    return mint(fa);}
...
Type nprin(int n, int r) {/*    if (!sz(fac)) com(0, -1);*/    if (r < 0 || r > n) return 0;return fac[n] * finv[n - r] % MOD;}
Type icom(int n, int r) {    const int NUM_ = 1400001;    static ll fac[NUM_ + 1], finv[NUM_ + 1], inv[NUM_ + 1];    if (fac[0] == 0) {        inv[1] = fac[0] = finv[0] = 1;        for (int i = 2; i <= NUM_; ++i) inv[i] = inv[MOD % i] * (MOD - MOD / i) % MOD;        for (int i = 1; i <= NUM_; ++i) fac[i] = fac[i - 1] * i % MOD, finv[i] = finv[i - 1] * inv[i] % MOD;    }    if (r < 0 || r > n) return 0;    return ((finv[r] * fac[n] % MOD) * finv[n - r]) % MOD;}
#define ncr com
#define ncri icom
//n種類でr個選ぶ
//n個の場所にr個の物を置く
...
Type nhri(int n, int r) { if (n == 0 && r == 0)return 1; else return icom(n + r - 1, r); }
...
//int型 mod m
template<typename T, typename U> mint mpow(const T& a, const U &b) {if (b < 0)return minv(mpow(a, -b));    mint x = a, res = 1;    U p = b;    while (p > 0) {        if (p & 1) res *= x;        x *= x;        p >>= 1;    }    return res;}
template<typename T, typename U, typename V> int mpowi(const T &a, const U &b, const V &m = MOD) {return mpow(a,b, m);}
...
//@formatter:off
/*@formatter:on*/
template<typename T> std::istream &operator>>(std::istream &stream, modint<T> &number) {
    typename common_type<typename modint<T>::Type, Type>::type x;
    stream >> x;
