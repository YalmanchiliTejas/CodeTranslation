...
///最下位の1になっているビットの桁の位置
inline unsigned int ctz64(unsigned long long x) { unsigned long r; _BitScanForward64(&r, x); return r; }
#endif
...
//__m128i buf[u6(1e6 / 16) + 2];


//constexpr u6 modval = 998244353ull;
//unsigned short d[u6(2e3 + 1)][u6(2e3 + 1)]{};
...
pfactors parsepfactors(u6 value) {
	pfactors result;
	result.count.resize(pfactors::tblsize);
	rep(i, pfactors::tblsize) {
		while (1) {
			if (value % primetbl[i] != 0)break;
			result.count[i]++;
			value /= primetbl[i];
		}
		if (value == 1)return result;
	}
	result.lcount.emplace(value, 1);
	return result;
}
template <typename Fn>
void primeetccall(Fn fn) {
	if (fn(2))return;
	if (fn(3))return;
	//vector<u6> pbuf{ 2,3 };
	for (u6 t = 1;; ++t) {
...
struct Calc {
	...
	auto calc() {
		...
		r %= moduloNo;
		r2 %= moduloNo;
		return (r*r%moduloNo + moduloNo - r2) % moduloNo * powm(2, moduloNo - 2) % moduloNo;
	}
	/*******************************************************************************/
};
