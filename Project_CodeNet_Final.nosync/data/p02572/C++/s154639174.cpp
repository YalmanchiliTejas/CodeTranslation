...
struct Combination {
  ...
  T nPk(int n, int r) const {
    if(r < 0 || n < r) return 0;
    return fact(n) * rfact(n - r);
  }

  T nCk(int p, int q) const {
    if(q < 0 || p < q) return 0;
    return fact(p) * rfact(q) * rfact(p - q);
  }

  T nHk(int n, int r) const {
    if(n < 0 || r < 0) return (0);
    return r == 0 ? 1 : nCk(n + r - 1, r);
  }
};
...
string llto26string1_indexed(ll n){
    string ans="";
    while(n){
        n--;
        ans+=(char)((n%26)+'a');
...
private:
    ...
    void setPrimeSet(){
        for(int i=2;i<N;i++){
            if(prime_sieve[i]==i){
                set_prime.emplace_back(i);
            }
        }        
    }
public:    
    osa_k(const int n):N(n){
        prime_sieve.resize(n);
        iota(prime_sieve.begin(),prime_sieve.end(),0);
        eratosthenes();
        setPrimeSet();
    }

    //例えばi=8の時、{2,2,2}
    vector<int> getFactor(int i) {
        vector<int> res;
        while(i>1){
            res.emplace_back(prime_sieve[i]);
            i/=prime_sieve[i];
        }
        return res;
    }
    //例えばi=12の時、{{2,2},{3,1}};
    vector<pair<int,int>> getFactorPow(int i) {
        vector<pair<int,int>> res;
        int old=prime_sieve[i];
        int count=0;
        while(i>1){
            if(old==prime_sieve[i]){
                count++;
            }
            else{
                res.emplace_back(make_pair(old,count));
                old=prime_sieve[i];
                count=1;
            }
            i/=prime_sieve[i];
    ...
    vector<int> getFactorSet(int i){
        ...
        //重複を省く
        sort(res.begin(),res.end());
        decltype(res)::iterator result = unique(res.begin(), res.end());
        res.erase(result, res.end());
        return res;
    }
    //素数の集合を得る {2,3,5,7,11,...}
    vector<int> getPrimeSet(){
        return set_prime;
    }
    //iが素数:true
...
class IsPrime{
private:
    const vector<ll> v = {3, 5, 7, 11, 13, 17, 19, 23};
...
bool SEARCH(ll mid,ll n,ll k,const vector<int> a){
...
//IsPrime prime;
//Combination<ModInt<MOD>> com(10000005);
