...
int nCr(int a,int b){
  if (b == 0 ){ return 0;}
  int i,r=1;for(i=1;i<=b;i++){r*=(a+1-i);r/=i;}return r;} //コンビネーション
...
long long llnCr(long long a,long long b){
  if (b == 0 ){ return 0;}
  long long i,r=1;for(i=1;i<=b;i++){r*=(a+1-i);r/=i;}return r;}
...
int main() {
  ...
  forr(i,1,n){
    cin >> v.at(i);
    sum.at(i) = v.at(i) + sum.at(i-1);
  }
  ll ansum=0;
  forn(i,n){
    ansum += (((sum.at(n-1)-sum.at(i))%1000000007)*v.at(i))%1000000007 ;
  }
  ANS(ansum%1000000007);
}