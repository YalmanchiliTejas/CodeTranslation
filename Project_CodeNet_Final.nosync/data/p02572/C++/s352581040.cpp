...
#define rep(i,n) for(int i=0,i##_fin=(n);i<i##_fin;i++)
#define brep(i,a,b) for(int i=(a),i##_fin=(b);i<=i##_fin;i++)
#define rrep(i,n) for(int i=(n)-1,now=0;i>=0;i--,now++)
#define rbrep(i,a,b) for(int i=(a),i##_fin=(b),now=0;i>=i##_fin;i--,now++)
#define xrep(i,n) for(int i=1,i##_fin=(n);i<=i##_fin;i++)
...
#define in_area(p,a,b) (0<=(p.first)&&0<=(p.second)&&(p.first)<(a)&&(p.second)<(b))
#define in__area(y,x,a,b) (0<=(y)&&0<=(x)&&(y)<(a)&&(x)<(b))
...
template<class T> long long modpow(long long m,T n,long long M=MOD){long long r=1;while(n>0){if(n&1){r*=m;r%=M;}m*=m;m%=M;n/=2;}return r;}
...
template<class T> void printd(const vector<T> &v,const string &d){for(int i=0,i_fin=(int)v.size()-1;i<i_fin;i++)cout<<v[i]<<d;cout<<*v.rbegin()<<"\n";}
...
template<class T> T gcd(const vector<T> &v){T r=v[0];for(int i=1,i_fin=v.size();i<i_fin;i++)r=gcd(r,v[i]);return r;}
template<class T> T lcm(const vector<T> &v){T r=v[0];for(int i=1,i_fin=v.size();i<i_fin;i++)r=lcm(r,v[i]);return r;}
template<class T> T vectorAdd(const T &u,const T &v)     {T r(u.size());for(int i=0,i_fin=u.size();i<i_fin;i++)r[i]=u[i]+v[i];return r;}
template<class T> T vectorSubtract(const T &u,const T &v){T r(u.size());for(int i=0,i_fin=u.size();i<i_fin;i++)r[i]=u[i]-v[i];return r;}
template<class T> T vectorMultiply(const T &u,const T &v){T r(u.size());for(int i=0,i_fin=u.size();i<i_fin;i++)r[i]=u[i]*v[i];return r;}
template<class T> T vectorDivide(const T &u,const T &v)  {T r(u.size());for(int i=0,i_fin=u.size();i<i_fin;i++)r[i]=u[i]/v[i];return r;}
template<class T> T dotProduct(const deque<bool> &u,const vector<T> &v){T r=0;for(int i=0,i_fin=u.size();i<i_fin;i++)if(u[i])r+=v[i];return r;}
template<class S,class T> typename common_type<S,T>::type dotProduct(const vector<S> &u,const vector<T> &v){typename common_type<S,T>::type r=0;for(int i=0,i_fin=u.size();i<i_fin;i++)r+=u[i]*v[i];return r;}
...
void solve(int N, vector<Int> A) {
  ...
  rep(i, N - 1) {
    ans += A[i] * cusum[i + 1];
    watch(A[i], cusum[i + 1], (Int) A[i] * cusum[i + 1]);
    ans %= MOD;
  }
