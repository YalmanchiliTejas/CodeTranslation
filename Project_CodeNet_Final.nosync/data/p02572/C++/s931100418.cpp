...
int main() {
    ...
    sum = 0;
    for(int $1=0, $e=(int)A.size(); $1<$e; ++$1) sum = mod((sum) + (A[$1]), 1e9 + 7);
    ret = 0;
    for(int i=0; i<A.size(); ++i) {
        sum = mod((sum) - (A[i]), 1e9 + 7);
        b = sum;
        b = mod_mul(b, A[i], 1e9 + 7);
        ret = mod((ret) + (b), 1e9 + 7);
    }
    print(ret);
    return 0;
