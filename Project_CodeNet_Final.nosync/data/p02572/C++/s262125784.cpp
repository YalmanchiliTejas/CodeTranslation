...
#define trace4(a, b, c, d) cerr << a << sp << b << sp << c << sp << d << nl;
...
int dx[4] = {0, 1, 0, -1};
int dy[4] = {1, 0, -1, 0};
int knightx[8] = {2, 2, 1, 1, -1, -1, -2, -2};
int knighty[8] = {1, -1, 2, -2, 2, -2, 1, -1};
...
int32_t main()
{
    ...
    while (tests--)
    {
        ...
        int a[n + 1], pref[n + 1];
        pref[0] = 0;
        f(i, 1, 1 + n) cin >> a[i], pref[i] = add(pref[i - 1], a[i]);
        int ans = 0;
        f(i, 2, 1 + n) cadd(ans, mult(a[i], pref[i - 1]));
        cout << ans;

        // RESTORE SEGTREE SIZE
    }
