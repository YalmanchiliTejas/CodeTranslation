...
public:
	void solve(std::istream& in, std::ostream& out)
	{
		...
		vll suffix(n, 1);
		suffix.back() = a.back();
		for(ll i = n - 2; i >= 0; --i) (suffix[i] = suffix[i + 1] + a[i]) %= MOD1;
		ll answer = 0;
		for(ll i = 0; i < n - 1; ++i) (answer += (a[i] * suffix[i + 1]) % MOD1) %= MOD1;
		printn(out, answer);
	}
	// REMOVE COMPILER OPTIMIZATION FLAGS IF RUNTIME ERRORS (CIN.TIE/COUT.TIE as well)
};
