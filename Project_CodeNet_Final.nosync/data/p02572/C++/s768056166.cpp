...
void smallpf(int n=100001){init1d(spf,100001,0);spf[1]=0;for(int i=2;i<=100000;i++){if (spf[i]==0){spf[i]=i;for(int j=2*i;j<=100000;j+=i){if (spf[j]==0){spf[j]=i;}}}}}
...
void solve()
{
	...
	suffix[n-1]=A[n-1];
	for(int i=n-2;i>=0;i--)
		suffix[i]=(A[i]%MOD+suffix[i+1]%MOD)%MOD;
	for(int i=0;i<n-1;i++)
	{
