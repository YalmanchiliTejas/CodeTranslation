...
// #define dbg(x...)
// #endif
int powerMod(int base,int exp) {int res=1;base%=mod;while(exp>0){if(exp&1)res=(res*base)%mod;base=(base*base)%mod;exp=exp>>1;}return res;}
...
{   
    int i,j,k;
    int n; cin>>n;
    vi a(n+1); fo1(i,n) cin>>a[i];

    vi pre(n+1);
    fo1(i,n) pre[i]=(a[i]+pre[i-1])%mod;

    int ans=0;
    fo1(i,n-1){
        int sm=(pre[n]-pre[i])%mod;
        if(sm<0) sm+=mod;
