...
void solve(ostringstream& aout, long long N, std::vector<long long> A);
void solve_TLE(ostringstream& aout, long long N, std::vector<long long> A);

class StressTest {
private:
	mt19937 m_RandEngine;
	bool judge_case(long long N, std::vector<long long> A) {
		ostringstream fast, tle;
		solve(fast, N, std::move(A));
		solve_TLE(tle, N, std::move(A));
		if (fast.str() == tle.str()) {
			return true;
		}
		else {
...
void solve(ostringstream& aout, long long N, std::vector<long long> A){
	vector<mint> cum(N + 1, mint(0));

	rep(i, N) {
		cum[i + 1] = cum[i] + mint(A[i]);
	}
