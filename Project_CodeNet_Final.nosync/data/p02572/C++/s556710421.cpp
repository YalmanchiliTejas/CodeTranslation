...
int main()
{
	...
	ModCalc mod(MOD);
	LLONG sum = accumulate(as.begin(), as.end(), 0LL) % MOD;
	LLONG ans = mod.ModMulti(sum, sum);
	for (const LLONG ai : as)
	{
		ans = mod.ModSubtract(ans, mod.ModPow(ai, 2));
	}
	ans = mod.ModDiv(ans, 2);
	cout << ans << endl;
}
