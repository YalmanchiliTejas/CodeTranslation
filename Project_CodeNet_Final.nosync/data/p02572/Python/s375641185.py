...
if __name__ == '__main__':
    N = intinput()
    indatas = list(mulinputs())

    # 出力
    # ans = ModInt(0)
    # for i in range(N):
    #     for j in range(i+1, N):
    #         a1, a2 = indatas[i], indatas[j]
    #         ans += ModInt(a1) * ModInt(a2)
    # print(ans)
    ruiseki = []
    for _ in range(len(indatas)):
        ruiseki.append(ModInt(0))
    ruiseki[0] = ModInt(indatas[0])
    for i in range(1,len(indatas)):
        ruiseki[i] = ModInt(indatas[i]) + ruiseki[i-1]
    ans = ModInt(0)
    b = ruiseki[len(ruiseki)-1]
