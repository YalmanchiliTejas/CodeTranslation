...
// dp[i][c][fixed] := i(1-indexed) まで決まってて、0以外がc個あって、fixed = true: 既にNイカであることが決まっている false: きまっていない
int dp[110][4][2];
...
signed main() {
	...
	rep(i, len) {
		rep(c, 4) {
			// 未確定
			if (N[i + 1] == '0') {
				// Ni+1 が 0 のとき、0をつけて未確定
				dp[i + 1][c][false] += dp[i][c][false];
			}
			else {
				// Ni+1 が0でないとき、0をつけて確定
				dp[i + 1][c][true] += dp[i][c][false];
				int Ni = N[i+1] - '0';
				if (c != 3) {
					// 0以外をつけて確定
					dp[i + 1][c + 1][true] += (Ni - 1) * dp[i][c][false];
					// Ni+1 をつけて未確定
					dp[i + 1][c + 1][false] += dp[i][c][false];
				}
			}

			// 確定してたら何を付けてもOK
			// 0 をつけたらcは増えない
			dp[i + 1][c][true] += dp[i][c][true];
			if (c != 3) {
				// 0 以外をつけるとcが増える
				dp[i + 1][c + 1][true] += 9 * dp[i][c][true];
			}

		}
	}

	int ans = dp[len][K][true] + dp[len][K][false];
