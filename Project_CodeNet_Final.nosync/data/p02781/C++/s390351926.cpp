...
inline void dobbysolver(int testcase)
{
    ...
    if (k == 1)
    {
        int ans = (s - 1) * 9;
        int d = 1;
        while (d <= (int)(n[0] - '0')) ans++, d++;
        cout << ans;
        return;
    }
    if (k == 2)
    {
        int ans = (s - 1) * (s - 2) / 2 * 81;
        int d = 1;
        while (d < (int)(n[0] - '0')) ans += (s - 1) * 9, d++;
        int i = 1;
        while (i < s && n[i] == '0') i++;
        if (i == s)
        {
    ...
    int ans = (s - 1) * (s - 2) * (s - 3) / 6 * 81 * 9;

    int d = 1;
    while (d < (int)(n[0] - '0')) ans += (s - 1) * (s - 2) / 2 * 81, d++;
    ...
    ans += (s - i - 1) * (s - i - 2) / 2 * 81;

    d = 1;
    while (d < (int)(n[i] - '0')) ans += (s - i - 1) * 9, d++;
    i++;
    while (i < s && n[i] == '0') i++;
    if (i == s)
    {
