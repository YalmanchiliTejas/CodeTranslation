...
template<typename T>
class RUQRMiQ :public SegTree<T, T> {
	using base = SegTree<T, T>;
public:
	RUQRMiQ(int size, const T& def = T()) :base(size, def, T(), [](T a, T b) {return min(a, b); }, [](T a, T b) {return b; }, [](T a, T b, int l, int r) {return b; }) {};
};
template<typename T>
class RUQRMaQ :public SegTree<T, T> {
	using base = SegTree<T, T>;
public:
	RUQRMaQ(int size, const T& def = T()) :base(size, def, T(), [](T a, T b) {return max(a, b); }, [](T a, T b) {return b; }, [](T a, T b, int l, int r) {return b; }) {};
};
class BIT {
	int n;
	int* bit;
...
public:
	...
	Matrix operator*(const Matrix& a) {
		if (this->n != a.n || this->zero != a.zero || this->e != a.e) {
		...
		rep(i, n) {
			rep(j, n) {
				res.vec[i][j] = zero;
				rep(k, n) {
					chmin(res.vec[i][j], vec[i][k] + a.vec[k][j]);
				}
			}
		}
		return res;
	...
	static Matrix<T> mypow(Matrix<T> a, int b) {
		if (!b)return Matrix<T>::measure(a.size(), a.zero, a.e);
		if (b % 2)return mypow(a, b - 1) * a;
		Matrix<T> memo = mypow(a, b / 2);
		return memo * memo;
	}
};
class mycomplex {
...
signed main() {
	...
	rep(i, n.size()) {
		rep(j, k + 1) {
			dp[i + 1][j][0] += dp[i][j][0];
			if (n[i] == '0')dp[i + 1][j][1] += dp[i][j][1];
			else dp[i + 1][j][0] += dp[i][j][1];
			REP(l, 9) {
				if (j < k) {
					dp[i + 1][j + 1][0] += dp[i][j][0];
					if (l < n[i] - '0')dp[i + 1][j + 1][0] += dp[i][j][1];
					if (l == n[i] - '0')dp[i + 1][j + 1][1] += dp[i][j][1];
				}
			}
		}
	}
