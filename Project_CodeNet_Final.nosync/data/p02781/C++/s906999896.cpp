...
signed main() {
    ...
    int k;
    cin >> k;
    mat<int> dp = mmat<int>(s.length(), 4);
    ...
    For(i, 1, s.length() - 1) {
        ...
        if (giricnt <= 3 and s[i] != '0') dp[i][giricnt]++;
        if (giricnt <= 2 and s[i] != '0')
            dp[i][giricnt + 1] += (s[i] - '0' - 1);

        if (s[i] != '0') giricnt++;
    }
    if (giricnt == k) dp[s.length() - 1][k]++;

    print(dp[s.length() - 1][k]);
