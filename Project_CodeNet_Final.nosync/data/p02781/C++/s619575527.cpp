 ...
 */
    template <class T> vector<vector<T>> vector2(int i, int j) { return vector<vector<T>>(i, vector<T>(j)); }
...
int main() {
    ...
    if (k == 1) {
        int ans = (s.length() - 1) * 9 + (s[s.length() - 1] - '0');
        cout << ans << endl;
    } else if (k == 2) {
        typedef pair<pp, pp> guess;
        guess g = {v[0], v[1]};
        long long ans = 0;
        for (int i = s.length() - 1; i >= 0; i--) for (int j = i - 1; j >= 0; j--)
        ...
        reverse(s.begin(), s.end());
        typedef pair<int, int> pp;
        typedef pair<pp, pair<pp, pp>> guess;
        long long ans = 0;
        guess g = {v[0], {v[1], v[2]}};
        for (int i = s.length() - 1; i >= 0; i--)
            for (int a = 1; a <= 9; a++) {
