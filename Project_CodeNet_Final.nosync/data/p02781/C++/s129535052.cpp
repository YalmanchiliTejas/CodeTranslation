...
#define for_eached__3(a,v, siki) [&](auto a) {auto b = a;for_each(b, v, siki);return b; }(a)
...
template<class T, class F> T mgr(signed ok, T ng, F f) {
    return mgr((T)ok, ng, f);
}
template<class T, class F> T mgr(T ok, signed ng, F f) {
    return mgr(ok, (T)ng, f);
}
...
template<class U> auto rev(vector<vector<U>> &a) {    vector<vector<U> > b(sz(a[0]), vector<U>(sz(a)));    rep(h, sz(a)) rep(w, sz(a[0]))b[w][h] = a[h][w];    return b;}
