...
public:
    void solve(istream& inp, ostream& outp) {
        ...
        function<int(int, int, bool)> go = [&](int pos, int rem, bool same) -> int {
            if (rem == 0) {
                return 1;
            }
            int res = 0;
            for (int i : range(pos, n.size())) {
                if (same) {
                    if (n[i] > '0') {
                        res += go(i + 1, rem - 1, false) * (n[i] - '0' - 1) + go(i + 1, rem - 1, true);
                        same = false;
                    }
                } else {
                    res += 9 * go(i + 1, rem - 1, false);
                }
            }
            return res;
        };
        out.printLine(go(0, k, true));
    }
};
