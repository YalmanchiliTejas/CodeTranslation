...
int main(){
  ...
  REP(i,digit){
    REP(j,i + 2){
      if(j > 0){
        dp[i+1][j][0] = dp[i][j][0] * 9 + dp[i][j][1] * max(ctoi(s[i])-1,(lint)0) + dp[i][j-1][0] + dp[i][j-1][1] * !!(ctoi(s[i]));
        dp[i+1][j][1] = dp[i][j][1] * !!(ctoi(s[i])) + dp[i][j-1][1] * !(ctoi(s[i]));
      }
      else{
        dp[i+1][j][0] = dp[i][j][0] * 9 + dp[i][j][1]* max(ctoi(s[i])-1,(lint)0);
        dp[i+1][j][1] = dp[i][j][1] * !!(ctoi(s[i]));
      }
    }
  }
  cout << dp[digit][digit-k][1] + dp[digit][digit-k][0];
}
