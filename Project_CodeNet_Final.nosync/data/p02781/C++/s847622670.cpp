...
void solve() {
	...
	if (k == 1) {
		...
		int ans = 81 * (sz(s) - 2) * (sz(s) - 1) / 2;
		ans += ((s[0] - '0') - 1) * 9 * (sz(s) - 1);
		...
		assert(k == 3);
		int ans = 81 * 9 * (sz(s) - 3) * (sz(s) - 2) * (sz(s) - 1) / 6;
		ans += ((s[0] - '0') - 1) * 81 * (sz(s) - 1) * (sz(s) - 2) / 2;
		for (int i = 1; i < sz(s); ++i) {
			if (s[i] != '0') {
				ans += 9 * (s[i] - '0' - 1) * (sz(s) - i - 1);
