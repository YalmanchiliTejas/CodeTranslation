...
int solve(string s, int k) {
	...
	if (k == 2) {
		...
		if (x < s.size()) ans += solve(s.substr(x, s.size() - x), 1);
		//最大の桁を少し小さくして使う場合
		ans += (int)((s[0] - '0') - 1) * 9 * (s.size() - 1);
		//最大の桁を使わない場合
	...
	if (k == 3) {
		...
		int x = 1;
		while (x < s.size() && s[x] == '0')x++;
		if (x < s.size())ans += solve(s.substr(x, s.size() - x), 2);
		//最大の桁を少し小さくして使う場合
		ans += (int)((s[0] - '0') - 1) * 81 * (s.size() - 1) * (s.size() - 2) / 2;
		//最大の桁を使わない場合
		ans += (s.size() - 1) * (s.size() - 2) * (s.size() - 3) / 6 * 729;
	}
	return ans;
}
