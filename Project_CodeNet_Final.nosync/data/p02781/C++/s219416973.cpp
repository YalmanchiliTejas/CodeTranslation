...
ll rec(int pos, int kv,bool tite) {
	...
	if (~memo[pos][kv][tite]) return memo[pos][kv][tite];
	ll res = 0;
	if(tite){
		if (ar[pos] != 0) {
			res += rec(pos + 1, kv + 1, true);
		}
		res += rec(pos + 1, kv, ar[pos] == 0);
		for (size_t i = 1; i < ar[pos]; i++)
		{
			res += rec(pos + 1, kv + 1, false);
		}
	}
	else {
		res += rec(pos + 1, kv,tite);

		for (size_t i = 1; i < 10; i++)
		{
			res += rec(pos + 1, kv + 1, tite);
		}
	}
	return memo[pos][kv][tite] = res;
}
void solv() {
	string v;
	cin >> v;
