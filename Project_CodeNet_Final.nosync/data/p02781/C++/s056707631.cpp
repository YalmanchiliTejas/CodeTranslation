...
template <typename sizetype, typename Tp, size_t N>
...
//vector<vector<vector<ll>>> v = make_vector({1, 2, 3}, 0LL);
/*-----8<-----library-----8<-----*/

//constexpr ll N_MAX = 1e100;
constexpr ll K_MAX = 3;
...
void solve() {
	...
	vector<vector<vector<ll>>> dp = make_vector({keta+1LL, 2LL, K+1LL}, 0LL);
	dp[0][0][0] = 1;  //初期条件。他は0で初期化されている
	for(ll i = 0; i < keta; i++){
		for(ll smaller = 0; smaller < 2; smaller++){
			for(ll j = 0; j <= K; j++){
				for(ll x = 0; x <= (smaller ? 9 : S[i]-'0'); x++){
					if(x!=0 && j==K)continue;
					if(x==0)
						dp[i + 1][smaller || x < S[i]-'0'] [j] += dp[i][smaller][j];
					else
						dp[i + 1][smaller || x < S[i]-'0'] [j+1] += dp[i][smaller][j];
