...
int main(){
  ...
  ll N = sz(S);
  ll K = in();
  // dp[i][j] 調査中の桁まで全部Sと一致⇒i=0。ちょうどj個。
  auto dp = Vector(2, 5, 0);
  dp[0][1] = 1; // S[0]
  dp[1][0] = 1; // 0
  dp[1][1] = S[0] - '1';  // 1 ～ S[0] - '1'
  rep(x, 1, N){
    ll n = S[x] - '0';
    auto dpNext = Vector(2, 5, 0);
    rep(j, 4){
      rep(k, 10){
        ll add = (k == 0 ? 0 : 1);
        if(k > n){
          dpNext[1][j + add] += dp[1][j];
        }else if(k == n){
          dpNext[0][j + add] += dp[0][j];
          dpNext[1][j + add] += dp[1][j];
        }else{
          dpNext[1][j + add] += dp[1][j];
          dpNext[1][j + add] += dp[0][j];
        }
      }
    }
    dp = dpNext;
