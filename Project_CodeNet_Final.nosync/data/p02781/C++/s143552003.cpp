...
{
  if (s[0] == '0')
    return f1(s.substr(1));
  ll keta = s.size();
  ll last = s[0] - '0';
  if (keta < 1)
    return 0;

  ll res = (keta - 1) * 9 + last;
  return res;
}
...
ll f2all(ll keta)
{
  ...
  {
    return f2all(keta - 1) + 9 * 9 * (keta - 1);
  }
}
...
ll f3all(ll keta)
{
  ...
  {
    //return f3all(keta - 1) + 9 * ((keta - 1) * (keta - 2) / 2 * 9);
    return f3all(keta - 1) + f2all(keta - 1) * 9;
  }
}
