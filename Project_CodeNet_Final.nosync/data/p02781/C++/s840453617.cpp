...
signed main() {
  ...
  static int memo[110][4][2];
  rep(i, 0, 110) rep(j, 0, 4) rep(k, 0, 2) memo[i][j][k] = INF;
  function<int(int, int, bool)> dfs = [&](int i, int k, bool less) -> int {
    if (k < 0) return 0;
    if (i == sz(N)) {
      return k == 0;
    }
    ...
    int ret = 0;
    if (less) {
      rep(d, 0, 10) { ret += dfs(i + 1, k - (d != 0), true); }
    } else {
      {
        int d = N[i] - '0';
        ret += dfs(i + 1, k - (d != 0), false);
      }
      rep(d, 0, N[i] - '0') { ret += dfs(i + 1, k - (d != 0), true); }
    }
    return MEMO = ret;
  };
