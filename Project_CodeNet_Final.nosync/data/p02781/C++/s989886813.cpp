...
ll dp[111][10][10];
ll rec(int idx, bool already = false, int k_cur = 0) {
    if (dp[idx][already][k_cur] != -1) return dp[idx][already][k_cur];
    if (k_cur > K) return 0;
    if (idx == sz) return already and (k_cur == K);
    ll ret = 0;
    if (already) {
        // zero
        ret += rec(idx + 1, true, k_cur);
        // other
        ret += rec(idx + 1, true, k_cur + 1) * 9;
    } else {
        int mx = N[idx] - '0';
        for (int i = 0; i <= mx; ++i) {
            if (i == 0) {
                ret += rec(idx + 1, i != mx, k_cur);
            } else {
                ret += rec(idx + 1, i != mx, k_cur + 1);
            }
        }
    }
    return dp[idx][already][k_cur] = ret;
}

signed main() {

    REP(i, 111) REP(j, 10) REP(k, 10) dp[i][j][k] = -1;
