...
int dp[M][M][2];
int K;
int yoyo(int pos, int cnt, int tight, vector<int> num)
{
        if (pos == num.size())
        {
                if (cnt <= K)
        ...
        for (int dig = 0; dig <= limit; dig++)
        {
                ...
                ans += yoyo(pos + 1, currCnt,
                            currTight, num);
        }
        return dp[pos][cnt][tight] = ans;
}

int funcction(string s)
{
        vector<int> num;
...
void solve()
{
        ...
        cin >> n;
        cin >> K;
        int ans = funcction(n) - funcction(L);
        K--;
        int res = funcction(n) - funcction(L);
        cout << ans - res << endl;
}
