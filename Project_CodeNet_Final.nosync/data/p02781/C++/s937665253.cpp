...
int main() {
	...
	vector<vector<SINT64>> DP(s.length()+1,vector<SINT64>(8));
	...
	rep(i,1,s.length()) {
		...
		if ((s[i]-'0') >= 2) {
			DP[i][6] += DP[i-1][1]*((s[i]-'0')-1);
			DP[i][7] += DP[i-1][2]*((s[i]-'0')-1);
//			DP[i][7] += DP[i-1][3]*((s[i]-'0')-1);
		}
		DP[i][6] += DP[i-1][5]*9;
		DP[i][7] += DP[i-1][6]*9;
	...
	if (K == 1) {
		put(DP[s.length()-1][1]+DP[s.length()-1][5]);
	}
	if (K == 2) {
		put(DP[s.length()-1][2]+DP[s.length()-1][6]);
	}
	if (K == 3) {
		put(DP[s.length()-1][3]+DP[s.length()-1][7]);
	}

	// rep(i,0,s.length()) {
	// 	puts(DP[i][1]);puts(DP[i][2]);puts(DP[i][3]);puts(DP[i][5]);puts(DP[i][6]);puts(DP[i][7]);
	// 	cout << endl;
	// }
