...
ll cnt(ll left, ll K, bool ok) {
  ...
  if (ok) {
    if (K == 3) {
      ll x = 9LL*9LL*9LL*L*(L-1)*(L-2)/6LL;
      // cout << left << "," << K << "," << ok << " =>" << x << endl;
      return x;
    }
    if (K == 2) {
      ll x = 9LL*9LL*L*(L-1)/2LL;
      // cout << left << "," << K << "," << ok << " =>" << x << endl;
      return x;
    }
    if (K == 1) {
  ...
  ll d = N[left]-'0';
  if (d > 0)
    x += 1LL * cnt(left+1, K, true);
  if (d > 1)
    x += (d-1LL) * cnt(left+1, K-1, true);
  if (d > 0)
    x += 1LL * cnt(left+1, K-1, false);
  else 
    x += 1LL * cnt(left+1, K, false);
  // cout << left << "," << K << "," << ok << " =>" << x << endl;
  return x;
}
