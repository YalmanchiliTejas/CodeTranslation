...
int main()
{
    ...
    REP(i,1,s.size()+1){
        if(i==1){
                dp[i][0][0] = 1;
                dp[i][1][0] = max((s[i-1]-'0')-1,0);
                dp[i][1][1] = 1;
        }
        else{
            dp[i][0][0] = dp[i-1][0][0];
            dp[i][1][0] = dp[i-1][0][0]*9+dp[i-1][1][0];
            dp[i][2][0] = dp[i-1][1][0]*9+dp[i-1][2][0];
            dp[i][3][0] = dp[i-1][2][0]*9+dp[i-1][3][0];
        }
        
        //cout<<s[i-1]<<" ";
        if(s[i-1] == '0'){
            if(i!=1){dp[i][0][1] = dp[i-1][0][1];
            dp[i][1][1] = dp[i-1][1][1];
            dp[i][2][1] = dp[i-1][2][1];
            dp[i][3][1] = dp[i-1][3][1];}
        } 
        else{
           if(i != 1){ dp[i][1][1] = dp[i-1][0][1];
            dp[i][2][1] = dp[i-1][1][1];
            dp[i][3][1] = dp[i-1][2][1];}
            //dp[i][4][1] = dp[i-1][3][1];
            dp[i][1][0] += dp[i-1][1][1]+dp[i-1][0][1]*max((s[i-1]-'0')-1,0);
            dp[i][2][0] += dp[i-1][2][1]+dp[i-1][1][1]*max((s[i-1]-'0')-1,0);
            dp[i][3][0] += dp[i-1][3][1]+dp[i-1][2][1]*max((s[i-1]-'0')-1,0);
        }
    }
