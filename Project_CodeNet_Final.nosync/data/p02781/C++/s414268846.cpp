...
// 桁DP、3を含む数字の合計
// dp[決めた桁数][未満フラグ][0でない数字の個数、ただしkを超える場合、k+1に分類]] := 場合の数
...
signed main() {
	...
	vvvi dp(102, vvi(2, vi(k+2,0)));
	dp[0][0][0] = 1;

	rep(i,n) rep(smaller,2) rep(j,k+2) {
		for (int x = 0; x <= (smaller ? 9 : a[i]); x++) {
			// ゼロでない数字がkを超えるなら、全てk+1で分類
			int num = j + (x != 0);
			if (num > k + 1) num = k + 1;

			dp[i+1][smaller || x < a[i]][num] += dp[i][smaller][j];
		}
	}
