...
#define _re(i,n) rei(i,0,n)
#define rei(i,a,b) for(ll i=ll(a);i<ll(b);++i)
#define re(...) _overload3(__VA_ARGS__,rei,_re,)(__VA_ARGS__)
...
#define parr(v) re(i,sz(v)){pk(v[i]);}p("");
#define pmat(v,n,m) re(i,n){re(j,m){pk(v[i][j]);}p("");}
#define pb push_back
#define sz(v) (v).size()
...
#define fi first
#define se second
#define pyn(x) {print_yes_no(x);return 0;}
...
struct graph{
    ll V;
    vector<VP > v;
    graph(ll V) : V(V), v(vector<VP >(V)){}
    void add_diedge(ll v1, ll v2, ll w){
        v[v1].pb({v2, w});
        v[v2].pb({v1, w});
    }
    void add_edge(ll v1, ll v2, ll w){
        v[v1].pb({v2, w});
    }
    VL vval = vector<ll>(V, 0);
    VL dist = vector<ll>(V, -1);
    VL prever = vector<ll>(V,-1);
    VL dijkstra(ll start){
    ...
    void dfs(ll now, ll prv, ll c){
        ...
        for(auto nxt : v[now])if(nxt.fi != prv){
            dfs(nxt.fi, now, vval[now]);
        }
    }
    void gprll(){
        re(i,V){ps(i);ps(": ");for(P x : v[i]){ps(x.fi);ps(",");ps(x.se);ps(" ");}p("");}
    }
    vector<VL> dd = vector<VL>(V, VL(V,inf)); 
    void warshall_floyd() {
      re(i,V)for(P e: v[i])dd[i][e.fi]=e.se;
      d(dd[0][2]);
      re(k,V)re(i,V)re(j,V)dd[i][j] = min(dd[i][j], dd[i][k] + dd[k][j]);
    }
};
...
ll a[100001],b[100001],c[100001],d,n,m,x,y,z,h,w,k,mi=inf,ma=-inf,ans,sum,cnt,s,t,l,r,res,tmp,itr,p,q;
bool flag;
...
int main(void){
    cin>>st>>k;
    //digit DP
    //dp[i][j]  val < n, digit i, used j of k 
    dp[0][0][0]=1; 
    re(i,sz(st))re(j,2)re(l,k+1){
        int lim = j ? 9 : st[i]-'0';
        re(d,lim+1){
            dp[i+1][j || d<lim][l + (d != 0)] += dp[i][j][l];
        }
    }
    re(j,2)ans+=dp[sz(st)][j][k];
    p(ans);
    return 0;
}
