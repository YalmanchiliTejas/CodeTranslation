...
int main(){
  ...
  s="0"+s;
  ll n=s.sz;
  vvvll dp(n,vvll(k+1,vll(2)));
  dp[0][0][0]=1;
  FOR(i,1,n){
    FOR(j,0,k+1){
      if(j!=k){
        if(s[i]!='0'){//0でない時にdp[][][0]からdp[][][1]に移行させる
          dp[i][j+1][0]+=dp[i-1][j][0];//上限
          dp[i][j][1]+=dp[i-1][j][0];//上からi桁目が0
          dp[i][j+1][1]+=dp[i-1][j][0]*(((s[i]-'0')+9)%10);
        }
        else{
          dp[i][j][0]+=dp[i-1][j][0];
        }
        dp[i][j][1]+=dp[i-1][j][1];//上からi桁目が0
