...
int main()
{
                ...
                for (int d = 0; d < 10; d++)
                {
                    ...
                    else
                    {
                        ...
                        if (d < num)
                        {
                            if (d == 0)
                            {
                                dp[i + 1][1][k + 1][0] += dp[i][j][k][0];
                                dp[i + 1][1][k + 1][1] += dp[i][j][k][1];
                            }
                            else
                            {
                                dp[i + 1][1][k][0] += dp[i][j][k][0] + dp[i][j][k][1];
                            }
                        }
                        else if (d == num)
                        {
                            if (d == 0)
                            {
                                dp[i + 1][0][k + 1][0] += dp[i][j][k][0];
                                dp[i + 1][0][k + 1][1] += dp[i][j][k][1];
                            }
                            else
                            {
                                dp[i + 1][0][k][0] += dp[i][j][k][0] + dp[i][j][k][1];
                            }
                        }
                    }
                }
    ...
    ll ans = dp[digit][0][digit - K][0] + dp[digit][1][digit - K][0];
    cout << ans << endl;
    return 0;
}
