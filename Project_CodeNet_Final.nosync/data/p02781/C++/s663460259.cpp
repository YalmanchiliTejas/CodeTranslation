...
int main(int argc, const char * argv[]) {
    ...
    for (int i=1; i<=length; i++) {
        if(n.at(i)>0){
            r1=n.at(i)-1;
            r2=1; // 入力した数値の該当する桁が０でないとき
        }else{
            r1=0;
            r2=0; // 入力した数値の該当する桁が０のとき
        }
        dp1.at(i).at(0)=1;
        ...
        //
        dp0.at(i).at(0)=0; // dp0.at(i).at(0)は常に０
        // 各桁の数が０なら前の桁までの情報をコピーする。
        if(n.at(i)==0){
    ...
    cout<<dp1.at(length).at(K)+dp0.at(length).at(K)<<endl;
    return 0;
}
