...
int main() {
  ...
  rep(i, n){
    rep(j, 10){
      if(j == s[i]-'0'){
        rep(k, 4){
          if(j == 0) dp[i+1][k][1] += dp[i][k][1];
          else dp[i+1][k+1][1] += dp[i][k][1];
        }
      }
      if(j < s[i]-'0'){
        rep(k, 4){
          if(j == 0) dp[i+1][k][0] += dp[i][k][1];
          else dp[i+1][k+1][0] += dp[i][k][1];
        }
      }
      rep(k, 4){
        if(j == 0) dp[i+1][k][0] += dp[i][k][0];
        else dp[i+1][k+1][0] += dp[i][k][0];
      }
    }
  }
