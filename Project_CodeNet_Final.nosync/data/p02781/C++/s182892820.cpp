...
int LIS(const vector<long long> &a) {
    int N = (int)a.size();
    vector<long long> dp(N, MINF);
    for (int i = 0; i < N; ++i) {
        // dp[k] >= a[i] となる最小のイテレータを見つける
        auto it = lower_bound(dp.begin(), dp.end(), a[i]);//ここのa[i]をa[i]+1とすれば広義単調増加列に関するLISとなる。
    ...
    return lower_bound(dp.begin(), dp.end(), MINF) - dp.begin();
}
...
int n=N.size();
ll dp[n][2][K+1];
rep(i,n)rep(j,2)rep(k,K+1)dp[i][j][k]=0;
rep(i,n){
    if(i==0){
        dp[0][0][1]=1;
        dp[0][1][1]=N[0]-'0'-1;
        ...
        if(N[i]=='0'){
            rep(k,K+1)dp[i][0][k]+=dp[i-1][0][k];
            rep(k,K)dp[i][1][k+1]+=dp[i-1][1][k]*9;
            rep(k,K+1)dp[i][1][k]+=dp[i-1][1][k];

        }
        else{
            rep(k,K)dp[i][0][k+1]+=dp[i-1][0][k];
            rep(k,K)dp[i][1][k+1]+=dp[i-1][0][k]*(N[i]-'0'-1);
            rep(k,K)dp[i][1][k+1]+=dp[i-1][1][k]*9;
            rep(k,K+1)dp[i][1][k]+=dp[i-1][0][k];
            rep(k,K+1)dp[i][1][k]+=dp[i-1][1][k];
