...
long long cc[101][4][2];

long long dp(int idx, int nonZero, int check){
  if(idx == (int)n.size())
    return nonZero == k;
  if(nonZero > k)
    return 0;
  long long &ans = cc[idx][nonZero][check];
  if(~ans)
    return ans;
  ans = 0;
  if(check){
    for(int i = 0; i <= (n[idx] - '0'); i++){
      ans += dp(idx + 1, nonZero + (i != 0), i == (n[idx] - '0'));
    }
  }else{
    for(int i = 0; i <= 9; i++){
      ans += dp(idx + 1, nonZero + (i != 0), 0);
    }
  }
  return ans;
}
