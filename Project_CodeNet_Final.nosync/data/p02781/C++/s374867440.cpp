...
namespace mine
{
	...
	char str[N];
	ll C(int n,int m){if(n<m)return 0; ll ret=1;fo(i,n-m+1,n) ret=ret*i;fo(i,1,m) ret/=i;return ret; }
	void main()
	{
		scanf("%s",str+1);int n=strlen(str+1);int k=qread();ll ans=0;
		int cnt=0;
		fo(i,1,n)
		{
			fo(j,0,str[i]-'0'-1)
			{
				int c2=cnt+(j!=0);
				if(c2<=k) ans+=C(n-i,k-c2)*qpower(9,k-c2);
			}cnt+=(str[i]!='0');
		}
		cnt=0;fo(i,1,n) cnt+=(str[i]!='0');ans+=cnt==k;write(ans);
	}
};//变量重名！
signed main()
{
