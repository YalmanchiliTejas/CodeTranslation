...
void solve(std::istream& cin, std::ostream& cout, std::ostream& cerr) {
    ...
    int s0 = s[0] - '0';
    if(K == 1){
        REP(i,N)FOR(ni,1,10)if(i != 0 || ni <= (s[0] - '0'))ans ++;
        BYE(ans);
    }
    int n1 = 1;
    while(n1 < N && s[n1] == '0')n1 ++;
    int n2 = n1 + 1;
    while(n2 < N && s[n2] == '0')n2 ++;
ERR(n1,n2)
    int  s1 = s[1] - '0';
    if(K == 2){
        auto ok2 = [&](int i,int j,int ni,int nj){if(i == 0){if(ni > s0)return 0;if(ni == s0 && j <= n1){if(nj > (s[j] - '0'))return 0;}} return 1;};
        REP(i,N)FOR(ni,1,10)REP(j,N)FOR(nj,1,10)if(i < j&& ok2(i,j,ni,nj)){ERR(i,ni,j,nj);ans ++;}
        BYE(ans);
    }
    int s2=  s[2] - '0';
    if(K == 3){
    auto ok3 = [&](int i,int j,int k,int ni,int nj,int nk){if(i == 0){if(ni > s0)return 0;if(ni == s0 && j <=n1){if(nj > s[j] - '0')return 0;if(ni == s0 && nj == (s[j] - '0' )&& k <= n2){if(nk > s[k] - '0')return 0;}}} return 1;};

    REP(i,N)FOR(ni,1,10)REP(j,N)FOR(nj,1,10)REP(k,N)FOR(nk,1,10)if(i <j && j< k && ok3(i,j,k,ni,nj,nk))ans ++;
    BYE(ans);}
