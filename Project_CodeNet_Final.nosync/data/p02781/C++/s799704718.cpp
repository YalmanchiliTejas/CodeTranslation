...
signed main(void){
  ...
  dp[0][0][0] = 1;
  REP(i, N.size()) REP(smaller, 2) REP(k, N.size()){
    int d = N[i]-'0';
    REP(nxt, 10){
      if(nxt < d)
        dp[i+1][1][(nxt!=0)+k] += dp[i][smaller][k];
      else if(nxt == d)
        dp[i+1][smaller][(nxt!=0)+k] += dp[i][smaller][k];
      else
        if(smaller) dp[i+1][smaller][(nxt!=0)+k] += dp[i][smaller][k];
    }
  }
