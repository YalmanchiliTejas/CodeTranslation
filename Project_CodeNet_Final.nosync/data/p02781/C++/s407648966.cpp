...
void _main()
{
    ...
    ll dp_small[105][4];
    memset(dp_small, 0, sizeof(dp_small));
    ll dp_eq[105][4];
    memset(dp_eq, 0, sizeof(dp_eq));

    ll n0 = n[0] - '0';
    dp_small[1][0]++;
    srep (i, 1, 10) {
        if (i < n0) dp_small[1][1]++;
        if (i == n0) dp_eq[1][1]++;
    }

    debug_printf("0: dp_small=%lld dp_eq=%lld\n",
                dp_small[1][0], dp_eq[1][0]);
    debug_printf("1: dp_small=%lld dp_eq=%lld\n",
                dp_small[1][1], dp_eq[1][1]);
    debug_printf("2: dp_small=%lld dp_eq=%lld\n",
                dp_small[1][2], dp_eq[1][2]);
    debug_printf("3: dp_small=%lld dp_eq=%lld\n",
                dp_small[1][3], dp_eq[1][3]);
    ...
    srep (i, 1, n.size()) {
        ...
        if (nn > 0) {
            rep (cnt, 4) {
                dp_small[i+1][cnt] += dp_small[i][cnt];
                dp_small[i+1][cnt] += dp_eq[i][cnt];
            }
        }
        else {
            rep (cnt, 4) {
                dp_small[i+1][cnt] += dp_small[i][cnt];
                dp_eq[i+1][cnt] += dp_eq[i][cnt];
            }
        }
        ...
        srep (dig, 1, 10) {
            if (dig < nn) {
                rep (cnt, 3) {
                    dp_small[i+1][cnt+1] += dp_small[i][cnt];
                    dp_small[i+1][cnt+1] += dp_eq[i][cnt];
                }
            }
            if (dig == nn) {
                rep (cnt, 3) {
                    dp_small[i+1][cnt+1] += dp_small[i][cnt];
                    dp_eq[i+1][cnt+1] += dp_eq[i][cnt];
                }
            }
            if (dig > nn) {
                rep (cnt, 3) {
        ...
        debug_printf("0: dp_small=%lld dp_eq=%lld\n",
                    dp_small[i+1][0], dp_eq[i+1][0]);
        debug_printf("1: dp_small=%lld dp_eq=%lld\n",
                    dp_small[i+1][1], dp_eq[i+1][1]);
        debug_printf("2: dp_small=%lld dp_eq=%lld\n",
                    dp_small[i+1][2], dp_eq[i+1][2]);
        debug_printf("3: dp_small=%lld dp_eq=%lld\n",
                    dp_small[i+1][3], dp_eq[i+1][3]);
    }

    ll ans = 0LL;
    ans += dp_small[n.size()][k];
    ans += dp_eq[n.size()][k];
    cout << ans << endl;
}
