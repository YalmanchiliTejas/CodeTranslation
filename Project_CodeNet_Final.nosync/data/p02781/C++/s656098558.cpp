...
ll find(ll d,ll k,ll minFlag,Array& num) {
	if (d == -1 && k == 0)return 1;
	else if (d == -1 || k == -1)return 0;
	if (dp[d][k][minFlag] != -1)return dp[d][k][minFlag];
	

	ll ret = 0;
	if (minFlag == 1) {
		ret += 9 * find(d - 1, k - 1, 1,num);
		ret += find(d - 1, k, 1,num);
	}
	else {
		if (num[d] == 0) {
			ret += find(d - 1, k, 0 ,num);
		}
		else {
			ret += find(d - 1, k-1, 0,num);
			ret += max(0LL, (num[d] - 1)) * find(d - 1, k - 1, 1,num);
			ret += find(d - 1, k, 1,num);
		}
	}
	return dp[d][k][minFlag] = ret;
}
...
int main() {
	...
	cin >> k;
	Array num(s.size());
	REP(i, s.size())num[s.size()-1-i] = s[i] - '0';
	REP(i, num.size())REP(j, k + 1)REP(l, 2)dp[i][j][l] = -1;

	cout << find(num.size() - 1, k, 0, num) << endl;
