...
void do2() {
	...
	ans += 9 * (posA - 1) * (posA - 2) * 9 / 2; // len - 1位数的情况 
	ans += (a - 1) * (posA - 1) * 9;
	ans += b;
	ans += (posB - 1) * 9;
}
...
void do3() {
	...
	ans += 9 * 9 * 9 * (posA - 1) * (posA - 2) * (posA - 3) / 6; // len - 1位数的情况 (朱世杰恒等式)
	ans += (a - 1) * 9 * 9 * (posA - 1) * (posA - 2) / 2;
	ans += 9 * 9 * (posB - 1) * (posB - 2) / 2; // posB上为0 
	ans += (b - 1) * (posB - 1) * 9;
	ans += c;
	ans += (posC - 1) * 9;
}
