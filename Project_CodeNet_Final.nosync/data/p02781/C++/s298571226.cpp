...
signed main() {
    ...
    auto dp = makeVector<intmax_t>(-1, N.size(), 2, 10);
    const auto res = makeFixPoint([&](auto rec, size_t i, bool tight, size_t cnt) -> intmax_t {
        if (i == N.size()) {
            return (cnt == K);
        }

        auto &ret = dp[i][tight][cnt];
        if (~ret) {
            return ret;
        }

        ret = 0;
        size_t lim = (tight ? N[i] - '0' : 9);
        reps(d, 0, lim) {
            ret += rec(i + 1, tight && d == lim, cnt + (d != 0));
        }
