...
void solve(){
    ...
    string s; ll k, n;
    in2(s, k); n = s.length();
    vl3 dp(2, vl2(k + 1, vl(n + 1, 0)));
    dp[0][0][0] = 1;
    For(nd, 1, n + 1){
        ll top = s[nd - 1] - '0';
        if (top){
            rep(j, k) dp[0][j + 1][nd] += dp[0][j][nd - 1];
            rep(j, k + 1) dp[1][j][nd] += dp[1][j][nd - 1] + dp[0][j][nd - 1];
            rep(j, k) dp[1][j + 1][nd] += 9 * dp[1][j][nd - 1] + (top - 1) * dp[0][j][nd - 1];
        }else{
            rep(j, k + 1) dp[0][j][nd] += dp[0][j][nd - 1];
            rep(j, k + 1) dp[1][j][nd] += dp[1][j][nd - 1];
            rep(j, k) dp[1][j + 1][nd] += 9 * dp[1][j][nd - 1];
        }
    }

    prl(dp[0][k][n] + dp[1][k][n]);
