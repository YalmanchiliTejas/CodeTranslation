...
const int MAX = 110;
ll dp[MAX][4][2]; // 0 : ギリギリじゃない 1: ギリギリ
int main(){
    iosetup;
    string S; cin >> S;
    int n = S.size();
    ...
    rep(i, 0, n){
        ...
        dp[i+1][0][0] = 1;
        rep(j, 0, 3){
            dp[i+1][j+1][0] = dp[i][j][0] * 9 + dp[i][j+1][0];
            if(S[i] != '0') dp[i+1][j+1][0] += dp[i][j+1][1];
            if(S[i] != '0') dp[i+1][j+1][0] += dp[i][j][1] * (S[i] - '0' - 1);
        }
        rep(j, 0, 3){
            if(S[i] == '0') dp[i+1][j+1][1] = dp[i][j+1][1];
