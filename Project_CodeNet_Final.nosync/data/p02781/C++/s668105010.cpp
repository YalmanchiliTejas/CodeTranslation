...
int used[101][2][2][101];
int mem[101][2][2][101];

Int dfs(Int idx, Int allZero, Int same, Int cnt, string s){
  if(idx >= Int(N.size())) return cnt == K;
  if(cnt > K) return 0;
  if(used[idx][allZero][same][cnt]++) return mem[idx][allZero][same][cnt];

  int res = 0;
  for(Int i=0;i<10;i++){
    Int nidx = idx + 1;
    Int nallZero = allZero && (i == 0);
    Int nsame = same && (N[idx] == i + '0');
    Int ncnt = cnt + (i != 0);
    if(same && N[idx] < i + '0') continue;
    res += dfs(nidx, nallZero, nsame, ncnt, "");
  }
  return mem[idx][allZero][same][cnt] = res;
