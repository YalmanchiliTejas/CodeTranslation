...
ll any(int pos, const std::string& s, int k) {
    if (k < 0) return 0;
    if (pos == isz(s)) return (k == 0);
    return any(pos+1, s, k) + 9 * any(pos+1, s, k-1);
}

ll fast(int pos, const std::string& s, int k) {
    if (k < 0) { return 0; }
    if (pos == isz(s)) { return (k == 0); }
    ll answ = 0;
    if ('0' < s[pos]) {
        //std::cerr << "call any(" << pos+1 << "," << s << "," << k << ")" << std::endl;
        answ += any(pos+1, s, k);
        //watchln(answ);
        answ += (s[pos] - '1') * any(pos+1,s,k-1);
    }
    //std::cerr << "call fast(" << pos+1 << "," << s << "," << k-1 << ")" << std::endl;
    auto ft = fast(pos+1,s,k-(s[pos]!='0'));
    //watchln(ft);
    return answ + ft;
}
...
void test() {
    for (int n = 1; n <= 10000; n++) {
        for (int k = 1; k <= 3; k++) {
            ...
            if (slow(n,k) != fast(0, s, k)) {
                std::cout << "n = " << n << std::endl;
                std::cout << "k = " << k << std::endl;
                std::cout << "slow = " << slow(n,k) << std::endl;
                std::cout << "fast = " << fast(0, s,k) << std::endl;
                std::exit(0);
            }
        }
...
int main() {
    ...
    while (std::cin >> s >> k) {
        //if (isz(s) <= 6) {
        //    std::cout << slow(std::stoi(s), k) << std::endl;
        //    return 0;
        //}
        std::cout << fast(0,s,k) << std::endl;
    }
    return 0;
}