    ...
    For(i, 1, n + 1){
        ...
        rep(num, k + 1){
            rep(d, 10){
                if (d < top){
                    if (d == 0) 
                        dp[1][num][i] += dp[1][num][i - 1] + dp[0][num][i - 1];
                    else if (num != 0)
                        dp[1][num][i] += dp[1][num - 1][i - 1] + dp[0][num - 1][i - 1];
                }else if (d == top){
                    if (d == 0){
                        dp[0][num][i] += dp[0][num][i - 1];
                        dp[1][num][i] += dp[1][num][i - 1];
                    }
                    else if (num != 0){
                        dp[0][num][i] += dp[0][num - 1][i - 1];
                        dp[1][num][i] += dp[1][num - 1][i - 1];
                    }
                }else if (num != 0){ // d > 0
                    dp[1][num][i] += dp[1][num - 1][i - 1];
                }
