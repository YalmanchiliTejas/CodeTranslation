...
int main()
{
    const auto N = in<std::string>();
    const auto K = in<int>();
    const int n  = N.size();
    auto dp      = make_v({n + 1, 2, K + 1}, 0LL);  // 上からi桁目,N未満,0の個数
    dp[0][0][0]  = 1LL;
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < 2; j++) {
            for (int l = 0; l <= K; l++) {
                for (int d = 0; d <= (j == 0 ? N[i] - '0' : 9); d++) {
                    const int ni = i + 1;
                    const int nj = (j == 1) or (d < (N[i] - '0'));
                    const int nl = l + (d != 0 ? 1 : 0);
                    if (nl > K) { continue; }
