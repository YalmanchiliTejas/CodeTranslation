...
ll solve(int i, int k, bool flg) {
    if (!k) return 1;
    if (i>=n) return 0;

    if (dp[i][k][flg]) return dp[i][k][flg];

    ll res = 0;
    if (flg) {
        res += solve(i+1, k, true);
        res += solve(i+1, k-1, true)*9;
        return res;
    }
    if (s[i]=='0') return solve(i+1, k, false);
    res += solve(i+1, k, true);
    res += solve(i+1, k-1, false);
    res += solve(i+1, k-1, true)*(s[i]-'1');
    return res;
}
