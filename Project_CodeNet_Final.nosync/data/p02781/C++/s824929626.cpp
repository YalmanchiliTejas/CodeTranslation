...
#define bitCheck(N,in)      (N >> (in - 1) & 1ll)
...
string s;
ll dp[200][10][2];
ll solve(int pos,int k,int can_over=false){
    if(k<0){
      return 0;  
    } 
    if(pos==sz(s)){
    ...
    auto& ans = dp[pos][k][can_over?1:0];
    if(ans!=-1)return ans;
    
    int lim = can_over?10 : (s[pos]-'0');

    ans = solve(pos+1,k,0<lim); // zero digit
    for(int i=1;i<=lim && i<10;i++){
        ans += solve(pos+1,k-1,i<lim);
    }
    return ans;
}
int main(){
