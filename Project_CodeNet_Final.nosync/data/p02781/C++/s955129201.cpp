...
long long pows(long long N, long long k) {
    long long res = 1;
...
ll solve(int i, int k, bool flag){
    if(i == N){
        if(k == 0) return 1;
        else return 0;
    }
    if(k == 0) return 1;
    if(flag) return com(N - i, k) * pows(9, k);
    else{
        if(S[i] == '0') return solve(i + 1, k, false);
        else{
            ll aa = solve(i + 1, k, true);
            ll bb = solve(i + 1, k - 1, true) * (S[i] - '1');
            ll cc = solve(i + 1, k - 1, false);
            return aa + bb + cc;
        }
    }
}
