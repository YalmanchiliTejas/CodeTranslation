...
ll solve(ll ind, ll cnt, ll bound){
    if(ind==n+1&&cnt==k){
        return 1ll;
    }
    else if(ind==n+1){
    ...
    if(vis[ind][cnt][bound]){
        return dp[ind][cnt][bound];
    }
    vis[ind][cnt][bound] = 1;
    if(bound){
        ll check = a[ind];
        ll mx = 0;
        if(check==0){
            mx = merge(mx, solve(ind+1, cnt, bound));
        }
        else{
            mx = merge(mx, solve(ind+1, cnt, bound^1));
            for(int i=1; i<check; i++){
                mx = merge(mx, solve(ind+1, cnt+1, bound^1));
            }
            mx = merge(mx, solve(ind+1, cnt+1, bound));
        }
        return dp[ind][cnt][bound] = mx;
    }
    else{
        return dp[ind][cnt][bound] = merge(solve(ind+1, cnt, bound), 9ll*solve(ind+1, cnt+1, bound));
    }
}
