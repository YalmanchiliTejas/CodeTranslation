...
void warshall_floyd(vector<vl>& dist){
    ll n = dist.size();
...
bool bellman_ford(ll start, ll goal, vector<Graph>& graph, vl& dist){
...
void solve(){
    ll nd = ns.length();
    vector<vector<vl>> dp(2, vector<vl>(k + 1, vl(nd + 1, 0)));
    ...
    For(i1, 1, nd + 1){
        ...
        rep(i2, k + 1){
            rep(state, 2){
                ...
                rep(d, 10){
                    sn = state;
                    if (!state && d > top) continue;
                    if (!state && d < top) sn = 1;
                    
                    if (d != 0 && i2 <= k - 1)
                        dp[sn][i2 + 1][i1] += dp[state][i2][i1 - 1];
                    else if (d == 0)
                        dp[sn][i2][i1] += dp[state][i2][i1 - 1];
                }
            }       
        }
    }

    prl(dp[1][k][nd] + dp[0][k][nd]);
}
