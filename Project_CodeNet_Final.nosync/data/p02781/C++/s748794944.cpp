...
INT(K);
ll dp[N.size()][2][K+1];
rep(i,N.size())rep(j,2)rep(k,K+1)dp[i][j][k]=0;
rep(i,N.size()){
    int num=ctoi(N[i]);
    if(i){
        // eq -> eq or less
        rep(k,K)rep(j,num+1)dp[i][j!=num][k+(j!=0)]+=dp[i-1][0][k];
        // less -> less
        rep(k,K)rep(j,10)dp[i][1][k+(j!=0)]+=dp[i-1][1][k];

        //k=K 0遷移
    
            dp[i][num!=0][K]+=dp[i-1][0][K];
            dp[i][1][K]+=dp[i-1][1][K];
        ...
        rep(j,num+1)dp[i][j!=num][j!=0]++;
    }
}
out(dp[N.size()-1][0][K]+dp[N.size()-1][1][K]);
}