...
signed main(){
	...
	int l = s.size();
	int k; cin >> k;
	vector<vector<vector<ll>>> dp(2, vector<vector<ll>>(l+1, vector<ll>(k+2)));
	dp[0][0][0] = 1;
	rep(i, l) {
		rep(j, k+1) {
			rep(n, 10) {
				dp[1][i+1][j + (n != 0)] += dp[1][i][j];
			}
			rep(n, s[i] - '0') {
				dp[1][i+1][j + (n != 0)] += dp[0][i][j];
			}
			dp[0][i+1][j + (s[i] != '0')] += dp[0][i][j];
		}
	}
	// rep(i, l+1) {
	// 	rep(j, k+1) {
