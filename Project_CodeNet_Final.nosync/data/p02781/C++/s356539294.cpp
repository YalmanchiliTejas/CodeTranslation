...
// dp[i][j][smaller] := i桁目まで決めたとき、残りj個の0以外の数を使うことができて、smaller ? 既にNイカであることが確定している : していない 場合の数
int dp[1010][4][2];
...
signed main() {
	...
	REPS(i, len) {
		...
		rep(j, K+1) {
			// smaller -> smaller
			// 0 を追加
			dp[i][j][true] += dp[i - 1][j][true];
			// 0以外 を追加
			if (j != 0) {
				dp[i][j - 1][true] += dp[i - 1][j][true] * 9;
			}

			// exact -> smaller(0のときはsmallerに遷移できない)
			if (now != 0) {
				// 0 を追加
				dp[i][j][true] += dp[i - 1][j][false];

				// 0以外 を追加
				if (j != 0) {
					dp[i][j - 1][true] += dp[i - 1][j][false] * (now - 1);
				}
			}
			...
			// 0を追加
			if (now == 0) {
				dp[i][j][false] += dp[i - 1][j][false];
			}
			// 0以外を追加できるのはjが0でないときだけ
			else if(j != 0) {
				dp[i][j - 1][false] += dp[i - 1][j][false];
			}
		}

	}

	cout << dp[len][0][false] + dp[len][0][true] << "\n";
