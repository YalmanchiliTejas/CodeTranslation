...
int main() {
	...
	for (int i = 1; i < SS.size(); i++) {
		...
		dp[0][0][i] = dp[0][0][i - 1];
		dp[0][1][i] = dp[0][0][i - 1] * 9 + dp[0][1][i - 1];
		if (A[i] != 0)dp[0][1][i] += dp[1][1][i - 1];

		dp[0][2][i] = dp[0][1][i - 1] * 9 + dp[0][2][i - 1] + dp[1][1][i-1]*max((ll)0,(A[i]-1));
		if (A[i] != 0)dp[0][2][i] += dp[1][2][i - 1];

		dp[0][3][i] = dp[0][2][i - 1] * 9 + dp[0][3][i - 1] + dp[1][2][i - 1] * max((ll)0, (A[i] - 1));
		if (A[i] != 0)dp[0][3][i] += dp[1][3][i - 1];

		if (A[i] == 0) {
			for (int j = 1; j <= 3; j++)dp[1][j][i] = dp[1][j][i - 1];
		}
		else {
			for (int j = 1; j <= 3; j++)dp[1][j][i] = dp[1][j-1][i - 1];
		}

	}

	cout << dp[0][K][SS.size() - 1] + dp[1][K][SS.size() - 1] << endl;
