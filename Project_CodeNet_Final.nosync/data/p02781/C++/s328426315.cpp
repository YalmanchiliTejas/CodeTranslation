...
signed main() {
    ...
    vector<vector<ll>> dp_det(digits+1, vector<ll>(k+2, 0));
    vector<vector<ll>> dp_ndet(digits+1, vector<ll>(k+2, 0));
    dp_ndet[0][0] = 1;

    int d;
    REP(i, digits) REP(j, k+1) {
        d = N[i] - '0';
        dp_det[i+1][j] += dp_det[i][j] + (d==0 ? 0 : dp_ndet[i][j]);
        dp_det[i+1][j+1] += dp_det[i][j]*9 + dp_ndet[i][j]*max(0, d-1);
        if (d==0) dp_ndet[i+1][j] += dp_ndet[i][j];
        if (d!=0) dp_ndet[i+1][j+1] += dp_ndet[i][j];
    }

    ll ans = (ll)dp_det[digits][k] + (ll)dp_ndet[digits][k];
    print(ans);
