...
void _main()
{
    ...
    srep (v, 1, n+1) {
        degs[v] = linklist[v].size();
        if (linklist[v].size() == 1) q.push(t2(v, 1));
    }

    while (!q.empty()) {
        auto vcnt = q.front(); q.pop();
        ll v = get<0>(vcnt);
        ll cnt = get<1>(vcnt);
        for (auto nv : linklist[v]) {
            degs[nv]--;
            if (degs[nv] <= 0LL) continue;
            cnts[nv].pb(cnt);
            if (degs[nv] == 1LL) {
                ll sum = 1LL;
                ll oddcnt = 0LL;
                for (auto cnt : cnts[nv]) {
                    sum += cnt;
                    if (cnt & 1LL) oddcnt++;
                }
                if ((n - sum) & 1LL) oddcnt++;
