...
void _main()
{
    ...
    S ll levels[100005];
    make_parental_relation(edgelist, root, n, parents, children, levels);

    S ll ccnts[100005];
    srep (v, 1, n+1) ccnts[v] = children[v].size();

    S V<ll> saved[100005];

    S queue<t2> q;
    srep (v, 1, n+1) if (!ccnts[v]) q.push(t2(v, 0));
    ...
    while (q.size()) {
        ...
        if (v == pv) continue;
        ccnts[pv]--;
        saved[pv].pb(chain+1LL);
        if (!ccnts[pv]) {
            if (pv == root) {
                ll ocnt = 0;
                for (auto cc : saved[pv]) if (cc & 1LL) ocnt++;
                if (ocnt != 1) {
                    cout << "First" << endl;
                    return;
                }
            }
            else if (children[pv].size() == 1) {
                q.push(t2(pv, saved[pv][0]));
            }
            else {
                ll ocnt = 0;
                debug_printf("---- pv=%lld saved\n", pv);
                debug_print(saved[pv]);
                for (auto cc : saved[pv]) if (cc & 1LL) ocnt++;
                if (ocnt >= 2) {
                    debug_printf("---- first v=%lld pv=%lld\n", v, pv);
                    cout << "First" << endl;
                    return;
                }
                if (ocnt) {
