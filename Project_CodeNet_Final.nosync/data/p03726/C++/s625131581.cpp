...
struct Info:InfoBase{
	...
	bool Ans(){
		return (*max_element(dp,dp+4))*2==s;
	}
	inline int&get(int a,int x,int b,int y){
		return dp[(x<<idx(a))|(y<<idx(b))];
	}
	void Join(int a,int b,int la,int lb,Info&l,int ra,int rb,Info&r,ClusterType type){
		...
		if(type==RAKE){
			...
			REP(i,4)dp[i]=-inf;
			REP(x,2)REP(y,2)REP(z,2)REP(w,2)if(y+w<=1)
				chmax(get(a,z,b,y+w),l.get(la,x,lb,y)+r.get(ra,z,rb,w));
		}else if(type==COMPRESS){
			//(a==la);
			//(lb==ra);
			//(rb==b);
			...
			REP(i,4)dp[i]=-inf;
			REP(x,2)REP(y,2)REP(z,2)REP(w,2)if(y+z<=1)
				chmax(get(a,x,b,w),l.get(la,x,lb,y)+r.get(ra,z,rb,w));
		}
...
signed main(){
	...
	printf("%s\n",!(tree.Expose(0)->Ans())?"First":"Second");
}
