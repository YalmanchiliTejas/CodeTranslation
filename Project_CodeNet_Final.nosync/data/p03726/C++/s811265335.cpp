...
int main() {
	...
	while (que.size()) {
		...
		ll next = -1;
		REP(i, graph[que.front()].size()) {
			if (used[graph[que.front()][i].to] == 0) {
				next = graph[que.front()][i].to;
			}
		}
		if (next == -1) {
			cout << "First";
		...
		if (used[que.front()] != 0 || used[next] != 0) {
			cout << "First";
			return 0;
		}
		else {
			used[que.front()] = 1;
			used[next] = 1;
			REP(i, graph[next].size()) {
				ma[graph[next][i].to]--;
				if (ma[graph[next][i].to] == 0 && used[graph[next][i].to] == 0) {
					cout << "First";
					return 0;
				}
				else if (ma[graph[next][i].to] == 1&&used[graph[next][i].to]==0) {
					que.push(graph[next][i].to);
				}
			}
		}
		que.pop();
