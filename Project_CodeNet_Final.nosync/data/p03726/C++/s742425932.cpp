...
int main() {
  ...
  set<P> st;
  for (ll i = 0; i < n; i++) st.insert(P(e[i].size(), i));
  for (ll i = 0, j = n/2-1; i <= j; i++) {
    P p = *st.begin(); st.erase(st.begin());
    a = p.second;
    b = *e[a].begin();
    st.erase(P(e[b].size(), b));
    auto tmp = e[b].begin();
    while (tmp != e[b].end()) {
      ll x = *tmp;
      st.erase(P(e[x].size(), x));
      e[x].erase(b);
      if (e[x].size() == 0) {
        if (a != x && i != j) {
          printf("First\n");
