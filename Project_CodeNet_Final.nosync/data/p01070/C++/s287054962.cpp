...
public:
    ...
    int operator[](const int idx)const{
     ...
     void get(string &t,int &x,int &y){
        ...
        if(s.compare(sa[ub],t.size(),t)!=0){
            x=y=-1;return;
        }
        x=ub;lb=ub;ub=s.size()+1;
        while(ub-lb>1){
            int mid=(ub+lb)/2;
...
struct segment_tree{
    ...
    void init(vint &v){
        dat.resize(SEG*2);
        rep(i,v.size())dat[i+SEG-1].pb(v[i]);
...
signed main(){
    ...
    cin>>s>>q;
    suffix_array sa(s);
    segment_tree seg;seg.init(sa.sa);

    while(q--){
        int l,r;
        string m;
        cin>>l>>r>>m;r++;
        if(r-l<m.size())cout<<0<<endl;
        else{
            int x,y;
            sa.get(m,x,y);
            if(x==-1)cout<<0<<endl;
            else cout<<seg.get(x,y,l,r-m.size()+1)<<endl;
        }
    }
    return 0;
}