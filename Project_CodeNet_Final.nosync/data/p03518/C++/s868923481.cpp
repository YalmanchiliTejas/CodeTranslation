...
int main() {
    ...
    vi dest; F0R(i,N) dest.pb(i);
    while (P != dest) {
        if (P.front() < P.back() && P.back() != N-1) move(N-1);
        else move(1);
    }
    pr(sz(ans));
    trav(x,ans) pr(x);
    ...
    while (sz(todo)) {
        ...
        auto A = a; swap(A.front(),A.back()); ins(A,a);
        A = a; FOR(i,1,sz(A)) swap(A[i],A[i-1]); ins(A,a);
    }
    vi dest; F0R(i,N) dest.pb(i);
    vi ret;
    while (sz(dest)) {
        auto a = dest; swap(a.front(),a.back());
        pr(dest);
        dest = pre[dest];
        if (a == dest) ret.pb(N-1);
        else if (sz(dest)) ret.pb(1);
    }
    reverse(all(ret));
    pr(sz(ret));
    trav(x,ret) pr(x);*/
