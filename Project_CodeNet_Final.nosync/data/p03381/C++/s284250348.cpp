...
template<class T> inline pair<vll, vll> RankOfi_AND_iOfRank(vector<T> &v){
	ll N = (ll)v.size();
	vll rank(N), idx(N);
	REP(i, N) idx[i] = i;
	sort(idx.begin(), idx.end(), [&](auto &a, auto &b){return v[a]<v[b]; });
	REPS(r, 1, N-1){
		ll i = idx[r], p = idx[r-1];
		rank[i] = (v[i] != v[p]) ? r : rank[p];
	}
	return std::move(make_pair(std::move(rank), std::move(idx)));
}
...
{
	ll n;  cin >> n;
	vector<ll> x = cinv<ll>(n);
	vll rank, idx;
	tie(rank,idx)=RankOfi_AND_iOfRank(x);
	rep(i,0,n-1){
		if (rank[i] <= n/2-1){
			cout << x[idx[n/2]] << '\n';
		}
