...
int _main()
{
    ...
    V<t2> x_idx_list;
    rep (i, n) {
        x_idx_list.pb(t2(xlist[i], i));
    }
    sort(x_idx_list);

    ll med1 = get<0>(x_idx_list[n/2-1]);
    ll med2 = get<0>(x_idx_list[n/2]);
    bool isleft[200005];
    memset(isleft, 0, sizeof(isleft));

    rep (i, n/2) {
        ll idx = get<1>(x_idx_list[i]);
        isleft[idx] = true;
    }

    debug_printf("---- med1 %lld\n", med1);
    debug_printf("---- med2 %lld\n", med2);
    debug_printf("---- isleft\n");
    debug_print(isleft, 4);

    rep (i, n) {
        if (isleft[i]) cout << med2 << endl;
        else cout << med1 << endl;
    }
