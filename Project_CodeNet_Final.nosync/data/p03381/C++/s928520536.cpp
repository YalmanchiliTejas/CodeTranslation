...
int main(int argc,char **argv){
	...
	std::sort(d,d+n,[](ds &u,ds &v){return u.dat<v.dat;});
	int m[2]={d[n/2].dat,d[n/2-1].dat};
	for(int i=0;i<n/2;++i) d[i].flag=0;
	for(int i=n/2;i<n;++i) d[i].flag=1;
	std::sort(d,d+n,[](ds &u,ds &v){return u.num<v.num;});
	for(int i=0;i<n;++i) out<m[d[i].flag]<'\n';
	out.print();
	in.close();
	out.close();
	return 0;
