...
template<class T> inline ll sum(const T& v){return reduce(all(v),0LL);}
...
void solve(ll n, vector<string> S) {
  ...
  for (int idx = 0; idx < n; idx++) {
    if (idx == 0) {
      for (char c : S[idx]) cross[c]++;
    } else {
      map<char, int> current;
      for (char c : S[idx]) current[c]++;

      for (auto &p : cross) {
        p.second = min(current[p.first], p.second);
      }
    }
  }

  string ans = "";
  for (pair<char, int> p : cross) {
    for (int iter = 0; iter < p.second; iter++) ans += p.first;
  }
