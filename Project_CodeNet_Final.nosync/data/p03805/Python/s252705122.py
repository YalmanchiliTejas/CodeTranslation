...
def check(vertex_dic :dict, passed :set, start :int, cnt :int):
    if cnt >= len(vertex_dic) and len(vertex_dic) == len(passed):
        return 1
    ...
    for i_next in next_set:
        ...
        ans += check(vertex_dic, next_passed, i_next, cnt)
    
    return ans
    
result=check(vertex_dic, {1}, 1 ,1 )
print(result)
