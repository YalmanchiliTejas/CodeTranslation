...
#define INF (lnt)1e18
#define MOD (lnt)(1e9 + 7) // AtCoderのMOD
...
MAIN() {
  ...
  lnt res = 0;
  vector<lnt> prm = init_comb(n);
  do {
    bool fOK = true;
    REP(i, n - 1) {
      if (not edg.count(P(prm.at(i), prm.at(i + 1)))) {
        fOK = false;
        break;
      }
    }
  ...
  } while (next_permutation(prm.begin() + 1, prm.end()));
