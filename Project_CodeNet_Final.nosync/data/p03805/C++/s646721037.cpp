...
void _main()
{
    ...
    V<ll> vlist;
    srep (v, 1, n+1) vlist.pb(v);
    ll ans = 0LL;

    debug_printf("conn[1]\n");
    debug_print(conn[1], 10);

    do {
        if (vlist[0] != 1) continue;
        bool ok = true;
        rep (i, n-1) {
            if (!conn[vlist[i]][vlist[i+1]]) ok = false;
        }
        if (ok) ans++;
    } while (next_permutation(ALL(vlist))) ;
