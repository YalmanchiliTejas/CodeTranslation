...
ll dfs(Path& p, int v, Matrix<bool>& adj) {
  // cerr << p.cur << endl;
  // for (auto a : p.route) cerr << a << " ";
  // cerr << "\n\n";

  if (sz(p.route) == v) return 1;

  ll ret = 0;
  int orig = p.cur;
  rep(i, 0, v) {
    if (!adj[p.cur][i]) continue;
    if (p.route.count(i)) continue;
...
int main(int argc, char* argv[]) {
  ll n, m;
  cin >> n >> m;
  Matrix<bool> adj(n, vec<bool>(n, false));
  for (int i = 0; i < m; i++) {
    int a, b;
    cin >> a >> b;
    a--, b--;
