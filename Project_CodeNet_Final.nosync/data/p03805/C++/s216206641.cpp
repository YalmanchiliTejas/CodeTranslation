...
template<typename T> T pow(T x,T y,ll m=MOD){T ans=1;while(y>0){if(y%2==1) ans=(ans*x)%m;y/=2;x=(x*x)%m;}return ans%m;} //using this instead of inbuilt pow due to precision issues in that
template<typename T> ll roundp(T x){ll ans=x;if(x-floor(x)<=0.001) ans=floor(x);else if(ceil(x)-x<=0.001) ans=ceil(x);return ans;}
vvi g;
int ans,glob;
void dfs(int n,vi vis,int ct){
	if(vis[n]) return;
	vis[n]=1;
	++ct;
//	trace2(n,ct);
...
int main(){
	int n,m,x,y;
	 cin>>n>>m; glob=n;g.resize(n);
	rep(i,0,m){
		cin>>x>>y; --x; --y; g[x].eb(y); g[y].eb(x);
	}
	vi vis; vis.resize(n); fill(all(vis),0);
	dfs(0,vis,0);
	cout<<ans<<endl;
}