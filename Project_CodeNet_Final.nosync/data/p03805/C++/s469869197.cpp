...
int main(){
  ...
  ll ans{};
  queue<tuple<ll,ll,deque<bool>>> Q;
  Q.emplace(0,-1,make_deque<bool>(N));
  while(!Q.empty()){
    auto fr = Q.front(); Q.pop();
    ll now = get<0>(fr);
    ll last = get<1>(fr);
    deque<bool> visited = get<2>(fr);
    if(visited[now]) continue;
    visited[now] = 1;
    if(accumulate(all(visited),0LL) == N){
      ans++;
      continue;
    }
    for(auto&& next : E[now]){
      if(last != next) Q.emplace(next,now,visited);
    }
  }
  cout << ans << rt;
}
