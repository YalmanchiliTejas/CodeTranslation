...
class PascalNCR{
  private:
    static const ll MOD = (1LL<<54);
    static const ll MAX_C = 1000LL;
    ll Com[MAX_C][MAX_C];
  public:
    void calc_com(){
...
class NCR{
  private:
    //MOD need to be prime number
    static const ll MAX = 210000LL;
...
vi a,b;
int ctr;
void dfs(Graph& g,int cur,vi visited){
  int visited_nodes = 0;
  REP(i,N){
    if(visited[i] == 1){
      visited_nodes++;
  ...
  FORE(e,g[cur]){
    if(visited[e.dst] == 0){
      vi l_visited = visited;
      l_visited[e.dst] = 1;
      dfs(g,e.dst,l_visited);
    }
  }
}
