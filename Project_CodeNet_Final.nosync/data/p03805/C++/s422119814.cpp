...
{
	//find first i,i+1 where d[i]<d[i+1]
	int rootidx = -1;
	for (int i = n - 2; i >= 0; --i)
	{
		if (data[i] < data[i + 1])
...
void dfs(Veci *p,char* visited, int nnodes, int current, int nvisited, int* npaths)
{
	visited[current] = 1;
	nvisited++;
	if(nvisited == nnodes)
	...
	repi(n)
	{
		int next = p[current].data[i];
		if (visited[next] == 1) continue;
		dfs(p, visited, nnodes, next, nvisited, npaths);
...
int main()
{
	...
	RUN_LOCAL("xxxtestcase_06.txt");
	int n = readi();
	int m = readi();
	static char visited[10] = { 0 };
	static Veci conn[10];
	repi(n)
	{
		init(&conn[i], 10);
