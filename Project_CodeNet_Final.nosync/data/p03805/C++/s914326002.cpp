...
int main(){
  ...
  int cnt = 0;
  bool willend = false;
  vector<int> permplus(n);
  rep(i,n){
    if(i == 0){
      permplus.at(i) = 1;
    }else{
      permplus.at(i) = perm.at(i-1)+1;
    }
  }
  while(true){
    bool isok = false;
    ...
    rep(i,n-1){
      rep(j,m){
        if((a.at(j) == permplus.at(i) && b.at(j) == permplus.at(i+1))
           ||(a.at(j) == permplus.at(i+1) && b.at(j) == permplus.at(i))){
          isok = true;
        }
      }
      if(!isok){
    ...
    willend = !(next_permutation(perm.begin(),perm.end()));
    if(willend){
      break;
    }
    rep(i,n){
