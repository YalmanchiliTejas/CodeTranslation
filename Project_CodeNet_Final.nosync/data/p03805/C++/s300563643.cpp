...
public:
    ...
    int dfs_count_subtree(int p, int i) {        childs[i] = 1;        fort(gi, g[i]) {            fort_init(gi, g[i]);            childs[i] += dfs_count_subtree(i, t);        }        return childs[i];    }
...
//    g.ing(N, M);
    ...
    do {
        if(p[0]!=0)goto end;
        rep(ni, N -1) {
            int i = p[ni], l = p[(ni - 1 + N) % N], r = p[(ni + 1) % N];
            if(!can[i][r])goto end;
