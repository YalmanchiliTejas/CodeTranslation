...
public:
    ...
    CommonGraph(const int&        num_vertex,
                ...
                const ll&         vertex_init_cost = (ll)1e18 + 1) :
        vertex_(num_vertex, vertex_init_cost),
        edge_(num_vertex),
        num_edge_(0),
        total_cost_(0),
    ...
    void setVertexCost(const ll& val) { std::fill(vertex_.begin(), vertex_.end(), val); };
     ...
     */
    void dfsSample(const int& index,
        ...
        for(const auto& edge : edge_[index]) {
            if(vertex_[edge.dst].cost == inf_val) {
                ...
                dfsSample(edge.dst, inf_val);
            }
        }
    }

    int dfs(const int& index) {
        if([&](){ll cost=0;for(auto& v:vertex_){cost+=v.cost;}return cost;}() == (ll)vertex_.size()) return 1;

        vertex_[0].cost = 1;
        int ret = 0;
        for(const auto& edge : edge_[index]) {
            if(vertex_[edge.dst].cost == 0) {
                vertex_[edge.dst].cost = 1;
                ret += dfs(edge.dst);
                vertex_[edge.dst].cost = 0;
            }
        }
        return ret;
    }
...
int main() {
    ...
    CommonGraph g(N, CommonGraph::Edge::Type::Undirected, 0);
    REP(i, M) {
        int a, b;
        std::cin >> a >> b;
        g.addEdge(a - 1, b - 1, 1);
