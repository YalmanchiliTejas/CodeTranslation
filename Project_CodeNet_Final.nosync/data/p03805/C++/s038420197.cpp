...
struct Path {
    ...
    Path(ll now, const bool checked[], int count) {
        this->now = now;
        memcpy(this->checked, checked, sizeof(this->checked));
        this->count = count;

        if (!this->checked[now]) {
            this->checked[now] = true;
            this->count++;
        }
    }
};
...
int _main()
{
    ...
    for (; !(q.empty()); ) {
        Path path = q.front(); q.pop();
        for (auto next : linklist[path.now]) {
            if (path.checked[next]) continue;

            Path np(next, path.checked, path.count);
