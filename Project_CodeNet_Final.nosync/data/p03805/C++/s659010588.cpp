...
void search( int N , vector<vector<int> >& cango , lli& ans , vector<int> notyet, int id )
{
    
  vector<int> temp = notyet;
  temp[id] = 0;
  ...
  REP(i,N)
    {
      if( cango[id][i] and temp[i] )
	{
	  search(N,cango,ans,temp,i);
	}
...
int main()
{
  ...
  int a,b;
  vector<vector<int> > cango(N,vector<int>(N,0));
  ...
  vector<int> notyet(N,1);
  lli ans = 0;
  
  search( N , cango , ans , notyet, 0 );
