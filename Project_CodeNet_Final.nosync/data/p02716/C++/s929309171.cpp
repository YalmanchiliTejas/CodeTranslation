...
struct Cumulative1D{
    ...
    Cumulative1D(ll n) : n(n){
        cum = vl(n + 1, 0);
    }

    void build(vl& vec){
        rep(i, n) cum[i + 1] = vec[i] + cum[i];
    }
...
void read_graph(ll n_vertex, ll n_edges, Graph& graph,
                ll cost = -1, ll directed = false, ll offset = -1){
...
void solve(){
    ...
    vl cum(n + 1, 0); cum[1] = cum[2] = as[0];
    For(i, 3, n + 1){
        ll nxt = as[i - 1];
        if (i % 2 == 0){
            dp[i] = max(dp[i - 2] + nxt, cum[i - 1]);
            cum[i] = cum[i - 1];
        }else{
            dp[i] = max(dp[i - 2] + nxt, dp[i - 1]);
            cum[i] = cum[i - 1] + as[i - 1];
        }
    }
