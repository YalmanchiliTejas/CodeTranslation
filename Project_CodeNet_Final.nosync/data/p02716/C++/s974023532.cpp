...
int main() {
    ...
    if (n % 2 == 0) {
        ...
        lv(i, a) {
            ...
            if (i % 2 == 1) {
                ...
                if (i)
                    tmp += max(prefix[i - 1], prefix[i - 2]);
            }
            if ((n - i - 1) % 2 == 1) {
                tmp += suffix[i + 1];
            } else {
                if (n - i - 1)
                    tmp += max(suffix[i + 1], suffix[i + 2]);
            }
            tmax(ans, tmp);
        }
        ou(ans);
    ...
    */
    auto f = vect<ll>({-inf<ll>()}, n + 1, 3, 2);
    f[0][0][0] = 0;
    lp(i, 0, n - 1) {
        lp(r, 0, 2) lp(c, 0, 1) {
            int max_choose = (i + 1) / 2;
            if (r <= max_choose && (c == 0 || r != max_choose)) {
                if (c == 1) {
                    int now_choose = max_choose - r;
                    int mc2 = (i + 2) / 2;
                    int nr = mc2 - now_choose;
                    // debu(nr);
                    if (nr <= 2)
                        tmax(f[i + 1][nr][0], f[i][r][c]);
                } else {
                    int now_choose = max_choose - r;
                    int mc2 = (i + 2) / 2;
                    int nr = mc2 - now_choose;
                    ...
                    // debu(i, r, c, nr);
                    assert(nr <= 2);
                    tmax(f[i + 1][nr][1], f[i][r][c] + a[i]);
                }
            }
        }
    }
    ou(max(f[n][(n + 1) / 2 - n / 2][0], f[n][(n + 1) / 2 - n / 2][1]));
    return 0;
}
