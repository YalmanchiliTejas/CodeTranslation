...
i%2==0 i/2-1 to i/2 (i==4, 1,2)

i%2==1 i/2 to (i+1)/2 (i==7, 3,4)
...
int main(void)
{
  ...
  for (ll i = 1; i <= n; i++)
  {
    ...
    else
    {
      if (i % 2 == 0)
      {
        DP[1][i] = max(DP[1][i - 2] + Ax, DP[2][i - 1]);
        DP[0][i] = max(DP[0][i - 2] + Ax, DP[1][i - 1]);
      }
      else
      {
        DP[2][i] = DP[2][i - 2] + Ax;
        DP[1][i] = max(DP[1][i - 2] + Ax, DP[1][i - 1]);
        DP[0][i] = max(DP[0][i - 2] + Ax, DP[0][i - 1]);
      }
    }
  }
  if (n % 2 == 0)
