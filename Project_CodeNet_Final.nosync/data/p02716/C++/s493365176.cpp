...
ll N,M,K,H,W,Q,ans=0;
v_ll A,B,C,P;
int main(){
    cin >> N;
    v_load(N,A);
    if (N%2==0){
        vv_ll V(N/2,v_ll(2,0));
        V[0][0]=A[0];
        V[0][1]=A[1];
        for(ll i=1;i<N/2;i++){
            V[i][0]=V[i-1][0]+A[2*i];
            V[i][1]=max(V[i-1][0],V[i-1][1])+A[2*i+1];
        }
        ll ans = max(V[N/2-1][0],V[N/2-1][1]);
        cout << ans << endl;
        return 0;
    }
    vv_ll V(N/2,v_ll(3,0));
    V[0][0]=A[0];
    V[0][1]=A[1];
    V[0][2]=A[2];
    for(ll i=1;i<N/2;i++){
        V[i][0]=V[i-1][0]+A[2*i];
        V[i][1]=max(V[i-1][0],V[i-1][1])+A[2*i+1];
        V[i][2]=max(max(V[i-1][0],V[i-1][1]),V[i-1][2])+A[2*i+2];
    }
    ll ans = max(max(V[N/2-1][0],V[N/2-1][1]),V[N/2-1][2]);
