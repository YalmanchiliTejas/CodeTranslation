...
int main(){
	...
	if(N%2){
		for(int i=0;i<3;i++)G[3*N].push_back({2-i+i*N,-A[2-i]});
	}else{
		for(int i=0;i<2;i++)G[3*N].push_back({1-i+i*N,-A[1-i]});
	}
	Dijk(G,3*N+1,3*N,ans);
	
	for(int i=0;i<3;i++){
		for(int j=0;j<=i;j++){
			if(i==0&&j==0 || vis[N-1-j+i*N]&&res>ans[N-1-j+i*N]){
				res=ans[N-1-j+i*N];
			}
		}
	}
