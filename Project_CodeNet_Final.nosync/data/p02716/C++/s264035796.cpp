...
struct ll_neg {
  ll val = -INF_LL;
  bool operator <(ll_neg rhs) {
    return val < rhs.val;
  }
};

ostream& operator<<(ostream& out, ll_neg x) {
  return out << x.val;
}
...
int main() {
  int n = IN;
  auto a = IN.vll(n);
  auto dp = vector(n+1, map<ll, ll_neg>{});
  dp[0][0] = ll_neg{0};
  rep(i, n) for (auto [k, v] : dp[i]) rep(j, i+1, i+5) {
    if (j <= n and (j - i > 1 or i == 0) and (n-j)/2 >= n/2 - (k+1))
      chmax(dp[j][k+1].val, v.val+a[j-1]);
    if (j <= n and (n-j+1)/2 >= n/2 - k)
     chmax(dp[j][k].val, v.val);
  }
  rep(i, n+1) debugv(dp[i]);
  OUT(dp[n][n/2].val);
}