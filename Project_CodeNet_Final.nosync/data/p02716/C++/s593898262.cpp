...
int main()
{
    ...
    for(int i=2;i<=n;i++)
    {
        for(int j=0;j<5;j++)
        {
            ...
            int pcc=(i-1)/2+j-var;
            pcc=pcc-i/2;
            if(pcc+var>=0&&pcc+var<5&&dp[i-1][j]!=inf)
                dp[i][pcc+var]=max(dp[i][pcc+var],dp[i-1][j]);
            cc++;
            if(cc+var>=0&&cc+var<5&&dp[i-2][j]!=inf)
                dp[i][cc+var]=max(dp[i][cc+var],dp[i-2][j]+a[i]);
            //dbg(i,j,(i/2)+j-var,dp[i][j]);
        }
    }
    print(dp[n][var]);
}