...
ll dp[2*MM][4][4][2], a[2*MM], n;
ll dpcall(int id, int fst, int scd, int ok){
    if(abs(scd-fst)>2)return -1e14;
    if(id==n){
        if(scd-fst==n%2)return 0;
        else return -1e14;
    }
    ll &ret = dp[id][fst][scd][ok];
    if(~ret)return ret;
    ret = -1e14;
    if(ok==1){
        return ret = dpcall(id+1, fst - min(fst, scd+1), scd+1 - min(fst, scd+1), 0);
    }
    ret = max(ret, a[id] + dpcall(id+1, fst+1 - min(fst+1, scd), scd-min(fst+1, scd), 1));
    ret = max(ret,dpcall(id+1, fst - min(fst, scd+1), scd+1 - min(fst, scd+1), 0));
    return ret;
}
