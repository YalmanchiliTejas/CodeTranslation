...
template<class T> struct ovv : vector<vector<optional<T>, allocator<optional<T>>>>{
    optional<T> emp;
//    vector<vector<optional<T>> > v ;
    using super = vector<vector<optional<T>, allocator<optional<T>>>>;
    ovv(int i, int j, T val = 0) : super(i, vector<optional<T>, allocator<optional<T>>>(j, val)){}
    /*@formatter:on*/
    optional<T> &operator()(int i, int j) {
        if (j < 0 || i < 0 || (this->size()) <= i || (sz(super::operator[](i))) <= j) {
//            emp.reset();
            return emp;
        } else { return super::operator[](i)[j]; }
    }
    //再帰ver 遅いと思う
...
private:
    ...
    template<class V, class H> optional<T> &gets(V &nowv, H i) {
        if constexpr(is_same2(H, nullopt_t)) { return emp; }
        else if constexpr(is_same2(H, optional<int>)) { if (i.has_value()) { return nowv[(int) i]; } else { return emp; }}
        else if constexpr(is_integral<H>::value) { return nowv[(int) i]; }
        else {
            assert2(0, "ov3 error not index");
            return emp;
        }
    }
    //todo const &消した
    template<class V, class H, class... U> optional<T> &gets(V &nowv, H i, U... tail) {
        if constexpr(is_same2(H, nullopt_t)) { return emp; }
        else if constexpr(is_same2(H, optional<int>)) { if (i.has_value()) { return gets(nowv[(int) i], tail...); } else { return emp; }}
        else if constexpr(is_integral<H>::value) { return gets(nowv[(int) i], tail...); }
        else {
            assert2(0, "ov3 error not index");
            return emp;
        }
