...
int main()
{
    ...
    loop(i, N) cin >> A[i];
    ll skip;
    //交互に置き続けるのが最密だとすると、何回スキップできるか?
    //N=偶数：101010
    //N=奇数：1010100
    N % 2 == 0 ? skip = 1 : skip = 2;
    loop(i, 200010) loop(j, 5) dp[i][j] = -1e18;
    dp[0][0] = 0;
    for (register int i = 0; i <= N; i++)
    {
        for (register int j = 0; j <= skip; j++)
        {
            if (j < skip)
                chmax(dp[i + 1][j + 1], dp[i][j]); //要素を1つスキップ
            if (i < N)
                chmax(dp[i + 2][j], dp[i][j] + A[i]); //スキップしない
        }
    }
    putout(dp[N + 1][skip]);
    return 0;
