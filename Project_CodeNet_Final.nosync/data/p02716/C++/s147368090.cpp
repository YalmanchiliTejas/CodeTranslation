...
int main()
{
    ...
    vec2 dp0(N + 1, vec(2, -INF)), dp1(N + 1, vec(2, -INF)), dp2(N + 1, vec(2, -INF));

    dp0[0][0] = 0;

    rep1(i, N){
        //前回選んでて今回選ばない
        chmax(dp0[i][0], dp0[i - 1][1]);
        chmax(dp1[i][0], dp1[i - 1][1]);
        chmax(dp2[i][0], dp2[i - 1][1]);

        //前回選んでなくて今回選ぶ
        chmax(dp0[i][1], dp0[i - 1][0] + A[i - 1]);
        chmax(dp1[i][1], dp1[i - 1][0] + A[i - 1]);
        chmax(dp2[i][1], dp2[i - 1][0] + A[i - 1]);

        //前回選んでなくて今回も選ばない
        chmax(dp1[i][0], dp0[i - 1][0]);
        chmax(dp2[i][0], dp1[i - 1][0]);
    }

    ll ans = -INF;
    if(N % 2 == 0) for(auto d : dp0.back()) chmax(ans, d);
    for(auto d : dp1.back()) chmax(ans, d);
    if(N % 2 == 1) for(auto d : dp2.back()) chmax(ans, d);
