...
void gen(int n, int k, int left, ll& ans) {
    if (left == 0) {
        ans++;
        return;
    }
    if (k >= n) return;
    if (n-k+1 <= left) return;
    // no take
    gen(n, k+1, left, ans);
    // take
    gen(n, k+2, left-1, ans);
}
...
int main()
{
    ...
    FOR(i, 2, n+1) {
        int x = arr[i-1];
        dp1[i] = max(
            dp1[i-2]+x, // take arr[i], find result for the rest of i-2
            i%2==1 ? max(dp1[i-1],dp2[i-1]) : dp2[i-1]    // don't take arr[i], find result for the rest of i-1
        );
        dp2[i] = max(
            dp2[i-2]+x,
            // (divCeil(i,2)-1==(i-2)/2 ? dp1[i-2]+x : divCeil(i,2)-1==divCeil(i-2,2) ? dp2[i-2]+x : 0),
            i%2==0?dp2[i-1]:-OO
        );
        dp1[i] = max(dp1[i], -OO);
        dp2[i] = max(dp2[i], -OO);
