...
Int used[200010][100];
Int mem[200010][100];
Int dfs(Int idx, Int passed, Int num){
  if(num > N / 2) return -INF;
  if(idx >= N) return num < N/2? -INF: 0;
  if(num + (N - idx + 1) / 2 < N/2) return -INF;
  if(used[idx][passed]++) return mem[idx][passed];
  Int a = dfs(idx + 2, passed, num + 1) + A[idx];
  Int b = dfs(idx + 1, passed + 1, num);
  Int res = max(a, b);
  return mem[idx][passed] = res;
}
