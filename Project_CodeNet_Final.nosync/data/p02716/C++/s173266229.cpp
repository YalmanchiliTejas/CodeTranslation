	...
	for(int i = 2; i <= n; i++) {
		for(int j = 0; j <= excess; j++) {
			for(int k = 0; k <= j; k++) {
				...
				if(last_take == 0) {
					if(excess < i - 1) continue;
					dp[i][i - 1] = max(dp[i][i - 1], a[i]);
					continue;
				}
				if(dp[last_take][j - k] == -inf) {
					continue;
				}
				dp[i][j] = max(dp[i][j], dp[last_take][j - k] + a[i]);
			}
		}
	}
	int ans = -inf;
