...
signed main()
{
  ...
  if ((N & 1) == 0)
  {
    ...
    FOR(i, 1, L + 1)
    {
      ...
      dp[i][0] = dp[i - 1][0] + a[idl];
      dp[i][1] = max(dp[i - 1][0], dp[i - 1][1]) + a[idr];
    }

    PRINTLLD(max(dp[L][0], dp[L][1]));
  }
  else
  {
    ++L;
    ...
    FOR(i, 1, L + 1)
    {
      ...
      dp[i][0][0] = dp[i - 1][0][0] + a[idl];
      dp[i][0][1] = max(dp[i - 1][0][0], dp[i - 1][0][1]) + a[idr];
      dp[i][1][0] = max({dp[i - 1][1][0] + a[idl], dp[i - 1][0][0], dp[i - 1][0][1]});
      dp[i][1][1] = max(dp[i - 1][1][0], dp[i - 1][1][1]) + a[idr];
    }

    PRINTLLD(max(dp[L][1][0], dp[L][1][1]));
  }
}
