...
signed main(){
    ...
    rep(i,n)a.pb(in());
    ll dp[n+1][10][2];
    rep(i,n+1)rep(j,10)dp[i][j][0]=-1e15;
    rep(i,n+1)rep(j,10)dp[i][j][1]=-1e15;
    dp[0][5][0]=0;
    rep(i,n){
        if((i&1)==0){
            rep2(j,0,8){
                chmax(dp[i+1][j][1],dp[i][j][0]+a[i]);
                chmax(dp[i+1][j][0],dp[i][j+1][0]);
                chmax(dp[i+1][j][0],dp[i][j+1][1]);
            }
        }
        else{
            rep2(j,1,9){
                chmax(dp[i+1][j][1],dp[i][j-1][0]+a[i]);
    ...
    if(n&1)
    cout<<max(dp[n][4][0],dp[n][4][1])<<endl;
    else
    cout<<max(dp[n][5][0],dp[n][5][1])<<endl;
