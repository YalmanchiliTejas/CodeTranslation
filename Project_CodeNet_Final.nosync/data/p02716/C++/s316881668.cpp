...
void solve() {
    ...
    VECCIN(A);
    map<PL, ll> memo;
    memo[PL(0, 0)] = 0, memo[PL(0, 1)] = A[0];
    memo[PL(1, 0)] = 0, memo[PL(1, 1)] = A[1];
    REP(i, N)
    FORR(j, i - 2, max(-1LL, i - 6)) FOR(k, (i - 1) / 2, (i + 1) / 2 + 1) {
        if(!memo.count(PL(j, k))) continue;
        if(!memo.count(PL(i, k + 1))) memo[PL(i, k + 1)] = -LINF;
        if(!memo.count(PL(i, k))) memo[PL(i, k)] = -LINF;
        chmax(memo[PL(i, k + 1)], memo[PL(j, k)] + A[i]);
        chmax(memo[PL(i, k)], memo[PL(j, k)]);
    }
    ll ma = -LINF;
    FOREACH(x, memo) {
        if(x.fi.se == N / 2) {
