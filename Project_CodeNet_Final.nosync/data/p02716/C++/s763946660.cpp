...
int main() {
  ...
  int n;
  cin >> n;
  for (int i = 0; i < n; i++) cin >> a[i/2+(i&1)*100030];
...
*/
  ...
  dp[1][0] = a[100030];
  for (int i = 1; i < n/2; i++) {
    dp[1][i] = (dp[0][i-1] > dp[1][i-1] ? dp[0][i-1] : dp[1][i-1]) + a[i+100030];
  }
  dp[2][0] = a[1];
  for (int i = 1; i < n/2; i++) {
    dp[2][i] = (dp[0][i-1] > dp[1][i-1] ? (dp[0][i-1] > dp[2][i-1] ? dp[0][i-1] : dp[2][i-1]) : (dp[1][i-1] > dp[2][i-1] ? dp[1][i-1] : dp[2][i-1])) + a[i+1];
  }
