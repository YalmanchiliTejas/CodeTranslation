...
LL dfs(int v, int r, int done, int N, vector<map<LL, LL>> &dp, vector<LL> &A) {
    if (dp[v].count(r)) return dp[v][r];
    if (done >= 4) return -INF;
    if (r == 0) return 0;
    if (v >= N) {
        if (r == 0) return 0;
        else return -INF;
    ...
    for (int i = 0; i < 3; i++) {
        int to = v + i;
        if (to < N and done + i <= 3) {
            chmax(ret, dfs(to + 2, r - 1, done + i, N, dp, A) + A[to]);
        }
    }
    return dp[v][r] = ret;
}
...
int main() {
    ...
    vector<map<LL, LL>> dp(700000);

    cout << dfs(0, N / 2, 0, N, dp, A) << endl;
}
