...
int n;
vll arr(2e5+5);
vector<vvll> dp(2e5+5, vvll(2, vll(3, -1)));
ll solve(int pos, int left, int sel=false){
  if((pos==n and left>0) or (sel and left>(n-pos)/2) or (!sel and left>(n-pos)/2+(n-pos)%2))return LLONG_MIN;
  // cout<<pos/2-(n/2-left)<<endl;
  if(left==0)return 0;
  if(dp[pos][sel][pos/2-(n/2-left)+1]!=-1)return dp[pos][sel][pos/2-(n/2-left)+1];
  ll ans=solve(pos+1, left, false);
  if(!sel){
    ll temp=solve(pos+1, left-1, true);
    if(temp!=LLONG_MIN)
    ans=max(ans, arr[pos]+temp);
  }
  return dp[pos][sel][pos/2-(n/2-left)+1]=ans;
}
