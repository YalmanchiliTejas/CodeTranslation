...
signed main() {
  ...
  REP(i, n) {
    for(auto x: dp[i][0]) {
      if(x.first < i / 2 - 10 || i / 2 + 10 < x.first) continue;
      dp[i+1][0][x.first] = x.second;
      dp[i+1][1][x.first+1] = x.second + a[i];
    }
    for(auto x: dp[i][1]) {
      if(x.first < i / 2 - 10 || i / 2 + 10 < x.first) continue;
      if(dp[i+1][0].count(x.first)) {
        dp[i+1][0][x.first] = max(x.second, dp[i+1][0][x.first]);
      }
      else {
        dp[i+1][0][x.first] = x.second;
      }
