...
int main() {
    ...
    vector<vector<vector<ll> > > dp(n, vector<vector<ll> >(7, vector<ll>(2, -INF)));;
    dp[0][get_jj(0, 0)][0] = 0;
    dp[0][get_jj(0, 1)][1] = a[0];
    for(int i = 0; i < n - 1; i++){
        for(int j = (i / 2) - 3; j <= (i / 2) + 3 && j <= (i + 1); j++){
            if(j < 0) continue;

            for(int k = 0; k < 2; k++) {
                if (dp[i][get_jj(i, j)][k] == -INF) continue;
                if (k == 1) {
                    if(get_jj(i + 1, j) >= 0 && get_jj(i + 1, j) < 7)
                        dp[i + 1][get_jj(i + 1, j)][1 - k] = max(dp[i + 1][get_jj(i + 1, j)][1 - k], dp[i][get_jj(i, j)][k]);
//                    cout << i << ' ' << j << ' ' << k << " -> " << i + 1 << ' ' << j << ' ' << 1 - k << '\n';
                }
                else{
                    if(get_jj(i + 1, j + 1) >= 0 && get_jj(i + 1, j + 1) < 7)
