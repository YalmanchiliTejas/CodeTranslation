...
#define o_multisetlli tree<lli,null_type,std::less_equal<lli>,rb_tree_tag,tree_order_statistics_node_update>
//member functions :
 ...
 int main()
{
  /*ifstream stdin;
    ...
    while(t--)
    {
        ...
        dp[0]=0;
        lli p=0;
        //dp[i] max sum you can get choosing exactly i/2 elements
        for(lli i=1;i<=n;i++)
        {
            if(i%2)
            {
                p+=a[i-1];
                dp[i]=max(dp[i-1],((i-2)!=-1 ?dp[i-2]+a[i-1]:0));
            }
            else
            {
                dp[i]=max(((i-2)!=-1 ?dp[i-2]+a[i-1]:0),p);
            }
        }
        cout<<dp[n]<<'\n';
    }
