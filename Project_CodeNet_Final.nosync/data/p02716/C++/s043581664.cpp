...
unordered_map<int, ll> dp[200200];
unordered_set<int> visited[200200];

ll rec(int cur, int used) {
    if (cur >= N) return (used == N / 2) ? 0 : -INFF;
    if (visited[cur].count(used)) return dp[cur][used];
    visited[cur].emplace(used);
    int cantake_mn = (N - cur) / 2;
    int cantake_mx = (N - cur + 1) / 2;
    if (used + cantake_mn > N / 2) return dp[cur][used] = -INFF;
    if (used + cantake_mx < N / 2) return dp[cur][used] = -INFF;
    return dp[cur][used] = max<ll>(A[cur] + rec(cur + 2, used + 1), rec(cur + 1, used));
}
