...
signed main(void) { cin.tie(0); ios::sync_with_stdio(false);
    ...
    if(n %2 == 0){
        ...
        rep(i, n) if(i%2) cin>>a[i/2]; else cin>>b[i/2];
        ruiseki <int> ra(a), rb(b);
        dump(ra.s, rb.s);
        rep(i, n/2+1){
            chmax(ans, rb.get(0, i)+ra.get(i, INF));
            dump(i, rb.get(0, i), ra.get(i, INF));
        }
        print(ans);
    }else{
        vector <int> a(n);
        cin>>a;
        vector<vector<vector<int>>> dp(2, vector<vector<int>>(5, vector<int>(n+10, -INF)));
        //使ったか使ってないか、二つ飛ばし何回やったか
        dp[1][0][0] = a[0];
        dp[0][1][0] = 0;
        for(int i=1;i<n;i++){
            rep(j, 2){
                rep(k, 3){
                    //j=0の時使わない
                    if(j == 0){
                        chmax(dp[j][k][i], dp[1][k][i-1]);
                        chmax(dp[j][k+1][i], dp[0][k][i-1]);
                    }else{//使う
                        chmax(dp[1][k][i], dp[0][k][i-1] + a[i]);
                    }
                }
            }
        }
