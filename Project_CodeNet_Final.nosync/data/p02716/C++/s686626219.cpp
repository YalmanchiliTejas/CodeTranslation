...
signed main() {
    ...
    if (N & 1) {
        ...
        rep(i, N / 2) rep(j, 3) {
            chmax(DP[i + 1][j], DP[i][j] + A[i * 2 + j]);
            if (j + 1 < 3) chmax(DP[i + 1][j + 1], DP[i][j] + A[i * 2 + j]);
        }
        cout << max({DP[N / 2][0], DP[N / 2][1], DP[N / 2][2]}) << rt;
    } else {
        ll DP[N / 2 + 1][2];
        rep(i, N / 2 + 1) rep(j, 2) DP[i][j] = -INF;
        DP[0][0] = DP[0][1] = 0;
        rep(i, N / 2) rep(j, 2) {
            chmax(DP[i + 1][j], DP[i][j] + A[i * 2 + j]);
            if (j + 1 < 2) chmax(DP[i + 1][j + 1], DP[i][j] + A[i * 2 + j]);
        }
        cout << max(DP[N / 2][0], DP[N / 2][1]) << rt;
    }
}
