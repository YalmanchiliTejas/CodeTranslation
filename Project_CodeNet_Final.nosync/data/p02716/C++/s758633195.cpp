...
ll solve(ll now,ll path,ll cnt){
  if(now>=N){
    return (cnt==N/2)?0:-inf;
  }
  if(path>N%2+1)return -inf;
  if(dp[now][path]!=-inf)return dp[now][path];
  ll i,res=-inf;
  if(N&1){
    rep(i,3)chmax(res,solve(now+i+2,path+i,cnt+1));
    return dp[now][path]=res+v[now];
  }else{
    rep(i,2)chmax(res,solve(now+i+2,path+i,cnt+1));
    return dp[now][path]=res+v[now];
  }
}
int main(){
  ...
  rep(i,N)cin>>v[i];
  if(N&1){
    rep(i,3)chmax(ans,solve(i,i,0));
    print(ans);
  }else{
    rep(i,2)chmax(ans,solve(i,i,0));
    print(ans);
  }
}