...
ll N;
vll A(300000, -1*INF);
vll even(300000, 0), odd(300000, 0);
vll esum(300000, 0), osum(300000, 0);
...
int main(){
  ...
  for(int i = 0; i < N; i++){
    cin >> A.at(i);
    if(i%2==0) even.at(i) = A.at(i);
    else odd.at(i) = A.at(i);
  }
  for(int i = 0; i < N; i++){
    esum.at(i+1) = esum.at(i) + even.at(i);
    osum.at(i+1) = osum.at(i) + odd.at(i);
  }
  //偶数桁まで
