...
int main() {
    ...
    constexpr ll LMINF = numeric_limits<ll>::min();
    vector dp(n+1, vector(3, LMINF));
    dp[0][0] = 0;
    REP(i, n) {
        REP(j, 3) {
            if (dp[i][j] == LMINF) continue;
            RANGE(k, j, min(j+1, 2LL)) {
                if (j == k || i % 2 != k % 2) {
                    chmax(dp[i+1][k], dp[i][j] + (i % 2 == k % 2 ? a[i] : 0));
                    // cerr << "(" << i << "," << j << "): " << dp[i][j] << " -> (" << i+1 << "," << k << "): " << dp[i+1][k] << "\n";
                }
            }
        }
    }
    // DUMP(dp);
    ll ans = max(dp[n][n%2], dp[n][n%2+1]);
    say(ans);
