...
#undef DEFINE_OUTPUT
// Variables:
const int debug = 0, NMAX = 256 * 1024;
// -----------------------------------------------------------------------------
int n;
vi a;
const ll INF = (ll)1e18L;
std::map<pii,ll> dp;
ll solve(int pos, int need) {
    // we can set any bit on pos
    pii key = {pos,need};
    auto it = dp.find(key);
    if (it != dp.end()) {
        return it->second;
    ...
    if (need * 2 - 1 > pos + 1) {
        ...
        res = solve(pos-1,need);
        if (need > 0) {
            std::remax(res, solve(pos-2,need-1)+a[pos]);
        }
    }
    //watchsp(pos); watchsp(need); watchln(res);
    return dp[key] = res;
}
int main() {
    while (std::cin >> n) {
        a.resize(n); std::cin >> a;
        dp = {};
        std::cout << solve(n-1, n/2) << std::endl;
    }
    return 0;
}