...
main() {
  ...
  repi(n + 1) repj(3) dp[i][j] = LLONG_MIN / 4;
  dp[0][0] = dp[0][1] = 0;
  repi(n) {
    updatemax(dp[i + 1][0], dp[i][0] + (i % 2 == 0 ? a[i] : 0));
    updatemax(dp[i + 1][1], dp[i][1] + (i % 2 == 1 ? a[i] : 0));
    updatemax(dp[i + 1][2], dp[i][2] + (i % 2 == 0 ? a[i] : 0));
    if (i % 2 == 0) updatemax(dp[i + 1][1], dp[i][0]);
    if (i % 2 == 1) updatemax(dp[i + 1][2], dp[i][1]);
  }
  // repj(3){
  //   repi(n + 1) cout << dp[i][j] << " ";
  //   cout << endl;
  // }
