...
ll solve3(const V<ll> &alist, ll n, const V<ll> &sumlist1, const V<ll> &sumlist2)
{
    SegmentTree<ll> tree(100005, -1e18, maxll);
    rep (i, n / 2 + 1) {
        ll sum = sumlist1[n/2+1] - sumlist1[i+1];
        sum += sumlist2[i];
        tree.update(i, sum);
    }
...
#endif
        ret = max(ret, sum + sum2 - sumlist2[i]);
    }

    return ret;
}

void solve_odd(const V<ll> &alist, ll n)
{
    V<ll> sumlist1;
    ll sum = 0;
    sumlist1.pb(sum);
    ...
    debug_printf("--- sumlist1\n");
    debug_print(sumlist1);
    ...
    debug_printf("--- sumlist2\n");
    debug_print(sumlist2);

    ll ans = -1e18;

    rep (i, n / 2 + 1) {
        ans = max(ans, sumlist1[n/2+1] - alist[i * 2]);
    }

    ans = max(ans, sumlist2[n/2]);

    rep (i, n / 2 + 1) {
        ans = max(ans, sumlist1[i] + sumlist2[n/2] - sumlist2[i]);
    }

    rep (i, n / 2 + 1) {
        ans = max(ans, sumlist2[i] + sumlist1[n/2+1] - sumlist1[i+1]);
    }

    ans = max(ans, solve3(alist, n, sumlist1, sumlist2));

    debug_printf("---- ans\n");
    cout << ans << endl;
}

void solve_even(const V<ll> &alist, ll n)
{
    V<ll> sumlist1;
    ll sum = 0;
    sumlist1.pb(sum);
...
void _main()
{
    ...
    if (n <= 3) {
        ll ret = -1e18;
        for (auto a : alist) ret = max(ret, a);
        cout << ret << endl;
        return;
    }

    if (n & 1LL) solve_odd(alist, n);
    else solve_even(alist, n);
}
