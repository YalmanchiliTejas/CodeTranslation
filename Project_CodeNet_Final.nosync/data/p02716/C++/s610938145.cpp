...
{
    int n; cin>>n;
    vector dp(n+1, vector(5,vector<long long>(3,INT64_MIN/2)));
    dp[0][1][1+(n&1)]=0;
    for(int i=0; i<n; ++i)
    {
        auto& crt=dp[i];
        ...
        for(int j=0; j<=3; ++j)
        {
            for(int k=0; k<=2; ++k)
            {
                if(k or !j) chmax(nxt[j+1][k-(j>0)],crt[j][k]);
                if(j) chmax(nxt[0][k],crt[j][k]+nval);
            }
        }
    }
    long long ans=INT64_MIN;
    for(int j=0; j<=3; ++j) chmax(ans,dp[n][j][j>0]);
    cout << ans << "\n";
}