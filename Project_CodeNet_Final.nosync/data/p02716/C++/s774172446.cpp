...
*/
  ...
  dp[1][0] = a[1];
  for (int i = 1; i < n/2; i++) {
    dp[1][i] = (dp[0][i-1] > dp[1][i-1] ? dp[0][i-1] : dp[1][i-1]) + a[i*2+1];
  }
  dp[2][0] = a[2];
  for (int i = 1; i < n/2; i++) {
    dp[2][i] = (dp[0][i-1] > dp[1][i-1] ? (dp[0][i-1] > dp[2][i-1] ? dp[0][i-1] : dp[2][i-1]) : (dp[1][i-1] > dp[2][i-1] ? dp[1][i-1] : dp[2][i-1])) + a[i*2+2];
  }
