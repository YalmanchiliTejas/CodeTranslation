...
void solve() {
	...
	auto f=[&](auto&& f,ll index, ll yuuyo,ll count){
		if(N<=index)return 0LL;
		if(dp[index][yuuyo]!=-INF)return dp[index][yuuyo];
		if(count==N/2)return 0LL;
		ll ans=-INF;
		for(ll i=0;i<=yuuyomax-yuuyo;i++){
			ll t=f(f,index+i+2,yuuyo+i,count+1)+a[index];
			chmax(ans,t);
		}
		return dp[index][yuuyo]=ans;
	};
