...
#endif
//todo fora_fを基本隠し、fora_fを使っているところは書き換える
...
template<class T> void sort(vector<T> &a, int l = -1, int r = -1) { set_lr12(l, r, sz(a)); sort(a.begin() + l, a.begin() + r); }
template<class T> void rsort(vector<T> &a, int l = -1, int r = -1) { set_lr12(l, r, sz(a)); sort(a.begin() + l, a.begin() + r, greater<T>()); };
...
#define debi(A) {was_deb=true;cerr<<debugName(A)<<" = "<<endl;rep(i, sz(A))cerr<<std::right << std::setw((int)(sz(tos(A[i]))+(i ? 1 : 0)))<<(i%10);cerr<<endl;rep(i, sz(A))cerr<<std::right << std::setw((int)(sz(tos(A[i]))+(i ? 1 : 0)))<<A[i];cerr<<endl;}
...
template<class T, class F> vector<T> sub(const vector<vector<T> >& A, int h, int w, int ah,int aw, F f){
    vector<T> res;
...
template<class T> vector<T> sub(const vector<vector<T> >& A, int h, int w, int ah,int aw){
