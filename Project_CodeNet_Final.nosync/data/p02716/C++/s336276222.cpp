...
int main(){cin.tie(0);ios::sync_with_stdio(false);
	int N;cin>>N;
	vvl d(N+1,vl(3,-e18)),D(N+1,vl(2,-e18));
	vl e(N+1,-e18);
	d[0][0]=D[0][0]=0;
	foor(i,1,N){
		ll A;cin>>A;
		...
		if(i>=3){
			chmax(d[i][1],d[i-3][0]+A);
			chmax(d[i][2],d[i-3][1]+A);
			chmax(D[i][1],D[i-3][0]+A);
		}
		if(i>=4){
			e[i]=d[i-4][0]+A;
	...
	if(N%2){
		print(max({d[N-2][0],d[N-1][1],d[N][2],e[N],D[N-1][0],D[N][1]}));
	}else{
		print(max({d[N-1][0],d[N][1],D[N][0]}));
	}
	return 0;
}
