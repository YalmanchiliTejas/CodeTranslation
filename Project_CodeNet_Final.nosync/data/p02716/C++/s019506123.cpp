...
void solve(){
  int n; cin >> n;
  V<ll> a(n); cin >> a;
  V3<ll> dp(n+5, V2<ll>(2,V<ll>(2,-LINF)));
  //dp[0][0][0] = 0;
  dp[0][1][0] = 0;
  FOR(i,0,n){
    if(i%2==0){
      if(dp[i][0][0]>-LINF)dp[i+1][0][1] = dp[i][0][0] + a[i];
      dp[i+1][0][0] = max(dp[i][1][1], dp[i][1][0]);
      if(dp[i][1][0]>-LINF)dp[i+1][1][1] = dp[i][1][0] + a[i];
    }else{
      dp[i+1][0][0] = max(dp[i][0][0], dp[i][0][1]);
      dp[i+1][1][0] = max(dp[i][1][0], dp[i][1][1]);
      if(dp[i][0][0]>-LINF)dp[i+1][1][1] = dp[i][0][0] + a[i];
    }
  }
  coutn(max(dp[n][(n%2)^1][0], dp[n][(n%2)^1][1]));
}
