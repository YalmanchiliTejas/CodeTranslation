...
int a[nax];
map <int, ll> with[nax], without[nax];
void crop(map <int, ll> &x) {
	auto it = x.end();
	REP(_, 5) if (it != x.begin()) it--;
	x.erase(x.begin(), it);
}
int main() {
	int n;
	...
	REP(i, n) {
		...
		crop(without[i]);
		without[i + 1] = with[i];
		for (auto [c, v] : without[i]) {
			if (!without[i + 1].count(c)) without[i + 1][c] = -llinf;
			maxi(without[i + 1][c], v);
		}
		for (auto [c, v] : without[i]) {
			if (!with[i + 1].count(c + 1)) with[i + 1][c + 1] = -llinf;
			maxi(with[i + 1][c + 1], v + a[i]);
		}
		debug(arr(with, i + 1), arr(without, i + 1), range(a, 0, i + 1));
		for (auto [c, v] : with[i + 1]) if (c == n / 2) maxi(ans, v);
		for (auto [c, v] : without[i + 1]) if (c == n / 2) maxi(ans, v);
	}
	printf("%lld\n", ans);
}
//#STAY AT HOME
