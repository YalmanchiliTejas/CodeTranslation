...
signed main(){
    ...
    for(int i=0;i<N;i++){
        for(int j=0;j<3;j++){
            ...
            if(i+2==N){
                if(j+1<=2) chmax(dp[N][j+1],dp[i][j]+a[i]);

            }else if(i+2==N+1) chmax(dp[N][j],dp[i][j]+a[i]);
            else chmax(dp[i+2][j],dp[i][j]+a[i]);
        }
    }
    //vout(dp);
    if(N%2==0) cout<<dp[N][1]<<endl;
