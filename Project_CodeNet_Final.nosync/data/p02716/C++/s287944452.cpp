...
template<class T, class U> constexpr bool chma(T &a, const U &b) {    if (a < b) {        a = b;        return true;    }    return false;}
template<class T, class U> constexpr bool chmi(T &a, const U &b) {    if (b < a) {        a = b;        return true;    }    return false;}
...
public:
    ...
    constexpr bool has_value() const { return !is_null; }
    //todo
#ifndef _DEBUG
...
template<class T> struct ov{
    ...
    optional<T> &operator()(int i) {if (i < 0 || sz(v) <= i) {return emp;} else { return v[i]; }}
...
//演算子==
//optional<int>(10) == 10
//todo まずconstexprを付ける

//null同士を比較する状況はおかしいのではないか
bool operator==(const nullopt_t &, const nullopt_t&){assert2(0, "nul == null  cant hikaku");return false;}
template<class T> constexpr bool operator==(const nullopt_t &, const T&){return false;}
template<class T> constexpr bool operator!=(const nullopt_t &, const T&){return true;}
template<class T> constexpr bool operator==(const T&, const nullopt_t &){return false;}
template<class T> constexpr bool operator!=(const T&, const nullopt_t &){return true;}

//template<class O, class T> constexpr bool operator==(const optional<O>& opt, const T& tem){if(opt.has_value()){return opt.value()==tem;}else return nullopt == tem;}
template<class O, class T> constexpr bool operator==(const optional<O>& opt, const T& tem){return opt.has_value() ? opt.value()==tem : nullopt == tem;}
template<class O, class T> constexpr bool operator!=(const optional<O>& opt, const T& tem){return opt.has_value() ? opt.value()!=tem : nullopt == tem;}
template<class O, class T> constexpr bool operator==(const T& tem, const optional<O>& opt){return opt.has_value() ? opt.value()==tem : nullopt == tem;}
template<class O, class T> constexpr bool operator!=(const T& tem, const optional<O>& opt){return opt.has_value() ? opt.value()!=tem : nullopt == tem;}


template<class Ol, class Or> constexpr bool operator==(const optional<Ol>& opl, const optional<Or>& opr){    if(opl.has_value() != opr.has_value()){        return false;    }else if(opl.has_value()){        return opl.value() == opr.value();    }else {        return nullopt == nullopt;    }}
template<class Ol, class Or> constexpr bool operator!=(const optional<Ol>& opl, const optional<Or>& opr){return !(opl == opr);}

//nullを

constexpr nullopt_t& operator +(const nullopt_t &, const nullopt_t&) {return nullopt;}
constexpr nullopt_t& operator -(const nullopt_t &, const nullopt_t&) {return nullopt;}
constexpr nullopt_t& operator *(const nullopt_t &, const nullopt_t&) {return nullopt;}
constexpr nullopt_t& operator /(const nullopt_t &, const nullopt_t&) {return nullopt;}
constexpr nullopt_t& operator +=(const nullopt_t &, const nullopt_t&) {return nullopt;}
...
template<class ANY> constexpr nullopt_t operator+(const nullopt_t&, const ANY &) {return nullopt;}
template<class ANY> constexpr nullopt_t operator-(const nullopt_t&, const ANY &) {return nullopt;}
template<class ANY> constexpr nullopt_t operator*(const nullopt_t&, const ANY &) {return nullopt;}
template<class ANY> constexpr nullopt_t operator/(const nullopt_t&, const ANY &) {return nullopt;}
template<class ANY> constexpr nullopt_t operator+(const ANY &, const nullopt_t &) {return nullopt;}
template<class ANY> constexpr nullopt_t operator-(const ANY &, const nullopt_t &) {return nullopt;}
template<class ANY> constexpr nullopt_t operator*(const ANY &, const nullopt_t &) {return nullopt;}
template<class ANY> constexpr nullopt_t operator/(const ANY &, const nullopt_t &) {return nullopt;}

template<class ANY> constexpr void operator+=(nullopt_t &, const ANY &) {}
template<class ANY> constexpr void operator-=(nullopt_t &, const ANY &) {}
template<class ANY> constexpr void operator*=(nullopt_t &, const ANY &) {}
template<class ANY> constexpr void operator/=(nullopt_t &, const ANY &) {}
template<class ANY> constexpr void operator+=(ANY &, const nullopt_t &) {}
template<class ANY> constexpr void operator-=(ANY &, const nullopt_t &) {}
template<class ANY> constexpr void operator*=(ANY &, const nullopt_t &) {}
template<class ANY> constexpr void operator/=(ANY &, const nullopt_t &) {}
...
//+-/*
//todo 速くできる
template<class T, class O> constexpr optional<O> operator+(const T &tem, const optional<O> &opt) {    if (opt.has_value()) { return optional<O>(tem + opt.value()); }    return optional<O>();}
template<class T, class O> constexpr optional<O> operator-(const T &tem, const optional<O> &opt) {    if (opt.has_value()) { return optional<O>(tem - opt.value()); }    return optional<O>();}
template<class T, class O> constexpr optional<O> operator*(const T &tem, const optional<O> &opt) {    if (opt.has_value()) { return optional<O>(tem * opt.value()); }    return optional<O>();}
template<class T, class O> constexpr optional<O> operator/(const T &tem, const optional<O> &opt) {    if (opt.has_value()) { return optional<O>(tem / opt.value()); }    return optional<O>();}

template<class T, class O> constexpr optional<O> operator+(const optional<O> &opt, const T &tem) { return tem + opt; }
template<class T, class O> constexpr optional<O> operator-(const optional<O> &opt, const T &tem) { return tem - opt; }
template<class T, class O> constexpr optional<O> operator*(const optional<O> &opt, const T &tem) { return tem * opt; }
template<class T, class O> constexpr optional<O> operator/(const optional<O> &opt, const T &tem) { return tem / opt; }
...
//(a+=null) != (a=a+null)
// a            null
template<class T, class O> constexpr void operator+=(T &tem, const optional<O> &opt) { if (opt.has_value()) { tem += opt.value(); }}
template<class T, class O> constexpr void operator-=(T &tem, const optional<O> &opt) { if (opt.has_value()) { tem -= opt.value(); }}
template<class T, class O> constexpr void operator*=(T &tem, const optional<O> &opt) { if (opt.has_value()) { tem *= opt.value(); }}
template<class T, class O> constexpr void operator/=(T &tem, const optional<O> &opt) { if (opt.has_value()) { tem /= opt.value(); }}

template<class T, class O> constexpr void operator+=(optional<O> &opt, const T &tem) { if (opt.has_value()) { opt.value() += tem; }}
template<class T, class O> constexpr void operator-=(optional<O> &opt, const T &tem) { if (opt.has_value()) { opt.value() -= tem; }}
template<class T, class O> constexpr void operator*=(optional<O> &opt, const T &tem) { if (opt.has_value()) { opt.value() *= tem; }}
template<class T, class O> constexpr void operator/=(optional<O> &opt, const T &tem) { if (opt.has_value()) { opt.value() /= tem; }}

template<class O> constexpr void operator+(optional<O> &opl, const optional<O> &opr) { if (opr.has_value()) { return opl + opr.value(); }}
template<class O> constexpr void operator-(optional<O> &opl, const optional<O> &opr) { if (opr.has_value()) { return opl - opr.value(); }}
template<class O> constexpr void operator*(optional<O> &opl, const optional<O> &opr) { if (opr.has_value()) { return opl * opr.value(); }}
template<class O> constexpr void operator/(optional<O> &opl, const optional<O> &opr) { if (opr.has_value()) { return opl / opr.value(); }}

template<class Ol, class Or> constexpr void operator+=(optional<Ol> &opl, const optional<Or> &opr) { if (opr.has_value()) { return opl += opr.value(); }}
template<class Ol, class Or> constexpr void operator-=(optional<Ol> &opl, const optional<Or> &opr) { if (opr.has_value()) { return opl -= opr.value(); }}
template<class Ol, class Or> constexpr void operator*=(optional<Ol> &opl, const optional<Or> &opr) { if (opr.has_value()) { return opl *= opr.value(); }}
template<class Ol, class Or> constexpr void operator/=(optional<Ol> &opl, const optional<Or> &opr) { if (opr.has_value()) { return opl /= opr.value(); }}
...
//null , optional, T
constexpr bool chma(nullopt_t &, const nullopt_t &) { return false; }
template<class T> constexpr bool chma(T &opt, const nullopt_t &) { return false; }
template<class T> constexpr bool chma(nullopt_t &, const T &opt) { return false; }
template<class T> constexpr bool chma(optional<T> &olv, const optional<T> &orv) { if (orv.has_value()) { return chma(olv, orv.value()); } else return false; }
...
//value_typeを持つ場合呼べる
//len回要素を追加する
template<class Iterable, class T = typename Iterable::value_type> Iterable tin(int len) {
    Iterable ret;
    T tem;
    while (len--) {
        cin >> tem;
...
template<class T> string deb_tos(const vector<vector<T> > &a, vi H, vi W, int key = -1) {
    stringstream ss;
    ss << endl;
    vi lens(sz(W));
    fora(h, H) {
...
template<class T> string deb_tos(const vector<vector<T> > &a, ll H = inf, ll W = inf, int key = -1) {
...
template<class T> string deb_tos(const vector<vector<vector<T> > > &a, ll H = inf) {
    stringstream ss;
    if (H == inf)H = DEB_LEN_H;
    H = min(H, sz(a));
    rep(i, H) {
        ss << endl;
...
template<class T> string deb_tos(vector<set<T> > &a, ll H = inf, ll W = inf, int key = -1) {
...
template<class T, class F> string deb_tos_f(vector<vector<T> > &a, F f, int key = -1) {
    vi hs, ws_;
    int H = sz(a), W = sz(a[0]);
    vi exh(H), exw(W);
    rep(h, H) {
...
template<class T, class F> string deb_tos_f(vector<vector<vector<T>>> &a, F f) {
    stringstream ss;
    int H = sz(a);
    if (sz(a) == 0)return ss.str();
...
__int128 to_bint(string &s) {
    __int128 ret = 0;
    for (ll i = 0; i < (ll) s.length(); ++i) if ('0' <= s[i] && s[i] <= '9') ret = 10 * ret + s[i] - '0';
    return ret;
}
...
ll rand(ll min, ll max) {
    assert(min <= max);
    if (min >= 0 && max >= 0) { return rand() % (max + 1 - min) + min; } else if (max < 0) { return -rand(-max, -min); } else { if (rand() % 2) { return rand(0, max); } else { return -rand(0, -min); }}
}
...
int div(nanoseconds a, nanoseconds b) {
    if (b < nanoseconds(1)) { return a / nanoseconds(1); }
    int v = a / b;
    return v;
}
//set_time();
...
vi ranvinc(ll n, ll min, ll max) {
    ...
    while (bad) {
        ...
        rep(i, n) {
            ...
            if (i)v[i] = v[i - 1] + rand(min, max - v[i - 1]); else v[i] = rand(min, max);
        }
    }
    return v;
}
...
template<class Iterable, class T = typename Iterable::value_type> vector<pair<T, int>> run_length(const Iterable &a) {
    vector<pair<T, int>> ret;
    ret.eb(a[0], 1);
    rep(i, 1, sz(a)) { if (ret.back().fi == a[i]) { ret.back().se++; } else { ret.eb(a[i], 1); }}
    return ret;
}
...
public:
    ...
    template<typename U> static Type normalize(const U &x) {
        Type v;
        if (-mod() <= x && x < mod()) v = static_cast<Type>(x); else v = static_cast<Type>(x % mod());
        if (v < 0) v += mod();
        return v;
    }
    ...
    template<typename U = T> typename enable_if<is_same<typename Modular<U>::Type, signed>::value, Modular>::type &operator*=(const Modular &rhs) {
        ...
        uint32_t xh = static_cast<uint32_t>(x >> 32), xl = static_cast<uint32_t>(x), d, m;asm("divl %4; \n\t": "=a" (d), "=d" (m): "d" (xh), "a" (xl), "r" (mod()));value = m;
#else
        value = normalize(static_cast<int64_t>(value) * static_cast<int64_t>(rhs.value));
#endif
        return *this;
    }
    template<typename U = T> typename enable_if<is_same<typename Modular<U>::Type, int64_t>::value, Modular>::type &operator*=(const Modular &rhs) {
...
mint com(int n, int r) {
    if (r < 0 || r > n) return 0;    /*nが大きくてrが小さい場合、nを上からr個掛ける*/    if (n >= sz(fac)) { if (n <= 4e6)setmod_resize(n + 1000); else return com_big(n, r); }
    return mint(finv[r] * fac[n] % MOD * finv[n - r]);
}
mint pom(int n, int r) {/*    if (!sz(fac)) com(0, -1);*/    if (r < 0 || r > n) return 0;
    return mint(fac[n] * finv[n - r]);
}
mint npr(int n, int r) {/*    if (!sz(fac)) com(0, -1);*/    if (r < 0 || r > n) return 0;
    return mint(fac[n] * finv[n - r]);
}
...
int nprin(int n, int r) {/*    if (!sz(fac)) com(0, -1);*/    if (r < 0 || r > n) return 0;
...
//int型 mod m
template<typename T, typename U, typename V> int mpow(const T a, const U b, const V m) {
    assert(b >= 0);
    int x = a, res = 1;
    U p = b;
    while (p > 0) {
...
//-k乗出来る
template<typename T, typename U> mint mpow(const T a, const U b) {/*    assert(b >= 0);*/    if (b < 0) { return minv(mpow(a, -b)); }
    int x = a, res = 1;
    U p = b;
    while (p > 0) {
        if (p & 1) (res *= x) %= MOD;
...
template<typename T, typename U, typename V> int mpowi(const T &a, const U &b, const V &m = MOD) {
    assert(b >= 0);
    int x = a, res = 1;
    U p = b;
    while (p > 0) {
...
template<class T, bool is_char = is_same2(T, char)> struct char_to_int { typedef int type; };
template<class T> struct char_to_int<T, false> { typedef T type; };
template<class Init, class T = typename char_to_int<Init>::type> class rui2 {//@formatter:off
    bool notBuild = true;
    bool dynamic_q = false;
    P dynamic_nex = P(0, 0);
    void build() {        notBuild = false;        tem.assign(H, vector<T>(W));        rep(h, H) { rep(w, W) { tem[h][w] = dat[h + 1][w + 1]; }}        for (int i = 1; i <= H; ++i) { for (int j = 1; j <= W; ++j) { dat[i][j] += dat[i - 1][j] + dat[i][j - 1] - dat[i - 1][j - 1]; }}    }
    void body_add_dynamic(P p, T v = 1) {
        if (notBuild) {
            tem.assign(H, vector<T>(W));
            notBuild = false;
        }
        int h = p.first, w = p.second;
        h++, w++;
        if (h > H || w > W || h <= 0 || w <= 0)return;
        dat[h][w] += dat[h - 1][w] + dat[h][w - 1] - dat[h - 1][w - 1] + v;
        tem[h - 1][w - 1] += v;
    }
public:
    vector<vector<T>> dat;
    vector<vector<T>> tem;
    int H, W;
    rui2(int H_, int W_) : dat(H_ + 1, vector<T>(W_ + 1, 0)), H(H_), W(W_) {}
    rui2(vector<vc> &a, char key = '#') : H(sz(a)), W(sz(a[0])) {        dat.assign(H + 1, vector<T>(W + 1));        rep(h, H) { rep(w, W) { dat[h + 1][w + 1] = a[h][w] == key; }}        build();    }
    rui2(vector<vector<Init>> &a) : H(sz(a)), W(sz(a[0])) {        dat.assign(H + 1, vector<T>(W + 1));        rep(h, H) { rep(w, W) { dat[h + 1][w + 1] = a[h][w]; }}        build();    }
    void add(int h, int w, T v = 1) {        if (!notBuild) {            cerr << "may need add_nynamic" << endl;            throw std::exception();        }        h++, w++;        if (h > H || w > W || h <= 0 || w <= 0)return;        dat[h][w] += v;    }
    //追加しながらbuildする
    // 左上から順番に埋める必要がある((hi<=h,wi<=w)がすべて埋まっていればいい)
    /*@formatter:on*/

    void add_dynamic(int h, int w, T v = 1) {
        dynamic_q = true;
        static string message = "add_dynamic h,w  not increase";
        assert2(h < H && w < W, message);
        P now = P(h, w);
        auto inc_nex = [&]() {
            if (dynamic_nex.second < W - 1)dynamic_nex.second++;
            else {
                dynamic_nex.first++;
                dynamic_nex.second = 0;
            }
        };
        assert(dynamic_nex <= now);
        while (dynamic_nex < now) {
            body_add_dynamic(dynamic_nex, 0);
            inc_nex();
        }
        body_add_dynamic(dynamic_nex, v);
        inc_nex();
    }
    /*@formatter:off*/
    const vector<T> &operator[](int i) { return tem[i]; }
    //縦 query
    T sumh_ng(int w, int sh, int gh) { return sum_ng(sh, gh, w, w + 1, true); }
    T sumh_ok(int w, int sh, int gh) { return sum_ok(sh, gh, w, w + 1, true); }
    //横 query
    T sumw_ng(int h, int sw, int gw) { return sum_ng(h, h + 1, sw, gw, true); }
    T sumw_ok(int h, int sw, int gw) { return sum_ok(h, h + 1, sw, gw, true); }
    //はみ出しng  最終的にここで集計する
    T sum_ng(int sh, int gh, int sw, int gw, bool debug = true) {
        if (gw > W || gh > H) return 0;
        if (sw < 0 || sh < 0) return 0;
        if (sh >= gh || sw >= gw) return 0;
        if (notBuild)build();
        if (dynamic_q && mp(gh - 1, gw - 1) >= dynamic_nex){
            add_dynamic(gh - 1, gw - 1, 0);
        }
        T rui_res = dat[gh][gw] - dat[sh][gw] - dat[gh][sw] + dat[sh][sw];
/*@formatter:off*/
#ifdef _DEBUG
        if (debug) {            if (sh + 1 == gh) {                deb(tem);                int h = sh;                cerr << "yoko : " << h << endl;                deb(sw, gw);                deb(rui_res);            } else if (sw + 1 == gw) {                deb(tem);                int w = sw;                cerr << "tate : " << w << endl;                deb(sh, gh);                deb(rui_res);            } else {                deb(tem);                deb(sh, gh, sw, gw);                deb(rui_res);            }        }
#endif
        return rui_res;
    }
    //はみ出しok
    /*@formatter:on*/
    T sum_ok(int sh, int gh, int sw, int gw, bool debug = true) {
        if (sh < 0)sh = 0;
        if (sw < 0)sw = 0;
        if (gh > H)gh = H;
        if (gw > W)gw = W;
        return sum_ng(sh, gh, sw, gw, debug);
    }
    /*@formatter:off*/
    T sum_ok(int gh, int gw, bool debug = true){return sum_ok(0,gh,0,gw, debug);}
    T sum_ng(int gh, int gw, bool debug = true){return sum_ng(0,gh,0,gw, debug);}
    //閉区間 左上と右下
    T sum_ok_close(P ul, P dr){return sum_ok(ul.first,dr.first+1,ul.second,dr.second+1);}
    T sum_ng_close(P ul, P dr){return sum_ng(ul.first,dr.first+1,ul.second,dr.second+1);}
};//@formatter:on
//rui2<int> r(vvi);
//rui2<char> r(vvc, key);
...
//    vvi(dp, N + 1, 10, -val);
    ...
    rep(i, 2, N) {
        rep(s, 4) {
            //2つ前
            chma(dp(i, s), dp(i - 2, s) + A[i]);
            if (i - 3 >= 0)chma(dp(i, s + 1), dp(i - 3, s) + A[i]);
            if (i - 4 >= 0)chma(dp(i, s + 2), dp(i - 4, s) + A[i]);
            if (i - 5 >= 0)chma(dp(i, s + 3), dp(i - 5, s) + A[i]);
        }
    }
    deb(dp);
//    if (N % 2) {
