...
int main(){
    ...
    rep(i, 0, N){
        ...
        if(i + 2 < N)chmax(dp[i+2][2],dp[i][2] + A[i+2]);
        if(i + 3 < N)chmax(dp[i+3][1],dp[i][0] + A[i+3]);
        if(i + 3 < N)chmax(dp[i+3][2],dp[i][1] + A[i+3]);
        if(i + 4 < N)chmax(dp[i+4][2],dp[i][0] + A[i+4]);
    }
    // rep(i, 0, N){
    //     cerr << dp[i] << endl;
    // }
    if(N%2 == 0) cout << dp[N-1][1] << endl;
    else cout << dp[N-1][2] << endl;
