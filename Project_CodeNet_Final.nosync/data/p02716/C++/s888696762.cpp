...
inline ll ph(ll i, ll j){
  return i*1000000+j;
}

ll rec(ll i, ll j){
  ll mi = i == 0 ? 1 : (i+1)/2;
  ll ma = i == n-1 ? (n-1)/2 : i/2+1;
  if(!(mi <= j && j <= ma)) return -INF;
  if(j == 1) return a[i];
  if(dp.count(ph(i, j))){
    return dp[ph(i, j)];
  }
  ...
  repr(k, i-4, i-1){
    if(k >= 0){
      chmax(ret, rec(k, j-1)+a[i]);
    }
  }
  return dp[ph(i, j)] = ret;
}

void solve(){
  ll ans = -INF;
  chmax(ans, rec(n-1, n/2));
  chmax(ans, rec(n-2, n/2));
  chmax(ans, rec(n-3, n/2));
  //dump(dp);
  print(ans);
}
