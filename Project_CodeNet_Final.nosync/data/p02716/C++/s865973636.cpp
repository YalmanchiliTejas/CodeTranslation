...
int rec(int pos,int taken){
    if(pos<0){
        if(taken)return -1ll*hell*hell;
        return 0;
    }
    if(dp.count(mp(pos,taken)))return dp[{pos,taken}];
    int rem=n-1-pos;
    int mxtk=(rem+1)/2;
    if(taken>(pos+2)/2)return -1ll*hell*hell;
    if(taken+mxtk<n/2)return -1ll*hell*hell;
    int donttake=rec(pos-1,taken);
    int takeit=rec(pos-2,taken-1)+a[pos];
    // trace(pos,taken,donttake,takeit);
    return dp[{pos,taken}]=max(donttake,takeit);
}
void meowmeow321()
{
    cin>>n;
    inputarr(a,n);
    cout<<rec(n-1,n/2)<<endl;
}
signed main()
{
    FAST;
