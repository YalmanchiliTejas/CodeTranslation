...
signed main() {
	int n; cin >> n;
	vel a(n);
	rep(i, n) { cin >> a[i]; a[i] += p1; }
	V<unordered_map<int, int>> use(n + 1);
	V<unordered_map<int, int>> nuse(n + 1);
	rep(i,n){
		for (int j = max(n / 2 - ((n - i + 1) / 2),(int)0); j <= (i + 1) / 2; j++) {
			mmax(nuse[i + 1][j], max(use[i][j], nuse[i][j]));
			mmax(use[i + 1][j + 1], nuse[i][j]+a[i]);
		}
	}
	cout << max(use[n][n / 2], nuse[n][n / 2])-(n/2)*p1 << endl;
	return 0;
}
