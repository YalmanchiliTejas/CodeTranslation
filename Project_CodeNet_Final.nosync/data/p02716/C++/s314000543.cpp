...
int main() {
	...
	ll n; cin >> n;
	vll a(n); cin >> a;
	vector<map<pair<bool, ll>, ll>> dp(n);
	dp[0][{false, 0}] = 0;
	dp[0][{true, 1}] = a[0];
	dp[0][{false, 1}] = -INF;
	dp[0][{true, 0}] = -INF;
	
	auto get_or = [](auto& mp, pair<bool, ll> key, ll val = -INF) -> ll {
		auto it = mp.find(key);
		return it != mp.end() ? it -> second
			: val;
	};
	rep(i, 1, n) {
		ll k = i / 2;
		dp[i][{false, k}] = max(get_or(dp[i - 1], { true, k }), get_or(dp[i - 1], {false, k}));
		dp[i][{true, k}] = get_or(dp[i - 1], {false, k-1}) + a[i];
		dp[i][{false, k + 1}] = max(get_or(dp[i - 1], { false, k + 1 }), get_or(dp[i - 1], {true, k+1}));
		dp[i][{true, k+1}] = get_or(dp[i - 1], { false, k}) + a[i];
	}
	cout << max(dp[n - 1][{false, (n)/2}], dp[n - 1][{true, (n) / 2}]) << endl;
