...
ll dp[200200][3];
bool visited[200200][3];

ll rec(int cur, int yoyuu) {
    if (yoyuu < 0) return -INFF;
    if (cur >= N) {
        // if (yoyuu) return -INFF;
        return 0;
    }
    if (visited[cur][yoyuu]) return dp[cur][yoyuu];
    visited[cur][yoyuu] = true;

    ll ret = -INFF;
    if (yoyuu > 0) {
        chmax(ret, rec(cur + 1, yoyuu - 1));
    }
    chmax(ret, A[cur] + rec(cur + 2, yoyuu));

    return dp[cur][yoyuu] = ret;
}
