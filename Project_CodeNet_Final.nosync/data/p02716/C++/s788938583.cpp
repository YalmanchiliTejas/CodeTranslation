  ...
  while(cin >> N){
    ...
    if(N % 2 == 0){
...

      {
        ...
        chmax(ans, accumulate(ALL(so), 0LL) - *min_element(ALL(so)));
      }


      vector<i64> so(N+1), se(N+1);
      ...
      REP(i,N){
        if(i % 2 == 0){
          chmax(ans, so[i] - so[0] + so[N] - so[i+1]);
          chmax(ans, so[i] - so[0] + se[N] - se[i+1]);
          chmax(ans, se[i] - se[0] + so[N] - so[i+1]);
          chmax(ans, se[i] - se[0] + se[N] - se[i+1]);
        }
      }

      {
        ...
        REP(j,3){
          REPE(i,N+2){
            if(dp[i][j] != LLONG_MIN) cerr << setw(4) << dp[i][j] << " ";
            else cerr << "---- ";
          }
          cerr << endl;
          }*/
