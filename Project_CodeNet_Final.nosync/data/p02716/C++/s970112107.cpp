...
unordered_map<int, i64> memo[200010];
i64 dp(int i, int cnt) {
    if (i >= N) {
        return (cnt == N / 2) ? 0 : -LINF;
    }
    if (cnt > N / 2 || cnt < i / 2 - 1) return -LINF;

    if (memo[i].count(cnt)) return memo[i][cnt];

    i64 ret = -LINF;
    chmax(ret, dp(i + 1, cnt));
    chmax(ret, dp(i + 2, cnt + 1) + a[i]);
