...
//--------------------------------------------------------------------------------//
ll A[200005], dp[200005][3], amax[3];
int main() {
    init();
    ll N;
    cin >> N;
    ...
    if(N%2==0){
        rep(i,N/2){
            ll a = 2 * i;
            dp[i + 1][1] = max(dp[i][0], dp[i][1]) + A[a + 1];
            dp[i + 1][0] = dp[i][0] + A[a];
        }
        ...
        rep(i,N/2){
            ll a = 2 * i;
            amax[0] = dp[i][0];
            rep(j, 2) amax[j + 1] = max(amax[j], dp[i][j + 1]);
            rep(j, 3) dp[i + 1][j] = amax[j] + A[a + j];
        }
        cout << *max_element(dp[N / 2], dp[N / 2] + 3) << endl;
    }
}