...
void solve() {
    ...
    vector<vector<pair<int, int>>> winnings;
    REP(i, n) {
        int a, b, c;
        cin >> a >> b >> c;
        zahyo.push_back(mp(b, c));
        winnings.push_back(vector<pair<int, int>>());
        REP(q, a){
            string c;
            cin >> c;
            int d;
            cin >> d;
            winnings.back().push_back(goods[c]);
            winnings.back().back().second -= d;
        }
    }
    auto calc = [](pair<int, int> now) {
        return abs(now.first) + abs(now.second);
    };
    REP(i, n) {
        dist[1 << i][i] = calc(zahyo[i]);
    }
    auto distance = [calc](pair<int, int> a, pair<int, int> b) {
        return calc(mp(a.first - b.first, a.second - b.second));
    };
    for (int i = 1; i < (1 << n); ++i) {
        REP(j, n) {
            for (int q = 0; q < n; ++q) {
                if (i & (1 << q)) continue;
                dist[(1 << q) + i][q] = min(dist[(1 << q) + i][q], dist[i][j] + distance(zahyo[j], zahyo[q]));
            }
        }
    }
    vector<pair<int, int>> knaps;
    int dp[100001] = {};
    REP(i, (1 << n)) {
        if (i == 0) continue;
        REP(q, 50001) {
        ...
        REP(q, n) {
            if (!((1 << q) & i)) continue;
            for (auto x : winnings[q]) {
                REP(j, w + 1) {
                    dp[j + x.first] = max(dp[j + x.first], dp[j] + x.second);
                }
            }
        }
        int maxdist = 1e9;
        REP(q, n) {
            maxdist = min(maxdist, dist[i][q] + calc(zahyo[q]));
        }
        knaps.push_back(mp(maxdist, dp[w]));
    }
    REP(q, 50001) {
        dp[q] = 0;
    }
    REP(q, knaps.size()) {
        REP(y, t - knaps[q].first + 1) {
            dp[y + knaps[q].first] = max(dp[y + knaps[q].first], dp[y] + knaps[q].second);
        }
    }
    cout << dp[t] << endl;
    return;
