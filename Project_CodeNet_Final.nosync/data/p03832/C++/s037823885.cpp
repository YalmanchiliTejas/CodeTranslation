...
//---------------------------------------------------------------
template<int MOD> struct ModInt { long long x;
    ...
    ModInt &operator*=(const ModInt &r) { x = x * r.x % MOD; return *this; }
...
int main() {
    ...
    for (int i = 1; i <= n; i++) for (int j = 0; j <= n; j++) {
        ...
        for (int k = c; k <= d; k++) {
            int rem = j - i * k;
            if (rem < 0) break;
            mint mul = com.P(n - rem, i * k) * com.ifac(i).pow(k) * com.ifac(k);
            dp[i][j] += dp[i - 1][rem] * mul;
        }
    }
    cout << dp[n][n] << endl;
    return 0;
