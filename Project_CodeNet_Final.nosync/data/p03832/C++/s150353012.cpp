...
const int MF = 1000005, MOD = 1000000007;
    ...
    for (int j = A; j <= B; j++)
    {
      ...
      for (int k = i - C * j; k >= i - D * j; k -= j)
      {
        if (k >= 0)
        {
          ...
          // if (i == 2 && j == 1)
          // {
          //   cout << binom(N - k, sel) << " " << dp[k][j - 1] << " " << sel << " " << j << " " << tmp << endl;
          // }
          dp[i][j] += binom(N - k, sel) * dp[k][j - 1] * fact(sel) / (fact(j).pow(tmp)) * inv_fact(tmp);
        }
      }
    }
  }
