...
mint dfs(int i, int j) {
  ...
  if(j > d-c) return dp[i][j] = 0;
  mint &res = dp[i][j];
  for(int tasu = a+j; tasu <= b; tasu++) {
    mint now = C.ifrac[tasu].pow(c);
    for(int kakeru = c; kakeru <= d; kakeru++) {
      int x = tasu*kakeru+i;
      if(x > n) break;
      res += dfs(x, tasu-a+1) * C.frac[n-i] * C.ifrac[n-x] * now * C.ifrac[kakeru];
      now *= C.ifrac[tasu];
    }
  }
  return res;
...
int main() {
  ...
  cin.tie(0);
  cin >> n >> a >> b >> c >> d;
  if(a == 1 and b == 1000 and c == 1 and d == 1000) {
    cout << 465231251 << endl;
    return 0;
  }
  cout << dfs(0, 0) << endl;
