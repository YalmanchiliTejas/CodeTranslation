...
template <class F, class S> auto operator-(pair<F, S> a) {
    a.first = -a.first, a.second = -a.second;
    return a;
}
template <class F, class S> auto operator+(pair<F, S> a, const pair<F, S> &b) {
...
template <class F, class S, class T> auto operator*=(pair<F, S> a, const T &b) {
...
int main() {
    ...
    int n, a, b, c, d;
    in(n, a, b, c, d);
    using inte = intm<int, int(1e9 + 7)>;
    auto f = vect<inte>({}, n + 1, n + 1);
    lp(i, 0, n) f[i][0] = 1;
    vector<inte> g(n + 1);
    auto comb = f;
    lp(i, 0, n) lp(j, 0, i) comb[i][j] = inte::binary_coefficient(i, j);
    g[0] = 1;
    lp(i, 1, n) g[i] = 1 / inte(i) * g[i - 1];
    lp(i, 1, n) {
        lp(j, 1, n) {
            // fij i sizes j students
            if (i < a || i > b)
                f[i][j] = f[i - 1][j];
            else {
                f[i][j] = f[i - 1][j];
                ...
                for (int k = 1; k * i <= j; ++k) {
                    ...
                    if (k >= c && k <= d)
                        f[i][j] +=
                            f[i - 1][j - k * i] * tmp * g[k] * comb[j][k * i];
                    // ou(k, f[i][j]);
                }
            }
            // ou(i, j, f[i][j]);
