        ...
        for (int i = 0; i < N; ++i) {
            for (int j = 0; j <= N; ++j) {
                ...
                for (Int k = C; k <= D && j + k * (i + 1) <= N; ++k) {
                    ...
                    dp[i + 1][j + k * (i + 1)] += dp[i][j] * cmb.fact[j + k * (i + 1)]
                                                  * cmb.ifact[i + 1].pow(k) * cmb.ifact[j] * cmb.ifact[k];
//                    dp[i + 1][j + k * (i + 1)] += dp[i][j] * cmb.nPk(N - j, k * (i + 1))
//                                                  / cmb.fact[i + 1].pow(k) / cmb.fact[k];
//                    cerr << N - j << "P" << k * (i + 1) << " / (" << i + 1 << ")! ^ " << k << " / " << k << "!" << endl;
//                    cerr <<
//                         (cmb.nPk(N - j, k * (i + 1))
//                          / cmb.fact[i + 1].pow(k)
//                          / cmb.fact[k]).value()
//                         << endl;
//                    cerr << ((cmb.nPk(N - j, k * (i + 1)) * cmb.ifact[i + 1].pow(k)) * cmb.ifact[k]).value() << endl;
                }
            }
        }
