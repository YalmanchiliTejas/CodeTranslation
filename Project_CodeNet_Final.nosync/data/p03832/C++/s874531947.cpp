...
int main(){
    ...
    cin >> N >> A >> B >> C >> D;
    create_mod_tables(N);
    static int64_t dp[1001];
    dp[N] = 1;
    for(int n=A; n<=B; n++) for(int i=0; i<=N; i++) for(int g=C; g<=D && i-g*n>=0; g++){
        int64_t res = perm_mod(i, g*n) * power_mod(fact_inv[n], g) % MOD * fact_inv[g] % MOD;
        add(dp[i-g*n], res * dp[i] % MOD);
    }
    cout << dp[0] << endl;
}
