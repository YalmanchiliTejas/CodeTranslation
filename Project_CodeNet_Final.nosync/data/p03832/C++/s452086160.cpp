...
ll C(ll a,ll b)
{
    return fac[a][1]*rev(fac[b][1])%MOD*rev(fac[a-b][1])%MOD;
}
int main()
{
    int n,a,b,c,d;
    ...
    while(scanf("%d%d%d%d%d",&n,&a,&b,&c,&d)!=EOF)
    {
        ...
        FOR(i,a,b)
        {
            FOR(j,0,n)
            {
                ...
                {
                    int pre = j-num*i;
                    dp[i][j] = (dp[i][j]+dp[i-1][j-num*i]*C(n-j+num*i,num*i)%MOD*fac[num*i][1]%MOD*rev(fac[i][num]*fac[num][1]%MOD)%MOD)%MOD;
                    num++;
                }
            }
        }
