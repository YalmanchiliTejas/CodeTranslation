...
#endif
//#STAY AT HOME
const int mod = 1e9 + 7, nax = 1e3 + 44;
ll fac[nax], fin[nax];
ll powe(ll b, ll e) {
	e = fix(e, mod - 1);
	ll r = 1;
...
void solve() {
	...
	for (int x = a; x <= b; ++x) {
		...
		for (int i = c; i <= d; ++i) {
			if (x * i > n) continue;
			ll cou = fac[x * i] * fin[i] % mod * powe(fac[x], -i) % mod;
			// debug(imie(x), imie(i), imie(cou));
			for (int s = 0; s + x * i <= n; ++s)
				add(dp2[s + x * i], dp[s] * cou % mod * cho(s + x * i, x * i));
		}
		dp = move(dp2);
		// debug(imie(x), "done", imie(dp));
	}
	printf("%lld\n", dp[n]);
}
void init() {
