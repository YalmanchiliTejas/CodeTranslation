...
int main() {
    ...
    int N, A, B, C, D; re(N, A, B, C, D);

    vmn dp(N + 1); dp[0] = 1;
    FOR (gsz, A, B + 1) {
        F0Rd (pc, sz(dp)) FOR (ng, C, D + 1) {
            if (pc + ng * gsz > N) break;
            dp[pc + ng * gsz] += dp[pc] * mn::ncr(N - pc, ng * gsz)
                * mn::fact(ng * gsz) * mn::finv(gsz).pow(ng) * mn::finv(ng);
        }
    }
    ps(dp.back());
