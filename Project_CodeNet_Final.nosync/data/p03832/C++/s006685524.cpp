...
#define for_each__2(a,right) do{fora(v,a){v right;}}while(0)
#define for_each__3(a,v, siki) do{fora(v,a){siki;}}while(0)
...
template<typename T, typename U, typename V> T mpow(const T a, const U b, const V m) {    assert(b >= 0);    int x = a, res = 1;    U p = b;    while (p > 0) {        if (p & 1) (res *= x) %= m;        (x *= x) %= m;        p >>= 1;    }    return res;}
//-k乗出来る
...
fps mul_sparse(fps& l,fps& r,int msize=-1){    vi inds;    rep(i, r.size()){        if(r[i]){            inds.push_back(i);        }    }    assert(sz(inds));    int n = l.size()+inds.back();    if(msize==-1){        msize = n-1;    }    fps ret(msize+1);    rep(i, l.size()){        for(auto d : inds){            if(i+d> msize)break;            ret[i+d] += l[i] * r[d];        }    }    return ret;}
//右から疎なfpsで割る、O(N) * 右の項数
fps div_sparse(fps l, fps &r, int msize = -1) {    vi inds;    rep(i, r.size()) { if (r[i]) { inds.push_back(i); }}    assert(sz(inds));    int n = l.size();    if (msize == -1) { msize = n - 1; }    msize = min(msize, n - 1);    fps ret(msize + 1);    rep(i, l.size()) {        if (l[i]) {            assert(i >= inds[0]);            mint dec_k = l[i] / r[inds[0]];            if (i - inds[0] > msize)break;            ret[i - inds[0]] += dec_k;            for (auto d : inds) {                if (i + d - inds[0] >= n)break;                l[i + d - inds[0]] -= dec_k * r[d];            }        }    }    return ret;}
...
// (x^l - x^r)^n
fps pow_term2(int l, int r, int n, int msize) {    if(l>r)swap(l,r);    fps ret = pow_term2(r - l, n, msize);    /*(x^l)^n * (1-x^(r-l))^n*/    ret.rshift(l * n);    if (sz(ret) > msize + 1)ret.resize(msize + 1);    return ret;}
