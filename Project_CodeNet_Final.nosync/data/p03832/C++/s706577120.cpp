/*
Date: 2017/01/15 19:44:55 Sunday
*/
#include<bits/stdc++.h>
using namespace std;
#define X first
...
int main(){
	...
	jc[0]=1;
	rep(i,1,1010)jc[i]=jc[i-1]*i%mo;
	rep(i,0,1010)inv[i]=powmod(jc[i],mo-2);
	CC[0][0]=1;
	rep(i,1,1010){
		CC[i][0]=1;
		rep(j,1,i)CC[i][j]=(CC[i-1][j]+CC[i-1][j-1])%mo;
	}
	dp[A-1][0]=1;
	rep(i,A,B){
		rep(j,0,n)if(j==0||j>=C&&j<=D){
			dep(k,n-j*i,0){
				ll q=dp[i-1][k]*CC[n-k][i*j]%mo;
				q=q*jc[i*j]%mo*powmod(inv[i],j)%mo*inv[j]%mo;
				dp[i][k+j*i]=(dp[i][k+j*i]+q)%mo;
			}
		}
		//rep(j,0,n)printf("%d %d %d\n",i,j,dp[i][j]);
	}
	cout<<dp[B][n]<<endl;
	return 0;
}