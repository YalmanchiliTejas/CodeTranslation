	...
	for (ll i = A; i <= B; i++) {
		for (ll j = 0; j <= N; j++) {
			...
			for (ll k = C; k <= D; k++) {
				ll n = j - k * i;
				if (n < 0) break;
				ll c = dp[i - 1][n] * modfac[N - n] % MOD;
				c = c * /*invmod(modfac[N - j], MOD)*/ modfacinv[N - j] % MOD;
				c = c * /*invmod(modfac[k], MOD)*/ modfacinv[k] % MOD;
				c = c * invmod(modpow(modfac[i], k, MOD), MOD) % MOD;
				dp[i][j] = (dp[i][j] + c) % MOD;
			}
		}
	}
