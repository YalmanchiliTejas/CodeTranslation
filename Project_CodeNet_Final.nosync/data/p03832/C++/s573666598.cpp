...
constexpr int64_t pow(int64_t x, int64_t n, int64_t mod) {
    if (n == 0) {
        return 1;
    } else {
        int64_t ret = pow(x, n / 2, mod) % mod;

        return (ret * ret % mod) * ((n & 1) ? x : 1) % mod;
    }
}
    ...
    for (int i = a; i <= b; ++i) {
        for (int j = 1; j <= n; ++j) {
            ...
            for (int k = c; k <= d && i * k <= j; ++k) {
                dp[i][j] += dp[i - 1][j - i * k] * fact[n - j + i * k] % mod *
                            fact_inv[n - j] % mod * pow(fact_inv[i], k, mod) %
                            mod * fact_inv[k] % mod;
                dp[i][j] %= mod;
            }
        }
