...
void _main()
{
    ...
    S ll perms[5000];
    S ll perm_invs[5000];
    make_perms(perms, perm_invs, 5000, MOD);

    S ll perm_inv_pows[5000][5000];
    rep (base, 5000) {
        ll val = 1LL;
        rep (pow, 5000) {
            perm_inv_pows[base][pow] = val;
            val *= perm_invs[base];
            val %= MOD;
        }
    }
    ...
    srep (u, umin, umax+1) {
        ...
        srep (gcnt, gmin, gmax+1) {
            rep (to, 1003 - gcnt * u) {
                ...
                coef *= perms[from];
                coef %= MOD;
                coef *= perm_invs[from-u*gcnt];
                coef %= MOD;
                coef *= perm_invs[gcnt];
                coef %= MOD;
                coef *= perm_inv_pows[u][gcnt];
                coef %= MOD;

                ll incr = dp_prev[from] * coef;
                incr %= MOD;
                dp[to] += incr;
                dp[to] %= MOD;
