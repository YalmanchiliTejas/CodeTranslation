...
//  LOCAL_OUTPUT; // ファイルに出力したいときのみ
  ...
  // テキトー考察
  //
  // dp[i][j] := (a + i)人のグループを作ろうとしていて、今までj人が分けられている通り数.
  // dp[i + 1][j] += dp[i][j], dp[i + 1][j + k * (a + i)] += dp[i][j] (c <= k <= d)
  // なので、O(n^3)になると思う.
  // j + k * (a + i) <= n , k <= (n - j) / (a + i)
  // なんかO(n^2 log n)らしい
  // なんか漸化式の構成より、計算量解析のほうが難易度が高い気がした.
  //
  // わかりやすい解説記事
  // http://drken1215.hatenablog.com/entry/2018/06/14/151800
  //
  cin >> n >> a >> b >> c >> d;
  rep(i, 1, n + 1) {
    ll now = i;
  ...
  rep(i, 0, b - a + 1) {
    rep(j, 0, 1001) {
      pinv[j] = mod_inverse(ModPow(f[a + i], j), MOD, true);
    }
    rep(j, 0, n + 1) {
