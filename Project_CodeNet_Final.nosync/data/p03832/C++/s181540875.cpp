...
ll pick(int from, int quan, int team){
	// cout << "PICK " << "from " << from << ' ' << "quan " << quan << ' ' << "team " << team << ' ' <<  C(from, quan) * fa[quan] % p * pow(fainv[quan/team], team, p) % p * inv[team] % p << '\n';
	return fa[from] * fainv[from-quan] % p * pow(fainv[quan/team], team, p) % p * fainv[team] % p;
	return C(from, quan) * fa[quan] % p * pow(fainv[quan/team], team, p) % p * inv[team] % p;
}
signed main(){
	...
	for(int per = gl;per <= gr;++per, cur ^= 1){
		...
		// for(int i = 1;i <= n;++i)cout << dp[cur][i] << " \n"[i == n];
	}
	cout << dp[!cur][n] << '\n';
}