...
struct ModFact{
    ...
    constexpr ModFact(const int sz = 200000) noexcept : fact(sz + 1), ifact(sz + 1) {
        fact[0] = mint(1);
        for(int i = 1; i <= sz; ++i)fact[i] = fact[i - 1] * mint(i);
        ifact[sz] = fact[sz].inv();
        for(int i = sz; i > 0; --i)ifact[i - 1] = ifact[i] * mint(i);
    }
    constexpr mint operator [] (const int id) const noexcept { return fact[id]; }
    constexpr mint operator () (const int n, const int r) noexcept {
        return fact[n] * ifact[n - r] * ifact[r];
    }
    constexpr mint nPr(const int n, const int r) noexcept {
    ...
    rep(i, 0, N){
        ...
        rep(j, 0, N){
            ...
            rep(k, 1, min(D, N / i)){
                ...
                now *= fac(pe, pe - i);
                pe -= i;
                if(k >= C)dp[i + 1][j + i * k] += dp[i][j] * now * fac.ifact[k];
            }
        }
    }
