...
//狭義の単調増加列 長さを返す
template<class T> int lis_up(vector<T> &a) {    int n = sz(a);    vi tail(n + 1, MAX(T));    rep(i, n) {        int id = lowerIndex(tail, a[i]);/**/        tail[id] = a[i];    }    return lowerIndex(tail, MAX(T));}
...
void solve() {
    din(n, a, b, c, d);
    vvm(dp, n + 1, n + 2);//[n]人をi人以上でさばく じょうきょうにする
    dp[n][0] = 1;
