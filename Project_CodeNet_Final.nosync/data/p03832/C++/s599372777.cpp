  ...
  REP(i, N + 1) {
    REP(j, B - A + 2) {
      ...
      FOR(k, C, D + 1) {
        int a = i + num * k;
        if (a > N) break;
        modint ch = math.C(N - i, num * k);
        modint div = modint(math.factinv[num]).pow(k) * math.fact[num * k];
        modint non = math.factinv[k];
        modint mul = ch * div * non;
        dp[a][j + 1] += dp[i][j] * mul;
      }
    }
  }
  io.writeln(dp[N][B - A + 1].x);
  return 0;
}