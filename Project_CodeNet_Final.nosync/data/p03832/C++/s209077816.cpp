...
ll n, a, b, c, d, dp[1001], kai[1001], kai_inv[1001];

int main() {
  scanf("%lld%lld%lld%lld%lld", &n, &a, &b, &c, &d);
  kai[0] = 1;
  for (ll i = 1; i <= n; i++) kai[i] = (kai[i-1] * i) % mod;
  for (ll i = 0; i <= n; i++) kai_inv[i] = invmod(kai[i], mod);
  dp[0] = 1;
  ll ans = 0;
  for (ll i = a; i <= b; i++) {
    ...
    for (ll x = n; x >= 0; x--) {
      ll temp = 1;
      for (ll m = 0; m < c; m++) temp = (temp * kai_inv[i]) % mod;
      for (ll m = c; m <= d; m++) {
        if (x - m * i < 0) break;
        ll mul = kai[n - x + m * i] * temp % mod;
        mul = mul * kai_inv[n - x] % mod;
        //printf("%lld(%lld, %lld, %lld, %lld!!\n", mul, n-x+m*i, n-x, i, temp);
        mul = mul * kai_inv[m] % mod;
        dp[x] = (dp[x] + dp[x-m*i] * mul) % mod;
        temp = (temp * kai_inv[i]) % mod;
      }
      //if (i == b && x == n) ans = (ans + dp[x]) % mod;
      //if (i == b && x == n && dp[x]) printf("%lld, %lld, %lld\n", x, dp[x]);
    }
  }
  printf("%lld\n", dp[n]);
}
