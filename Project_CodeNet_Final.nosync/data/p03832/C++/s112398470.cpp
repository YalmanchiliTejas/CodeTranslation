...
int main(){
  ...
  f[0]=1;
  FOR(i,1001) f[i+1]=(f[i]*(i+1))%MOD;
  ios::sync_with_stdio(0); cin.tie(0);
  lli n,a,b,c,d; cin>>n>>a>>b>>c>>d;
  dp[a][0]=1;
  FORU(i,a,b) {
    FOR(j,n+1) dp[i+1][j]=dp[i][j];
    FORU(j,c,d) if(j*i <= n) FOR(k,n+1) if(dp[i][k]&&k+j*i<=n) {
        (dp[i+1][k+j*i] += mul(dp[i][k],mul(cnk[n-k][i*j],mul(f[i*j],invmod(mul(f[j],fexp(f[i],j))))))) %= MOD;
      }
  }
  cout << dp[b+1][n] << endl;
  return 0;
