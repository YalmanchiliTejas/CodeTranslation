...
int thedge_set(set<ll> S,ll x){//独立集合Sのいずれかと頂点xの間に辺があるか
	S.insert(x);
	string s="";
	rep(i,n)s+="0";
	for(auto t:S)s[t]='1';
...
void dfs(ll x,set<ll> S,ll score){//xとSの間の辺を探す(一つはある）
	if(S.size()==1){
		FANS_A.push_back(x);
	...
	ll vl=thedge_set(L,x);
	if(vl)dfs(x,L,vl);
	if(score==1&&vl==1)return;
	ll vr;
	if(score==1)vr=1;
	else vr=thedge_set(R,x);
	if(vr)dfs(x,R,vr);
}
int main(){
	cin>>n;
