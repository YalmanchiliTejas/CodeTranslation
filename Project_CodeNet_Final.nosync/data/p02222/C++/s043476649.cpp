...
void solve(){
  ...
  auto dfs = [&](auto rec,int cur,int par = -1) -> int{
    ...
    each(dst , g[cur]){
      if(dst == par) continue;
      ret.pb(rec(rec , dst , cur) + 1);
    }
    int n = *max_element(all(ret));
    return dp[cur] = n;
  };
  dfs(dfs , 0);
  trc(dp);
  ...
  int mi = 2;
  auto efs = [&](auto rec,int pval,int cur,int par =-1) -> void{
    vp ret;
    ret.eb(pval , par);
    each(dst , g[cur]){
      if(dst == par) continue;
    ...
    if(ret.size() > 2){
      if(ret[2].first != 0)
        if(ret[0].first != ret[1].first || ret[1].first != ret[2].first )
          amax(mi , ret[0].first + ret[2].first + 1);
        else 
          amax(mi , ret[0].first + ret[2].first);
    }
    each(dst , g[cur]){
      if(dst == par) continue;
      rec(rec , ret[0 + (ret[0].se == dst)].first + 1 , dst , cur);
    }
  };
  efs(efs , 0 , 0);
  trc(mi);
  string ans(N ,'1');
  for(int i = 3 ; i < min(mi,N) ; i++){
    ans[i - 1] = '0';
  }
  out(ans);
}
