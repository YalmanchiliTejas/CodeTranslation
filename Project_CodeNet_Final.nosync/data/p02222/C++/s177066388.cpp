...
signed main(){
    ...
    while(q.size()){
        d2=q.front();
        q.pop();
        each(i,g[d2])if(chmin(cost1[i],cost1[d2]+1)){
            q.push(i);
            back1[i]=d2;
        }
    }
    ...
    while(q.size()){
        ll at=q.front();
        q.pop();
        each(i,g[at])if(chmin(cost2[i],cost2[at]+1)){
            q.push(i);
            back2[i]=at;
        }
    }
    ll second=0;
    rep(n)if(i!=d2&&g[i].size()==1)chmax(second,cost1[i]);
    rep(n)if(i!=d1&&g[i].size()==1)chmax(second,cost2[i]);
    rep(i,second+1,n+1)ans[i]=1;
    if(second==cost1[d2]){
        ans[second]=1;
        rep(n)if(cost1[i]==second||cost2[i]==second)if(i!=d1&&i!=d2)if(cost1[i]!=second||cost2[i]!=second){
            ans[second]=0;
            break;
        }
    }
