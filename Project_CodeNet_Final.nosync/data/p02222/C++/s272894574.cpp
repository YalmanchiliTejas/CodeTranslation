...
int main(){
  ...
  ll n;cin>>n;
  using AR=array<ll,3>;
  auto f1=[&](AR arr1,AR arr2){
    vector<ll>tmp(6);
    rep(i,0,3)tmp[i]=arr1[i];
    rep(i,0,3)tmp[i+3]=arr2[i];
    sort(ALLR(tmp));
    return AR({tmp[0],tmp[1],tmp[2]});
  };
  auto f2=[&](AR arr1,ll val){
    return AR({arr1[0]+val,0,0});
  };
  ReRooting<AR,ll>reroot(n,f1,f2,{0,0,0});
  rep(i,0,n-1){
    ll a,b;cin>>a>>b;a--;b--;
  ...
  rep(i,0,n){
    if(v[i][2]<=0)continue;
    //cout<<v[i][0] spa v[i][1] spa v[i][2]<<endl;
    chmax(mx,min(2*v[i][0],v[i][0]+v[i][2]+1));
  }
  //cout<<mx<<endl;
  string ret(n,'0');
  rep(i,0,n){
    if(i==0||i==1||i+1>=mx)ret[i]='1';
  }
  cout<<ret<<endl;
  return 0;
}
