...
int depth_dfs(int now,int back) {
    ...
    for (auto x : vertexs[now]) {
        if (x == back) continue;
        depth[now] = max(depth[now], depth_dfs(x, now) + 1);
    }
    return depth[now];
}
void dfs(int now, int back, int value) {
    vector<pair<int, int>> gekoa;
    for (auto x : vertexs[now]) {
        if (x == back) continue;
        gekoa.push_back(mp(depth[x], x));
    }
    if(value != 0)
        gekoa.push_back(mp(value, -1));
    if (gekoa.size() != 0) {
        sort(ALL(gekoa));
        reverse(ALL(gekoa));
        if (gekoa.size() >= 3) {
            int targetter = 0;
            if (gekoa[0].first == gekoa[1].first&&gekoa[1].first == gekoa[2].first) {
                if ((gekoa[0].first + gekoa[2].first) % 2 == 0) {
                    targetter = -1;
                }
            }
            ans = max(ans, (gekoa[2].first + gekoa[0].first) + targetter);
        }
    }
    gekoa.push_back(mp(0, -1));
    sort(ALL(gekoa));
    reverse(ALL(gekoa));

    for (auto x : vertexs[now]) {
        if (x == back) continue;
        int target = gekoa[0].first;
        if (gekoa[0].second == x) {
            target = gekoa[1].first;
        }
        dfs(x, now, target + 1);
    }
    return;
