...
int func_main(){
    var_exp = letterOf(var_K, var_S);
    var_i = Var(1);
    for (int cnt_0=0;cnt_0<var_N.asNumber();cnt_0++){
        if ((letterOf(var_i, var_S) == var_exp)){
            var_ans = Var(var_ans.asString() + letterOf(var_i, var_S).asString());
        } else {
            var_ans = Var(var_ans.asString() + Var("*").asString());
        }
        var_i = (var_i + Var(1));
    }
    cout << var_ans << endl;
