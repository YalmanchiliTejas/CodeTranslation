...
int max_dist(vector<vector<int>> matrix, int i, int j){
    int ans = 0;
    queue<ar<int, 3>> qu;
    qu.push({i,j, -1});
    int n = matrix.size();
    ...
    while(!qu.empty()){
        ...
        ans = max(ans, dist);
        visited[i1][j1] = 1;
        if(i1 > 0 && !visited[i1-1][j1] && matrix[i1-1][j1]){
            visited[i1-1][j1] = 1;
            qu.push({i1-1,j1,dist});
        }
        if(j1 > 0 && !visited[i1][j1-1] && matrix[i1][j1-1]){
            visited[i1][j1-1] = 1;
            qu.push({i1,j1-1,dist});
        }
        if(i1 < (n-1) && !visited[i1+1][j1] && matrix[i1+1][j1]){
            visited[i1+1][j1] = 1;
            qu.push({i1+1,j1,dist});
        }
        if(j1 < (m-1) && !visited[i1][j1+1] && matrix[i1][j1+1]){
            visited[i1][j1+1] = 1;
            qu.push({i1,j1+1,dist});
        }
    }
...
void bfs(vector<vector<ll>> &graph, vector<ll> &dist, ll u){
    dist[u] = 1;
    queue<ll> q;
    q.push(u);
    while(!q.empty()){
