...
int cnt(int level, int eat){
	if(eat <= 0) return 0;
	if(eat == th[level]){
		return mt[level];
	}else{
		if(eat == (th[level] + 1)/2){
			return 1 + cnt(level-1, th[level-1]);
		}else if(eat < (th[level] + 1)/2){
			return cnt(level - 1, eat-1);
		}else{
			return 1 + mt[level - 1] + cnt(level - 1, eat - 2 - th[level-1]);
		}
	}
}	
