...
void solve(){
    /*
    height: h[i + 1] = 2 * h[i] + 3
    meats: m[i + 1] = 2 * m[i] + 1
    - if x == h[i] + 2 then cnt += m[i] + 1 and break
    - if x == 1 then cnt += 0 and break
    - if x == h[i + 1] then cnt += m[i + 1] and break.
    - if 1 < x < h[i] + 2 then x = x - 1 and i = i - 1
    - if h[i] + 2 < x < h[i + 1] then x = x - h[i] - 2 and i = i - 1, cnt += m[i] + 1
    */

    h[0] = 1, m[0] = 1;
    rep(i, n) h[i + 1] = 2 * h[i] + 3, m[i + 1] = 2 * m[i] + 1;

    ll idx = n - 1;
    while (true){
        if (idx + 1 == 0) {answer++; break;}
        else if (x == h[idx] + 2) {answer += m[idx] + 1; break;}
        else if (x == 1) break;
        else if (x == h[idx + 1]) {answer += m[idx + 1]; break;}
        else if (1 < x && x < h[idx] + 2){x--; idx--;}
        else if (h[idx] + 2 < x && x < h[idx + 1]){x -= h[idx] + 2; answer += m[idx] + 1; idx--;}
    }
