...
vector<pair<llong, llong>> levels;

llong dfs(int level, llong X)
{
	if (level == 0)
	{
		return X;
	...
	if (X < 1) { return pat; } X--;
	if (X < levels[level - 1].first) { return pat + dfs(level - 1, X); } X -= levels[level - 1].first; pat += levels[level - 1].second;
	if (X < 1) { return pat; } X--; pat++;
	if (X < levels[level - 1].first) { return pat + dfs(level - 1, X); } X -= levels[level - 1].first; pat += levels[level - 1].second;
	if (X < 1) { return pat; } X--;
...
{
	{
		levels.emplace_back(1, 1);
		FOR(i, 1, 50)
		{
			const auto b = levels.back();
			levels.emplace_back(b.first * 2 + 3, b.second * 2 + 1);
		}
	}
	READ(llong, N, X);
