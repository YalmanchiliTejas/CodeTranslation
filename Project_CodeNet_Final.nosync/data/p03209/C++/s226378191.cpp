...
ll atu[60], p[60];
ll burger(ll level, ll X){
	if(!level) return 0<X;
	if(X<=1+atu[level-1]) return burger(level-1, X-1);
	else return p[level-1] + 1 + burger(level-1, X-2-atu[level-1]);
}

int main() {
	in(ll, N, X);
	p[0] = atu[0] = 1;
	loop(i, 55) atu[i+1] = 2*atu[i] + 3;
	loop(i, 55) p  [i+1] = 2*  p[i] + 1;
	out(burger(N, X));
}
