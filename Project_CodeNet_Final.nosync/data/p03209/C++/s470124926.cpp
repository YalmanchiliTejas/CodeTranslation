...
public:
	...
	void set(LL n) {
		if (true) {
			...
			for (LL i = 1;i <= n;i++) {
				this->layer[i] = this->layer[i - 1] * 2 + 3;
				this->meet[i] = this->meet[i - 1] * 2 + 1;
			}
	...
	LL sum(LL x,LL n) {
		...
		if (x <= 1 + this->layer[n - 1]) {
			return this->sum(x - 1, n - 1);
		}

		if (x == 2 + this->layer[n - 1]) {
			return this->meet[n - 1] + 1;
		}

		if (x <= 2 + this->layer[n - 1] * 2) {
			return this->meet[n - 1] + 1 + this->sum(x - 2 - this->layer[n - 1], n - 1);
		}

		return this->meet[n - 1] * 2 + 1;
	}
};
