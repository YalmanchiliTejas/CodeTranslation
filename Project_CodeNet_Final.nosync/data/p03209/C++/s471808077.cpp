...
s64 get_b(int level, s64 left) {
	// cout << level << ":" << left << endl;
	if (level == 0) {
		assert(left < 2);
		return left;
	}
	if (left == 0) return left;
	s64 times = left/num[level];
	// assert(!times);
	s64 res = times*b[level];
	left %= num[level];
	if (left > num[level-1]+1) {
		res++;
		res += b[level-1];
		res += get_b(level-1, left - (num[level-1]+2));
	} else if (left) {
		res += get_b(level-1, left - 1);
	}
	return res;
}
...
int main() {
	...
	b[0] = 1;
	num[0] = 1;
	FOR(i,n) b[i+1] = b[i] * 2 + 1, num[i+1] = num[i]*2 + 3;
