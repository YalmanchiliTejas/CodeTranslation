...
Int calc_P(Int L,Int n){
	if(L==0){
		if(n==0)return 0;
		else return 1;
	}
	if(n>=num[L])return num_P[L];
	if(n==1)return 0;
	n--;
	if(n<=num[L-1])return calc_P(L-1,n);
	n-=num[L-1];
	if(n==1)return 1 + num_P[L-1];
	n--;
	return calc_P(L-1,n) + num_P[L-1] + 1;
}
