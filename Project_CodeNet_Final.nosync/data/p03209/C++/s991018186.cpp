...
vector<ll>size(51),putty(51);
ll dp(ll n,ll x){
    if(x<1)return 0;
    if(n<0)return 0;
    if(x==1)return !n;
    if(x==size[n])return putty[n];
    if(x<size[n-1]+1)return dp(n-1,x-1);
    if(x==size[n-1]+1)return putty[n-1];
    if(x==size[n-1]+2)return putty[n-1]+1;
    return putty[n-1]+1+dp(n-1,x-size[n-1]-2);
}
signed main(){
    size[0]=1;
    putty[0]=1;
    rep(50)size[i+1]=size[i]*2+3;
    rep(50)putty[i+1]=putty[i]*2+1;
    LL(n,x);
    out(dp(n,x));
}
