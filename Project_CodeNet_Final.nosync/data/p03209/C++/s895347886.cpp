...
LL f(LL level, LL X, vector<LL> &patty, vector<LL> &all) {
    if (level == 0) {
        if (X > 0)return 1;
        else return 0;
    }
    if (X <= 0) return 0;
    if (X <= all[level - 1] + 1) return f(level - 1, X - 1, patty, all);
    if (X == all[level - 1] + 2)return patty[level - 1] + 1;
    if (X <= 2 * all[level - 1] + 2) return patty[level - 1] + 1 + f(level - 1, X - all[level - 1] - 2, patty, all);
...
int main() {
    ...
    vector<LL> patty(MAX + 1, 0);
    vector<LL> all(MAX + 1, 0);
    patty[0] = all[0] = 1;
    for (int i = 1; i <= MAX; i++) {
