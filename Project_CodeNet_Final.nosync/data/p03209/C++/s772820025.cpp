...
int len(int L) {
  return (L == 0 ? 1 : 3 + 2 * len(L-1));
}

int pate(int L) {
  return (L == 0 ? 1 : 1 + 2 * pate(L-1));
}

int f(int L, int x) {
  if (L == 0 && x > 0) return 1;
  if (x <= 1) return 0;
  if (x == len(L)) return pate(L-1) * 2 + 1;
  
  if (x-1 < (len(L)-1) / 2) return f(L-1, x-1);
  else return f(L-1, x - (len(L-1)+2)) + pate(L-1) + 1; 
}
