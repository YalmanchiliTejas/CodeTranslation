...
ll pa(ll paty,ll al,ll eat) {
	ll xx = 0;
	if (al == eat)return paty;
	else if (eat <= 0)return 0;
	else if (al == 1)return 1;
	...
	if (eat > al / 2 + 1) {
		return paty - pa((paty - 1) / 2, (al - 3) / 2, (al - eat) - 1);
	}
	else {
		return pa((paty - 1) / 2, (al - 3) / 2, (eat - 1));
	}
