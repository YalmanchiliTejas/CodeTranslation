...
long long solve(long long now, long long p){
    if(now == 0) return p == 1;
    if(p < 2) return 0;
    if(dp[now][1] - 1 <= p) return dp[now][0];
    if(dp[now][1] / 2 + 1 == p) return dp[now - 1][0] + 1;
    if(dp[now][1] / 2 < p) return solve(now - 1, p - (dp[now][1] / 2) - 1) + dp[now - 1][0] + 1;
    else return solve(now - 1, p - 1);
}
