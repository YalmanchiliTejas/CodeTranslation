...
ll rec(int m, ll k) {
	// dbs(m, k);
	assert(0 <= m);
	assert(0 <= k); assert(k <= b[m]);	
	if (m == 0) return k;
	// dbs(b[m], b[m-1]);
	if (k == 0) return 0;	
	else if (k <= 1 + b[m - 1]) return rec(m - 1, k - 1);
	else if (k == 1 + b[m - 1] + 1) return rec(m - 1, b[m - 1]) + 1;
	else if (k < 1 + b[m - 1] + 1 + b[m - 1]) return rec(m - 1, b[m - 1]) + 1 + rec(m - 1, k - (1 + b[m - 1] + 1));
	else return rec(m - 1, b[m - 1]) * 2 + 1;
}
