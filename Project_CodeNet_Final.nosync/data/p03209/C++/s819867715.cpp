...
ll dfs(int level, ll x) {
  ...
  if(x == b[level]) return p[level];
  else if(b[level-1]+2 < x) return p[level-1]+1+dfs(level-1, x-b[level-1]-2);
  else if(x == b[level-1]+2) return p[level-1]+1;
  else if(x > 1) return dfs(level-1, x-1);
  else return 0;
}
