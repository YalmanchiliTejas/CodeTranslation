...
ll thickness[55], patty[55];

ll rec(int level, ll len) {
  if (level == 0) return 1;
  if (len >= thickness[level]) return patty[level];
  if (len > 2 + thickness[level - 1]) return 1 + patty[level - 1] + rec(level - 1, len - 2 - thickness[level - 1]);
  else if (len == 2 + thickness[level - 1]) return 1 + patty[level - 1];
  else if (len <= 1) return 0;
  else return rec(level - 1, len - 1);
}
...
signed main() {
  ...
  thickness[0] = 1;
  REP(i, 52) thickness[i + 1] = thickness[i] * 2 + 3;

  patty[0] = 1;
  REP(i, 52) patty[i + 1] = patty[i] * 2 + 1;
