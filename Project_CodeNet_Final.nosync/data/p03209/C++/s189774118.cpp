...
static M<ll, ll> cache;

ll solve(ll sizes[51], ll level, ll to_eat)
{
    debug_printf("to_eat=%lld level=%lld\n", to_eat, level);
    assert(to_eat <= sizes[level]);

    if (level == 0LL) return (to_eat ? 1LL : 0LL);

    if (to_eat == sizes[level]) {
        if (cache.find(to_eat) != cache.end()) {
            return cache[to_eat];
        }
    }

    if (to_eat <= 1LL) return 0LL;
    if (1LL < to_eat && to_eat <= 1LL + sizes[level-1]) {
        return solve(sizes, level - 1LL, to_eat - 1LL);
    }
    if (to_eat == 1LL + sizes[level-1] + 1LL) {
        return solve(sizes, level - 1LL, to_eat - 2LL) + 1LL;
    }

    ll ret = solve(sizes, level - 1LL, sizes[level-1]) + 1LL;
    ll to_eat_remain = to_eat - 1LL - sizes[level-1] - 1LL;

    if (to_eat_remain <= sizes[level-1]) ret += solve(sizes, level - 1LL, to_eat_remain);
    else {
        assert(to_eat == sizes[level]);
        ret += solve(sizes, level - 1LL, to_eat_remain - 1LL);
        cache[to_eat] = ret;
    }
...
int _main()
{
    ...
    static ll sizes[51];
    sizes[0] = 1LL;
    rep (i, 50) sizes[i+1] = sizes[i] * 2LL + 3LL;
    debug_printf("sizes[2]=%lld\n", sizes[2]);

    cout << solve(sizes, n, x) << endl;
