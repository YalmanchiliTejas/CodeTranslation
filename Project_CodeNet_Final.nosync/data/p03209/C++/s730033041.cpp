...
ll on_bit(ll x, int pos) {x |= (1ll << pos); return x;}
int off_bit(int x, int pos) {x &= ~(1 << pos); return x;}
bool is_on_bit(ll x, int pos) {return ((x & (1ll << pos)) != 0);}
...
ll f_left(int n, ll x);
ll f_right(int n, ll x);
ll f(int n, ll x);
...
ll f(int n, ll x){
	if(n==0)return 1LL;
	if(len[n]==x)return (1ll<<(n+1))-1;
	ll res=f_right(n,min((len[n])/2+1,x));
	if(x>(len[n])/2+1)res+=f_left(n,x-((len[n])/2+1));
	return res;
}
...
int main(){
	...
	sf("%d%lld",&n,&x);
	len[0]=1;
	fr(i,1,51)len[i]=len[i-1]+len[i-1]+3;
	pf("%lld\n",f(n,x));
	return 0;
}
...
/*
	...
	5 BP P PB
	13 BBPPPB P BPPPBB
	29 BBBPPPBPBPPPBB P BBPPPBPBPPPBBB
*/