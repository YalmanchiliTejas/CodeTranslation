...
int s(int n) { return (1LL<<(n+2))-3; }
int c(int n) { return (1LL<<(n+1))-1; }

int f(int n, int x) {
    dump(n,x);
    assert(0 <= n); assert(0 <= x); assert(x <= s(n));
    if (n == 0) return x;
    if (x <= 1) return 0;
    if (x <= 1+s(n-1)) return f(n-1,x-1);
    if (x == 2+s(n-1)) return c(n-1)+1;
    if (x <= 2+2*s(n-1)) return c(n-1)+1+f(n-1,x-(2+s(n-1)));
    if (x == s(n)) return c(n);
    assert(0); return -1;
}
