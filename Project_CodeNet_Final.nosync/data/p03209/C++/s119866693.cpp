...
// code
// ------------------------------------------------
ll solve(ll n, ll x, vll &b ,vll & p){
    ...
    if(x == 1){
        ret = (n == 0 ? 1 : 0);
    } else if(1 < x && x <= b[i - 1] + 1){
        ret = solve(n - 1,x - 1,b,p);
    } else if(x == b[i - 1] + 2){
        ret = p[i - 1] + 1;
    } else if(b[i - 1] + 2 < x && x <= 2 + 2 * b[i-1]){
        ret = solve(n - 1, x - b[i-1] - 2, b ,p) + p[i-1] + 1;
    } else {
        ret = 2 * p[i- 1] + 1;
    }
