...
ll n, x, sum[51], psum[51];

ll search(ll m, ll s) {
  // BxxxPxxxB
  if (s == 0) return 0;
  if (s == sum[m]) return psum[m];
  if (s == sum[m] - 1) return psum[m];
  if (s == 1) return 0;
  if (s <= sum[m-1]) return search(m-1, s-1);
  if (s == sum[m-1] + 1) return psum[m-1];
  if (s == sum[m-1] + 2) return psum[m-1] + 1;
  return psum[m-1] + search(m-1, s-sum[m-1]-2) + 1;
}

int main() {
  scanf("%lld%lld", &n, &x);
  sum[0] = psum[0] = 1;
  for (ll i = 1; i <= n; i++) sum[i] = 3 + sum[i-1] * 2, psum[i] = 1 + psum[i-1] * 2;
  printf("%lld\n", search(n, x));
}
