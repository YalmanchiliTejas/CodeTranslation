...
ll ref(int n,ll x){
     if(x<=1||n==0){if(n==0)return 1; else return 0;}
     if(A.at(n-1)+2>x){return ref(n-1,x-1);}
     if(A.at(n-1)+2==x){return (B.at(n-1)+1);}
 if(A.at(n-1)+2<x){return (B.at(n-1)+1+ref(n-1,x-A.at(n-1)-2));}
}
