...
int main(){
    ll N,X;cin>>N>>X;
    vl dp(N+1,0),sz(N+1,1);
    REP(i,N){
        dp[i+1]=2+2*dp[i];
        sz[i+1]=3+2*sz[i];
    }
    auto bun =[&](auto &&f,ll n,ll x)->ll{
        if(n==0||x==0) return 0;
        if(x==sz[n]) return dp[n];
        ll res=0;
        if(x>sz[n]/2) res=dp[n]/2+f(f,n-1,x-sz[n]/2-1);
        else res=1+f(f,n-1,x-1);
        return res;
    };
    cout<<X-bun(bun,N,X)<<endl;
