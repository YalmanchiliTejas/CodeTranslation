...
ll sizeof_b(int lvl) {
	if (lvl == 0) return 1;
	return sizeof_b(lvl - 1) * 2 + 3;
}

ll sizeof_bp(int lvl) {
	if (lvl == 0) return 1;
	return sizeof_bp(lvl - 1) * 2 + 1;
}

ll go(ll need, int lvl) {
	--need;
	if (!need) return 0;
	ll sz = sizeof_b(lvl - 1);
	ll ans = 0;
	if (need >= sz) {
		ans += sizeof_bp(lvl - 1);
		need -= sz;
