...
ll dfs(ll l, ll s){
  ...
  if(s > pow(2,l+2)-4) return pow(2,l+1)-1;
  if(s>0 && pow(2,l+1)-2>=s) tmp += dfs(l-1, min((ll)pow(2,l+1)-3, s-1));
  if(s>pow(2,l+1)-2) tmp += pow(2,l);
  if(s>pow(2,l+1)-1) tmp += dfs(l-1, s-pow(2,l+1)+1);
  // cout << l << sp <<tmp << rt;
  return tmp;
}
