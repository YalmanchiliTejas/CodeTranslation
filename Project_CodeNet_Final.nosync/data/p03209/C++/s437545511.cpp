...
ll numGu[MAX_level + 1];


// パティ・バン
map <pair <int, ll>, pair <ll, ll>> mMemo;
pair <ll, ll> dfs(int level, ll x)
{
	if (x <= 0) x = 0;
	if (x >= numGu[level]) x = numGu[level];

	if (mMemo.count(make_pair(level, x)))
	{
		return mMemo[make_pair(level, x)];
	}

	pair <ll, ll>& memo = mMemo[make_pair(level,x)];

	if (level==0)
	{
		if (x >= 1)
		{
			return memo = make_pair(1, 0);
		}
		else
		{
	...
	auto l1 = dfs(level - 1, x);
	ret.first += l1.first;
	ret.second += l1.second;

	x = max(0LL, x - l1.first - l1.second);
	...
	auto l2 = dfs(level - 1, x);
	ret.first += l2.first;
	ret.second += l2.second;

	x = max(0LL, x - l2.first - l2.second);
...
int main()
{
	...
	for (int i = 0; i < 50; ++i)
	{
		numGu[i + 1] = numGu[i] * 2 + 3;
	}


	mMemo.clear();

	pair <ll, ll> ret = dfs(N, X);
