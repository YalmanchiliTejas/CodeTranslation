...
void UnionFind::unite(int x, int y) { // xとyの木を併合
  ...
  size[root(x)] += ys;
  size[root(y)] += xs;
  //cout << x << " " << y << " " << size[root(x)] << " " << size[root(y)] << endl; 
...
ll dfs(ll level, ll eat){
  ...
  if(eat <= 1){
    ret = 0;
  }else if(eat <= num[level-1]+1){
    ret = dfs(level-1,eat-1);
  }else if(eat <= num[level-1]+2){
    ret = dfs(level-1,eat-2)+1;
  }else if(eat <= 2*num[level-1]+2){
    ret = dfs(level-1,num[level-1])+1+dfs(level-1,eat-(num[level-1]+2));
  }else{
    ret = 2*dfs(level-1,num[level-1])+1;
  }
