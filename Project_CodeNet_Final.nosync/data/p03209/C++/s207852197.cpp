...
// }}}
// Topological Sort {{{
bool _visit(ll n, vector<ll> &marks, vector<vector<ll>> &edges, vector<ll> &res) {
    if (marks[n] == 1) {
        return false;
    } else if (marks[n] == 0) {
        marks[n] = 1;
...
// 返値は，頂点番号を要素とするベクトル
// 閉路があれば返値の要素数を0にする
vector<ll> topologicalSort(vector<vector<ll>> edges) {
    ll N = edges.size();
    // 結果
    vector<ll> res;
...
ll length(ll n){
    static vector<ll> memo(51, -1);
    if (memo[n] != -1)
        return memo[n];
    else if (n==0)
        return memo[n] = 1;
    else
        return memo[n] = 3 + 2*length(n-1);
}

ll f(ll n, ll x) {
    static map<PLL, ll> memo;
    auto it = memo.find(PLL(n,x));
    if (it != memo.end()) {
        return it->second;
    }

    if (n == 0) {
        return memo[PLL(n,x)] = x>0;
    }

    // 0, length(n-1), 1, length(n-1), 0
    return memo[PLL(n,x)] = f(n-1, minll(length(n-1), maxll(0, x-1))) +
        f(n-1, minll(length(n-1), maxll(0, x-(2+length(n-1))))) + 
        (x>=2+length(n-1));
