...
Ty ipow(Ty x, Ty n) {
	Ty ret = Ty(1);
	for (Ty i = Ty(0); i < n; i++) ret *= x;
	return ret;
}

map<pll, ll> memo;

// level N バーガーの [0, r) にあるパティの数 (0-indexed)
ll countP(ll N, ll r) {
	if (N < 0) return 0;
	//dump(N, r);
	if (memo.find(pll(N, r)) != memo.end())
		return memo[pll(N, r)];
	// 真ん中の P の位置
	ll m = ipow(2LL, N + 1) - 2;
	if (r <= m) {
		ll a = countP(N - 1, r - 1);
		return memo[pll(N - 1, r - 1)] = a;
	}
	ll a = ipow(2LL, N) - 1;
	ll b = countP(N - 1, r - m - 1);
	return memo[pll(N, r)] = a + 1LL + b;
}
