...
//		ll cur=0;
		rep(l,0,n)
		{
//			ll s=(((n+k)&1)?mod-1:1ll)*ksm(2,(k*l+pw2[k])%(mod-1))%mod*C(n,k)%mod;
			ll s=(((n+k)&1)?mod-1:1ll)*SHIT::ksm((k*l+pw2[k])%(mod-1))%mod*C(n,k)%mod;
			(ans+=F[l][n-k]*s%mod)%=mod;
//			rep(i,0,n) (ans+=C(n-k,i)%mod*S[i][l]%mod*s)%=mod;
		}
//		(ans+=(((n+k)&1)?mod-1:1ll)*C(n,i)%mod*C(n-i,k)%mod*ksm(2,ksm(2,k,mod-1))%mod)%=mod; 
	}
//	chktime();
	cout<<ans;
}