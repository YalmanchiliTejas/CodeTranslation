...
vector<int> MODS = { 1000000007, 988244353 }; // 実行時に決まる
...
signed main(){
  ...
  vector<mint> beki(n+1,1),fact(n+1,1),inv(n+1,1);
  ...
  dp[0][0] = 1;
  REP(i,n+1) REP(j,n+1){
    dp[i+1][j] += dp[i][j]; // 取らない
    dp[i+1][j] += dp[i][j]*j; // 取って入れる
    dp[i+1][j+1] += dp[i][j]; // 取って新しく作る
  }
...
int MAX = 3010;
  ...
  REP(i,n+1) REP(j,n+1) assert(dp[i][j]==core[i][j]);

  mint ans = 0;
  REP(i,n+1){
    mint x = 0, y = modpow((mint)2,(n-i)), t = 1;
    mint z = modpow((mint)2,calc(n-i));
    REP(j,i+1) x += dp[i][j]*t*z, t *= y;
    x *= fact[n]*inv[i]*inv[n-i];
    if(i%2) ans -= x;
    else ans += x;
  }
