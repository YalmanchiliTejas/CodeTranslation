...
int main()
{
	...
	while(cin >> n >> mint::mod) {
		...
		RREP(i, n+1) memo[0][i] = 1;
		RREP(i, n+1) memo[1][i] = modpow(2, n-i, mint::mod);;
		FOR(i, 1, n+1) REP(j, n+1) {
			memo[i][j] = memo[i-1][j] * memo[1][j];
		}
		REP(i, n+1) dpite(ALL(memo[i]));

		// choose x group
		auto f = [&](int cn, int x) -> mint {
			DEBUG(memo[x][cn]);
			DEBUG(modpow(2, x * (n-cn), mint::mod));
			return get(cn, x) * memo[x][cn] * ppow[n-cn];
		};
		// auto f = [&](int cn, int x) -> mint {
