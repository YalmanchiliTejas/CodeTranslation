...
struct Combination{
	...
	mll operator () (ll n, ll r){
		if (n<0 || r<0 || n-r<0) return mll(0);
		return this->f[n] * this->g[n-r] * this->g[r];
	}
...
void solve()
{
	...
	vmll t((N/2+1)*(N/2+1)+1); //2^i
	t[0] = 1;
	REPS(i, 1, (N/2+1)*(N/2+1)){
		t[i] = t[i-1]*2;
	}
	...
	vvmll S_(N+1, vmll(N+1));
	auto S = [&](ll P, ll k)->mll&{
		static mll zero=0;
		if (k<0) return zero;
		return S_[P][k];
	};
