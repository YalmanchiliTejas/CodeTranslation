...
vector<mint<>> kaijo;
auto memory = exvector(mint<>(0), 2100, 2100);
mint<> comb(int n, int k) { return kaijo[n] / kaijo[k] / kaijo[n - k]; }
vector<mint<>> powpow2;
vector<mint<>> pow2;
vector<vector<mint<>>> stering;
...
void init() {
  ...
  stering = exvector(mint<>(0), N, N);
  for (LL i = 0; i < N; i++)
    for (LL j = 1; j <= i; j++) {
      if (j == 1 || j == i)
        stering[i][j] = 1;
      else {
        stering[i][j] = stering[i - 1][j - 1] + stering[i - 1][j] * j;
      }
    }
}
...
int main() {
  ...
  for (int k = 0; k <= N; k++) {
    mint<> res = 0;
    for (int p = 0; p <= k; p++) {
      res += stering[k + 1][p + 1] * pow2[(N - k) * p];
    }
    res *= powpow2[N - k];
    if (k % 2 == 1) res *= -1;
    ans += res * comb(N, k);
