...
/* <sr.m.nck> */
	...
	// NOT_VERIFIED
	// O(1), before:O(N)
	inline int nCk(int n, int k, int mod) {
...
/* </foxy.memo-area> */
	...
	N.times(kk) {
		...
		int pyon = 0;
		int block = mpow(2, N - k, M), zoi = 1;

		VI pp = p;
		(N + 1).times(l) {
			p[l] = dig(0ll, pp, l - 1) + pp[l] * (l + 1);
			p[l] %= M;
		...
		int sign = k % 2 == 0 ? +1 : -1;
		ans += sign * nCk(N, k, M) * mpow(2, mpow(2, N - k, M - 1), M) % M * pyon;
		ans %= M;
		if(ans < 0) ans += M;
		ddd(k, ans, sign, nCk(N, k, M));
		ddd(mpow(2, block, M), pyon);
	}
...
/* </foxy.memo-area> */
	...
	for(int kk = 0, kk_rabP_to = N; kk < kk_rabP_to; ++kk) {
		int k = kk + 1;
		...
		VI pp = p;
		for(int l = 0, l_rabP_to = (N + 1); l < l_rabP_to; ++l) {
			p[l] = dig(0ll, pp, l - 1) + pp[l] * (l + 1);
			p[l] %= M;
