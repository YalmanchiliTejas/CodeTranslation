...
int MOD=1000000007; //1000000000000000003LL
...
int main(){
    ...
    while(scanf("%d%d", &n, &m)!=EOF){
        ...
        for(int i=1; i<=n; i++){
            c[i][0]=1;
            for(int j=1; j<=i; j++){
                c[i][j]=MOD_ADD(c[i-1][j-1], c[i-1][j]);
            }
        }
        memset(g, 0, sizeof(g));
        g[0][0]=1;
        for(int i=1; i<=n; i++){
            ...
            for(int j=1; j<=i; j++){
                //discard
                MOD_ADD(&g[i][j], g[i-1][j]);
                //create new
                MOD_ADD(&g[i][j], g[i-1][j-1]);
                //union
                MOD_ADD(&g[i][j], MOD_MUL(g[i-1][j], j));
        ...
        for(int i=0; i<=n; i++){
            int cur=0;
            for(int j=0; j<=i; j++){
                //fprintf(stderr, "[%d][%d]: %d*%d*(%d^%d)\n", i, j, g[i][j], p[n-i], b[n-i], j);
                MOD_ADD(&cur, MOD_MUL(MOD_MUL(g[i][j], p[n-i]), MOD_POW(b[n-i], j)));
            }
