...
void MAIN() {
    ...
    vector<Mint> W(1<<15);
    vector<Mint> X(1<<15);
    REP (k, g+1) X[k] = fact_inv[k+1];
    W[0] = 1;
    for (int i=0; (1<<i) <= a; i++) {
	ntt::ntt(X);
	if (a>>i & 1) {
	    ntt::ntt(W);
	    REP (j, W.size()) W[j] *= X[j];
	    ntt::ntt(W, true);
	    fill(W.begin()+g+1, W.end(), 0);
	}
	REP (j, X.size()) X[j] *= X[j];
	ntt::ntt(X, true);
	fill(X.begin()+g+1, X.end(), 0);
    }

    Mint ans = 0;
    REP (j, g+1) {
	ans += W[j] * fact[j+a] * nPk(g, j) * fact[g-j].pow(2) * nCk(a+g, g-j);
    }
    ans *= fact[a];
