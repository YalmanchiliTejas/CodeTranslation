...
int main() {
  ...
  for(char trump; cin >> trump;) {
    ...
    rep(t, 4) rep(i, 13) { string s; cin >> s; cards[t].push_back(s); }
    ...
    rep(i, 13) {
      ...
      if(trump_ids.size() == 1) {
        ...
        int mxid = -1; char max = '0';
        rep(k, trump_ids.size()) {
          if(less_than(max, cards[trump_ids[k]][i][0])) { max = cards[trump_ids[k]][i][0]; mxid = trump_ids[k]; }
        }
        points[mxid] ++;
        lead_t = mxid;
      }
        ...
        rep(t, 4) {
          if(cards[lead_t][i][1] == cards[t][i][1]) {
            if(less_than(max, cards[t][i][0])) { max = cards[t][i][0]; mxid = t; }
          }
        }
        points[mxid] ++;
        lead_t = mxid;
      }
//      rep(t, 4) cout << cards[t][i] << " "; cout << endl; rep(t, 4) cout << points[t] << " "; cout << endl;
    }
