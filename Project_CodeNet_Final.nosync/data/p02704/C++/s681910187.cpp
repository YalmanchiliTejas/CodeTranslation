...
int h, w;
bool opy[500], opx[500];
uint64_t y[500], x[500], by[500], bx[500];
uint64_t ans[500][500], b[500][500];
...
bool solvebit(){
	...
	if(min(rh, rw) >= 2){
		...
		if(rw == 1){
			...
			bool g = false;
			for(int i = 0; i < h; i++){
				if(b[i][rx[0]] == bx[rx[0]]){ g = true; break;}
				if(b[i][rx[0]] == -1){
					b[i][rx[0]] = bx[rx[0]];
					g = true; break;
				}
			...
			bool g = false;
			for(int i = 0; i < w; i++){
				if(b[ry[0]][i] == by[ry[0]]){ g = true; break; }
				if(b[ry[0]][i] == -1){
					b[ry[0]][i] = by[ry[0]];
					g = true; break;
				}
...
int main(){
	...
	for(int k = 0; k < 64; k++){
		bool g = false;
		for(int i = 0; i < h; i++){
			by[i] = (y[i] & (1uLL << k)) != 0uLL; g |= by[i];
		}
		for(int i = 0; i < w; i++){
			bx[i] = (x[i] & (1uLL << k)) != 0uLL; g |= bx[i];
		}
		if(!g) continue; 
		if(!solvebit()) f = false;
		for(int i = 0; i < h; i++) for(int j = 0; j < w; j++){
			ans[i][j] += b[i][j] * (1uLL << k);
		}
	}
	if(!f){
		cout << -1 << endl;
