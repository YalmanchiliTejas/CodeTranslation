...
bool check(vector<vector<int>> A,vector<int> s,vector<int> t,vector<int> u,vector<int> v){
	int n=A.size();
	rep(i,n){
		int z;
		if(s[i]==AND){
...
pair<bool,vector<vector<int>>> solve(int n,vector<int> s,vector<int> t,vector<int> u,vector<int> v){
	...
	if((s0>0 && t1>0) || (s1>0 && t0>0)) return {false,{}};

	if(R.size()==1){
		int x=u[R[0]];
		rep(j,C.size()){
			if((v[C[j]]==0 && s0>0) || (v[C[j]]==1 && s1>0)) res[R[0]][C[j]]=x;
			else                                             res[R[0]][C[j]]=v[C[j]];
		}
	}
	else if(C.size()==1){
		int x=v[C[0]];
		rep(i,R.size()){
			if((u[R[i]]==0 && t0>0) || (u[R[i]]==1 && t1>0)) res[R[i]][C[0]]=x;
			else                                             res[R[i]][C[0]]=u[R[i]];
		}
	}
	else if(R.size()>=2 && C.size()>=2){
		rep(i,R.size()) rep(j,C.size()) res[R[i]][C[j]]=(i+j)%2;
	}
	return {check(res,s,t,u,v),res};
}
