...
ll n, s[500], t[500];
ull u[500], v[500], ans[500][500];

bool check(ull bit) {
  vector<ll> rest1, rest2;
  map<ll, bool> mp1, mp2;
  for (ll i = 0; i < n; i++) {
    if (s[i] == 0 && (u[i] & bit)) {
      for (ll j = 0; j < n; j++) {
        ans[i][j] |= bit;
        if (t[j] && (v[j] & bit) == 0) return false;
        if (t[j] && (v[j] & bit)) mp2[j] = true;
      }
    } else if (s[i] && (u[i] & bit) == 0) {
      for (ll j = 0; j < n; j++) {
        if (t[j] == 0 && (v[j] & bit)) return false;
        if (t[j] == 0 && (v[j] & bit) == 0) mp2[j] = true;
      }
    } else {
      rest1.push_back(i);
    }
  ...
  for (ll j = 0; j < n; j++) {
    if (t[j] == 0 && (v[j] & bit)) {
      for (ll i = 0; i < n; i++) {
        ans[i][j] |= bit;
        if (s[i] && (u[i] & bit)) mp1[i] = true;
      }
    } else if (t[j] && (v[j] & bit) == 0) {
      for (ll i = 0; i < n; i++) {
        if (s[i] == 0 && (u[i] & bit) == 0) mp1[i] = true;
      }
    } else {
      rest2.push_back(j);
    }
  }
  if (rest1.size() == 0 || rest2.size() == 0) {
    for (ll i = 0; i < n; i++) {
      ll sum, sum2 = u[i] & bit;
      if (s[i] == 0) { // and
        sum = bit;
        for (ll j = 0; j < n; j++) sum = sum & (ans[i][j] & bit);
      } else {
        sum = 0;
        for (ll j = 0; j < n; j++) sum = sum | (ans[i][j] & bit);
      }
    ...
    for (ll j = 0; j < n; j++) {
      ...
      if (t[j] == 0) { // and
        sum = bit;
        for (ll i = 0; i < n; i++) sum = sum & (ans[i][j] & bit);
      } else {
        sum = 0;
        for (ll i = 0; i < n; i++) sum = sum | (ans[i][j] & bit);
      }
  ...
  if (rest1.size() == 1) {
    ...
    if (s[x]) { // and 1
      for (ll i = 0; i < n; i++) if (ans[x][i] & bit) return true;
    } else {
      for (ll i = 0; i < n; i++) if ((ans[x][i] & bit) == 0) return true;
    }
    return false;
  }
  if (rest2.size() == 1) {
    ...
    if (t[x]) { // and 1
      for (ll i = 0; i < n; i++) if (ans[i][x] & bit) return true;
    } else {
      for (ll i = 0; i < n; i++) if ((ans[i][x] & bit) == 0) return true;
    }
    return false;
  }
  for (ll i = 0; i < rest1.size(); i++) {
    for (ll j = 0; j < rest2.size(); j++) {
      if ((i + j) % 2) ans[rest1[i]][rest2[j]] |= bit;
    }
  }
  return true;
}
...
int main() {
  ...
  for (ll i = 0; i < 64; i++) if (!check(1ull << i)) {
    cout << -1 << endl;
    return 0;
  }
  for (ll i = 0; i < n; i++) {
    for (ll j = 0; j < n; j++) cout << ans[i][j] << (j == n-1 ? "\n" : " ");
  }
}
