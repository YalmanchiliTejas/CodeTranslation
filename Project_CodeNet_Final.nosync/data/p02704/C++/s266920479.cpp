...
bool solve(ull bit) {
  ...
  REP(r, N) {
    if (S[r] == 0 && (U[r] & bit) == 0) {
      bool ok = false;
      REP(c, N) if (mat[r][c] == 0) {
        ok = true;
      }
      if (!ok) --R;
      else REP(c, N) if (mat[r][c] == -1) mat[r][c] = 1;
      ...
      if (!ok) ++R;
      else REP(c, N) if (mat[r][c] == -1) mat[r][c] = 0;
    }
  }
  int C = 0;
  REP(c, N) {
    if (T[c] == 0 && (V[c] & bit) == 0) {
      bool ok = false;
      REP(r, N) if (mat[r][c] == 0) {
        ok = true;
      }
      if (!ok) --C;
      else REP(r, N) if (mat[r][c] == -1) mat[r][c] = 1;
    } else if (T[c] == 1 && (V[c] & bit) == bit) {
      bool ok = false;
      REP(r, N) if (mat[r][c] == 1) {
        ok = true;
      }
      if (!ok) ++C;
      else REP(r, N) if (mat[r][c] == -1) mat[r][c] = 0;
    }
  }
  /*
  REP(r, N) {
