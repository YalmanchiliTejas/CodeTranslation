...
int colc[MAX_N][2], rowc[MAX_N][2];
int colu[MAX_N], rowu[MAX_N];
...
struct Solution {
    ...
    bool solve(int k) {
        ...
        for(int i : range(N)) {
            for(int j : range(N)) {
                if((row[i] == 2 and col[j] == 1) or (row[i] == 1 and col[j] == 2)) {
                    return false;
                }
                ...
                if(row[i] == 2 or col[j] == 2) {
                    x = 0;
                } else if(row[i] == 1 or col[j] == 1) {
                    x = 1;
                } else if((row[i] == 0 and rowc[i][0] == 0 and rowu[i] == N - 1) or
                        (col[j] == 0 and colc[j][0] == 0 and colu[j] == N - 1)) {
                    x = 0;
                    ++rowu[i];
                    ++colu[j];
                    ++rowc[i][x];
                    ++colc[j][x];
                } else if((row[i] == 3 and rowc[i][1] == 0 and rowu[i] == N - 1) or
                        (col[j] == 3 and colc[j][1] == 0 and colu[j] == N - 1)) {
                    x = 1;
                    ++rowu[i];
                    ++colu[j];
                    ++rowc[i][x];
                    ++colc[j][x];
                } else if((row[i] == 0 and rowc[i][0] == 0) or (col[j] == 0 and colc[j][0] == 0)) {
                    x = 0;
                    ++rowu[i];
                    ++colu[j];
                    ++rowc[i][x];
                    ++colc[j][x];
                } else if((row[i] == 3 and rowc[i][1] == 0) or (col[j] == 3 and colc[j][1] == 0)) {
                    x = 1;
                    ++rowu[i];
                    ++colu[j];
                    ++rowc[i][x];
        ...
        for(int i : range(N)) {
            if((row[i] == 0 and rowc[i][0] == 0) or (col[i] == 0 and colc[i][0] == 0) or
                    (row[i] == 3 and rowc[i][1] == 0) or (col[i] == 3 and colc[i][1] == 0)) {
                return false;
            }
        }
        return true;
