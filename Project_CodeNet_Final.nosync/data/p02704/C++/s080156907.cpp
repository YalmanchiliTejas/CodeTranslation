...
bool check(ull reslist[64][505][505],
           const V<ull> &slist, const V<ull> &tlist,
           const V<ull> &ulist, const V<ull> &vlist, ull n, ull pos)
{
    rep (hh, n) {
        ll cnt1 = 0;
        rep (ww, n) cnt1 += reslist[pos][hh][ww];
        if (slist[hh] == 0) {
            if ((1ULL<<pos) & ulist[hh]) {
    ...
    rep (ww, n) {
        ll cnt1 = 0;
        rep (hh, n) cnt1 += reslist[pos][hh][ww];
        if (tlist[ww] == 0) {
            if ((1ULL<<pos) & vlist[ww]) {
                if (cnt1 != n) return false;
            }
            else {
                if (cnt1 == n) return false;
...
bool solve(ull reslist[64][505][505],
           const V<ull> &slist, const V<ull> &tlist,
           const V<ull> &ulist, const V<ull> &vlist, ull n, ull pos)
{
    S bool pmt1[505][505];
    memset(pmt1, 0, sizeof(pmt1));
    S ll cnt1h[505];
    memset(cnt1h, 0, sizeof(cnt1h));
    S ll cnt1w[505];
    memset(cnt1w, 0, sizeof(cnt1w));

    rep (i, n) rep (j, n) {
        bool flag_h = ((1ULL<<pos) & ulist[i]);
        bool flag_w = ((1ULL<<pos) & vlist[j]);

        if (slist[i] == 0 && tlist[j] == 0) {
            if (flag_h || flag_w) {
                reslist[pos][i][j] = 1;
                cnt1h[i]++;
                cnt1w[j]++;
            }
            else reslist[pos][i][j] = 0;
        }
        if (slist[i] == 1 && tlist[j] == 1) {
            if (!flag_h || !flag_w) reslist[pos][i][j] = 0;
            else {
                reslist[pos][i][j] = 1;
                cnt1h[i]++;
                cnt1w[j]++;
        ...
        if (slist[i] == 0 && tlist[j] == 1) {
            if (flag_h && flag_w) reslist[pos][i][j] = 1;
            if (flag_h && !flag_w) return false;
            if (!flag_h && flag_w) {
                reslist[pos][i][j] = 0;
                pmt1[i][j] = true;
            }
    ...
    if (check(reslist, slist, tlist, ulist, vlist, n, pos)) return true;

    rep (hh, n) {
        bool flag = ((1ULL<<pos) & ulist[hh]);
        if (slist[hh] == 0) continue;
        if (!flag) continue;
        if (cnt1h[hh]) continue;
        bool changed = false;
        rep (ww, n) {
            //if ((1ULL<<pos) & vlist[ww]) continue;
            if (tlist[ww] == 1) continue;
            if (cnt1w[ww] == n-1) continue;
            if (!pmt1[hh][ww]) continue;
            reslist[pos][hh][ww] = 1;
            cnt1w[ww]++;
            debug_printf("--- changed 1\n");
            changed = true;
            break;
        }
    ...
    rep (ww, n) {
        bool flag = ((1ULL<<pos) & vlist[ww]);
        if (tlist[ww] == 0) continue;
        if (!flag) continue;
        if (cnt1w[ww]) continue;
        bool changed = false;
        rep (hh, n) {
            //if ((1ULL<<pos) & ulist[hh]) continue;
            if (slist[hh] == 1) continue;
            if (cnt1h[hh] == n-1) continue;
            if (!pmt1[hh][ww]) continue;
            reslist[pos][hh][ww] = 1;
            cnt1h[hh]++;
            debug_printf("--- changed 2\n");
            changed = true;
            break;
        }
        if (!changed) return false;
    ...
    if (!check(reslist, slist, tlist, ulist, vlist, n, pos)) return false;
...
{
    ll n; llin(n);
    S V<ull> slist; llinul1(slist, n);
    S V<ull> tlist; llinul1(tlist, n);
    S V<ull> ulist; llinul1(ulist, n);
    S V<ull> vlist; llinul1(vlist, n);

    S ull reslist[64][505][505];
    rep (pos, 64) {
        if (!solve(reslist, slist, tlist, ulist, vlist, n, pos)) {
            cout << -1 << endl;
            return;
        }
    }

    S ull anslist[505][505];
    memset(anslist, 0, sizeof(anslist));
    rep (pos, 64) rep (i, n) rep (j, n) {
        if (!reslist[pos][i][j]) continue;
        anslist[i][j] |= (1LL<<pos);
    }

    debug_printf("---- ans\n");
    rep (i, n) {
        rep (j, n) cout << anslist[i][j] << " ";
        cout <<endl;
    }
}
