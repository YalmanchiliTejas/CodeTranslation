...
void SpecialInference(info_t& one,info_t& another,bool isRow,int pos){
    ...
    for(int r=1;r<=N;r++){
        ...
        if (!bitTarget||one.zeroCount[pos][r]<N) continue;
        
        int c=1;
        while(c<=N){
            if (another.opcat[c]==0&&(another.targets[c]&mask)==0&&another.zeroCount[pos][c]>1) break;
            c++;
        }
        if (c>N) continue;
        if (isRow)
...
int main(){
    ...
    for(int bit=0;bit<64;bit++){
        ...
        for(int r=1;r<=N;r++)
            for(int w=1;w<=N;w++)
                if (((matrix[r][w]>>bit)&1)==0) row.zeroCount[bit][r]++,col.zeroCount[bit][w]++;
