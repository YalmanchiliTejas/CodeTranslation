    ...
    REP(teas, 64) {
        ...
        REP(q, n) {
            {
                ...
                else if (S[q] == 1 and ok == 0) {
                    //eliminated.
                    //no bits should be appear.
                }
                else {
                    x.push_back(q);
                }
        ...
        if (x.size() > 1 and y.size() > 1) {
            REP(t, 2) {
                ...
                REP(i, n) {
                    REP(q, n) {
                        pre_grid[i][q] = grid[i][q];
                    }
                }
                if (t == 1) {
                    REP(i, x.size()) {
                        REP(q, y.size()) {
                            pre_grid[x[i]][y[q]] |= baseline;
                        }
                    }
                }
                REP(q, max(x.size(), y.size())) {
                    pre_grid[x[q % x.size()]][y[q % y.size()]] ^= baseline;
                }
                if (true or check_bit(baseline) == true) {
                    REP(i, n) {
                        REP(q, n) {
                            grid[i][q] = pre_grid[i][q];
                        }
                    }
                    break;
                }
            ...
            REP(q, x.size()) {
                if (S[x[q]] == 0) {
                    ...
                    if (ok == 1) {
                        ...
                        grid[x[q]][y[0]] |= baseline;
                    }
                }
            }
        }
        if (teas != 63) {
            baseline *= (unsigned long long)2LL;
        }
    }
    if (check() == false) {
        cout << -1 << endl;
