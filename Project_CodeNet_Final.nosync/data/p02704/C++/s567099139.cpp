...
signed main() {
	...
	REP(k, 64) {
		...
		if (fill == 0) {
			REP(i, n) REP(j, n) a[i][j] = (i + j) % 2;
		}
		else if (fill == 3) {
			assert(col != 3);
			int y = (col == 1) ? 1 : 0;
			REP(i, n) REP(j, n) if (a[i][j] == -1) a[i][j] = y;
		}
		else {
			if (col == 3) {
				REP(i, n) REP(j, n) if (a[i][j] == -1) a[i][j] = (i + j) % 2;
			}
			else {
				if (fill == 1) {
					if (cnt == n) {}
						...
						int x = -1;
						REP(i, n) if (a[i][0] == -1) x = i;
						int y = (col == 1) ? 0 : 1;
						REP(i, n) {
							if (t[i] == 1 - y && v[i] == 1 - y) a[x][i] = 1 - y;
						}
						int pre = y;
						REP(i, n) {
							if (a[x][i] == -1) {
						...
						int pre = 0;
						REP(i, n) {
							REP(j, n) if (a[i][j] == -1) a[i][j] = (pre + j) % 2;
							pre = 1 - pre;
						}
					}
				}
					...
					if (cnt == n) {}
						...
						int x = -1;
						REP(i, n) if (a[0][i] == -1) x = i;
						int y = (col == 1) ? 0 : 1;
						REP(i, n) {
							if (s[i] == 1 - y && u[i] == 1 - y) a[i][x] = 1 - y;
						}
						int pre = y;
						REP(i, n) {
							if (a[i][x] == -1) {
						...
						int pre = 0;
						REP(i, n) {
							REP(j, n) if (a[j][i] == -1) a[j][i] = (pre + j) % 2;
							pre = 1 - pre;
						}
					}
				}
