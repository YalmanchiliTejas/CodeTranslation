...
#define F(va) {\
  if(!~tmp[i][j]){\
    ...
    if(++S[i]<n&&IN)que.push({S[i],i,'S',s[i]});\
    if(++T[j]<n&&IN)que.push({T[j],j,'T',t[j]});\
  }\
}
void AL(){
  int KO=1;
  while(KO--){
    REP(i)
      if(s[i]==U){
        bool f=0;
        REP(j)f|=tmp[i][j]==s[i];
        if(f)REP(j){KO|=tmp[i][j]<0;F(s[i]^1);}
      }
    REP(j)
      if(t[j]==V){
        bool f=0;
        REP(i)f|=tmp[i][j]==t[j];
        if(f)REP(i){KO|=tmp[i][j]<0;F(t[j]^1);}
      }
  }
}
...
int main(){
  ...
  while(++bit<64){
    ...
    REP(i)REP(j)if(same(s[i],(u[i]>>bit)&1,t[j],(v[j]>>bit)&1))F(s[i]);
    AL();
    IN=1;
