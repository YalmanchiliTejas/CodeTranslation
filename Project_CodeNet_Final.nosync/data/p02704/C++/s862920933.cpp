...
signed main() {
    ...
    REP(t, 2) {
        REP(i, n) {
            ull acc = S[i] ? 0 : std::numeric_limits<ull>::max();
            if (std::accumulate(ans[i].begin(), ans[i].end(), acc,
                                [&](Int x, Int y) { return S[i] ? x | y : x & y; }) != U[i])
                no();
        }
    }
    // ull all = std::numeric_limits<ull>::max();
    // REP(i, n) {
    //     ull acc = S[i] ? 0 : all;
    //     REP(j, n) {
    //         if (S[i]) {
