...
int main() {
	...
	REP(i, n)cin >> u[i];
	REP(i, n)cin >> v[i];
	vector<vector<ull>> ans(n, vector<ull>(n, -1));
	REP(i, n) REP(j, n) {
		if (s[i] == 1 && t[j] == 1)ans[i][j] = u[i] & v[j];
		if (s[i] == 0 && t[j] == 0)ans[i][j] = u[i] | v[j];
	}
	REP(i, n) {
		if (s[i] == 0) {
			REP(j, n) {
				if (ans[i][j] == -1)ans[i][j] = u[i];
				else ans[i][j] |= u[i];
			}
		}
	}
	...
	REP(i, n) {
		ull temp = ans[i][0];
		rep(j, 1, n) {
			if (s[i] == 0)temp &= ans[i][j];
			else temp |= ans[i][j];
		}
		if (temp == u[i])continue;
		if ((temp | u[i]) > u[i]) {
			cout << -1 << "\n";
			return 0;
		}
		...
		REP(k, n) {
			ull j = (now + k + 1) % n;
			if (t[j] == 0 && j != now) {
				ans[i][j] |= nec, now = j, flag = true;
				break;
			}
		}
		if (flag == false && now != -1)ans[i][now] |= nec;
		else if (flag == false && now == -1) {
			cout << -1 << "\n";
			return 0;
		}
	}
	...
	REP(i, n) {
		...
		REP(k, n) {
			ll j = (now + k + 1) % n;
			if (s[j] == 0 && j != now) {
				ans[j][i] |= nec, now = j, flag = true;
				break;
			}
		}
		if (flag == false && now != -1)ans[now][i] |= nec;
		else if (flag == false && now == -1) {
			cout << -1 << "\n";
			return 0;
		}
