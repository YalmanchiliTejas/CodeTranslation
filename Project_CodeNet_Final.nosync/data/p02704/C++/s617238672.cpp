...
vector<vector<int>>
Transpose(int n, const vector<vector<int>>& matrix) {
  vector<vector<int>> rv = MakeVector2D(n, n, 0);
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < n; j++) {
      rv[j][i] = matrix[i][j];
    }
...
Solve(int n, vector<Constraint> rows, vector<Constraint> cols) {
  ...
  auto fill_all_row = [&](int r, int v) -> bool {
    for (int c = 0; c < n; c++) {
  ...
  for (int r = 0; r < n; r++) {
    if (rows[r] == ALL_ONE) {
      if (!fill_all_row(r, 1)) return nullopt;
    } else if (rows[r] == ALL_ZERO) {
      if (!fill_all_row(r, 0)) return nullopt;
    } else {
      open_rows.push_back(r);
    }
  }

  auto fill_all_col = [&](int c, int v) -> bool {
    for (int r = 0; r < n; r++) {
      if (matrix[r][c] == -1) matrix[r][c] = v;
      else if (matrix[r][c] != v) return false;
    }
  ...
  for (int c = 0; c < n; c++) {
    if (cols[c] == ALL_ONE) {
      if (!fill_all_col(c, 1)) return nullopt;
    } else if (cols[c] == ALL_ZERO) {
      if (!fill_all_col(c, 0)) return nullopt;
    } else {
      open_cols.push_back(c);
    }
  }

  auto verify_any_row = [&](int r, int v) -> bool {
    vector<int> bits(2);
    for (int c = 0; c < n; c++) {
      if (matrix[r][c] == -1) matrix[r][c] = v;
      bits[matrix[r][c]] = 1;
    }
    if (!bits[v]) return false;
    return true;
  };
    ...
    if (open_rows.size() > 0) {
      for (int open_row : open_rows) {
        if (rows[open_row] == ANY_ZERO) {
          if (!verify_any_row(open_row, 0)) return nullopt;
        } else if (rows[open_row] == ANY_ONE) {
          if (!verify_any_row(open_row, 1)) return nullopt;
        }
      }
      return matrix;
    } else if (open_cols.size() > 0) {
      auto optional_t_matrix = Solve(n, cols, rows);
      if (!optional_t_matrix) return nullopt;
      return Transpose(n, *optional_t_matrix);
    }
  }
