...
for k in range(64):
    ...
    if blank_rows or blank_cols:
        ...
        for i,(s,u) in enumerate(zip(S,U)):
            x = (u>>k)&1
            if s==x:
                if s==0 and (not row_has_0[i]) and len(blank_cols) == 0: invalid_exit()
                if s==1 and (not row_has_1[i]) and len(blank_cols) == 0: invalid_exit()
                if j is not None:
                    y = (V[j]>>k)&1
                    if x==y:
                        fill(i,j,x)
    ...
    if blank_rows or blank_cols:
        ...
        for j,(t,v) in enumerate(zip(T,V)):
            x = (v>>k)&1
            if t==x:
                if t==0 and (not col_has_0[j]) and len(blank_rows) == 0: invalid_exit()
                if t==1 and (not col_has_1[j]) and len(blank_rows) == 0: invalid_exit()
                if i is not None:
                    y = (U[i]>>k)&1
                    if x==y:
                        fill(i,j,x)
