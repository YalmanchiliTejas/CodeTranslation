// http://judge.u-aizu.ac.jp/onlinejudge/creview.jsp?rid=2447127&cid=RitsCamp17Day2
// http://judge.u-aizu.ac.jp/onlinejudge/description.jsp?id=1615
// http://natsugiri.hatenablog.com/entry/2016/06/28/231351
...
struct maximum_flow_lr {
    ...
    flow_type maximum_flow(int s, int t) {
        ...
        // }
        // for (auto &e : flow.g[T]) {
        //     if (flow.residue(flow.reverse(e))> 0) {
        //         return -1;
        //     }
        // }
        // return f;
...
bool ok(int l, int r) {
    maximum_flow_lr<dinic> dn(n*n + n + 2);
    int S = n*n + n;
    int T = S + 1;
    rep(i, n*n) {
        dn.add_edge(S, i, 0, 1);
    ...
    rep(i, m) {
        int u, v;
        tie(u, v) = es[i];
        dn.add_edge(u*n + v, u + n*n, 0, 1);
        dn.add_edge(u*n + v, v + n*n, 0, 1);
    }
    rep(i, n) {
        dn.add_edge(i + n*n, T, l, r);
    }
...
int main() {
    while (cin >> n >> m && n) {
        es.resize(m);
        rep(i, m) cin >> es[i].first >> es[i].second, es[i].first--, es[i].second--;
        int ansl = 0, ansr = n;
        int l = 0;
        for (int r = 0; r <= n; ++r) {
            while (l <= r && ok(l, r)){
...
// // ????¬???° [lb, ub], ?????§????????? x ??§?????§??????????????°
// int match(int lb, int ub, int x) {
//     maximum_flow_lr<dinic> dn(n + m + 2);
//     rep(i, n) rep(j, m) {
//         if (c[i][j] <= x) {
//             dn.add_edge(i, j + n, 0, 1);
//         }
//     }
//     int S = n + m;
//     int T = S + 1;
//     rep(i, n) {
//         dn.add_edge(S, i, lb, ub);
//     }
//     rep(i, m) {
//         dn.add_edge(i + n, T, 1, 1);
//     }
//     return dn.maximum_flow(S, T);
// }
...
//         int minx = 0, maxx = 1e9 + 1;
//         while (minx + 1 < maxx) {
//             int x = (minx + maxx) / 2;
//             int f = match(m / n, (m + n - 1) / n, x);
//             if (f == m) {
//                 maxx = x;
//             } else {
//                 minx = x;
