...
template<class S,class T> void sortBySecond(vector<pair<S,T>> &v){sort(all(v),[](auto &L,auto &R){if(L.second==R.second)return L.first<R.first;return L.second<R.second;});}
...
template<class S,class T> pair<S,T> min(const map<S,T> &m){pair<S,T> r=*m.begin();for(const auto &p:m)if(r.second>p.second)r=p;return r;}
template<class S,class T> pair<S,T> max(const map<S,T> &m){pair<S,T> r=*m.begin();for(const auto &p:m)if(r.second<p.second)r=p;return r;}
...
int main() {
  ...
  vector<int> cols_ignored, rows_ignored;

  str all_dots(W, '.');

  rep(i, H) {
    if (a[i] == all_dots) {
      rows_ignored.emplace_back(i);
    }
  }
  ...
  rep(j, W) {
    ...
    if (flag) {
      cols_ignored.emplace_back(j);
    }
  }

  rep(i, H) {
    rep(j, W) {
      if (!in(i, rows_ignored) && !in(j, cols_ignored)) {
        putchar(a[i][j]);
      }
    }
    if (!in(i, rows_ignored)) {
      putchar('\n');
    }
  }
