...
signed main(){
    ...
    int h,w; cin >> h >> w;
    char hw[h][w]; REP(i,h)REP(j,w) cin >> hw[i][j];
    REP(i,h){ /*横*/
        bool flag = 1;
        REP(j,w){if(hw[i][j]=='#') flag = 0;}
        if(flag) REP(j,w) hw[i][j] = '1';
    }
    REP(i,w){ /*縦*/
        bool flag = 1;
        REP(j,h){if(hw[j][i]=='#') flag = 0;}
        if(flag) REP(j,h) hw[j][i] = '1';
    }
    bool super_first = 1;
    REP(i,h){bool first = 1; REP(j,w){if(hw[i][j]!='1'){if(!super_first) if(first) cout << endl; super_first = 0; first = 0; cout << hw[i][j];}}}
    cout << endl;
