...
void solve(ostringstream& aout, long long N, long long M, std::vector<long long> A);
void solve_TLE(ostringstream& aout, long long N, long long M, std::vector<long long> A);
...
private:
	...
	bool judge_case(long long N, long long M, std::vector<long long> A) {
		...
		solve(fast, N, M, std::move(A));
		solve_TLE(tle, N, M, std::move(A));
		if (fast.str() == tle.str()) {
			return true;
		}
		else {
...
signed main() {
	...
	rep(i, H) {
		if (mapData[i] != dif) {
			m2.push_back(mapData[i]);
		}
	}

	int newRow = m2.size();
	vector<string> ans(newRow);
