...
ll h, w;
char a[100][101];
bool allH[100], allW[100];

int main() {
  scanf("%lld%lld", &h, &w);
  for (ll i = 0; i < h; i++) scanf("%s", a[i]);
  for (ll i = 0; i < h; i++) {
    allW[i] = true;
    for (ll j = 0; j < w; j++) if (a[i][j] == '#') allW[i] = false;
  }
  for (ll i = 0; i < w; i++) {
    allH[i] = true;
    for (ll j = 0; j < h; j++) if (a[j][i] == '#') allH[i] = false;
  }
  for (ll i = 0; i < h; i++) {
    if (allW[i]) continue;
    for (ll j = 0; j < w; j++) if (!allH[j]) printf("%c", a[i][j]);
    printf("\n");
  }
}
