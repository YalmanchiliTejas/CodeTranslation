...
//    cout << fixed << setprecision(15);
   ...
   i32 h,w, tmp1 = 0, tmp2 = 0; cin >> h >> w;
   vec<bool> panish1(h), panish2(w);
   vec<string> grid(h);
   rep(i,h) cin >> grid[i];
   ...
   i32 nh = h-tmp1, nw = w-tmp2;
   vec<string> answer;
    rep(i,h) {
        if(panish1[i]) continue;
        string tmp;
        rep(j,w) {
            if(panish2[j]) continue;
