...
int main() {
    ...
    vector<bool> flag_x(H, true), flag_y(W, true);
    rep(i, 0, H) rep(j, 0, W) flag_x[i] = flag_x[i] & (a[i][j] == '.');
    rep(j, 0, W) rep(i, 0, H) flag_y[j] = flag_y[j] & (a[i][j] == '.');
    rep(i, 0, H) {
        if (flag_x[i]) continue;
        rep(j, 0, W) {
            if (flag_y[j]) continue;
            cout << a[i][j];
        }
        output();
    }
