...
int main(){
  ...
  ll H, W;
  cin >> H >> W;
  auto elimh = mvec<bool>(H);
  auto elimw = mvec<bool>(W);
  auto vec = mvec<char>(H,W);
  ...
  rep(i,0,H){
    bool exist{};
    rep(j,0,W){
      if(vec[i][j] == '#') elimh[i] = 1;
    }
    if(exist == false) elimh.push_back(i); 
  }
  rep(i,0,W){
    bool exist{};
    rep(j,0,H){
      if(vec[j][i] == '#') elimw[i] = 1;
    }
  }
  rep(i, 0, H){
    if(elimh[i]){
      rep(j, 0, W){
        if(elimw[j]) cout << vec[i][j];
      }
      cout << rt;
    }
  }
