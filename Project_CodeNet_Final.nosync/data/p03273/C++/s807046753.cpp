...
int main() {
  ...
  vint rows_to_be_skipped, cols_to_be_skipped;

  rep(i, H) {
    if (a[i] == str(W, '.')) {
      rows_to_be_skipped.emplace_back(i);
    }
  }
  ...
  rep(j, W) {
    ...
    if (b) cols_to_be_skipped.emplace_back(j);
  }

  bool r;

  rep(i, H) {
    r = !in(i, rows_to_be_skipped);
    rep(j, W) {
      if (r && !in(j, cols_to_be_skipped)) printf("%c", a[i][j]);
    }
    if (r) puts("");
  }
