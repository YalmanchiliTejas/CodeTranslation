...
void init(int h, int w, Matrix &matrix) {
    matrix = vector<vector<ll>>(h, vector<ll>(w));
    /*for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < n; j++)
        {
...
Matrix &operator*(Matrix &matrix, Matrix &matrix1) {
    ...
    Matrix res;
    res = vector<vector<ll>>(height, vector<ll>(width2, 0));
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width2; j++) {
            for (int k = 0; k < width; k++) {
                res[i][j] = Madd(res[i][j], Mmul(matrix[i][k], matrix1[k][j]));
...
int main() {
  ...
  rep(i, 0LL, n) {
    rep(j, 0LL, m) {
      y = Madd(y, Mdivide(Mmul(Madd(1LL, i), i), 2LL));
      y = Madd(y, Mdivide(Mmul(Msub(n, i), Msub(n, Madd(i, 1LL))), 2LL));
      x = Madd(x, Mdivide(Mmul(Madd(1LL, j), j), 2LL));
      x = Madd(x, Mdivide(Mmul(Msub(m, j), Msub(m, Madd(j, 1LL))), 2LL));
    }
  }
  ll c = comb(n * m - 2, k - 2);
  cout << Mdivide(Mmul(Madd(Mmul(x, n), Mmul(y, m)), c), 2LL) << endl;
#ifdef LOCAL_DEFINE
