...
string to_string(edge e){return "{" + to_string(e.to) + ", " + to_string(e.cost) + "}";}
template <typename A, typename B>
...
ll modf(ll x, ll m){return (x % m + m) % m;}
...
struct MaxFlow {
    ...
    void add_edge(ll from, ll to, ll capacity = 1) {
...
struct Doubling { // ABC167D
    ...
    */
    ...
    // edge[i]: i -> edge[i] by one operation
    ...
    // return the result obtained when starting from i and advancing x times
    ll get(ll i, ll x){
        ll ret = i;
        rep(bit, sz){
            if (!(x >> bit & 1)) continue;
    ...
    // return the iteration x where the value exceeds j for the first time.
    // if it is not possible, then return n.
...
public:
    ...
    void _init(ll source, ll parent, ll d, ll l){
        depth[source] = d;
        ...
        rep(i, tree[source].size()){
            ...
            _init(target, source, d + 1, cost + l);
        }
    }
...
struct BinaryIndexedTree{
    ...
    BinaryIndexedTree(ll n, ll ini = 0):
        dat(n + 1, ini), n(n), ini(ini){M = 1; while (M <= n) M <<= 1;};
...
struct SegmentTree{
	...
	void update(ll idx, ll val, bool force = false){
		idx += n - 1;
        if (!force){
...
struct StronglyConnectedComponents{
    /*
    dag: [cmp idx][# of connected cmp]
        the index of connected cmp
    cmp: [cmp idx][# of vertex included in a cmp]
        the index of vertex
    cmp_idx: [vertex idx]
        the component which the vertex belongs to

    Use graph_rev when you would like to know the vertex with zero indegree
    */
    ll n, n_cmp;
...
struct CombinationMemo{
    ll sz, mod;
    vl fact, fact_inv, minv;

    CombinationMemo(ll sz, ll _mod):
        sz(sz), mod(_mod){
            fact.resize(sz + 5);
            fact_inv.resize(sz + 5);
            minv.resize(sz + 5);
            init();
        }
    ...
    void init(){
        ...
        For(i, 2, sz + 3){
            ...
            fact_inv[i] = fact_inv[i - 1] * minv[i] % mod;
        }
    }
    ...
    ll nCk(ll n, ll r){
        ...
        ll val = (fact[n] * fact_inv[n - r]) % mod;
        val *= fact_inv[r];
        return val % mod;
    }
...
struct Grid2D{
    ...
    Grid2D(ll w, ll h, vb2& block) : Width(w), Height(h){
        graph.resize(w * h);
        _build(block);
    }
    ...
    void _build(vb2& block, ll val = 1){
    ...
    */
    ...
    ll dfs(vl& c, ll src){
        ...
        for (Edge& e : graph[src]){
            ...
            ret += tmp;
            if (mod2) x[e.idx] = modf(x[e.idx], 2);
        }
...
struct BigInt10{
    ...
    ll subseq_mod(ll l, ll r, ll m){
        ...
        ll sz = num.size();
        ll tot = mod[m][sz];
        ll ls = modf(tot - mod[m][sz - l], m);
        ll rs = mod[m][sz - r];
        return (tot - ls - rs + 2 * m) % m;
    }

    bool operator<(BigInt10& n2) const{
        if (num.length() < n2.size()) return true;
        else if (num.length() > n2.size()) return false;
        rep(i, num.length()) if (ll(num[i] - '0') != n2[i]) return ll(num[i] - '0') < n2[i];
        return false;
    }
...
istream& operator>>(istream& is, BigInt10& a) {return is >> a.num;}
ostream& operator<<(ostream& os, BigInt10& a) {return os << a.num;}

BigInt10 add(BigInt10 s1, BigInt10 s2){
    ll sz1 = s1.size(), sz2 = s2.size(), sz = max(sz1, sz2);
    ll r = abs(sz2 - sz1); string a(r, '0');
    if (sz1 < sz2)
        s1.num = a + s1.num;
    else if (sz1 > sz2)
        s2.num = a + s2.num;
    ...
    bool inc = false;
    rep(i, sz){
        ll nxt = s1[sz - 1 - i] + s2[sz - 1 - i] + inc;
        if (nxt >= 10) inc = true;
        else inc = false;
        s1.num[sz - 1 - i] = to_string(nxt % 10)[0];
    }
    if (inc) s1.num = "1" + s1.num;
    return s1;
}

BigInt10 add(BigInt10 s, ll x){return add(s, BigInt10(to_string(x)));}

BigInt10 subtract(BigInt10 s1, BigInt10 s2){
    ll sz1 = s1.size(), sz2 = s2.size(), sz = max(sz1, sz2);
    assert(sz1 >= sz2);
    ll r = abs(sz2 - sz1); string a(r, '0');
    if (sz1 > sz2)
    ...
    bool dec = false;
    rep(i, sz){
        ll nxt = s1[sz - 1 - i] - s2[sz - 1 - i] - dec;
        if (nxt < 0) dec = true;
        else dec = false;
        s1.num[sz - 1 - i] = to_string((nxt + 10) % 10)[0];
    }
    ll n0 = 0;
    while (s1.num[n0] == '0') n0++;
    s1.num = substring(s1.num, n0, sz); 
    return s1;
}

BigInt10 subtract(BigInt10 s, ll x){return subtract(s, BigInt10(to_string(x)));}

BigInt10 multiply(BigInt10 s, ll x){
    assert(x <= 10);
    assert(x >= 0);

    if (x == 0) return BigInt10("0");
    else if (x == 1) return BigInt10(s.num);
    if (x == 10) return BigInt10(s.num + "0");

    ll sz = s.size(); ll inc = 0;
    rep(i, sz){
        ll nxt = s[sz - 1 - i] * x + inc;
        inc = nxt / 10;
        s.num[sz - 1 - i] = to_string(nxt % 10)[0];
    }
    if (inc) s.num = to_string(inc) + s.num;
    return s;
}

BigInt10 multiply(BigInt10 s1, BigInt10 s2){
    ll sz = s2.size();
    BigInt10 ret;

    rep(i, sz){
        ll x = s2[sz - 1 - i];
        ret = add(ret, multiply(s1, x));
        s1.num += "0";
    }
    return ret;
}

BigInt10 shift(BigInt10 s, ll k){
    if (k >= s.size()) return BigInt10("0");
    s.num = substring(s.num, 0, s.size() - k - 1);
    return s;
}
...
istream& operator>>(istream& is, BigInt2& a) {return is >> a.num;}
ostream& operator<<(ostream& os, BigInt2& a) {return os << a.num;}

struct MedianManager{
    ll sz = 0, median = 0;
    priority_queue<ll> lower;
    priority_queue<ll, vl, greater<ll>> upper;
    vl in, out;
    ll ini;

    MedianManager(ll _ini = -INF){
        ini = _ini;
        in = out = vl(2, _ini);
    }
    ...
    void update(ll v){
        sz++;
        if (sz == 1) median = v, in[0] = in[1] = out[0] = out[1] = ini;
        else if (sz % 2 == 0){
            if (v < median) lower.push(v), upper.push(median), in[0] = v, in[1] = median, out[0] = out[1] = ini;
            else lower.push(median), upper.push(v), in[0] = median, in[1] = v, out[0] = out[1] = ini;
        }else if (sz % 2){
            ll lower_top = lower.top(), upper_bottom = upper.top();
            if (v < lower_top) median = lower_top, lower.pop(), lower.push(v), in[0] = v, out[0] = median, in[1] = out[1] = ini;
            else if (v > upper_bottom) median = upper_bottom, upper.pop(), upper.push(v), in[1] = v, out[1] = median, in[0] = out[0] = ini;
            else median = v, in[0] = out[0] = in[1] = out[1] = ini;
        }
    }

    ll size(){return sz;}
    pl medians(){return sz % 2 ? mp(median, median) : mp(lower.top(), upper.top());}
};

struct RangeTypeQuery{
    ll n, cur, Q;
    vl types, most_right, ans;
    vtp3 queries;
    BinaryIndexedTree bit;

    RangeTypeQuery(ll _n, ll _Q, BinaryIndexedTree _bit): bit(_bit){
        n = _n, Q = _Q, cur = 0;
        queries.resize(Q); ans.resize(Q);
        types.resize(n); most_right = vl(n, -1);
    }

    void get_queries(ll offset=-1){
        rep(i, Q){
            ll l, r; in2(l, r);
            l += offset, r += offset;
            queries[i] = {r, l, i};
    ...
    void get_sequence(ll offset=-1){
        rep(i, n){
            in1(types[i]);
            types[i] += offset;
            // type num must be less than n.
            assert(types[i] <= n - 1);
        }
    }
    ...
    void update(ll nxt){
        while (cur <= nxt){
            if (most_right[types[cur]] != -1)
                bit.update(most_right[types[cur]], -1);
            most_right[types[cur]] = cur;
            bit.update(most_right[types[cur]], 1);
            cur++;
        }
    }
...
vl eratosthenes(ll n, vl& div){
    // return the list of prime number up to n
    div = vl(n + 1, 0); vl ps;
    rep(i, n + 1) div[i] = i;
    For(i, 2, n + 1){
        if(div[i] == i){
...
map<ll, ll> quick_prime_factorization(vl& div, ll n){
    // ABC177E using the div from eratosthenes
    map<ll, ll> table;
    while (n != 1){
        table[div[n]]++;
        n /= div[n];
...
vl divisor_table(ll n, bool asc = true){
    vl table;
    ll i = 1;
    while(i * i <= n){
        if (n % i == 0){
    ...
    if (asc) sort(all(table));
    else sort(rall(table));

    return table;
}

ll euler_function(ll x){
    // the number of coprime numbers for x (including 1)
    vl dt = divisor_table(x, false);
    ll ret = x; map<ll, ll> cnt;
...
vl base_converter(ll num, ll base, ll sz){
    ...
    while (cur < sz) coefs[cur++] = 0;
    return coefs;
}

string indexed_alphabet(ll leng, ll idx){
    // 0-indexed
    // returns the string whose order is the-idx-th in the set of strings of the given length.
    string ret = "";
...
void dijkstra(ll start, Graph& graph, vl& dist,
    ...
    ll n = graph.size(); dist = vl(n, INF); dist[start] = 0;
...
void update_warshall_floyd(vl2& dist, ll v1, ll v2, ll cost){
    ll n = dist.size();
    if (!chmin(dist[v1][v2], cost)) return;
    dist[v2][v1] = dist[v1][v2];
    rep(i, n) rep(j, n) dist[i][j] = min(dist[i][j], dist[i][v1] + dist[v1][j]);
    rep(i, n) rep(j, n) dist[i][j] = min(dist[i][j], dist[i][v2] + dist[v2][j]);
}
...
template<typename T>
void read_vector(ll n, vector<T>& v, ll offset = 0){
    v.resize(n);
    rep(i, n){
        in1(v[i]);
        if (offset) v[i] += offset;
    }
}

template<typename T>
void read_vector(ll n, vector<T>& v1, vector<T>& v2, ll offset1 = 0, ll offset2 = 0){
    v1.resize(n); v2.resize(n);
    rep(i, n){
        in2(v1[i], v2[i]);
        if (offset1) v1[i] += offset1;
        if (offset2) v2[i] += offset2;
    }
}
...
ll diameter_of_tree(Graph& tree){
    ...
    dfs(dfs, 0, -1, 0); v1 = vM;
    dfs(dfs, vM, -1, 0); v2 = vM;
    return dM;
}

void trit_search(ll n, function<void(ll, ll)> func){
    rep(bit, 1LL << n){
        ll sub = bit;
        do{
            func(bit, sub);
...
ll convert_string_to_int(string s, ll n_below_decimal){
    // return double(s) * coef
    ll ret = 0, comma = s.length(), n10 = 1, coef = power_normal(10, n_below_decimal);
    rep(i, s.length()) if (s[i] == '.') comma = i;
    revrep(i, comma){
        ll num = s[i] - '0';
        ret += n10 * num;
        n10 *= 10;
    }
    ret *= coef; n10 = coef / 10;
    For(i, comma + 1, s.length()){
        ll num = s[i] - '0';
        ret += n10 * num;
        n10 /= 10;
    }
...
void iter_combination(ll n, ll k, function<void(ll)> func){
    /*
    ...
    */
    ...
    auto next_combination = [&](ll sub) -> ll{
        ll x = sub & -sub, y = sub + x;
        if (x != 0) return (((sub & ~y) / x) >> 1) | y;
        else return INF;
    };

    ll M = 1LL << n;
    for (ll bit = (1LL << k) - 1; bit < M; bit = next_combination(bit))
        func(bit);
}

ll count_in_sorted_seq(vl& as, ll val){
    ll lb = lower_bound(all(as), val) - as.begin();
    ll ub = upper_bound(all(as), val) - as.begin();
    return ub - lb;
}

bool win_first_player(vl& grundy_nums){
    ll x = 0;
    for (ll& gn: grundy_nums) x ^= gn;
    return x != 0;
}

ll create1001001(ll num, ll m, ll num_of_1, ll nb = 31){
    // e.g. 1001001001 -> num = 3
    // e.g. 111111 -> num = 1 (REPUNIT)
    // Calculate the modulo m of 100..100...100..1 (the length of each 00... is num - 1. The # of 1 is 2^i.)
    // Then calculate the modulo m of 100...100...100...1 (The # of 1 is num_of_1 using the memo.)
    vl memo(nb, 1); ll v = 0, tot = 1, coef = power_mod(10, num, m);
    rep(j, nb){
        ll c10 = power_mod(coef, 1LL << j, m);
        memo[j + 1] = ((memo[j] * c10) % m + memo[j]) % m;
        if (num_of_1 >> j & 1){
            (v += (memo[j] * tot) % m) %= m;
            (tot *= power_mod(coef, 1LL << j, m)) %= m;
        }
    }
    return v;
}

ll GaussJordanBitVector(vl &bs){
    ll n = bs.size();
    ll rank = 0; ll j = 0;
    revrep(i, N_DIGITS){
...
ll GaussJordan(vl2& A, ll mod, bool is_extended = false, bool det = false){
    /*
    Ax = b
    ...
    is_extended: bool
        if A includes the term of b (e.g. when solving linear equation), true
        otherwise false.

    Property: The product of diagonal elements of upper triangle matrix is equal to the determinant of this matrix.
    */

    ll rank = 0, m = A.size(), n = A[0].size();
    rep(r, m) rep(c, n) A[r][c] = modf(A[r][c], mod);

    ll pivot, inv, ret = 1; vl tmp;
    if (det) tmp = vl(n);
    rep(c, n){
        if (is_extended && c == n - 1) break;
        pivot = -1;
        For(r, rank, m) if (A[r][c] != 0) {pivot = r; break;}
        if (pivot == -1){
            if (det) return 0;
            continue;
        }
        ...
        swap(A[pivot], A[rank]);
        ret = modf(ret * A[rank][c], mod), inv =  modinv(A[rank][c], mod);
        vl& ax = det ? tmp : A[rank];
        rep(c2, n) ax[c2] = A[rank][c2] * inv % mod;

        rep(r, m) if (r != rank && A[r][c]){
            ll base = A[r][c];
            rep(c2, n)
                A[r][c2] = modf(A[r][c2] - ax[c2] * base, mod);
        }
        ++rank;
    }
...
ll linear_equation(vl2 A, vl b, vl &res){
    ...
    ll rank = GaussJordan(M, true);
    For(r, rank, m) if (M[r][n]) return -1;
...
ll determinant(vl2& A, ll mod){return GaussJordan(A, mod, false, true);};
...
/*
    ...
    Grundy number (ARC038C)
    void grundy(ll x, vl& gn){ // the example from the ari book
        // the player can take a[0], a[1], ..., a[k - 1] coins from 1 mountain
        set<ll> s;
        rep(i, k) if (a[i] <= x) s.insert(gn[x - a[i]])
        
        while (s.count(gn[x])) gn[x]++;
    }
*/
...
void solve(){
    /*
    *** 注意 ***
    ・マイナスの整数の除算が0 (マイナスのときはCeil関数になる)になって，マイナスの判定ができていない．
    ・MODの逆元を取るときは素数か否かに注意．
    ・Queue, stack, deque等で必ず`empty`なら飛ばすようにする．
    ・set, multisetで`RE`を起こすときは初期段階で解法に影響が出ないようなダミーをsetに入れておくとエラーを回避できる．
    ・掛け算のOverflow注意．(x * y > 10^18　-> x > 10^18 / y) のような形で判定するとよい．
    ・総和を取る公式 (1+2+...+n = n * (n + 1) / 2)の計算時等で最終的には10^18に収まる場合は`/2`の順序を早める．
    ・Bit演算をするときに`a ^ b != c`のような式は代入演算子になったりするので注意． (ABC150F)
    ・aborted (core dumped) のエラーが出ているときは変数の二重宣言やベクトルの区間外参照を確認．
    ・`tuple`のコンパイルエラーは`make_tuple(a, b, c, ...)`で回避．
    ・グラフの最短経路と木の最短経路は同じDFSでも更新方法に注意. (ABC148E)
    ・Running Errorの１つとして，関数の返り値のタイプが定義されているのに何も返していないことが考えられる．
    ・Running Errorとしてベクトルサイズが0であるにもかかわらずsortをしている場合がある．
    ・境界条件は必ず試す．
    ・Overflow不等式: (前提条件：a, b, c > 0)
        1. a * b <= c -> a <= c / b
        2. a * b <  c -> a <= c / b && (c % b || a != c / b)
        3. c <= a * b -> (c + b - 1) / b <= a
        4. c <  a * b -> (c + b - 1) / b <= a && (c % b || a != c / b)
    */
    // generate_array(size, min, max);
    ll n, m, k; in3(n, m, k);
    mint ans = 0;
    mint coef = combination(n * m - 2, k - 2, MOD);

    auto func = [&](ll x, ll y) -> mint{
        // x, x - 1, ..., 1, 0, 1, 2, ..., m - x
        mint vx = x * (x + 1) / 2 + (m - x - 1) * (m - x) / 2;
        mint vy = y * (y + 1) / 2 + (n - y - 1) * (n - y) / 2;
        mint ret = vx * n + vy * m;
        return ret;
    };

    rep(y, n) rep(x, m) ans += func(x, y);
    ans = (ans * coef) / 2;
    prl(ans);
