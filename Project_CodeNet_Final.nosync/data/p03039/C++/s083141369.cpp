...
int main()
{
    ...
    for(int i = 1; i <= rows; i++)
    {
        ...
        x_contribution += (columns*one_x_contribution)%MOD;// cout << " X = " << x_contribution << endl;
    }
    ...
    for(int i = 1; i <= columns; i++)
    {
        ...
        y_contribution += (rows*one_y_contribution)%MOD; //cout << " Y = " << y_contribution << endl;
    }

    long long sum_pair_distance = (x_contribution + y_contribution)%MOD;// cout << "N = " << no_of_times_each_pair_is_counted << " S = " << sum_pair_distance << endl;
