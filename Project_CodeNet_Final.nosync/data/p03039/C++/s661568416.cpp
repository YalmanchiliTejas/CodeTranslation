...
inline LL mod_bipow(LL x, LL y) {   // x^y by bisection method
    if (y == 0) return 1;
...
inline LL mod_inv(LL x) { return mod_bipow(x, MOD - 2); }   // x^{-1} = x^{MOD-2} (MOD: prime number)
inline LL mod_div(LL a, LL b) { return mod_mul(a, mod_inv(b)); }   // a/b = a*b^{-1}
