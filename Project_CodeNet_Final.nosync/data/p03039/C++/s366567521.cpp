...
signed main(){
    ...
    Mint<> sum = 0;
    for(int i = 0; i < n; i++){
        sum += Mint<>(m) * Mint<>(m) * (Mint<>(i) + Mint<>(i) * Mint<>(i - 1) / 2 + Mint<>(n - 1 - i) + Mint<>(n - 1 - i) * Mint<>(n - 1 - i - 1) / 2);
    }
    for(int i = 0; i < m; i++){
        sum += Mint<>(n) * Mint<>(n) * (Mint<>(i) + Mint<>(i) * Mint<>(i - 1) / 2 + Mint<>(m - 1 - i) + Mint<>(m - 1 - i) * Mint<>(m - 1 - i - 1) / 2);
    }
    Comb<Mint<>> C(n * m);
    cout << sum / 2 * C.C(n * m - 2, k - 2) << endl;
    return 0;
}