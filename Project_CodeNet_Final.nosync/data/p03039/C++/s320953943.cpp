...
void prep() {
    ...
    ifact[2*N-1] = inv(fact[2*N-1]);
    repi(i, 2*N-2, -1) ifact[i] = mul(i+1, ifact[i+1]);
}
...
{   
    ...
    int ans = 0;
    rep(i, 0, n) {
        int x = nC2(i+1), y = nC2(n-i);
        ans = add(ans, mul(mul(m, m), add(x, y)));
    }

    rep(j, 0, m) {
        int x = nC2(j+1), y = nC2(m-j);
        ans = add(ans, mul(mul(n, n), add(x, y)));
    }

    ans = mul(ans, nCr(n*m-2, k-2));
    ans = div(ans, 2);
