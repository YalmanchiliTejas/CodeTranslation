...
inline void solve_me_senpai() {
    ...
    int ncr = 1;
    rep(i, k - 2) {
        ncr = (((ncr * (N - i)) % MOD) * (binexp(i + 1, MOD - 2, MOD))) % MOD;
    }
    int ans = 0;
    rep(i, n) {
        rep(j, m) {
            int t = (((m * (n - i - 1)) % MOD) * ncr) % MOD;
            ans = (ans + (t * (MOD - i - 1)) % MOD) % MOD;
            t = (((m * i) % MOD) * ncr) % MOD;
            ans = (ans + (t * (i + 1)) % MOD) % MOD;
         //  cout << ans << "\n";
            t = (((n * (m - j - 1)) % MOD) * ncr) % MOD;
            ans = (ans + (t * (MOD - j - 1)) % MOD) % MOD;
            t = (((n * j) % MOD) * ncr) % MOD;
           // cout << ans << "\n";
            ans = (ans + (t * (j + 1)) % MOD) % MOD;
        }
    }
    o(ans);
}
