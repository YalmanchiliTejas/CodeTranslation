...
int main(){
	...
	ll yoko=(((powMod(N,3,MOD)-N+MOD)%MOD)*invMod(6,MOD))%MOD;
	yoko=yoko*powMod(M,2,MOD);
	yoko%=MOD;
	ll tate=(((powMod(M,3,MOD)-M+MOD)%MOD)*invMod(6,MOD))%MOD;
	tate=tate*powMod(N,2,MOD);
	tate%=MOD;
	cout<<(((yoko+tate)%MOD)*pat)%MOD<<endl;
}