...
signed main() {
  ...
  dp[0].fi = 0;
  dp[0].se = 0;
  rep2(i, 1, n*m) dp[i].se = i;
  rep2(i, 0, n)rep2(j, 0, m) {
    if (i == 0 && j == 0) continue;
    if (i >= 1) {
      int plus = dp[m*(i-1)+j].se % mod;
      addm(dp[m*i+j].fi, dp[m*(i-1)+j].fi+plus);
      plus = (j * (j+1)) / 2 + 1 * (m-1-j) + (1+m-1-j) * (m-j-1)/2 + 1;
      addm(dp[m*i+j].fi, plus);
    }
    else {
      addm(dp[m*i+j].fi, dp[m*i+j-1].fi+j);
    }
  }
  int ans = 0;
  init_fact(n*m);
  rep2(i,0, n)rep2(j, 0, m) {
    if (i == 0 && j == 0) continue;
    addm(ans, dp[m*i+j].fi * nCk(n*m-2, k-2) % mod);
    //cerr << dp[m*i+j].fi << endl;
  }
  cout << ans << endl;
}
