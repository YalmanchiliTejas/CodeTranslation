...
void solve() {
    ...
    decltype(auto) fac  = factorial_table<200001>();
    decltype(auto) ifac = ifactorial_table<200001>();

    ModP ans = 0;

    // 縦
    FOR(d, 1, H) {
        ans += combination_count_fac(H*W-2,K-2,fac,ifac) * d * (H-d) * W*W;
    }

    // 横
    FOR(d, 1, W) {
        ans += combination_count_fac(H*W-2,K-2,fac,ifac) * d * (W-d) * H*H;
    }
