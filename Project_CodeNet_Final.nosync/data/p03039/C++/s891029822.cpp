...
int main()
{
	...
	modll III = N*N*(M-1)*M*(M+1)/3 + M*M*(N-1)*N*(N+1)/3;
	C[1] = 0;
	REPS(k, 2, K){
		modll I = C[k-1] * (NM - k + 1);
		modll II = (k-1)*P[k-2]*III;
		C[k] = I + II;
	}
