...
#if __cplusplus >= 201703L
template<class T = int, class A, class B> auto my_pow(A a, B b) {
    if constexpr(is_floating_point<T>::value) {return pow((T) a, (T) b);}
#ifdef _DEBUG   /*返り値が小数で無いのに、引数に小数が渡された場合エラー*/
...
template<class T> T tin(int len = 0){assert2(!(len &&  !is_vector<T>::value), "tin not vector");T a(len);    cin>>a;    return a;}
...
//template<size_t A>string out_m2(T (&a)[A]){return out_m2(vector<T>(begin(a), end(a)));}
//template<size_t A, size_t B>string out_m2(T (&a)[A][B]){return out_m2(vector<vector<T > >(begin(a), end(a)));}
//template<size_t A, size_t B, size_t C>string out_m2(T (&a)[A][B][C]){return out_m2(vector<vector<vector<T> > >(begin(a), end(a)));}
...
//template<class T> T zzz(const vector<T> &A, int l = -1, int r = -1) {
//    out("call1");
//    T s = 0;
//    set_lr12(l, r, sz(A));
//    rep(i, l, r)s += A[i];
//    return s;
//}
//template<class T> auto zzz(const vector<vector<T>> &A) {
//    out("call2");
//    decl2(A) s = 0;
//    rep(i, sz(A))s += zzz(A[i]);
//    return s;
//}
...
template<class T> string out_m2(ruiC<T> &a) {return out_m2(a.rui);}
...
template<class T> string out_m2(rruiC<T> &a) {return out_m2(a.rui);}
...
public:
    ...
    my_optional(T v) : v(v), is_null(false) {}
...
template<class T> struct ov : std::vector<optional<T>, std::allocator<optional<T> >> {
    optional<T> emp;
    ov(int i, int v = 0) : std::vector<optional<T>, std::allocator<optional<T> >>(i, v) {}
    ov(const vector<T>& rhs) : std::vector<optional<T>, std::allocator<optional<T> >>(rhs.begin(), rhs.end()) {}
//    void operator=(const vector<T> & rhs){
//        (*this) = ov<T>(rhs);
//    }
    optional<T> &operator()(int i) {        if (i < 0 || (*this).size() <= i) {            emp.reset();            return emp;        } else {            return std::vector<optional<T>, std::allocator<optional<T> >>::operator[](i);        }    }
    optional<T> &operator()(const optional<T>& i){
        if(i.has_value())return operator()(i.value());
        else{
            emp.reset();
            return emp;
        }
    }
    template<class... U >void operator[](U...) {        assert2(0, "cant [] ovec");        return;    }
};
template<class T> struct ovv : std::vector<vector<optional<T>>, std::allocator<vector<optional<T>> >> {
    optional<T> emp = nullopt;
    ovv(int i, int j, int v = 0) : std::vector<vector<optional<T>>, std::allocator<vector<optional<T>> >>(i, vector<optional<T>, allocator<optional<T>>>(j, v)) {}
    optional<T> &operator()(int i, int j) {        if (i < 0 || j < 0 || (*this).size() <= i || ((*this)[i]).size() <= j) {            emp.reset();            return emp;        } else {            return (*this)[i][j];        }    }
    template<class... U >void operator[](U...) {        assert2(0, "cant [] ovec");        return;    }
};
template<class T> struct ov3 : std::vector<vector<vector<optional<T>>>, std::allocator<vector<vector<optional<T>>> >> {
    optional<T> emp = nullopt;
    ov3(int i, int j, int k, int v = 0) : std::vector<vector<vector<optional<T>>>, std::allocator<vector<vector<vector<optional<T>>>> >>(i, vector<vector<optional<T>>, allocator<vector<optional<T>>>>(j, vector<optional<T>, allocator<optional<T>>>(k, v))) {}
    optional<T> &operator()(int i, int j, int k) {        if (i < 0 || j < 0 || k < 0 || (*this).size() <= i || ((*this)[i]).size() <= j || ((*this)[i][j]).size() <= k) {            emp.reset();            return emp;        } else {            return (*this)[i][j][k];        }    }
};
//nullopt_t
template<class T> void operator+=(nullopt_t &, const T &rhs) {}
template<class T> void operator-=(nullopt_t &, const T &rhs) {}
template<class T> void operator*=(nullopt_t &, const T &rhs) {}
template<class T> void operator/=(nullopt_t &, const T &rhs) {}
template<class T> T operator+=(T &a, const nullopt_t &b) {}
template<class T> T operator-=(T &a, const nullopt_t &b) {}
template<class T> T operator*=(T &a, const nullopt_t &b) {}
template<class T> T operator/=(T &a, const nullopt_t &b) {}

template<class T> void operator+(const nullopt_t, const T &rhs) {}
template<class T> void operator-(const nullopt_t, const T &rhs) {}
template<class T> void operator*(const nullopt_t, const T &rhs) {}
template<class T> void operator/(const nullopt_t, const T &rhs) {}
template<class T> T operator+(const T &a, const nullopt_t &b) {}
template<class T> T operator-(const T &a, const nullopt_t &b) {}
template<class T> T operator*(const T &a, const nullopt_t &b) {}
template<class T> T operator/(const T &a, const nullopt_t &b) {}
...
//optional<T>
template<class T, class U> void operator+=(optional<T> &lhs, const U &rhs) { if (lhs.has_value())lhs.value() += rhs; }
template<class T, class U> void operator-=(optional<T> &lhs, const U &rhs) { if (lhs.has_value())lhs.value() -= rhs; }
template<class T, class U> void operator*=(optional<T> &lhs, const U &rhs) { if (lhs.has_value())lhs.value() *= rhs; }
template<class T, class U> void operator/=(optional<T> &lhs, const U &rhs) { if (lhs.has_value())lhs.value() /= rhs; }

template<class T, class U> void operator+=(U &lhs, const optional<T> &rhs) { if (rhs.has_value())lhs += rhs.value(); }
template<class T, class U> void operator-=(U &lhs, const optional<T> &rhs) { if (rhs.has_value())lhs -= rhs.value(); }
template<class T, class U> void operator*=(U &lhs, const optional<T> &rhs) { if (rhs.has_value())lhs *= rhs.value(); }
template<class T, class U> void operator/=(U &lhs, const optional<T> &rhs) { if (rhs.has_value())lhs /= rhs.value(); }
...
void solve() {
    in(N, M);
    din(K);
    auto calc = [K](int H, int W) {
        mint base = com(H*W-2, K-2);
        deb(H, W, base);
        base *= W;
        base *= W;
        mint res = 0;
        rep(h, H) {
            mint r = base * (sig0(h) + sig0(H - 1 - h));
            deb(h, r, res);
            res += r;
        }
        deb(res);
