...
int main(){
  ...
  rep(i,N){
    int u = i,  d = N - 1 - i;
    ll cur =  mdiv ( mmul( (u+1) , u) + mmul( (d + 1) , d ) , 4 ); //最後のわる２
    cur = mmul ( cur , (M * M) % mod );
    ans = (ans + cur) % mod;
  }
  rep(i,M){
    int l = i,  r = M - 1 - i;
    ll cur =  mdiv ( mmul( (l+1) , l) + mmul( (r + 1) , r ) , 4 ); //最後のわる２
    cur = mmul ( cur , (N * N) % mod );
    ans = (ans + cur) % mod;
  }
