    ...
    def __init__(self, n, a=None):
        ...
        if a is not None:
            ...
            for k in range(num-2, -1, -1):
                self.seg[k] = max(self.seg[(k<<1)+1], self.seg[(k<<1)+2])
...
#                 self.update(i,item)
    ...
    def _query(self,a,b,k=0,l=0,r=None):
        ...
        if r <= a or b <= l:
            ...
            return max(self._query(a,b,(k<<1)+1,l,(l+r)>>1),self._query(a,b,(k<<1)+2,(l+r)>>1,r))
    ...
    def _find_right(self,a,b,x,k,l,r):
        if self.seg[k]<x or r<=a or b<=l: # x以上を満たせない or 区間が重複しない
            ...
            vr = self._find_right(a,b,x,(k<<1)+2,(l+r)>>1,r)
            if vr>=0:
                return vr
            else:
                return self._find_right(a,b,x,(k<<1)+1,l,(l+r)>>1)
    ...
    def _find_left(self,a,b,x,k,l,r):
        if self.seg[k]<x or r<=a or b<=l: # x以上を満たせない or 区間が重複しない
            ...
            vl = self._find_left(a,b,x,(k<<1)+1,l,(l+r)>>1)
            if vl<self.num:
                return vl
            else:
                return self._find_left(a,b,x,(k<<1)+2,(l+r)>>1,r)
    def query_index(self,a,b,k=0,l=0,r=None):
        ...
        if r <= a or b <= l:
            ...
            return max(self.query_index(a,b,(k<<1)+1,l,(l+r)>>1),self.query_index(a,b,(k<<1)+2,(l+r)>>1,r))
    def _index(self, k):
        if k>=self.num:
            return k - (self.num-1)
        else:
            if self.seg[(k<<1)+1]>=self.seg[(k<<1)+2]:
                return self._index((k<<1)+1)
            else:
                return self._index((k<<1)+2)
