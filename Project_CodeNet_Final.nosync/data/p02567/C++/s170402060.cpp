...
int getleft(int x, int val, int k = 0, int l = 0, int r = - 1) {
    // 最初に呼び出されたときの対象区間は [0, n)
    if(r < 0) r = n;

    // 要求区間と対象区間が交わらない -> 適当に返す(今回は最小値なので、結果に影響しないような大きい値を返しておく)
    if(r <= x || n <= l) return INF;
    ...
    if(x <= l && r <= n)
	{
		...
		if (getmax(l, (l+r)/2) >= val)
			return (getleft(x, val, 2*k+1, l, (l+r)/2));
		if (getmax((l+r)/2, r) >= val)
			return (getleft(x, val, 2*k+2, (l+r)/2, r));
		return (INF);
	}
    ...
    // 左側の子を vl ・ 右側の子を vr としている
    // 新しい対象区間は、現在の対象区間を半分に割ったもの
    int vl = getleft(x, val, 2*k+1, l, (l+r)/2);
    int vr = getleft(x, val, 2*k+2, (l+r)/2, r);
	if (vl != INF) return (vl);
    if (vr != INF) return (vr);
	return (INF);
}
