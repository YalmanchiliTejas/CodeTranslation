...
void _main()
{
    ll n; llin(n);
    ll qcnt; llin(qcnt);
    V<ll> alist; alist.pb(-1e18); llina(alist, n);

    n++;

    S SegmentTree<ll> tree(200005, -1e18, maxll);
    rep (i, n) tree.update(i, alist[i]);
    ...
    rep (xx, qcnt) {
        ...
        ll idx; llin(idx);
        ll border; llin(border);

        ll left = idx+1;
        ll right = n;
        ll nowend = (left+right) / 2LL;
        ll validend = n+1;

        for ( ; left <= right; nowend = (left+right) / 2LL) {
            ll nowval = tree.query(idx, nowend);
            if (nowval >= border) {
                validend = min(validend, nowend);
                right = nowend - 1LL;
            }
            else {
                left = nowend + 1LL;
