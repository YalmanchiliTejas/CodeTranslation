...
//    readint(test_cases);
    ...
    forr(t, 1, test_cases) {
        ...
        SegmentTree2<int> seg(a, [](int x, int y){return max(x,y);}, -modd);
        forr(i,0,q) {
            readint(a); readint(x); readint(v);
            if (a==1) {
                seg.SetVal(x-1, v);
            }
            if (a==2) {
                cout << seg.GetEvaluation(x-1,v) << endl;
            }
            if (a==3) {
                cout <<  binary_search([&seg,x,v](ll j){
                    return seg.GetEvaluation(x-1, j) >= v;
                }, x, n+1) << endl;
            }
        }
