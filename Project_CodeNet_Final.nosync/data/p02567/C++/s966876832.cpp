...
template<class T> auto min(const T& a) -> decltype(*min_element(all(a))) { return *min_element(all(a)); }
template<class T> auto max(const T& a) -> decltype(*max_element(all(a))) { return *max_element(all(a)); }
...
signed main(){
    ...
    rep(q){
        LL(type, a, b);
        a--;
        if(type == 1) seg.set(n - 1 - a, b);
        elif(type == 2) out(seg.get(n - b, n - a));
        else out(n - seg.find_first([&](ll x){ return x < b; }, n - a) + 1);
    }
}
