...
namespace haar_lib {
  ...
  public:
    ...
    template <typename F = std::greater_equal<value_type>>
    int lower_bound(const int l, const int r, value_type x, F f = F()) const {
...
namespace solver {
  ...
  void solve(){
    ...
    auto seg = segment_tree<max_monoid<int>>(N);
    ...
    for(int i = 0; i < Q; ++i){
      ...
      if(type == 1){
        ...
        std::cout << seg.lower_bound(
          x - 1, N, v,
          [v](const auto &a, const auto &b){return a.value_or(INT_MAX) >= b.value_or(INT_MAX);}
        ) + 1 << "\n";
      }
    }
  }
