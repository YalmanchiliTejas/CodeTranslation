#line 1 "atcoder-workspace/practice2/j/j.cpp"
#include <iostream>

#line 2 "Library/data_structure/segment_tree/basic.hpp"
#include <cassert>
#include <vector>
...
#line 4 "atcoder-workspace/practice2/j/j.cpp"
...
int main() {
  ...
  struct mono {
    int value = 0;
    mono operator+(mono rhs) { return {std::max(value, rhs.value)}; }
  };
  int n, q;
  std::cin >> n >> q;
  ...
  while (q--) {
    ...
    switch (tp) {
      ...
      case 2: {
        std::cout << seg.fold(x, y).value << "\n";
        break;
      }
      case 3: {
        std::cout << seg.right_search(x, [&](mono m) {
          return m.value < y;
        }) + 1 << "\n";
        break;
      }
