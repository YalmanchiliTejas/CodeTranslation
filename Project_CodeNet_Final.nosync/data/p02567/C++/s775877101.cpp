...
public:
    ...
    */
    ...
    int binary_search(int a, int b, const std::function<bool(Monoid)> &f, int k = 0, int l = 0, int r = -1) {
        if (r < 0) r = n;
        if (r <= a || b <= l) return n;
        if (!f(node[k])) return n;
        if (a <= l && r <= b && l == r - 1) return l;
        int ret_l = binary_search(a, b, f, 2 * k + 1, l, (r - l) / 2 + l);
        if (ret_l != n) return ret_l;
        int ret_r = binary_search(a, b, f, 2 * k + 2, (r - l) / 2 + l, r);
        //assert(ret_r < n);
        return ret_r;
    }
